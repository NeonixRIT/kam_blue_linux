...
from scapy.utils import do_graph, hexdump, make_table, make_lined_table, make_tex_table, issubtype
...
QueryAnswer = NamedTuple('QueryAnswer', [('query', Packet), ('answer', Packet)])
...
class _PacketList(Generic[_Inner]):
    ...
    def __getitem__(self, item):
        ...
        if issubtype(item, BasePacket):
            return self.__class__(
                [x for x in self.res if item in self._elt2pkt(x)],  # noqa: E501
    ...
    def multiplot(
        ...
        **kargs,  # type: Any
        ...
        if plot_xy:
            lines = [plt.plot(*zip(*pl), **dict(kargs, label=k)) for k, pl in six.iteritems(d)]
        else:
            lines = [plt.plot(pl, **dict(kargs, label=k)) for k, pl in six.iteritems(d)]
