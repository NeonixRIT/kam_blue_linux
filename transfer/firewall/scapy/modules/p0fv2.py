...
def p0f_correl(x, y):
    ...
    if len(xopt) == len(yopt):
        ...
        for i in range(len(xopt)):
            if not (
                xopt[i] == yopt[i]
                or (len(yopt[i]) == 2 and len(xopt[i]) > 1 and yopt[i][1] == '*' and xopt[i][0] == yopt[i][0])
                or (len(yopt[i]) > 2 and len(xopt[i]) > 1 and yopt[i][1] == '%' and xopt[i][0] == yopt[i][0] and int(xopt[i][1:]) % int(yopt[i][2:]) == 0)
            ):
                same = False
                break
        if same:
            ...
            if opt[0] == 'M':
                ...
                if opt[1:] == '*':
                    ...
                    if wscale_hint is not None and wscale_hint % coef == 0:
                        ...
                        options.append(('WScale', coef * RandNum(min=1, max=(2**8 - 1) // coef)))  # noqa: E501
                else:
                ...
                if int(opt[1:]) in TCPOptions[0]:
                    ...
                    options.append((optname, struct.unpack(optstruct, RandString(struct.calcsize(optstruct))._fix())))  # noqa: E501
                else:
