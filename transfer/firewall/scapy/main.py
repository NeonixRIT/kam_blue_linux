...
def _usage():
    # type: () -> None
    print('Usage: scapy.py [-s sessionfile] [-c new_startup_file] ' '[-p new_prestart_file] [-C] [-P] [-H]\n' 'Args:\n' '\t-H: header-less start\n' '\t-C: do not read startup file\n' '\t-P: do not read pre-startup file\n')
    sys.exit(0)
...
def load_module(name, globals_dict=None, symb_list=None):
    ...
    """
    _load('scapy.modules.' + name, globals_dict=globals_dict, symb_list=symb_list)
...
def load_contrib(name, globals_dict=None, symb_list=None):
    ...
    try:
        ...
        _load('scapy.contrib.' + name, globals_dict=globals_dict, symb_list=symb_list)
    except ImportError as e:
        # if layer not found in contrib, try in layers
        try:
            load_layer(name, globals_dict=globals_dict, symb_list=symb_list)
        except ImportError:
...
def _scapy_builtins():
    ...
    return {k: v for k, v in six.iteritems(importlib.import_module('.all', 'scapy').__dict__.copy()) if _validate_local(k)}
...
def interact(mydict=None, argv=None, mybanner=None, loglevel=logging.INFO):
    ...
    if conf.fancy_prompt:
        ...
        if mini_banner:
            ...
            the_banner.append('   |')
        banner_text = '\n'.join(logo + banner for logo, banner in six.moves.zip_longest((conf.color_theme.logo(line) for line in the_logo), (conf.color_theme.success(line) for line in the_banner), fillvalue=''))
    else:
    ...
    if IPYTHON:
        ...
        try:
            ...
            log_loading.warning("traitlets not available. Some Scapy shell features won't be " 'available.')
            try:
                start_ipython(display_banner=False, user_ns=SESSION, exec_lines=['print("""' + banner + '""")'])
            except Exception:
            ...
            try:
                ...
                cfg.TerminalInteractiveShell.separate_in = ''
            if int(IPython.__version__[0]) >= 6:
                cfg.TerminalInteractiveShell.term_title_format = 'Scapy %s' % conf.version
