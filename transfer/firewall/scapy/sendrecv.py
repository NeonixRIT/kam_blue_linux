...
# Typing imports
from scapy.compat import Any, Callable, Dict, Iterator, List, Optional, Tuple, Type, Union, cast
...
class SndRcvHandler(object):
    ...
    def __init__(
        ...
        while retry >= 0:
            ...
            if multi:
                remain = [p for p in itertools.chain(*six.itervalues(self.hsent)) if not hasattr(p, '_answered')]
            else:
...
def send(
    ...
    **kargs,  # type: Any
    ...
    return _send(x, lambda iface: iface.l3socket(), iface=iface, **kargs)
...
def _interface_selection(
    iface,  # type: Optional[_GlobInterfaceType]
        ...
        **karg,  # type: Any
        ...
        if opened_socket is not None:
            if isinstance(opened_socket, list):
                ...
                sniff_sockets.update((s, label) for s, label in six.iteritems(opened_socket))
            else:
        ...
        if offline is not None:
            ...
            if isinstance(offline, list) and all(isinstance(elt, str) for elt in offline):
                ...
                # Dict of files
                sniff_sockets.update((PcapReader(fname if flt is None else tcpdump(fname, args=['-w', '-'], flt=flt, getfd=True, quiet=quiet)), label) for fname, label in six.iteritems(offline))
        ...
        if not sniff_sockets or iface is not None:
            ...
            if isinstance(iface, list):
                ...
                sniff_sockets.update((_RL2(ifname)(type=ETH_P_ALL, iface=ifname, **karg), iflabel) for ifname, iflabel in six.iteritems(iface))
            else:
        ...
        try:
            ...
            while sniff_sockets and self.continue_sniff:
                ...
                for s in sockets:
                    ...
                    # check
                    if (stop_filter and stop_filter(p)) or (0 < count <= session.count):
                        self.continue_sniff = False
                        break
                # Removed dead sockets
                for s in dead_sockets:
                    del sniff_sockets[s]
                    if len(sniff_sockets) == 1 and close_pipe in sniff_sockets:  # type: ignore
...
def bridge_and_sniff(
    ...
    **kargs,  # type: Any
    ...
    return sniff(opened_socket={sckt1: if1, sckt2: if2}, prn=prn, *args, **kargs)
