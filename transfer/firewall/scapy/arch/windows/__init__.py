...
from scapy.interfaces import NetworkInterface, InterfaceProvider, dev_from_index, resolve_iface, network_name
...
def _encapsulate_admin(cmd):
    ...
    # ! Behold !
    return 'powershell /command "Start-Process cmd ' '-windowstyle hidden -Wait -PassThru -Verb RunAs ' "-ArgumentList '/c %s'\"" % cmd
...
class WinProgPath(ProgPath):
    ...
    def _reload(self):
        ...
        self.usbpcapcmd = win_find_exe('USBPcapCMD', installsubdir='USBPcap', env='programfiles')
        self.powershell = win_find_exe('powershell', installsubdir='System32\\WindowsPowerShell\\v1.0', env='SystemRoot')
        self.cscript = win_find_exe('cscript', installsubdir='System32', env='SystemRoot')
        self.cmd = win_find_exe('cmd', installsubdir='System32', env='SystemRoot')
        if self.wireshark:
            try:
                new_manuf = load_manuf(os.path.sep.join(self.wireshark.split(os.path.sep)[:-1]) + os.path.sep + 'manuf')
...
if conf.prog.tcpdump and conf.use_npcap:
    ...
    if not windump_ok:
        log_loading.warning('The installed Windump version does not work with Npcap! ' "Refer to 'Winpcap/Npcap conflicts' in scapy's installation doc")
    del windump_ok
...
class NetworkInterface_Win(NetworkInterface):
    ...
    def _npcap_set(self, key, val):
        ...
        res, code = _exec_cmd(_encapsulate_admin(' '.join([_WlanHelper, self.guid[1:-1], key, val])))
    ...
    def setmode(self, mode):
        ...
        # According to https://nmap.org/npcap/guide/npcap-devguide.html#npcap-feature-dot11  # noqa: E501
        self._check_npcap_requirement()
        _modes = {0: 'managed', 1: 'monitor', 2: 'master', 3: 'wfd_device', 4: 'wfd_owner', 5: 'wfd_client'}
...
class WindowsInterfacesProvider(InterfaceProvider):
    ...
    def _pcap_check(cls):
        ...
        if _detect:
            ...
            log_interactive.warning('Scapy has detected that your pcap service is not running !')
            if not conf.interactive or _ask_user():
                ...
                if succeed:
                    ...
                    return
        log_loading.warning('Could not start the pcap service! ' "You probably won't be able to send packets. " 'Check your winpcap/npcap installation ' 'and access rights.')
    ...
    def load(self, NetworkInterface_Win=NetworkInterface_Win):
        ...
        for netw, if_data in six.iteritems(conf.cache_pcapiflist):
            ...
            if data:
                ...
                # Only in [Wi]npcap
                index -= 1
                data = {'name': name, 'description': name, 'index': index, 'guid': guid, 'network_name': netw, 'mac': '00:00:00:00:00:00', 'ipv4_metric': 0, 'ipv6_metric': 0, 'ips': ips, 'flags': flags}
...
def get_ip_from_name(ifname, v6=False):
    ...
    Deprecated.
    """
    warnings.warn('get_ip_from_name is deprecated. Use the `ip` attribute of the iface ' 'or use get_ips() to get all ips per interface.', DeprecationWarning)
...
if conf.use_pcap:
    ...
    def open_pcap(
        iface,  # type: Union[str, NetworkInterface]
        *args,  # type: Any
        **kargs,  # type: Any
    ):
...
def _read_routes_c(ipv6=False):  # noqa: F811
    ...
    for route in GetIpForwardTable2(af):
        ...
        if ipv6:
            _append_route6(routes, dest, netmask, nexthop, netw, lifaddr, metric)
        else:
            routes.append((atol(dest), itom(int(netmask)), nexthop, netw, ip, metric))
    return routes
...
def _route_add_loopback(
    ...
    loopback_route = (loop_net, loop_mask, '0.0.0.0', adapter.network_name, '127.0.0.1', 1)
...
class _NotAvailableSocket(SuperSocket):
    ...
    def __init__(self, *args, **kargs):
        # type: (*Any, **Any) -> None
        raise RuntimeError('Sniffing and sending packets is not available at layer 2: ' 'winpcap is not installed. You may use conf.L3socket or' 'conf.L3socket6 to access layer 3')
