        ...
        def load_winpcapy():
            ...
            try:
                ...
                while p:
                    ...
                    description = plain_str(p.contents.description or '')  # DESC
        ...
        if WINDOWS:
            if conf.interactive:
                log_loading.critical(
                    'Npcap/Winpcap is not installed ! See ' 'https://scapy.readthedocs.io/en/latest/installation.html#windows'  # noqa: E501
                )
        else:
            if conf.interactive:
...
if conf.use_pcap:
    ...
    class _PcapWrapper_libpcap:  # noqa: F811
        ...
        def next(self):
            ...
            if not c > 0:
                return None, None
            ts = self.header.contents.ts.tv_sec + float(self.header.contents.ts.tv_usec) / 1e6
            pkt = bytes(
                bytearray(
