...
from scapy.interfaces import InterfaceProvider, IFACES, NetworkInterface, network_name
...
def get_if_raw_addr(ifname):
    ...
    # Get ifconfig output
    subproc = subprocess.Popen([conf.prog.ifconfig, ifname], close_fds=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    # Get IPv4 addresses
    addresses = [line.strip() for line in plain_str(stdout).splitlines() if 'inet ' in line]
...
def get_if_raw_hwaddr(ifname):
    ...
    stdout, stderr = subproc.communicate()
    if subproc.returncode:
        raise Scapy_Exception('Failed to execute ifconfig: (%s)' % plain_str(stderr).strip())

    # Get MAC addresses
    addresses = [line.strip() for line in plain_str(stdout).splitlines() if ('ether' in line or 'lladdr' in line or 'address' in line)]
    if not addresses:
    ...
    for bpf in range(256):
        try:
            ...
            if ex.errno == 13:  # Permission denied
                raise Scapy_Exception(('Permission denied: could not open /dev/bpf%i. ' 'Make sure to be running Scapy as root ! (sudo)') % bpf)
            continue
...
class BPFInterfaceProvider(InterfaceProvider):
    ...
    def load(self):
        ...
        for ifname, index in _get_ifindex_list():
            ...
            if_data = {'name': ifname, 'network_name': ifname, 'description': ifname, 'flags': ifflags, 'index': index, 'ip': ip, 'ips': [x[0] for x in ips if x[2] == ifname] + [ip], 'mac': mac}
