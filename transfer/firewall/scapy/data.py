...
def load_protocols(filename, _fallback=None, _integer_base=10, _cls=DADict[int, str]):
...
class EtherDA(DADict[int, str]):
    ...
    def __setitem__(self, attr, val):
        ...
        if isinstance(attr, str):
            attr, val = val, attr
            warnings.warn('ETHER_TYPES now uses the integer value as key !', DeprecationWarning)
        super(EtherDA, self).__setitem__(attr, val)

    def __getitem__(self, attr):
        # type: (int) -> Any
        if isinstance(attr, str):
            warnings.warn("Please use 'ETHER_TYPES.%s'" % attr, DeprecationWarning)
...
def load_ethertypes(filename):
    ...
    prot = load_protocols(filename or "Scapy's backup ETHER_TYPES", _fallback=DATA, _integer_base=16, _cls=EtherDA)
...
class ManufDA(DADict[str, Tuple[str, str]]):
    ...
    def reverse_lookup(self, name, case_sensitive=False):
        ...
        return {k: v for k, v in six.iteritems(self.d) if filtr(name, v)}
    ...
    with open(filename, 'rb') as fdesc:
        for line in fdesc:
            try:
                ...
                log_loading.warning("Couldn't parse one line from [%s] [%r]", filename, line, exc_info=True)
    return manufdb
...
if WINDOWS:
    ...
    TCP_SERVICES, UDP_SERVICES, SCTP_SERVICES = load_services(
        os.path.join(
            os.environ['SystemRoot'],
            'system32',
            'drivers',
    ...
    MANUFDB = ManufDA()
    manuf_path = select_path(['/usr', '/usr/local', '/opt', '/opt/wireshark', '/Applications/Wireshark.app/Contents/Resources'], 'share/wireshark/manuf')
    if manuf_path:
        try:
