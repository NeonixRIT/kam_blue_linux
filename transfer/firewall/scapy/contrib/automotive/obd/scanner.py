...
from scapy.contrib.automotive.obd.obd import OBD, OBD_S03, OBD_S07, OBD_S0A, OBD_S01, OBD_S06, OBD_S08, OBD_S09, OBD_NR, OBD_S02, OBD_S02_Record
from scapy.config import conf
from scapy.packet import Packet
from scapy.themes import BlackAndWhite

from scapy.contrib.automotive.scanner.enumerator import ServiceEnumerator, _AutomotiveTestCaseScanResult, _AutomotiveTestCaseFilteredScanResult
from scapy.contrib.automotive.scanner.executor import AutomotiveTestCaseExecutor
from scapy.contrib.automotive.ecu import EcuState
from scapy.contrib.automotive.scanner.test_case import AutomotiveTestCaseABC, _SocketUnion
...
class OBD_Service_Enumerator(OBD_Enumerator):
    ...
    def get_supported(self, socket, state, **kwargs):
        ...
        super(OBD_Service_Enumerator, self).execute(socket, state, scan_range=range(0, 0xFF, 0x20), exit_scan_on_first_negative_response=True, **kwargs)
        ...
        for _, _, r, _, _ in self.results_with_positive_response:
            ...
            try:
                supported += [int(i[-2:], 16) for i in getattr(dr, key, ['xxx00'])]
            except TypeError:
                pass
    ...
    def execute(self, socket, state, **kwargs):
        ...
        if full_scan:
            ...
            super(OBD_Service_Enumerator, self).execute(socket, state, scan_range=supported_pids, **kwargs)
...
class OBD_S01_Enumerator(OBD_Service_Enumerator):
    ...
    def _get_table_entry_y(self, tup):
        ...
        if resp is None:
            ...
            return 'NR' if resp.service == 0x7F else '%s' % resp.data_records[0].lastlayer().name
...
class OBD_S02_Enumerator(OBD_Service_Enumerator):
    ...
    def _get_initial_requests(self, **kwargs):
        # type: (Any) -> Iterable[Packet]
        scan_range = kwargs.pop('scan_range', range(0x100))  # type: Iterable[int]  # noqa: E501
        return (OBD() / OBD_S02(requests=[OBD_S02_Record(pid=[x])]) for x in scan_range)
...
class OBD_S06_Enumerator(OBD_Service_Enumerator):
    ...
    def _get_table_entry_y(self, tup):
        ...
        if resp is None:
            ...
            return 'NR' if resp.service == 0x7F else '0x%02x %s' % (req.mid[0], resp.data_records[0].sprintf('%OBD_S06_PR_Record.mid%'))
...
class OBD_S08_Enumerator(OBD_Service_Enumerator):
    ...
    def _get_table_entry_y(self, tup):
        ...
        if resp is None:
            ...
            return 'NR' if resp.service == 0x7F else '0x%02x %s' % (tup[1].tid[0], resp.data_records[0].lastlayer().name)
...
class OBD_S09_Enumerator(OBD_Service_Enumerator):
    ...
    def _get_table_entry_y(self, tup):
        ...
        if resp is None:
            ...
            return 'NR' if resp.service == 0x7F else '0x%02x %s' % (tup[1].iid[0], resp.data_records[0].lastlayer().name)
...
class OBD_Scanner(AutomotiveTestCaseExecutor):
    ...
    def default_test_case_clss(self):
        ...
        return [OBD_S01_Enumerator, OBD_S02_Enumerator, OBD_S06_Enumerator, OBD_S08_Enumerator, OBD_S09_Enumerator, OBD_S03_Enumerator, OBD_S07_Enumerator, OBD_S0A_Enumerator]
