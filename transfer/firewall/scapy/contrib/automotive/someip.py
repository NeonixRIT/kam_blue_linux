...
class SOMEIP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XBitField('method_id', 0, 15), lambda pkt: pkt.sub_id == 0),
        ConditionalField(XBitField('event_id', 0, 15), lambda pkt: pkt.sub_id == 1),
        IntField('len', None),
        ...
        ConditionalField(BitField('offset', 0, 28), lambda pkt: SOMEIP._is_tp(pkt)),
        ConditionalField(BitField('res', 0, 3), lambda pkt: SOMEIP._is_tp(pkt)),
        ConditionalField(BitField('more_seg', 0, 1), lambda pkt: SOMEIP._is_tp(pkt)),
    ]
...
def _MAKE_COMMON_IP_SDOPTION_FIELDS_DESC():
    return [XByteField('res_tail', 0), ByteEnumField('l4_proto', 0x11, {0x06: 'TCP', 0x11: 'UDP'}), ShortField('port', 0)]
...
class SD(_SDPacketBase):
    ...
    FLAGSDEF = {'REBOOT': _sdFlag(mask=0x80, offset=7), 'UNICAST': _sdFlag(mask=0x40, offset=6)}
    ...
    fields_desc = [
        ...
        PacketListField('entry_array', None, _sdentry_class, length_from=lambda pkt: pkt.len_entry_array),
        FieldLenField('len_option_array', None, length_of='option_array', fmt='!I'),
        PacketListField('option_array', None, _sdoption_class, length_from=lambda pkt: pkt.len_option_array),
    ]
