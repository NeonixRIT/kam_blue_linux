...
from scapy.compat import Optional, List, Type, Any, Tuple, Iterable, Dict, cast, Callable, orb
...
from scapy.contrib.automotive.gm.gmlanutils import GMLAN_InitDiagnostics, GMLAN_TesterPresentSender
from scapy.contrib.automotive.gm.gmlan import GMLAN, GMLAN_SA, GMLAN_RD, GMLAN_TD, GMLAN_RMBA, GMLAN_RDBI, GMLAN_RDBPI, GMLAN_IDO, GMLAN_NR, GMLAN_WDBI, GMLAN_DC, GMLAN_PM
...
from scapy.contrib.automotive.scanner.enumerator import ServiceEnumerator, _AutomotiveTestCaseScanResult, StateGeneratingServiceEnumerator
...
__all__ = [
    'GMLAN_Scanner',
    'GMLAN_ServiceEnumerator',
    'GMLAN_RDBIEnumerator',
    'GMLAN_RDBPIEnumerator',
...
class GMLAN_WDBIEnumerator(GMLAN_Enumerator):
    ...
    _supported_kwargs.update({'rdbi_enumerator': (GMLAN_RDBIEnumerator, None)})
    ...
    def _get_initial_requests(self, **kwargs):
        ...
        if rdbi_enumerator is None:
            ...
            return (GMLAN() / GMLAN_WDBI(dataIdentifier=t.resp.dataIdentifier, dataRecord=bytes(t.resp)[2:]) for t in rdbi_enumerator.filtered_results if t.resp.service != 0x7F and len(bytes(t.resp)) >= 2)
        else:
...
class GMLAN_SAEnumerator(GMLAN_Enumerator, StateGenerator):
    ...
    _supported_kwargs = copy.copy(GMLAN_Enumerator._supported_kwargs)
    _supported_kwargs.update({'keyfunction': (None, None)})
    ...
    def _evaluate_retry(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        if response.service == 0x7F and self._get_negative_response_code(response) in [0x22, 0x37]:
    ...
    def _evaluate_response(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        if response is not None and response.service == 0x67 and response.subfunction % 2 == 1:
    ...
    def get_seed_pkt(sock, level=1):
        ...
        for _ in range(10):
            ...
            if seed is None:
                return None
            elif seed.service == 0x7F and GMLAN_Enumerator._get_negative_response_code(seed) != 0x37:
    ...
    def get_new_edge(self, socket, config):
        ...
        try:
            if last_resp.service != 0x67 or last_resp.subfunction % 2 != 1:
                return None
...
class GMLAN_PMEnumerator(GMLAN_Enumerator, StateGeneratingServiceEnumerator):
    ...
    def execute(self, socket, state, timeout=1, execution_time=1200, **kwargs):
        ...
        if supported:
            self._store_result(state, GMLAN() / GMLAN_PM(), GMLAN(service=0xE5))
        else:
            self._store_result(state, GMLAN() / GMLAN_PM(), GMLAN() / GMLAN_NR(returnCode=0x11, requestServiceId=0xA5))
...
class GMLAN_RMBAEnumerator(GMLAN_Enumerator):
    ...
    _supported_kwargs = copy.copy(GMLAN_Enumerator._supported_kwargs)
    _supported_kwargs.update({'probe_width': (int, lambda x: x >= 0), 'random_probes_len': (int, lambda x: x >= 0), 'sequential_probes_len': (int, lambda x: x >= 0)})

    _supported_kwargs_doc = (
        GMLAN_Enumerator._supported_kwargs_doc
        + """
    ...
    def __init__(self):
        ...
        self.random_probes_len = min(10 ** conf.contribs['GMLAN']['GMLAN_ECU_AddressingScheme'], 0x5000)
