...
try:
    ...
    log_automotive.info(
        "Specify \"conf.contribs['GMLAN'] = " "{'treat-response-pending-as-answer': True}\" to treat " "a negative response 'RequestCorrectlyReceived-" "ResponsePending' as answer of a request. \n" 'The default value is False.'
    )
...
class GMLAN_IDO(Packet):
    subfunctions = {0x02: 'disableAllDTCs', 0x03: 'enableDTCsDuringDevCntrl', 0x04: 'wakeUpLinks'}
...
class GMLAN_RFRD(Packet):
    subfunctions = {0x01: 'readFailureRecordIdentifiers', 0x02: 'readFailureRecordParameters'}
    name = 'ReadFailureRecordData'
    fields_desc = [ByteEnumField('subfunction', 0, subfunctions), ConditionalField(PacketField('dtc', b'', GMLAN_DTC), lambda pkt: pkt.subfunction == 0x02)]
...
class GMLAN_RFRDPR_RFRI(Packet):
    failureRecordDataStructureIdentifiers = {0x00: 'PID', 0x01: 'DPID'}
    ...
    dataIdentifiers = ObservableDict(
        {
            ...
            0x9F: '$9F: PreviousStoredRepairShopCodeOrTesterSerialNumbers ' '(PSRSCOTSN)',
            ...
            0xE9: '$E9: Subnet_Config_List_GMLANFrontObjectExpansionBus ' '(SCLGFOEB)',
...
class GMLAN_RDBPI(Packet):
    dataIdentifiers = ObservableDict({0x0005: 'OBD_EngineCoolantTemperature', 0x000C: 'OBD_EngineRPM', 0x001F: 'OBD_TimeSinceEngineStart'})
...
class GMLAN_RDBPKTI(Packet):
    ...
    subfunctions = {0x00: 'stopSending', 0x01: 'sendOneResponse', 0x02: 'scheduleAtSlowRate', 0x03: 'scheduleAtMediumRate', 0x04: 'scheduleAtFastRate'}

    fields_desc = [XByteEnumField('subfunction', 0, subfunctions), ConditionalField(FieldListField('request_DPIDs', [], XByteField('', 0)), lambda pkt: pkt.subfunction > 0x0)]
...
class GMLAN_RMBA(Packet):
    ...
    fields_desc = [
        MultipleTypeField(
            [(XShortField('memoryAddress', 0), lambda pkt: GMLAN.determine_len(2)), (X3BytesField('memoryAddress', 0), lambda pkt: GMLAN.determine_len(3)), (XIntField('memoryAddress', 0), lambda pkt: GMLAN.determine_len(4))],
...
class GMLAN_SAPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XShortField('securitySeed', 0), lambda pkt: pkt.subfunction % 2 == 1),
    ]
...
class GMLAN_RD(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [(XShortField('memorySize', 0), lambda pkt: GMLAN.determine_len(2)), (X3BytesField('memorySize', 0), lambda pkt: GMLAN.determine_len(3)), (XIntField('memorySize', 0), lambda pkt: GMLAN.determine_len(4))],
...
class GMLAN_TD(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [(XShortField('startingAddress', 0), lambda pkt: GMLAN.determine_len(2)), (X3BytesField('startingAddress', 0), lambda pkt: GMLAN.determine_len(3)), (XIntField('startingAddress', 0), lambda pkt: GMLAN.determine_len(4))],
...
class GMLAN_PM(Packet):
    subfunctions = {0x01: 'requestProgrammingMode', 0x02: 'requestProgrammingMode_HighSpeed', 0x03: 'enableProgrammingMode'}
...
class GMLAN_RDI(Packet):
    subfunctions = {0x80: 'readStatusOfDTCByDTCNumber', 0x81: 'readStatusOfDTCByStatusMask', 0x82: 'sendOnChangeDTCCount'}
...
class GMLAN_NR(Packet):
    ...
    fields_desc = [XByteEnumField('requestServiceId', 0, GMLAN.services), ByteEnumField('returnCode', 0, negativeResponseCodes), ShortField('deviceControlLimitExceeded', 0)]

    def answers(self, other):
        return self.requestServiceId == other.service and (self.returnCode != 0x78 or conf.contribs['GMLAN']['treat-response-pending-as-answer'])
