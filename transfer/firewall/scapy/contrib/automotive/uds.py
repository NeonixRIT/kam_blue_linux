...
try:
    ...
    log_loading.info("Specify \"conf.contribs['UDS'] = " "{'treat-response-pending-as-answer': True}\" to treat " "a negative response 'requestCorrectlyReceived-" "ResponsePending' as answer of a request. \n" 'The default value is False.')
...
class UDS_DSC(Packet):
    diagnosticSessionTypes = ObservableDict({0x00: 'ISOSAEReserved', 0x01: 'defaultSession', 0x02: 'programmingSession', 0x03: 'extendedDiagnosticSession', 0x04: 'safetySystemDiagnosticSession', 0x7F: 'ISOSAEReserved'})
    name = 'DiagnosticSessionControl'
    fields_desc = [ByteEnumField('diagnosticSessionType', 0, diagnosticSessionTypes)]
...
class UDS_DSCPR(Packet):
    ...
    fields_desc = [ByteEnumField('diagnosticSessionType', 0, UDS_DSC.diagnosticSessionTypes), StrField('sessionParameterRecord', b'')]
...
# #########################ER###################################
class UDS_ER(Packet):
    resetTypes = {0x00: 'ISOSAEReserved', 0x01: 'hardReset', 0x02: 'keyOffOnReset', 0x03: 'softReset', 0x04: 'enableRapidPowerShutDown', 0x05: 'disableRapidPowerShutDown', 0x41: 'powerDown', 0x7F: 'ISOSAEReserved'}
    name = 'ECUReset'
    fields_desc = [ByteEnumField('resetType', 0, resetTypes)]
...
class UDS_ERPR(Packet):
    ...
    fields_desc = [ByteEnumField('resetType', 0, UDS_ER.resetTypes), ConditionalField(ByteField('powerDownTime', 0), lambda pkt: pkt.resetType == 0x04)]
...
# #########################CC###################################
class UDS_CC(Packet):
    controlTypes = {0x00: 'enableRxAndTx', 0x01: 'enableRxAndDisableTx', 0x02: 'disableRxAndEnableTx', 0x03: 'disableRxAndTx'}
    name = 'CommunicationControl'
    fields_desc = [
        ByteEnumField('controlType', 0, controlTypes),
        BitEnumField('communicationType0', 0, 2, {0: 'ISOSAEReserved', 1: 'normalCommunicationMessages', 2: 'networkManagmentCommunicationMessages', 3: 'networkManagmentCommunicationMessages and ' 'normalCommunicationMessages'}),
...
class UDS_CCPR(Packet):
    ...
    fields_desc = [ByteEnumField('controlType', 0, UDS_CC.controlTypes)]
...
class UDS_AUTH(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XByteField('communicationConfiguration', 0), lambda pkt: pkt.subFunction in [0x01, 0x02, 0x5]),
        ConditionalField(XShortField('certificateEvaluationId', 0), lambda pkt: pkt.subFunction == 0x04),
        ConditionalField(XStrFixedLenField('algorithmIndicator', 0, length=16), lambda pkt: pkt.subFunction in [0x05, 0x06, 0x07]),
        ConditionalField(FieldLenField('lengthOfCertificateClient', None, fmt='H', length_of='certificateClient'), lambda pkt: pkt.subFunction in [0x01, 0x02]),
        ConditionalField(XStrLenField('certificateClient', b'', length_from=lambda p: p.lengthOfCertificateClient), lambda pkt: pkt.subFunction in [0x01, 0x02]),
        ConditionalField(FieldLenField('lengthOfProofOfOwnershipClient', None, fmt='H', length_of='proofOfOwnershipClient'), lambda pkt: pkt.subFunction in [0x03, 0x06, 0x07]),
        ConditionalField(XStrLenField('proofOfOwnershipClient', b'', length_from=lambda p: p.lengthOfProofOfOwnershipClient), lambda pkt: pkt.subFunction in [0x03, 0x06, 0x07]),
        ConditionalField(FieldLenField('lengthOfChallengeClient', None, fmt='H', length_of='challengeClient'), lambda pkt: pkt.subFunction in [0x01, 0x02, 0x06, 0x07]),
        ConditionalField(XStrLenField('challengeClient', b'', length_from=lambda p: p.lengthOfChallengeClient), lambda pkt: pkt.subFunction in [0x01, 0x02, 0x06, 0x07]),
        ConditionalField(FieldLenField('lengthOfEphemeralPublicKeyClient', None, fmt='H', length_of='ephemeralPublicKeyClient'), lambda pkt: pkt.subFunction == 0x03),
        ConditionalField(XStrLenField('ephemeralPublicKeyClient', b'', length_from=lambda p: p.lengthOfEphemeralPublicKeyClient), lambda pkt: pkt.subFunction == 0x03),
        ConditionalField(FieldLenField('lengthOfCertificateData', None, fmt='H', length_of='certificateData'), lambda pkt: pkt.subFunction == 0x04),
        ConditionalField(XStrLenField('certificateData', b'', length_from=lambda p: p.lengthOfCertificateData), lambda pkt: pkt.subFunction == 0x04),
        ConditionalField(FieldLenField('lengthOfAdditionalParameter', None, fmt='H', length_of='additionalParameter'), lambda pkt: pkt.subFunction in [0x06, 0x07]),
        ConditionalField(XStrLenField('additionalParameter', b'', length_from=lambda p: p.lengthOfAdditionalParameter), lambda pkt: pkt.subFunction in [0x06, 0x07]),
    ]
...
class UDS_AUTHPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XStrFixedLenField('algorithmIndicator', 0, length=16), lambda pkt: pkt.subFunction in [0x05, 0x06, 0x07]),
        ConditionalField(FieldLenField('lengthOfChallengeServer', None, fmt='H', length_of='challengeServer'), lambda pkt: pkt.subFunction in [0x01, 0x02, 0x05]),
        ConditionalField(XStrLenField('challengeServer', b'', length_from=lambda p: p.lengthOfChallengeServer), lambda pkt: pkt.subFunction in [0x01, 0x02, 0x05]),
        ConditionalField(FieldLenField('lengthOfCertificateServer', None, fmt='H', length_of='certificateServer'), lambda pkt: pkt.subFunction == 0x02),
        ConditionalField(XStrLenField('certificateServer', b'', length_from=lambda p: p.lengthOfCertificateServer), lambda pkt: pkt.subFunction == 0x02),
        ConditionalField(FieldLenField('lengthOfProofOfOwnershipServer', None, fmt='H', length_of='proofOfOwnershipServer'), lambda pkt: pkt.subFunction in [0x02, 0x07]),
        ConditionalField(XStrLenField('proofOfOwnershipServer', b'', length_from=lambda p: p.lengthOfProofOfOwnershipServer), lambda pkt: pkt.subFunction in [0x02, 0x07]),
        ConditionalField(FieldLenField('lengthOfSessionKeyInfo', None, fmt='H', length_of='sessionKeyInfo'), lambda pkt: pkt.subFunction in [0x03, 0x06, 0x07]),
        ConditionalField(XStrLenField('sessionKeyInfo', b'', length_from=lambda p: p.lengthOfSessionKeyInfo), lambda pkt: pkt.subFunction in [0x03, 0x06, 0x07]),
        ConditionalField(FieldLenField('lengthOfEphemeralPublicKeyServer', None, fmt='H', length_of='ephemeralPublicKeyServer'), lambda pkt: pkt.subFunction in [0x01, 0x02]),
        ConditionalField(XStrLenField('ephemeralPublicKeyServer', b'', length_from=lambda p: p.lengthOfEphemeralPublicKeyServer), lambda pkt: pkt.subFunction in [0x1, 0x02]),
        ConditionalField(FieldLenField('lengthOfNeededAdditionalParameter', None, fmt='H', length_of='neededAdditionalParameter'), lambda pkt: pkt.subFunction == 0x05),
        ConditionalField(XStrLenField('neededAdditionalParameter', b'', length_from=lambda p: p.lengthOfNeededAdditionalParameter), lambda pkt: pkt.subFunction == 0x05),
    ]
...
class UDS_TP(Packet):
    name = 'TesterPresent'
    fields_desc = [ByteField('subFunction', 0)]
...
class UDS_TPPR(Packet):
    ...
    fields_desc = [ByteField('zeroSubFunction', 0)]
...
class UDS_ATP(Packet):
    ...
    fields_desc = [ByteEnumField('timingParameterAccessType', 0, timingParameterAccessTypes), ConditionalField(StrField('timingParameterRequestRecord', b''), lambda pkt: pkt.timingParameterAccessType == 0x4)]
...
class UDS_ATPPR(Packet):
    ...
    fields_desc = [ByteEnumField('timingParameterAccessType', 0, UDS_ATP.timingParameterAccessTypes), ConditionalField(StrField('timingParameterResponseRecord', b''), lambda pkt: pkt.timingParameterAccessType == 0x3)]
...
class UDS_CDTCS(Packet):
    ...
    fields_desc = [ByteEnumField('DTCSettingType', 0, DTCSettingTypes), StrField('DTCSettingControlOptionRecord', b'')]
...
class UDS_CDTCSPR(Packet):
    ...
    fields_desc = [ByteEnumField('DTCSettingType', 0, UDS_CDTCS.DTCSettingTypes)]
...
class UDS_ROE(Packet):
    ...
    fields_desc = [ByteEnumField('eventType', 0, eventTypes), ByteField('eventWindowTime', 0), StrField('eventTypeRecord', b'')]
...
class UDS_ROEPR(Packet):
    ...
    fields_desc = [ByteEnumField('eventType', 0, UDS_ROE.eventTypes), ByteField('numberOfIdentifiedEvents', 0), ByteField('eventWindowTime', 0), StrField('eventTypeRecord', b'')]
...
class UDS_LCPR(Packet):
    ...
    fields_desc = [ByteEnumField('linkControlType', 0, UDS_LC.linkControlTypes)]
...
class UDS_RDBI(Packet):
    ...
    fields_desc = [FieldListField('identifiers', None, XShortEnumField('dataIdentifier', 0, dataIdentifiers))]
...
class UDS_RSDBIPR(Packet):
    ...
    fields_desc = [XShortEnumField('dataIdentifier', 0, UDS_RSDBI.dataIdentifiers), ByteField('scalingByte', 0), StrField('dataRecord', b'', fmt='B')]
...
class UDS_RDBPI(Packet):
    ...
    fields_desc = [ByteEnumField('transmissionMode', 0, transmissionModes), ByteEnumField('periodicDataIdentifier', 0, periodicDataIdentifiers), StrField('furtherPeriodicDataIdentifier', b'', fmt='B')]
...
class UDS_RDBPIPR(Packet):
    ...
    fields_desc = [ByteField('periodicDataIdentifier', 0), StrField('dataRecord', b'', fmt='B')]
...
class UDS_DDDI(Packet):
    ...
    subFunctions = {0x1: 'defineByIdentifier', 0x2: 'defineByMemoryAddress', 0x3: 'clearDynamicallyDefinedDataIdentifier'}
    fields_desc = [ByteEnumField('subFunction', 0, subFunctions), StrField('dataRecord', b'', fmt='B')]
...
class UDS_DDDIPR(Packet):
    ...
    fields_desc = [ByteEnumField('subFunction', 0, UDS_DDDI.subFunctions), XShortField('dynamicallyDefinedDataIdentifier', 0)]
...
class UDS_WDBI(Packet):
    ...
    fields_desc = [XShortEnumField('dataIdentifier', 0, UDS_RDBI.dataIdentifiers)]
...
class UDS_RDTCI(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteField('DTCSeverityMask', 0), lambda pkt: pkt.reportType in [0x07, 0x08]),
        ConditionalField(XByteField('DTCStatusMask', 0), lambda pkt: pkt.reportType in [0x01, 0x02, 0x07, 0x08, 0x0F, 0x11, 0x12, 0x13]),
        ConditionalField(ByteField('DTCHighByte', 0), lambda pkt: pkt.reportType in [0x3, 0x4, 0x6, 0x10, 0x09]),
        ConditionalField(ByteField('DTCMiddleByte', 0), lambda pkt: pkt.reportType in [0x3, 0x4, 0x6, 0x10, 0x09]),
        ConditionalField(ByteField('DTCLowByte', 0), lambda pkt: pkt.reportType in [0x3, 0x4, 0x6, 0x10, 0x09]),
        ConditionalField(ByteField('DTCSnapshotRecordNumber', 0), lambda pkt: pkt.reportType in [0x3, 0x4, 0x5]),
        ConditionalField(ByteField('DTCExtendedDataRecordNumber', 0), lambda pkt: pkt.reportType in [0x6, 0x10]),
    ]
...
class UDS_RDTCIPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XByteField('DTCStatusAvailabilityMask', 0), lambda pkt: pkt.reportType in [0x01, 0x07, 0x11, 0x12, 0x02, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x13, 0x15]),
        ConditionalField(ByteEnumField('DTCFormatIdentifier', 0, {0: 'ISO15031-6DTCFormat', 1: 'UDS-1DTCFormat', 2: 'SAEJ1939-73DTCFormat', 3: 'ISO11992-4DTCFormat'}), lambda pkt: pkt.reportType in [0x01, 0x07, 0x11, 0x12]),
        ConditionalField(ShortField('DTCCount', 0), lambda pkt: pkt.reportType in [0x01, 0x07, 0x11, 0x12]),
        ConditionalField(StrField('DTCAndStatusRecord', b''), lambda pkt: pkt.reportType in [0x02, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x13, 0x15]),
        ConditionalField(StrField('dataRecord', b''), lambda pkt: pkt.reportType in [0x03, 0x04, 0x05, 0x06, 0x08, 0x09, 0x10, 0x14]),
    ]
...
class UDS_RC(Packet):
    ...
    name = 'RoutineControl'
    fields_desc = [ByteEnumField('routineControlType', 0, routineControlTypes), XShortEnumField('routineIdentifier', 0, routineControlIdentifiers)]
...
class UDS_RCPR(Packet):
    ...
    fields_desc = [
        ...
        XShortEnumField('routineIdentifier', 0, UDS_RC.routineControlIdentifiers),
    ]
...
class UDS_RU(Packet):
    name = 'RequestUpload'
    fields_desc = [
        ByteEnumField('dataFormatIdentifier', 0, UDS_RD.dataFormatIdentifiers),
        BitField('memorySizeLen', 0, 4),
        BitField('memoryAddressLen', 0, 4),
        ConditionalField(XByteField('memoryAddress1', 0), lambda pkt: pkt.memoryAddressLen == 1),
        ConditionalField(XShortField('memoryAddress2', 0), lambda pkt: pkt.memoryAddressLen == 2),
...
class UDS_TD(Packet):
    name = 'TransferData'
    fields_desc = [ByteField('blockSequenceCounter', 0), StrField('transferRequestParameterRecord', b'', fmt='B')]
...
class UDS_TDPR(Packet):
    ...
    fields_desc = [ByteField('blockSequenceCounter', 0), StrField('transferResponseParameterRecord', b'', fmt='B')]
...
class UDS_RFT(Packet):
    ...
    modeOfOperations = {0x00: 'ISO/SAE Reserved', 0x01: 'Add File', 0x02: 'Delete File', 0x03: 'Replace File', 0x04: 'Read File', 0x05: 'Read Directory'}
    ...
    fields_desc = [
        ...
        FieldLenField('filePathAndNameLength', None, length_of='filePathAndName', fmt='H'),
        StrLenField('filePathAndName', b'', length_from=lambda p: p.filePathAndNameLength),
        ConditionalField(BitField('compressionMethod', 0, 4), lambda p: p.modeOfOperation not in [2, 5]),
        ConditionalField(BitField('encryptingMethod', 0, 4), lambda p: p.modeOfOperation not in [2, 5]),
        ConditionalField(FieldLenField('fileSizeParameterLength', None, fmt='B', length_of='fileSizeUnCompressed'), lambda p: UDS_RFT._contains_file_size(p)),
        ConditionalField(StrLenField('fileSizeUnCompressed', b'', length_from=lambda p: p.fileSizeParameterLength), lambda p: UDS_RFT._contains_file_size(p)),
        ConditionalField(StrLenField('fileSizeCompressed', b'', length_from=lambda p: p.fileSizeParameterLength), lambda p: UDS_RFT._contains_file_size(p)),
    ]
...
class UDS_RFTPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(FieldLenField('lengthFormatIdentifier', None, length_of='maxNumberOfBlockLength', fmt='B'), lambda p: p.modeOfOperation != 2),
        ConditionalField(StrLenField('maxNumberOfBlockLength', b'', length_from=lambda p: p.lengthFormatIdentifier), lambda p: p.modeOfOperation != 2),
        ConditionalField(BitField('compressionMethod', 0, 4), lambda p: p.modeOfOperation != 0x02),
        ConditionalField(BitField('encryptingMethod', 0, 4), lambda p: p.modeOfOperation != 0x02),
        ConditionalField(FieldLenField('fileSizeOrDirInfoParameterLength', None, length_of='fileSizeUncompressedOrDirInfoLength'), lambda p: p.modeOfOperation not in [1, 2, 3]),
        ConditionalField(StrLenField('fileSizeUncompressedOrDirInfoLength', b'', length_from=lambda p: p.fileSizeOrDirInfoParameterLength), lambda p: p.modeOfOperation not in [1, 2, 3]),
        ConditionalField(StrLenField('fileSizeCompressed', b'', length_from=lambda p: p.fileSizeOrDirInfoParameterLength), lambda p: p.modeOfOperation not in [1, 2, 3, 5]),
    ]
...
class UDS_IOCBI(Packet):
    ...
    dataIdentifiers = ObservableDict()
    fields_desc = [XShortEnumField('dataIdentifier', 0, dataIdentifiers), ByteField('controlOptionRecord', 0), StrField('controlEnableMaskRecord', b'', fmt='B')]
...
class UDS_IOCBIPR(Packet):
    ...
    fields_desc = [XShortField('dataIdentifier', 0), StrField('controlStatusRecord', b'', fmt='B')]
...
class UDS_NR(Packet):
    ...
    fields_desc = [XByteEnumField('requestServiceId', 0, UDS.services), ByteEnumField('negativeResponseCode', 0, negativeResponseCodes)]

    def answers(self, other):
        return self.requestServiceId == other.service and (self.negativeResponseCode != 0x78 or conf.contribs['UDS']['treat-response-pending-as-answer'])
        ...
        while not self._stopped.is_set() and not self._socket.closed:
            for p in self._pkts:
                try:
                    ...
                    log_automotive.exception('Exception in TesterPresentSender: %s', e)
                    break
