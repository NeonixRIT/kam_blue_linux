...
from scapy.contrib.automotive.xcp.cto_commands_slave import (
    ...
    TransportLayerCmdGetDAQIdResponse,
    SegmentInfoMode0PositiveResponse,
    SegmentInfoMode1PositiveResponse,
    SegmentInfoMode2PositiveResponse,
...
bind_layers(CTORequest, TransportLayerCmdGetSlaveId, pid=0xF2, sub_command_code=0xFF)
bind_layers(CTORequest, TransportLayerCmdGetDAQId, pid=0xF2, sub_command_code=0xFE)
bind_layers(CTORequest, TransportLayerCmdSetDAQId, pid=0xF2, sub_command_code=0xFD)
...
class DTO(Packet):
    ...
    fields_desc = [
        ConditionalField(XByteField('fill', 0x00), lambda _: identification_field_needs_alignment()),
        ConditionalField(StrLenField('daq', b'', length_from=lambda _: get_daq_length()), lambda _: get_daq_length() > 0),
        ConditionalField(StrLenField('timestamp', b'', length_from=lambda _: get_timestamp_length()), lambda _: get_timestamp_length() > 0),
        ConditionalField(StrLenField('data', b'', length_from=lambda _: get_daq_data_field_length()), lambda _: get_daq_data_field_length() > 0),
    ]
