...
from scapy.compat import Any, Union, List, Optional, Dict, Tuple, Set, Callable, TYPE_CHECKING
...
class AutomotiveTestCase(AutomotiveTestCaseABC):
    ...
    def check_kwargs(cls, kwargs):
        ...
        for k, v in kwargs.items():
            if k not in cls._supported_kwargs.keys():
                raise Scapy_Exception('Keyword-Argument %s not supported for %s' % (k, cls.__name__))
            ti, vf = cls._supported_kwargs[k]
            if ti is not None and not isinstance(v, ti):
                raise Scapy_Exception("Keyword-Value '%s' is not instance of type %s" % (k, str(ti)))
            if vf is not None and not vf(v):
                raise Scapy_Exception("Validation Error: '%s: %s' is not in the allowed " 'value range' % (k, str(v)))
    ...
    def _show_state_information(self, **kwargs):
        # type: (Any) -> str
        completed = [(state, self._state_completed[state]) for state in self.scanned_states]
        return make_lined_table(completed, lambda tup: ('Scan state completed', tup[0], tup[1]), dump=True) or ''
