...
from scapy.compat import Any, Union, List, Optional, Dict, Callable, Type, cast
...
from scapy.contrib.automotive.scanner.test_case import AutomotiveTestCaseABC, _SocketUnion, _CleanupCallable, StateGenerator, TestCaseGenerator, AutomotiveTestCase
...
class AutomotiveTestCaseExecutor:
    ...
    def __init__(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        self.configuration = AutomotiveTestCaseExecutorConfiguration(test_cases or self.default_test_case_clss, **kwargs)
    ...
    def state_paths(self):
        ...
        """
        paths = [Graph.dijkstra(self.state_graph, self._initial_ecu_state, s) for s in self.state_graph.nodes if s != self._initial_ecu_state]
        return sorted([p for p in paths if p] + [[self._initial_ecu_state]], key=lambda x: x[-1])
    ...
    def scan_completed(self):
        # type: () -> bool
        return all(t.has_completed(s) for t, s in product(self.configuration.test_cases, self.final_states))
    ...
    def reconnect(self):
        ...
        if self.reconnect_handler:
            try:
                ...
                log_automotive.exception("Exception '%s' during socket.close", e)
        ...
        if self.socket.closed:
            raise Scapy_Exception('Socket closed even after reconnect. Stop scan!')
    ...
    def execute_test_case(self, test_case, kill_time=None):
        ...
        test_case.pre_execute(self.socket, self.target_state, self.configuration)
        ...
        if kill_time:
            ...
            test_case_kwargs['execution_time'] = min(max_execution_time, cur_execution_time)

        log_automotive.debug('Execute test_case %s with args %s', test_case.__class__.__name__, test_case_kwargs)

        test_case.execute(self.socket, self.target_state, stop_event=self._stop_scan_event, **test_case_kwargs)
        test_case.post_execute(self.socket, self.target_state, self.configuration)
        ...
        if hasattr(test_case, 'runtime_estimation'):
            ...
            if estimation is not None:
                log_automotive.debug('[i] Test_case %s: TODO %d, ' 'DONE %d, TOTAL %0.2f', test_case.__class__.__name__, estimation[0], estimation[1], estimation[2])
    ...
    def scan(self, timeout=None):
        ...
        while kill_time > time.time():
            ...
            for p, test_case in product(self.state_paths, self.configuration.test_cases):
                log_automotive.info('Scan path %s', p)
                terminate = kill_time <= time.time()
                if terminate or self._stop_scan_event.is_set():
                    log_automotive.debug('Execution time exceeded. Terminating scan!')
                    break

                final_state = p[-1]
                if test_case.has_completed(final_state):
                    log_automotive.debug('State %s for %s completed', repr(final_state), test_case)
                    continue

                try:
                    if not self.enter_state_path(p):
                        log_automotive.error('Error entering path %s', p)
                        continue
                    log_automotive.info('Execute %s for path %s', str(test_case), p)
                    ...
                    if isinstance(e, OSError):
                        log_automotive.exception('OSError occurred, closing socket')
                        self.socket.close()
                    if cast(SuperSocket, self.socket).closed and self.reconnect_handler is None:
                        log_automotive.critical('Socket went down. Need to leave scan')
                        raise e
                finally:
                    self.cleanup_state()

            if not test_case_executed:
                log_automotive.info('Execute failure or scan completed. Exit scan!')
                break
    ...
    def enter_state_path(self, path):
        ...
        if path[0] != self._initial_ecu_state:
            raise Scapy_Exception('Initial state of path not equal reset state of the target')
    ...
    def enter_state(self, prev_state, next_state):
        ...
        state_changed = trans_func(self.socket, self.configuration, trans_kwargs)
        if state_changed:
        ...
        for f in self.cleanup_functions:
            ...
            try:
                if not f(self.socket, self.configuration):
                    log_automotive.info('Cleanup function %s failed', repr(f))
            except (OSError, ValueError, Scapy_Exception) as e:
