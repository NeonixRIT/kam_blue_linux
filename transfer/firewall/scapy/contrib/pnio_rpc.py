...
MAU_TYPE = {0x0000: 'Radio', 0x001E: '1000-BaseT-FD'}

MAU_EXTENSION = {0x0000: 'None', 0x0100: 'Polymeric-Optical-Fiber'}
...
MEDIA_TYPE = {0x00: 'Unknown', 0x01: 'Copper cable', 0x02: 'Fiber optic cable', 0x03: 'Radio communication'}
...
RPC_INTERFACE_UUID = {
    ...
    'UUID_IO_ControllerInterface': UUID('dea00002-6c97-11d1-8271-00a02442df7d'),
    'UUID_IO_SupervisorInterface': UUID('dea00003-6c97-11d1-8271-00a02442df7d'),
    'UUID_IO_ParameterServerInterface': UUID('dea00004-6c97-11d1-8271-00a02442df7d'),
}
...
ALARM_CR_TRANSPORT = {0x0: 'RTA_CLASS_1', 0x1: 'RTA_CLASS_UDP'}
...
class DiagnosisItem(AlarmItem):
    fields_desc = [
        ...
        ConditionalField(cond=lambda p: p.getfieldval('UserStructureIdentifier') in [0x8002, 0x8003], fld=XShortField('ExtChannelErrorType', 0)),
        ConditionalField(cond=lambda p: p.getfieldval('UserStructureIdentifier') in [0x8002, 0x8003], fld=XIntField('ExtChannelAddValue', 0)),
        ConditionalField(cond=lambda p: p.getfieldval('UserStructureIdentifier') == 0x8003, fld=XIntField('QualifiedChannelQualifier', 0)),
    ]
...
PE_OPERATIONAL_MODE.update({i: 'PE_EnergySavingMode_{}'.format(i) for i in range(0x1, 0x20)})
...
class PRAL_AlarmItem(AlarmItem):
    fields_desc = [
        ...
        StrLenField('PRAL_ReasonAddValue', '', length_from=lambda x: x.len - 10),
    ]
...
class NDRData(Packet):
    ...
    fields_desc = [
        EField(FieldLenField('args_length', None, fmt='I', length_of='blocks'), endianness_from=dce_rpc_endianess),
        EField(FieldLenField('max_count', None, fmt='I', length_of='blocks'), endianness_from=dce_rpc_endianess),
        EField(IntField('offset', 0), endianness_from=dce_rpc_endianess),
        EField(FieldLenField('actual_count', None, fmt='I', length_of='blocks'), endianness_from=dce_rpc_endianess),
...
class PNIOServiceReqPDU(Packet):
    ...
    fields_desc = [
        EField(FieldLenField('args_max', None, fmt='I', length_of='blocks'), endianness_from=dce_rpc_endianess),
        NDRData,
    ]
    overload_fields = {
        DceRpc4: {
...
class PNIOServiceResPDU(Packet):
    ...
    fields_desc = [
        EField(IntEnumField('status', 0, ['OK']), endianness_from=dce_rpc_endianess),
        NDRData,
    ]
    overload_fields = {
        DceRpc4: {
