...
class SebekHead(Packet):
    name = 'Sebek header'
    fields_desc = [XIntField('magic', 0xD0D0D0), ShortField('version', 1), ShortEnumField('type', 0, {'read': 0, 'write': 1, 'socket': 2, 'open': 3}), IntField('counter', 0), IntField('time_sec', 0), IntField('time_usec', 0)]
...
class SebekV3Sock(Packet):
    ...
    fields_desc = [
        ...
        ShortField('sport', 0),
        ShortEnumField('call', 0, {'bind': 2, 'connect': 3, 'listen': 4, 'accept': 5, 'sendmsg': 16, 'recvmsg': 17, 'sendto': 11, 'recvfrom': 12}),
