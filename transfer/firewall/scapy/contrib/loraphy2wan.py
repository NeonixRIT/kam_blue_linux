...
class DevAddrElem(Packet):
    name = 'DevAddrElem'
    fields_desc = [XByteField('NwkID', 0x0), LEX3BytesField('NwkAddr', b'\x00' * 3)]
...
class EIRP_DwellTime(Packet):
    name = 'EIRP_DwellTime'
    fields_desc = [BitField('RFU', 0b0, 2), BitEnumField('DownlinkDwellTime', 0b0, 1, DwellTimes), BitEnumField('UplinkDwellTime', 0b0, 1, DwellTimes), BitEnumField('MaxEIRP', 0b0000, 4, MaxEIRPs)]
...
class ADRparam(Packet):
    name = 'ADRparam'
    fields_desc = [BitField('Limit_exp', 0b0000, 4), BitField('Delay_exp', 0b0000, 4)]
...
class DeviceTimeAns(Packet):
    name = 'DeviceTimeAns'
    fields_desc = [IntField('SecondsSinceEpoch', 0), ByteField('FracSecond', 0x00)]
...
class MACCommand_up(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketListField('Reset', b'', ResetInd, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x01)),
        ConditionalField(PacketListField('LinkCheck', b'', LinkCheckReq, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x02)),
        ConditionalField(PacketListField('LinkADR', b'', LinkADRReq, length_from=lambda pkt: 4), lambda pkt: (pkt.CID == 0x03)),
        ConditionalField(PacketListField('DutyCycle', b'', DutyCycleReq, length_from=lambda pkt: 4), lambda pkt: (pkt.CID == 0x04)),
        ConditionalField(PacketListField('RXParamSetup', b'', RXParamSetupReq, length_from=lambda pkt: 4), lambda pkt: (pkt.CID == 0x05)),
        ConditionalField(PacketListField('DevStatus', b'', DevStatusReq, length_from=lambda pkt: 2), lambda pkt: (pkt.CID == 0x06)),
        ConditionalField(PacketListField('NewChannel', b'', NewChannelReq, length_from=lambda pkt: 5), lambda pkt: (pkt.CID == 0x07)),
        ConditionalField(PacketListField('RXTimingSetup', b'', RXTimingSetupReq, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x08)),
        # specific to 1.1 from here
        ConditionalField(PacketListField('TxParamSetup', b'', TxParamSetupReq, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x09)),
        ConditionalField(PacketListField('DlChannel', b'', DlChannelReq, length_from=lambda pkt: 4), lambda pkt: (pkt.CID == 0x0A)),
        ConditionalField(PacketListField('Rekey', b'', RekeyInd, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0B)),
        ConditionalField(PacketListField('ADRParamSetup', b'', ADRParamSetupReq, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0C)),
        ConditionalField(PacketListField('DeviceTime', b'', DeviceTimeReq, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x0D)),
        ConditionalField(PacketListField('ForceRejoin', b'', ForceRejoinReq, length_from=lambda pkt: 2), lambda pkt: (pkt.CID == 0x0E)),
        ConditionalField(PacketListField('RejoinParamSetup', b'', RejoinParamSetupReq, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0F)),
    ]
...
class MACCommand_down(Packet):
    ...
    fields_desc = [
        ByteEnumField('CID', 0, CIDs_up),
        ConditionalField(PacketListField('Reset', b'', ResetConf, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x01)),
        ConditionalField(PacketListField('LinkCheck', b'', LinkCheckAns, length_from=lambda pkt: 2), lambda pkt: (pkt.CID == 0x02)),
        ConditionalField(PacketListField('LinkADR', b'', LinkADRAns, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x03)),
        ConditionalField(PacketListField('DutyCycle', b'', DutyCycleAns, length_from=lambda pkt: 4), lambda pkt: (pkt.CID == 0x04)),
        ConditionalField(PacketListField('RXParamSetup', b'', RXParamSetupAns, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x05)),
        ConditionalField(PacketListField('DevStatusAns', b'', RXParamSetupAns, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x06)),
        ConditionalField(PacketListField('NewChannel', b'', NewChannelAns, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x07)),
        ConditionalField(PacketListField('RXTimingSetup', b'', RXTimingSetupAns, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x08)),
        ConditionalField(PacketListField('TxParamSetup', b'', TxParamSetupAns, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x09)),
        ConditionalField(PacketListField('DlChannel', b'', DlChannelAns, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0A)),
        ConditionalField(PacketListField('Rekey', b'', RekeyConf, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0B)),
        ConditionalField(PacketListField('ADRParamSetup', b'', ADRParamSetupAns, length_from=lambda pkt: 0), lambda pkt: (pkt.CID == 0x0C)),
        ConditionalField(PacketListField('DeviceTime', b'', DeviceTimeAns, length_from=lambda pkt: 5), lambda pkt: (pkt.CID == 0x0D)),
        ConditionalField(PacketListField('RejoinParamSetup', b'', RejoinParamSetupAns, length_from=lambda pkt: 1), lambda pkt: (pkt.CID == 0x0F)),
    ]
...
class FOpts(Packet):
    ...
    fields_desc = [
        ConditionalField(
            ...
            lambda pkt: (pkt.FCtrl[0].FOptsLen > 0 and pkt.MType & 0b1 == 0 and pkt.MType >= 0b010),
        ),
        ConditionalField(
            PacketListField(
                'FOpts_down',
            ...
            lambda pkt: (pkt.FCtrl[0].FOptsLen > 0 and pkt.MType & 0b1 == 1 and pkt.MType <= 0b101),
        ),
    ]
...
class FHDR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketListField('FCtrl', b'', FCtrl_Link, length_from=lambda pkt: 1), lambda pkt: ((pkt.MType & 0b1 == 1 and pkt.MType <= 0b101 and (pkt.MType & 0b10 > 0)) or (pkt.MType & 0b1 == 0 and pkt.MType >= 0b010))),
        ConditionalField(LEShortField('FCnt', 0), lambda pkt: (pkt.MType >= 0b010 and pkt.MType <= 0b101)),
        ConditionalField(
            PacketListField('FOpts_up', b'', MACCommand_up, length_from=lambda pkt: pkt.FCtrl[0].FOptsLen),  # noqa: E501
            FOptsUpShow,
        ),
        ConditionalField(
            PacketListField('FOpts_down', b'', MACCommand_down, length_from=lambda pkt: pkt.FCtrl[0].FOptsLen),  # noqa: E501
            FOptsDownShow,
        ),
    ]
...
JoinReqTypes = {0xFF: 'Join-request', 0x00: 'Rejoin-request type 0', 0x01: 'Rejoin-request type 1', 0x02: 'Rejoin-request type 2'}


class Join_Request(Packet):
    name = 'Join_Request'
    fields_desc = [StrFixedLenField('AppEUI', b'\x00' * 8, 8), StrFixedLenField('DevEUI', b'\00' * 8, 8), LEShortField('DevNonce', 0x0000)]
...
class Join_Accept(Packet):
    ...
    dcflist = False
    fields_desc = [
        LEX3BytesField('JoinAppNonce', 0),
        LEX3BytesField('NetID', 0),
        XLEIntField('DevAddr', 0),
        DLsettings,
        XByteField('RxDelay', 0),
        ConditionalField(
            StrFixedLenField('CFList', b'\x00' * 16, 16),  # noqa: E501
...
class FRMPayload(Packet):
    name = 'FRMPayload'
    fields_desc = [
        ConditionalField(MultipleTypeField(datapayload_list, StrField('DataPayload', '', remain=4)), lambda pkt: (dpload_type(pkt) is not None)),
        ConditionalField(PacketListField('Join_Request_Field', b'', Join_Request, length_from=lambda pkt: 18), lambda pkt: (pkt.MType == 0b000)),
        ConditionalField(PacketListField('Join_Accept_Field', b'', Join_Accept, count_from=lambda pkt: 1), lambda pkt: (pkt.MType == 0b001 and LoRa.encrypted is False)),
        ConditionalField(StrField('Join_Accept_Encrypted', 0), lambda pkt: (pkt.MType == 0b001 and LoRa.encrypted is True)),  # noqa: E501
        ConditionalField(
            PacketListField(
...
class MACPayload(Packet):
    name = 'MACPayload'
    eFPort = False
    fields_desc = [FHDR, ConditionalField(ByteEnumField('FPort', 0, FPorts), lambda pkt: (pkt.MType >= 0b010 and pkt.MType <= 0b101 and pkt.FCtrl[0].FOptsLen == 0)), FRMPayload]
...
class MHDR(Packet):  # Same for 1.0 as for 1.1
    ...
    fields_desc = [BitEnumField('MType', 0b000, 3, MTypes), BitField('RFU', 0b000, 3), BitField('Major', 0b00, 2)]


class PHYPayload(Packet):
    name = 'PHYPayload'
    fields_desc = [MHDR, MACPayload, ConditionalField(XIntField('MIC', 0), lambda pkt: (pkt.MType != 0b001 or LoRa.encrypted is False))]
...
class LoRa(Packet):  # default frame (unclear specs => taken from https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5677147/)  # noqa: E501
    ...
    fields_desc = [XBitField('Preamble', 0, 4), XBitField('PHDR', 0, 16), XBitField('PHDR_CRC', 0, 4), PHYPayload, ConditionalField(XShortField('CRC', 0), lambda pkt: (pkt.MType & 0b1 == 0))]
