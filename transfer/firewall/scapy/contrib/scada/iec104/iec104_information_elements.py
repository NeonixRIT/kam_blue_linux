...
from scapy.contrib.scada.iec104.iec104_fields import IEC60870_5_4_NormalizedFixPoint, IEC104SignedSevenBitValue, LESignedShortField, LEIEEEFloatField
from scapy.fields import BitEnumField, ByteEnumField, ByteField, ThreeBytesField, BitField, LEShortField, LESignedIntField
...
class IEC104_IE_CommonQualityFlags:
    ...
    IV_FLAG_VALID = 0
    IV_FLAG_INVALID = 1
    IV_FLAGS = {IV_FLAG_VALID: 'valid', IV_FLAG_INVALID: 'invalid'}

    NT_FLAG_CURRENT_VALUE = 0
    NT_FLAG_OLD_VALUE = 1
    NT_FLAGS = {NT_FLAG_CURRENT_VALUE: 'current value', NT_FLAG_OLD_VALUE: 'old value'}

    SB_FLAG_NOT_SUBSTITUTED = 0
    SB_FLAG_SUBSTITUTED = 1
    SB_FLAGS = {SB_FLAG_NOT_SUBSTITUTED: 'not substituted', SB_FLAG_SUBSTITUTED: 'substituted'}

    BL_FLAG_NOT_BLOCKED = 0
    BL_FLAG_BLOCKED = 1
    BL_FLAGS = {BL_FLAG_NOT_BLOCKED: 'not blocked', BL_FLAG_BLOCKED: 'blocked'}

    EI_FLAG_ELAPSED_TIME_VALID = 0
    EI_FLAG_ELAPSED_TIME_INVALID = 1
    EI_FLAGS = {EI_FLAG_ELAPSED_TIME_VALID: 'elapsed time valid', EI_FLAG_ELAPSED_TIME_INVALID: 'elapsed time invalid'}
...
class IEC104_IE_SIQ(IEC104_IE_CommonQualityFlags):
    ...
    SPI_FLAGS = {SPI_FLAG_STATE_OFF: 'off', SPI_FLAG_STATE_ON: 'on'}
...
class IEC104_IE_DIQ(IEC104_IE_CommonQualityFlags):
    ...
    DPI_FLAGS = {DPI_FLAG_STATE_UNDEFINED_OR_TRANSIENT: 'undefined/transient', DPI_FLAG_STATE_OFF: 'off', DPI_FLAG_STATE_ON: 'on', DPI_FLAG_STATE_UNDEFINED: 'undefined'}
...
class IEC104_IE_QDS(IEC104_IE_CommonQualityFlags):
    ...
    OV_FLAG_OVERFLOW = 1
    OV_FLAGS = {OV_FLAG_NO_OVERFLOW: 'no overflow', OV_FLAG_OVERFLOW: 'overflow'}
...
class IEC104_IE_VTI:
    ...
    TRANSIENT_STATE_FLAGS = {TRANSIENT_STATE_DISABLED: 'device not in transient state', TRANSIENT_STATE_ENABLED: 'device in transient state'}

    informantion_element_fields = [BitEnumField('transient_state', 0, 1, TRANSIENT_STATE_FLAGS), IEC104SignedSevenBitValue('value', 0)]
...
class IEC104_IE_NVA:
    ...
    informantion_element_fields = [IEC60870_5_4_NormalizedFixPoint('normed_value', 0)]
...
class IEC104_IE_SVA:
    ...
    informantion_element_fields = [LESignedShortField('scaled_value', 0)]
...
class IEC104_IE_R32_IEEE_STD_754:
    ...
    informantion_element_fields = [LEIEEEFloatField('scaled_value', 0)]
...
class IEC104_IE_BCR:
    ...
    CA_FLAGS = {CA_FLAG_COUNTER_NOT_ADJUSTED: 'counter not adjusted', CA_FLAG_COUNTER_ADJUSTED: 'counter adjusted'}

    CY_FLAG_NO_OVERFLOW = 0
    CY_FLAG_OVERFLOW = 1
    CY_FLAGS = {CY_FLAG_NO_OVERFLOW: 'no overflow', CY_FLAG_OVERFLOW: 'overflow'}
...
class IEC104_IE_SPE:
    ...
    GS_FLAGS = {GS_FLAG_NO_GENERAL_TRIGGER: 'general trigger', GS_FLAG_GENERAL_TRIGGER: 'no general trigger'}
    ...
    SL_FLAGS = {SL_FLAG_START_OPR_PHASE_L1_NO_TRIGGER: 'no start of operation', SL_FLAG_START_OPR_PHASE_L1_TRIGGER: 'start of operation'}
    ...
    SIE_FLAGS = {SIE_FLAG_START_OPR_PHASE_IE_NO_TRIGGER: 'no start of operation', SIE_FLAG_START_OPR_PHASE_IE_TRIGGER: 'start of operation'}

    # direction of the started protection event
    SRD_FLAG_DIRECTION_FORWARD = 0
    SRD_FLAG_DIRECTION_BACKWARD = 1
    SRD_FLAGS = {SRD_FLAG_DIRECTION_FORWARD: 'forward', SRD_FLAG_DIRECTION_BACKWARD: 'backward'}
...
class IEC104_IE_OCI:
    ...
    GC_FLAGS = {GC_FLAG_NO_GENERAL_COMMAND_OFF: 'no general off', GC_FLAG_GENERAL_COMMAND_OFF: 'general off'}
    ...
    CL_FLAGS = {CL_FLAG_NO_COMMAND_L1_OFF: 'no command off', CL_FLAG_COMMAND_L1_OFF: 'no command off'}
...
class IEC104_IE_BSI:
    ...
    informantion_element_fields = [BitField('bsi', 0, 32)]
...
class IEC104_IE_FBP:
    ...
    informantion_element_fields = [LEShortField('fbp', 0)]
...
class IEC104_IE_QOC:
    ...
    GENERATED_ATTRIBUTES = [
        ('QU_FLAG_RESERVED_COMPATIBLE', 'reserved - compatible', QU_FLAGS, 4, 8),
        ('QU_FLAG_RESERVED_PREDEFINED_FUNCTION', 'reserved - predefined function', QU_FLAGS, 9, 15),
        ('QU_FLAG_RESERVED_PRIVATE', 'reserved - private', QU_FLAGS, 16, 31),
    ]

    SE_FLAG_EXECUTE = 0
    SE_FLAG_SELECT = 1
    SE_FLAGS = {SE_FLAG_EXECUTE: 'execute', SE_FLAG_SELECT: 'select'}

    informantion_element_fields = [BitEnumField('s_or_e', 0, 1, SE_FLAGS), BitEnumField('qu', 0, 5, QU_FLAGS)]
...
class IEC104_IE_SCO(IEC104_IE_QOC):
    ...
    SCS_FLAG_STATE_ON = 1
    SCS_FLAGS = {SCS_FLAG_STATE_OFF: 'off', SCS_FLAG_STATE_ON: 'on'}

    informantion_element_fields = IEC104_IE_QOC.informantion_element_fields + [BitField('reserved', 0, 1), BitEnumField('scs', 0, 1, SCS_FLAGS)]
...
class IEC104_IE_DCO(IEC104_IE_QOC):
    ...
    informantion_element_fields = IEC104_IE_QOC.informantion_element_fields + [BitEnumField('dcs', 0, 2, DCS_FLAGS)]
...
class IEC104_IE_RCO(IEC104_IE_QOC):
    ...
    informantion_element_fields = IEC104_IE_QOC.informantion_element_fields + [BitEnumField('rcs', 0, 2, RCO_FLAGS)]
...
class IEC104_IE_CP56TIME2A(IEC104_IE_CommonQualityFlags):
    ...
    GEN_FLAGS = {GEN_FLAG_REALTIME: 'real time', GEN_FLAG_SUBSTITUTED_TIME: 'substituted time'}

    SU_FLAG_NORMAL_TIME = 0
    SU_FLAG_SUMMER_TIME = 1
    SU_FLAGS = {SU_FLAG_NORMAL_TIME: 'normal time', SU_FLAG_SUMMER_TIME: 'summer time'}
...
class IEC104_IE_CP56TIME2A_START_TIME(IEC104_IE_CP56TIME2A):
    ...
    informantion_element_fields = [
        ...
        BitEnumField('start_weekday', 0, 3, IEC104_IE_CP56TIME2A.WEEK_DAY_FLAGS),
...
class IEC104_IE_CP56TIME2A_STOP_TIME(IEC104_IE_CP56TIME2A):
    ...
    informantion_element_fields = [
        ...
        BitEnumField('stop_weekday', 0, 3, IEC104_IE_CP56TIME2A.WEEK_DAY_FLAGS),
...
class IEC104_IE_CP24TIME2A(IEC104_IE_CP56TIME2A):
    ...
    informantion_element_fields = IEC104_IE_CP56TIME2A.informantion_element_fields[:4]
...
class IEC104_IE_CP16TIME2A:
    ...
    informantion_element_fields = [LEShortField('sec_milli', 0)]
...
class IEC104_IE_CP16TIME2A_ELAPSED:
    ...
    informantion_element_fields = [LEShortField('elapsed_sec_milli', 0)]
...
class IEC104_IE_CP16TIME2A_PROTECTION_ACTIVE:
    ...
    informantion_element_fields = [LEShortField('prot_act_sec_milli', 0)]
...
class IEC104_IE_CP16TIME2A_PROTECTION_COMMAND:
    ...
    informantion_element_fields = [LEShortField('prot_cmd_sec_milli', 0)]
...
class IEC104_IE_COI:
    ...
    LPC_FLAGS = {LPC_FLAG_LOCAL_PARAMETER_UNCHANGED: 'unchanged', LPC_FLAG_LOCAL_PARAMETER_CHANGED: 'changed'}

    COI_FLAG_LOCAL_POWER_ON = 0
    COI_FLAG_LOCAL_MANUAL_RESET = 1
    COI_FLAG_REMOTE_RESET = 2

    COI_FLAGS = {COI_FLAG_LOCAL_POWER_ON: 'local power on', COI_FLAG_LOCAL_MANUAL_RESET: 'manual reset', COI_FLAG_REMOTE_RESET: 'remote reset'}

    GENERATED_ATTRIBUTES = [('COI_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', COI_FLAGS, 3, 31), ('COI_FLAG_PRIVATE_RESERVED', 'private reserved', COI_FLAGS, 32, 127)]

    informantion_element_fields = [BitEnumField('local_param_state', 0, 1, LPC_FLAGS), BitEnumField('coi', 0, 7, COI_FLAGS)]
...
class IEC104_IE_QOI:
    ...
    GENERATED_ATTRIBUTES = [
        ('QOI_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', QOI_FLAGS, 1, 19),
        ('QOI_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', QOI_FLAGS, 37, 63),
        ('QOI_FLAG_PRIVATE_RESERVED', 'private reserved', QOI_FLAGS, 64, 255),
    ]

    informantion_element_fields = [ByteEnumField('qoi', 0, QOI_FLAGS)]
...
class IEC104_IE_QCC:
    ...
    RQT_FLAGS = {
        ...
        RQT_FLAG_GROUP_1_COUNTER_INTERROGATION: 'counter group 1 ' 'interrogation',
        RQT_FLAG_GROUP_2_COUNTER_INTERROGATION: 'counter group 2 ' 'interrogation',
        RQT_FLAG_GROUP_3_COUNTER_INTERROGATION: 'counter group 3 ' 'interrogation',
        RQT_FLAG_GROUP_4_COUNTER_INTERROGATION: 'counter group 4 ' 'interrogation',
        RQT_FLAG_GENERAL_COUNTER_INTERROGATION: 'general counter ' 'interrogation',
    }

    GENERATED_ATTRIBUTES = [
        ('RQT_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', RQT_FLAGS, 6, 31),
    ...
    FRZ_FLAGS = {FRZ_FLAG_QUERY: 'query', FRZ_FLAG_SAVE_COUNTER_WITHOUT_RESET: 'save counter, no counter reset', FRZ_FLAG_SAVE_COUNTER_AND_RESET: 'save counter and reset counter', FRZ_FLAG_COUNTER_RESET: 'reset counter'}

    informantion_element_fields = [BitEnumField('frz', 0, 2, FRZ_FLAGS), BitEnumField('rqt', 0, 6, RQT_FLAGS)]
...
class IEC104_IE_QPM:
    ...
    GENERATED_ATTRIBUTES = [('KPA_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', KPA_FLAGS, 5, 31), ('KPA_FLAG_PRIVATE_RESERVED', 'private reserved', KPA_FLAGS, 32, 63)]

    LPC_FLAG_LOCAL_PARAMETER_MOT_CHANGED = 0
    LPC_FLAG_LOCAL_PARAMETER_CHANGED = 1
    LPC_FLAGS = {LPC_FLAG_LOCAL_PARAMETER_MOT_CHANGED: 'local parameter not changed', LPC_FLAG_LOCAL_PARAMETER_CHANGED: 'local parameter changed'}
...
class IEC104_IE_QPA:
    ...
    QPA_FLAGS = {
        ...
        QPA_FLAG_ACT_DEACT_LOADED_PARAM_OA_0: 'act/deact loaded parameters ' 'for object addr 0',
        QPA_FLAG_ACT_DEACT_LOADED_PARAM: 'act/deact loaded parameters for ' 'given object addr',
        QPA_FLAG_ACT_DEACT_CYCLIC_TX: 'act/deact cyclic transfer of object ' 'given by object addr',
    }

    GENERATED_ATTRIBUTES = [('QPA_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', QPA_FLAGS, 4, 127), ('QPA_FLAG_PRIVATE_RESERVED', 'private reserved', QPA_FLAGS, 128, 255)]

    informantion_element_fields = [ByteEnumField('qpa', 0, QPA_FLAGS)]
...
class IEC104_IE_QRP:
    ...
    QRP_FLAGS = {QRP_FLAG_UNUSED: 'unsued', QRP_FLAG_GENERAL_PROCESS_RESET: 'general process reset', QRP_FLAG_RESET_EVENT_BUFFER: 'reset event buffer'}

    GENERATED_ATTRIBUTES = [
        ('QRP_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', QRP_FLAGS, 3, 127),
        ('QRP_FLAG_PRIVATE_RESERVED', 'private reserved', QRP_FLAGS, 128, 255),
    ]

    informantion_element_fields = [ByteEnumField('qrp', 0, QRP_FLAGS)]
...
class IEC104_IE_FRQ:
    ...
    FR_FLAGS = {FR_FLAG_UNUSED: 'unused'}

    GENERATED_ATTRIBUTES = [
        ('FR_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', FR_FLAGS, 1, 63),
        ('FR_FLAG_PRIVATE_RESERVED', 'private reserved', FR_FLAGS, 64, 127),
    ]

    FRACK_FLAG_POSITIVE_ACK = 0
    FRACK_FLAG_NEGATIVE_ACK = 1
    FRACK_FLAGS = {FRACK_FLAG_POSITIVE_ACK: 'positive ack', FRACK_FLAG_NEGATIVE_ACK: 'negative ack'}

    informantion_element_fields = [BitEnumField('fr_ack', 0, 1, FRACK_FLAGS), BitEnumField('fr', 0, 7, FR_FLAGS)]
...
class IEC104_IE_SRQ:
    ...
    SR_FLAGS = {SR_FLAG_UNUSED: 'unused'}

    GENERATED_ATTRIBUTES = [
        ('SR_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', SR_FLAGS, 1, 63),
        ('SR_FLAG_PRIVATE_RESERVED', 'private reserved', SR_FLAGS, 64, 127),
    ]

    SLOAD_FLAG_SECTION_READY = 0
    SLOAD_FLAG_SECTION_NOT_READY = 1
    SLAOD_FLAGS = {SLOAD_FLAG_SECTION_READY: 'section ready', SLOAD_FLAG_SECTION_NOT_READY: 'section not ready'}

    informantion_element_fields = [BitEnumField('section_load_state', 0, 1, SLAOD_FLAGS), BitEnumField('sr', 0, 7, SR_FLAGS)]
...
class IEC104_IE_SCQ:
    ...
    SEL_CALL_ERR_FLAGS = {
        ...
        SEL_CALL_ERR_FLAG_REQ_MEM_AREA_NO_AVAIL: 'requested memory area ' 'not available',
        SEL_CALL_ERR_FLAG_INVALID_CHECKSUM: 'invalid checksum',
        SEL_CALL_ERR_FLAG_UNEXPECTED_COMMUNICATION_SERVICE: 'unexpected ' 'communication ' 'service',
    ...
    GENERATED_ATTRIBUTES = [
        ('SEL_CALL_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', SEL_CALL_FLAGS, 8, 10),
        ('SEL_CALL_FLAG_PRIVATE_RESERVED', 'private reserved', SEL_CALL_FLAGS, 11, 15),
        ('SEL_CALL_ERR_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', SEL_CALL_ERR_FLAGS, 6, 10),
        ('SEL_CALL_ERR_FLAG_PRIVATE_RESERVED', 'private reserved', SEL_CALL_ERR_FLAGS, 11, 15),
    ]

    informantion_element_fields = [BitEnumField('errors', 0, 4, SEL_CALL_ERR_FLAGS), BitEnumField('select_call', 0, 4, SEL_CALL_FLAGS)]
...
class IEC104_IE_LSQ:
    ...
    GENERATED_ATTRIBUTES = [
        ('LSQ_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', LSQ_FLAGS, 5, 127),
        ('LSQ_FLAG_PRIVATE_RESERVED', 'private reserved', LSQ_FLAGS, 128, 255),
    ]

    informantion_element_fields = [ByteEnumField('lsq', 0, LSQ_FLAGS)]
...
class IEC104_IE_AFQ:
    ...
    ACK_FILE_OR_SEC_FLAGS = {
        ...
        ACK_FILE_OR_SEC_FLAG_POSITIVE_ACK_FILE_TRANSFER: 'positive acknowledge' ' file transfer',
        ACK_FILE_OR_SEC_FLAG_NEGATIVE_ACK_FILE_TRANSFER: 'negative acknowledge' ' file transfer',
        ACK_FILE_OR_SEC_FLAG_POSITIVE_ACK_SECTION_TRANSFER: 'positive ' 'acknowledge ' 'section transfer',
        ACK_FILE_OR_SEC_FLAG_NEGATIVE_ACK_SECTION_TRANSFER: 'negative ' 'acknowledge ' 'section transfer',
    }
    ...
    ACK_FILE_OR_SEC_ERR_FLAGS = {
        ...
        ACK_FILE_OR_SEC_ERR_FLAG_REQ_MEM_AREA_NO_AVAIL: 'requested memory ' 'area not available',
        ACK_FILE_OR_SEC_ERR_FLAG_INVALID_CHECKSUM: 'invalid checksum',
        ACK_FILE_OR_SEC_ERR_FLAG_UNEXPECTED_COMMUNICATION_SERVICE: 'unexpected' ' communica' 'tion ' 'service',
        ACK_FILE_OR_SEC_ERR_FLAG_UNEXPECTED_FILENAME: 'unexpected file name',
        ACK_FILE_OR_SEC_ERR_FLAG_UNEXPECTED_SECTION_NAME: 'unexpected ' 'section name',
    }

    GENERATED_ATTRIBUTES = [
        ('ACK_FILE_OR_SEC_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', ACK_FILE_OR_SEC_FLAGS, 5, 10),
        ('ACK_FILE_OR_SEC_FLAG_PRIVATE_RESERVED', 'private reserved', ACK_FILE_OR_SEC_FLAGS, 11, 15),
        ('ACK_FILE_OR_SEC_ERR_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', ACK_FILE_OR_SEC_ERR_FLAGS, 6, 10),
        ('ACK_FILE_OR_SEC_ERR_FLAG_PRIVATE_RESERVED', 'private reserved', ACK_FILE_OR_SEC_ERR_FLAGS, 11, 15),
    ]

    informantion_element_fields = [BitEnumField('errors', 0, 4, ACK_FILE_OR_SEC_ERR_FLAGS), BitEnumField('ack_file_or_sec', 0, 4, ACK_FILE_OR_SEC_FLAGS)]
...
class IEC104_IE_NOF:
    ...
    informantion_element_fields = [LEShortField('file_name', 0)]
...
class IEC104_IE_NOS:
    ...
    informantion_element_fields = [ByteField('section_name', 0)]
...
class IEC104_IE_LOF:
    ...
    informantion_element_fields = [ThreeBytesField('file_length', 0)]
...
class IEC104_IE_LOS:
    ...
    informantion_element_fields = [ByteField('segment_length', 0)]
...
class IEC104_IE_CHS:
    ...
    informantion_element_fields = [ByteField('checksum', 0)]
...
class IEC104_IE_SOF:
    ...
    STATUS_FLAGS = {STATUS_FLAG_UNUSED: 'unused'}

    GENERATED_ATTRIBUTES = [('STATUS_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', STATUS_FLAGS, 1, 15), ('STATUS_FLAG_PRIVATE_RESERVED', 'private reserved', STATUS_FLAGS, 16, 32)]

    LFD_FLAG_NEXT_FILE_OF_DIR_FOLLOWS = 0
    LFD_FLAG_LAST_FILE_OF_DIR = 1
    LFD_FLAGS = {LFD_FLAG_NEXT_FILE_OF_DIR_FOLLOWS: 'next file of dir follows', LFD_FLAG_LAST_FILE_OF_DIR: 'last file of dir'}

    FOR_FLAG_NAME_DEFINES_FILE = 0
    FOR_FLAG_NAME_DEFINES_SUBDIR = 1
    FOR_FLAGS = {FOR_FLAG_NAME_DEFINES_FILE: 'name defines file', FOR_FLAG_NAME_DEFINES_SUBDIR: 'name defines subdirectory'}

    FA_FLAG_FILE_WAITS_FOR_TRANSFER = 0
    FA_FLAG_FILE_TRANSFER_IS_ACTIVE = 1
    FA_FLAGS = {FA_FLAG_FILE_WAITS_FOR_TRANSFER: 'file waits for transfer', FA_FLAG_FILE_TRANSFER_IS_ACTIVE: 'transfer of file active'}

    informantion_element_fields = [BitEnumField('fa', 0, 1, FA_FLAGS), BitEnumField('for_', 0, 1, FOR_FLAGS), BitEnumField('lfd', 0, 1, LFD_FLAGS), BitEnumField('status', 0, 5, STATUS_FLAGS)]
...
class IEC104_IE_QOS:
    ...
    QL_FLAGS = {QL_FLAG_UNUSED: 'unused'}

    GENERATED_ATTRIBUTES = [('QL_FLAG_COMPATIBLE_RESERVED', 'compatible reserved', QL_FLAGS, 1, 63), ('QL_FLAG_PRIVATE_RESERVED', 'private reserved', QL_FLAGS, 64, 127)]

    SE_FLAG_EXECUTE = 0
    SE_FLAG_SELECT = 1
    SE_FLAGS = {SE_FLAG_EXECUTE: 'execute', SE_FLAG_SELECT: 'select'}

    informantion_element_fields = [BitEnumField('action', 0, 1, SE_FLAGS), BitEnumField('ql', 0, 7, QL_FLAGS)]
...
class IEC104_IE_SCD:
    ...
    CD_FLAGS = {CD_FLAG_STATE_NOT_CHANGED: 'state not changed', CD_FLAG_STATE_CHANGED: 'state changed'}
...
class IEC104_IE_TSC:
    ...
    informantion_element_fields = [LEShortField('tsc', 0)]
