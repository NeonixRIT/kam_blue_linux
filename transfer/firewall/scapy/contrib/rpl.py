...
from scapy.fields import ByteEnumField, ByteField, IP6Field, ShortField, BitField, BitEnumField, FieldLenField, StrLenField, IntField, ConditionalField
from scapy.layers.inet6 import ICMPv6RPL, icmp6ndraprefs, _IP6PrefixField


# https://www.iana.org/assignments/rpl/rpl.xhtml#mop
RPLMOP = {0: 'No Downward routes', 1: 'Non-Storing', 2: 'Storing with no multicast support', 3: 'Storing with multicast support', 4: 'P2P Route Discovery'}
...
class RPLOptRIO(_RPLGuessOption):
    ...
    fields_desc = [
        ByteEnumField('otype', 3, RPLOPTSSTR),
        FieldLenField('len', None, length_of='prefix', fmt='B', adjust=lambda pkt, x: x + 6),
        ByteField('plen', None),
        BitField('res1', 0, 3),
...
class RPLOptDODAGConfig(_RPLGuessOption):
    ...
    fields_desc = [
        ByteEnumField('otype', 4, RPLOPTSSTR),
        ByteField('len', 14),
        BitField('flags', 0, 4),
        BitField('A', 0, 1),
        BitField('PCS', 0, 3),
...
class RPLOptTgt(_RPLGuessOption):
    ...
    name = 'RPL Target'
    fields_desc = [ByteEnumField('otype', 5, RPLOPTSSTR), FieldLenField('len', None, length_of='prefix', fmt='B', adjust=lambda pkt, x: x + 2), ByteField('flags', 0), ByteField('plen', 0), _IP6PrefixField('prefix', None)]
...
class RPLOptTIO(_RPLGuessOption):
    ...
    fields_desc = [
        ByteEnumField('otype', 6, RPLOPTSSTR),
        FieldLenField('len', None, length_of='parentaddr', fmt='B', adjust=lambda pkt, x: x + 4),
        BitField('E', 0, 1),
        BitField('flags', 0, 7),
...
class RPLOptSolInfo(_RPLGuessOption):
    ...
    fields_desc = [
        ByteEnumField('otype', 7, RPLOPTSSTR),
        ByteField('len', 19),
...
class RPLOptPIO(_RPLGuessOption):
    ...
    fields_desc = [
        ByteEnumField('otype', 8, RPLOPTSSTR),
        ByteField('len', 30),
        ByteField('plen', 64),
        BitField('L', 0, 1),
        BitField('A', 0, 1),
...
class RPLOptTgtDesc(_RPLGuessOption):
    ...
    fields_desc = [ByteEnumField('otype', 9, RPLOPTSSTR), ByteField('len', 4), IntField('descriptor', 0)]
...
class RPLOptPadN(_RPLGuessOption):
    ...
    name = 'PadN'
    fields_desc = [ByteEnumField('otype', 0x01, RPLOPTSSTR), FieldLenField('optlen', None, length_of='optdata', fmt='B'), StrLenField('optdata', '', length_from=lambda pkt: pkt.optlen)]
...
class RPLDIS(_RPLGuessMsgType, _RPLGuessOption):
    ...
    fields_desc = [ByteField('flags', 0), ByteField('reserved', 0)]
...
class RPLDAO(_RPLGuessMsgType, _RPLGuessOption):
    ...
    fields_desc = [ByteField('RPLInstanceID', 50), BitField('K', 0, 1), BitField('D', 0, 1), BitField('flags', 0, 6), ByteField('reserved', 0), ByteField('daoseq', 1), ConditionalField(IP6Field('dodagid', None), lambda pkt: pkt.D == 1)]
...
class RPLDAOACK(_RPLGuessMsgType, _RPLGuessOption):
    ...
    fields_desc = [ByteField('RPLInstanceID', 50), BitField('D', 0, 1), BitField('reserved', 0, 7), ByteField('daoseq', 1), ByteField('status', 0), ConditionalField(IP6Field('dodagid', None), lambda pkt: pkt.D == 1)]
...
class RPLDCO(_RPLGuessMsgType, _RPLGuessOption):
    ...
    fields_desc = [ByteField('RPLInstanceID', 50), BitField('K', 0, 1), BitField('D', 0, 1), BitField('flags', 0, 6), ByteField('status', 0), ByteField('dcoseq', 1), ConditionalField(IP6Field('dodagid', None), lambda pkt: pkt.D == 1)]
...
class RPLDCOACK(_RPLGuessMsgType, _RPLGuessOption):
    ...
    fields_desc = [ByteField('RPLInstanceID', 50), BitField('D', 0, 1), BitField('flags', 0, 7), ByteField('dcoseq', 1), ByteField('status', 0), ConditionalField(IP6Field('dodagid', None), lambda pkt: pkt.D == 1)]
