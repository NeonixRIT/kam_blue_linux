...
class IFETlv(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField('length', None, length_of='value', adjust=lambda pkt, x: x + 4),
        MultipleTypeField(
            [
                (PadField(ShortField('value', 0), 4, padwith=b'\x00'), lambda pkt: pkt.type in IFE_TYPES_SHORT),
                (PadField(IntField('value', 0), 4, padwith=b'\x00'), lambda pkt: pkt.type in IFE_TYPES_INT),
            ],
...
class IFE(Packet):
    ...
    fields_desc = [
        FieldLenField('mdlen', None, length_of='tlvs', adjust=lambda pkt, x: x + 2),
