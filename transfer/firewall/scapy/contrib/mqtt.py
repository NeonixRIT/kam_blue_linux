...
from scapy.fields import FieldLenField, BitEnumField, StrLenField, ShortField, ConditionalField, ByteEnumField, ByteField, PacketListField
...
QOS_LEVEL = {0: 'At most once delivery', 1: 'At least once delivery', 2: 'Exactly once delivery'}
...
class MQTT(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField('DUP', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('QOS', 0, 2, QOS_LEVEL),
        BitEnumField('RETAIN', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
...
PROTOCOL_LEVEL = {3: 'v3.1', 4: 'v3.1.1', 5: 'v5.0'}
...
class MQTTConnect(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField('usernameflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('passwordflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('willretainflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('willQOSflag', 0, 2, QOS_LEVEL),
        BitEnumField('willflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('cleansess', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        BitEnumField('reserved', 0, 1, {0: 'Disabled', 1: 'Enabled'}),
        ShortField('klive', 0),
        ...
        ConditionalField(FieldLenField('wtoplen', None, length_of='willtopic'), lambda pkt: pkt.willflag == 1),
        ConditionalField(StrLenField('willtopic', '', length_from=lambda pkt: pkt.wtoplen), lambda pkt: pkt.willflag == 1),
        ConditionalField(FieldLenField('wmsglen', None, length_of='willmsg'), lambda pkt: pkt.willflag == 1),
        ConditionalField(StrLenField('willmsg', '', length_from=lambda pkt: pkt.wmsglen), lambda pkt: pkt.willflag == 1),
        ConditionalField(FieldLenField('userlen', None, length_of='username'), lambda pkt: pkt.usernameflag == 1),
        ConditionalField(StrLenField('username', '', length_from=lambda pkt: pkt.userlen), lambda pkt: pkt.usernameflag == 1),
        ConditionalField(FieldLenField('passlen', None, length_of='password'), lambda pkt: pkt.passwordflag == 1),
        ConditionalField(StrLenField('password', '', length_from=lambda pkt: pkt.passlen), lambda pkt: pkt.passwordflag == 1),
    ]
...
RETURN_CODE = {0: 'Connection Accepted', 1: 'Unacceptable protocol version', 2: 'Identifier rejected', 3: 'Server unavailable', 4: 'Bad username/password', 5: 'Not authorized'}
...
class MQTTPublish(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ShortField('msgid', None), lambda pkt: (pkt.underlayer.QOS == 1 or pkt.underlayer.QOS == 2)),
        StrLenField('value', '', length_from=lambda pkt: pkt.underlayer.len - pkt.length - 2 if pkt.underlayer.QOS == 0 else pkt.underlayer.len - pkt.length - 4),
    ]
...
class MQTTTopic(Packet):
    name = 'MQTT topic'
    fields_desc = [FieldLenField('length', None, length_of='topic'), StrLenField('topic', '', length_from=lambda pkt: pkt.length)]
...
class MQTTSubscribe(Packet):
    name = 'MQTT subscribe'
    fields_desc = [ShortField('msgid', None), PacketListField('topics', [], pkt_cls=MQTTTopicQOS)]


ALLOWED_RETURN_CODE = {0: 'Success', 1: 'Success', 2: 'Success', 128: 'Failure'}


class MQTTSuback(Packet):
    name = 'MQTT suback'
    fields_desc = [ShortField('msgid', None), ByteEnumField('retcode', None, ALLOWED_RETURN_CODE)]


class MQTTUnsubscribe(Packet):
    name = 'MQTT unsubscribe'
    fields_desc = [ShortField('msgid', None), PacketListField('topics', [], pkt_cls=MQTTTopic)]
