...
class ModbusPDU01ReadCoilsResponse(_ModbusPDUNoPayload):
    ...
    fields_desc = [XByteField('funcCode', 0x01), BitFieldLenField('byteCount', None, 8, count_of='coilStatus'), FieldListField('coilStatus', [0x00], ByteField('', 0x00), count_from=lambda pkt: pkt.byteCount)]


class ModbusPDU01ReadCoilsError(_ModbusPDUNoPayload):
    name = 'Read Coils Exception'
    fields_desc = [XByteField('funcCode', 0x81), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusPDU02ReadDiscreteInputsRequest(_ModbusPDUNoPayload):
    name = 'Read Discrete Inputs'
    fields_desc = [XByteField('funcCode', 0x02), XShortField('startAddr', 0x0000), XShortField('quantity', 0x0001)]
...
class ModbusPDU02ReadDiscreteInputsResponse(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x02), BitFieldLenField('byteCount', None, 8, count_of='inputStatus'), FieldListField('inputStatus', [0x00], ByteField('', 0x00), count_from=lambda pkt: pkt.byteCount)]


class ModbusPDU02ReadDiscreteInputsError(Packet):
    name = 'Read Discrete Inputs Exception'
    fields_desc = [XByteField('funcCode', 0x82), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusPDU03ReadHoldingRegistersRequest(_ModbusPDUNoPayload):
    name = 'Read Holding Registers'
    fields_desc = [XByteField('funcCode', 0x03), XShortField('startAddr', 0x0000), XShortField('quantity', 0x0001)]
...
class ModbusPDU03ReadHoldingRegistersError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x83), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusPDU04ReadInputRegistersRequest(_ModbusPDUNoPayload):
    name = 'Read Input Registers'
    fields_desc = [XByteField('funcCode', 0x04), XShortField('startAddr', 0x0000), XShortField('quantity', 0x0001)]
...
class ModbusPDU04ReadInputRegistersError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x84), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU05WriteSingleCoilError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x85), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusPDU06WriteSingleRegisterRequest(_ModbusPDUNoPayload):
    name = 'Write Single Register'
    fields_desc = [XByteField('funcCode', 0x06), XShortField('registerAddr', 0x0000), XShortField('registerValue', 0x0000)]
...
class ModbusPDU06WriteSingleRegisterError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x86), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU07ReadExceptionStatusError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x87), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU08DiagnosticsRequest(_ModbusPDUNoPayload):
    name = 'Diagnostics'
    fields_desc = [XByteField('funcCode', 0x08), XShortEnumField('subFunc', 0x0000, _diagnostics_sub_function), FieldListField('data', [0x0000], XShortField('', 0x0000))]
...
class ModbusPDU08DiagnosticsError(_ModbusPDUNoPayload):
    ...
    fields_desc = [XByteField('funcCode', 0x88), ByteEnumField('exceptionCode', 1, _modbus_exceptions)]
...
class ModbusPDU0BGetCommEventCounterResponse(_ModbusPDUNoPayload):
    ...
    fields_desc = [XByteField('funcCode', 0x0B), XShortField('status', 0x0000), XShortField('eventCount', 0xFFFF)]


class ModbusPDU0BGetCommEventCounterError(_ModbusPDUNoPayload):
    name = 'Get Comm Event Counter Exception'
    fields_desc = [XByteField('funcCode', 0x8B), ByteEnumField('exceptionCode', 1, _modbus_exceptions)]
...
class ModbusPDU0CGetCommEventLogResponse(_ModbusPDUNoPayload):
    ...
    fields_desc = [XByteField('funcCode', 0x0C), ByteField('byteCount', 8), XShortField('status', 0x0000), XShortField('eventCount', 0x0108), XShortField('messageCount', 0x0121), FieldListField('event', [0x20, 0x00], XByteField('', 0x00))]


class ModbusPDU0CGetCommEventLogError(_ModbusPDUNoPayload):
    name = 'Get Comm Event Log Exception'
    fields_desc = [XByteField('funcCode', 0x8C), XByteField('exceptionCode', 1)]
...
class ModbusPDU0FWriteMultipleCoilsResponse(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x0F), XShortField('startAddr', 0x0000), XShortField('quantityOutput', 0x0001)]


class ModbusPDU0FWriteMultipleCoilsError(Packet):
    name = 'Write Multiple Coils Exception'
    fields_desc = [XByteField('funcCode', 0x8F), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU10WriteMultipleRegistersResponse(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x10), XShortField('startAddr', 0x0000), XShortField('quantityRegisters', 0x0001)]


class ModbusPDU10WriteMultipleRegistersError(Packet):
    name = 'Write Multiple Registers Exception'
    fields_desc = [XByteField('funcCode', 0x90), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU11ReportSlaveIdError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x91), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusReadFileSubRequest(Packet):
    name = 'Sub-request of Read File Record'
    fields_desc = [ByteField('refType', 0x06), ShortField('fileNumber', 0x0001), ShortField('recordNumber', 0x0000), ShortField('recordLength', 0x0001)]
...
class ModbusPDU14ReadFileRecordRequest(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x14), ByteField('byteCount', None)]
...
class ModbusReadFileSubResponse(Packet):
    name = 'Sub-response'
    fields_desc = [
        BitFieldLenField('respLength', None, 8, count_of='recData', adjust=lambda pkt, p: p * 2 + 1),
        ByteField('refType', 0x06),
        FieldListField('recData', [0x0000], XShortField('', 0x0000), count_from=lambda pkt: (pkt.respLength - 1) // 2),
    ]
...
class ModbusPDU14ReadFileRecordResponse(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x14), ByteField('dataLength', None)]
...
class ModbusPDU14ReadFileRecordError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x94), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusWriteFileSubRequest(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('recordData', [0x0000], ShortField('', 0x0000), length_from=lambda pkt: pkt.recordLength * 2),
    ]
...
class ModbusPDU15WriteFileRecordRequest(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x15), ByteField('dataLength', None)]
...
class ModbusPDU15WriteFileRecordError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x95), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU16MaskWriteRegisterRequest(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x16), XShortField('refAddr', 0x0000), XShortField('andMask', 0xFFFF), XShortField('orMask', 0x0000)]
...
class ModbusPDU16MaskWriteRegisterError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x96), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDU17ReadWriteMultipleRegistersRequest(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('writeRegistersValue', [0x0000], XShortField('', 0x0000), count_from=lambda pkt: pkt.byteCount),
    ]
...
class ModbusPDU17ReadWriteMultipleRegistersError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x97), ByteEnumField('exceptCode', 1, _modbus_exceptions)]


class ModbusPDU18ReadFIFOQueueRequest(Packet):
    name = 'Read FIFO Queue'
    fields_desc = [XByteField('funcCode', 0x18), XShortField('FIFOPointerAddr', 0x0000)]
...
class ModbusPDU18ReadFIFOQueueError(Packet):
    ...
    fields_desc = [XByteField('funcCode', 0x98), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
_read_device_id_object_id = {0x00: 'VendorName', 0x01: 'ProductCode', 0x02: 'MajorMinorRevision', 0x03: 'VendorUrl', 0x04: 'ProductName', 0x05: 'ModelName', 0x06: 'UserApplicationName'}
...
_read_device_id_more_follow = {0x00: 'No', 0x01: 'Yes'}


class ModbusPDU2B0EReadDeviceIdentificationRequest(Packet):
    name = 'Read Device Identification'
    fields_desc = [XByteField('funcCode', 0x2B), XByteField('MEIType', 0x0E), ByteEnumField('readCode', 1, _read_device_id_codes), ByteEnumField('objectId', 0x00, _read_device_id_object_id)]


class ModbusPDU2B0EReadDeviceIdentificationResponse(Packet):
    name = 'Read Device Identification'
    fields_desc = [
        XByteField('funcCode', 0x2B),
...
class ModbusPDU2B0EReadDeviceIdentificationError(Packet):
    name = 'Read Exception Status Exception'
    fields_desc = [XByteField('funcCode', 0xAB), ByteEnumField('exceptCode', 1, _modbus_exceptions)]
...
class ModbusPDUUserDefinedFunctionCodeRequest(_ModbusPDUNoPayload):
    ...
    fields_desc = [
        ModbusByteEnumField('funcCode', 0x00, _userdefined_funccode_request, 'Unknown user-defined request function Code'),
        StrFixedLenField('payload', '', 255),
    ]
...
class ModbusPDUUserDefinedFunctionCodeResponse(_ModbusPDUNoPayload):
    ...
    fields_desc = [
        ModbusByteEnumField('funcCode', 0x00, _userdefined_funccode_response, 'Unknown user-defined response function Code'),
        StrFixedLenField('payload', '', 255),
    ]
...
class ModbusPDUUserDefinedFunctionCodeError(_ModbusPDUNoPayload):
    ...
    fields_desc = [
        ModbusByteEnumField('funcCode', 0x00, _userdefined_funccode_error, 'Unknown user-defined error function Code'),
        StrFixedLenField('payload', '', 255),
    ]
...
class ModbusObjectId(Packet):
    name = 'Object'
    fields_desc = [ByteEnumField('id', 0x00, _read_device_id_object_id), BitFieldLenField('length', None, 8, length_of='value'), StrLenField('value', '', length_from=lambda pkt: pkt.length)]
