...
class GETPORT_Call(Packet):
    name = 'GETPORT Call'
    fields_desc = [IntField('prog', 0), IntField('vers', 0), IntField('prot', 0), IntField('port', 0)]
...
bind_layers(RPC_Call, GETPORT_Call, program=100000, pversion=2, procedure=3)
...
class Map_Entry(Packet):
    ...
    fields_desc = [IntField('prog', 0), IntField('vers', 0), IntField('prot', 0), IntField('port', 0), IntField('value_follows', 0)]
...
class DUMP_Reply(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('mappings', [], Map_Entry, next_cls_cb=lambda pkt, lst, cur, remain: Map_Entry if pkt.value_follows == 1 and (len(lst) == 0 or cur.value_follows == 1) and len(remain) > 4 else None),
    ]
