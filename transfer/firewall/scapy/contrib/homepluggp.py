...
class CM_SLAC_PARM_REQ(Packet):
    ...
    fields_desc = [ByteField('ApplicationType', 0x0), ByteField('SecurityType', 0x0), StrFixedLenField('RunID', b'\x00' * 8, 8)]
...
class VS_ATTENUATION_CHARACTERISTICS_MME(Packet):
    ...
    fields_desc = [
        MACField('EVMACAddress', '00:00:00:00:00:00'),
        FieldLenField('NumberOfGroups', None, count_of='Groups', fmt='B'),
        ByteField('NumberOfCarrierPerGroupe', 0),
        StrFixedLenField('Reserved', b'\x00' * 7, 7),
        PacketListField('Groups', '', HPGP_GROUP, length_from=lambda pkt: pkt.NumberOfGroups),
    ]
...
class CM_SLAC_MATCH_REQ(Packet):
    ...
    fields_desc = [ByteField('ApplicationType', 0x0), ByteField('SecurityType', 0x0), FieldLenField('MatchVariableFieldLen', None, length_of='VariableField', fmt='<H'), PacketField('VariableField', SLAC_varfield(), SLAC_varfield)]
...
class CM_SLAC_MATCH_CNF(Packet):
    ...
    fields_desc = [ByteField('ApplicationType', 0x0), ByteField('SecurityType', 0x0), FieldLenField('MatchVariableFieldLen', None, length_of='VariableField', fmt='<H'), PacketField('VariableField', SLAC_varfield_cnf(), SLAC_varfield_cnf)]
...
class CM_SET_KEY_REQ(Packet):
    name = 'CM_SET_KEY_REQ'
    fields_desc = [
        ByteEnumField('KeyType', 0x0, KeyType_list),
        XIntField('MyNonce', 0),
...
class CM_SET_KEY_CNF(Packet):
    name = 'CM_SET_KEY_CNF'
    fields_desc = [ByteEnumField('Result', 0x0, HPGP_codes), XIntField('MyNonce', 0), XIntField('YourNonce', 0), ByteField('PID', 0), ShortField('ProtoRunNumber', 0), ByteField('ProtoMessNumber', 0), ByteField('CCoCapability', 0)]
