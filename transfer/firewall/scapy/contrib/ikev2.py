...
from scapy.packet import Packet, bind_top_down, bind_bottom_up, split_bottom_up, split_layers, Raw
...
IKEv2AttributeTypes = {
    ...
    'Extended Sequence Number': (5, {'No ESN': 0, 'ESN': 1}, 0),
}
...
IKEv2TrafficSelectorTypes = {7: 'TS_IPV4_ADDR_RANGE', 8: 'TS_IPV6_ADDR_RANGE', 9: 'TS_FC_ADDR_RANGE'}

IKEv2ConfigurationPayloadCFGTypes = {1: 'CFG_REQUEST', 2: 'CFG_REPLY', 3: 'CFG_SET', 4: 'CFG_ACK'}
...
IKEv2_exchange_type.extend(['IKE_SA_INIT', 'IKE_AUTH', 'CREATE_CHILD_SA', 'INFORMATIONAL', 'IKE_SESSION_RESUME'])
...
class RawTrafficSelector(TrafficSelector):
    name = 'IKEv2 Encrypted Traffic Selector'
    fields_desc = [ByteEnumField('TS_type', None, IKEv2TrafficSelectorTypes), ByteEnumField('IP_protocol_ID', None, IPProtocolIDs), FieldLenField('length', None, 'load', 'H', adjust=lambda pkt, x: x + 4), PacketField('load', '', Raw)]
...
class ConfigurationAttribute(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (IPField('value', '127.0.0.1'), lambda x: x.length == 4 and x.type in (1, 2, 3, 4, 6, 20)),
                (IP6Field('value', '::1'), lambda x: x.length == 16 and x.type in (10, 12, 21)),
            ],
...
class IKEv2_payload_CP(IKEv2_class):  # RFC 7296, section 3.15
    ...
    fields_desc = [
        ...
        X3BytesField('res2', 0),
        PacketListField('attributes', None, ConfigurationAttribute, length_from=lambda x: x.length - 8),
    ]
...
class NON_ESP(Packet):  # RFC 3948, section 2.2
    fields_desc = [XIntField('non_esp', 0x0)]
...
class NAT_KEEPALIVE(Packet):  # RFC 3948, section 2.2
    fields_desc = [XByteField('nat_keepalive', 0xFF)]
...
def ikev2scan(ip, **kwargs):
    ...
    return sr(IP(dst=ip) / UDP() / IKEv2(init_SPI=RandString(8), exch_type=34) / IKEv2_payload_SA(prop=IKEv2_payload_Proposal()), **kwargs)  # noqa: E501
