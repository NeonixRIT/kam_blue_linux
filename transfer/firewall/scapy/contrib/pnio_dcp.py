...
DCP_OPTIONS = {0x00: 'reserved', 0x01: 'IP', 0x02: 'Device properties', 0x03: 'DHCP', 0x04: 'Reserved', 0x05: 'Control', 0x06: 'Device Initiative', 0xFF: 'All Selector'}
...
DCP_SUBOPTIONS = {
    ...
    # control
    0x05: {0x00: 'Reserved', 0x01: 'Start Transaction', 0x02: 'End Transaction', 0x03: 'Signal', 0x04: 'Response', 0x05: 'Reset Factory Settings', 0x06: 'Reset to Factory'},
    # device initiative
...
class DCPFullIPBlock(Packet):
    fields_desc = [
        ...
        IPField('netmask', '255.255.255.0'),
        IPField('gateway', '192.168.0.1'),
        FieldListField('dnsaddr', [], IPField('', '0.0.0.0'), count_from=lambda x: 4),
        PadField(StrLenField('padding', b'\x00', length_from=lambda p: p.dcp_block_length % 2), 1, padwith=b'\x00'),
    ]
...
class DCPNameOfStationBlock(Packet):
    fields_desc = [
        ...
        ShortEnumField('block_info', 0, BLOCK_INFOS),
        StrLenField('name_of_station', 'et200sp', length_from=lambda x: x.dcp_block_length - 2),
        PadField(StrLenField('padding', b'\x00', length_from=lambda p: p.dcp_block_length % 2), 1, padwith=b'\x00'),
    ]
...
class DCPAliasNameBlock(Packet):
    fields_desc = [
        ...
        ShortEnumField('block_info', 0, BLOCK_INFOS),
        StrLenField('alias_name', 'et200sp', length_from=lambda x: x.dcp_block_length - 2),
        PadField(StrLenField('padding', b'\x00', length_from=lambda p: p.dcp_block_length % 2), 1, padwith=b'\x00'),
    ]
...
class DCPOEMIDBlock(Packet):
    fields_desc = [
        ByteEnumField('option', 2, DCP_OPTIONS),
        MultiEnumField('sub_option', 8, DCP_SUBOPTIONS, fmt='B', depends_on=lambda p: p.option),
        LenField('dcp_block_length', None),
        ShortEnumField('block_info', 0, BLOCK_INFOS),
        XShortField('vendor_id', 0x002A),
        XShortField('device_id', 0x0313),
...
def guess_dcp_block_class(packet, **kargs):
    ...
    class_switch_case = {
        # IP
        0x01: {0x01: 'DCPMACBlock', 0x02: 'DCPIPBlock'},
        # Device Properties
        0x02: {
        ...
        # Control
        0x05: {0x00: 'Reserved (0x00)', 0x01: 'Start Transaction (0x01)', 0x02: 'End Transaction (0x02)', 0x03: 'Signal (0x03)', 0x04: 'DCPControlBlock', 0x05: 'Reset Factory Settings (0x05)', 0x06: 'Reset to Factory (0x06)'},
        # Device Inactive
        0x06: {0x00: 'Reserved (0x00)', 0x01: 'DCPDeviceInitiativeBlock'},
        # ALL Selector
        0xFF: {0xFF: 'ALL Selector (0xff)'},
    }
...
class ProfinetDCP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(MultiEnumField('sub_option', 3, DCP_SUBOPTIONS, fmt='B', depends_on=lambda p: p.option), lambda pkt: pkt.service_type == 0),
        # calculate the len fields - workaround
        ConditionalField(LenField('dcp_block_length', 0), lambda pkt: pkt.service_type == 0),
        # DCP SET REQUEST #
        ConditionalField(ShortEnumField('block_qualifier', 1, BLOCK_QUALIFIERS), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0),
        # (Common) Name Of Station
        ConditionalField(
            MultipleTypeField(
                [
                    (StrLenField('name_of_station', 'et200sp', length_from=lambda x: x.dcp_block_length - 2), lambda pkt: pkt.service_id == 4),
                ],
                StrLenField('name_of_station', 'et200sp', length_from=lambda x: x.dcp_block_length),
            ),
            lambda pkt: pkt.service_type == 0 and pkt.option == 2 and pkt.sub_option == 2,
        ),
        # DCP SET REQUEST #
        # MAC
        ConditionalField(MACField('mac', '00:00:00:00:00:00'), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0 and pkt.option == 1 and pkt.sub_option == 1),
        # IP
        ConditionalField(IPField('ip', '192.168.0.2'), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0 and pkt.option == 1 and pkt.sub_option in [2, 3]),
        ConditionalField(IPField('netmask', '255.255.255.0'), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0 and pkt.option == 1 and pkt.sub_option in [2, 3]),
        ConditionalField(IPField('gateway', '192.168.0.1'), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0 and pkt.option == 1 and pkt.sub_option in [2, 3]),
        # Full IP
        ConditionalField(FieldListField('dnsaddr', [], IPField('', '0.0.0.0'), count_from=lambda x: 4), lambda pkt: pkt.service_id == 4 and pkt.service_type == 0 and pkt.option == 1 and pkt.sub_option == 3),
        # DCP IDENTIFY REQUEST #
        # Name of station (handled above)
        # Alias name
        ConditionalField(StrLenField('alias_name', 'et200sp', length_from=lambda x: x.dcp_block_length), lambda pkt: pkt.service_id == 5 and pkt.service_type == 0 and pkt.option == 2 and pkt.sub_option == 6),
        # implement further REQUEST fields if needed ....
        # DCP RESPONSE BLOCKS #
        ConditionalField(PacketListField('dcp_blocks', [], guess_dcp_block_class, length_from=lambda p: p.dcp_data_length), lambda pkt: pkt.service_type == 1),
    ]
