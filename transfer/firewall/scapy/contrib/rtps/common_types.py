    ...
    def set_endianness(self, pkt):
        ...
        if isinstance(self.endianness, str) and self.endianness:
            if isinstance(self.fld, UUIDField):
                self.fld.uuid_fmt = UUIDField.FORMAT_LE if self.endianness == '<' else UUIDField.FORMAT_BE
...
class LocatorPacket(EPacket):
    name = 'RTPS Locator'
    fields_desc = [
        EField(XIntField('locatorKind', 0), endianness=FORMAT_LE, endianness_from=None),
        EField(IntField('port', 0), endianness=FORMAT_LE, endianness_from=None),
        ConditionalField(ReversePadField(IPField('address', '0.0.0.0'), 20), lambda p: p.locatorKind == 0x1),
        ConditionalField(XStrFixedLenField('hostId', 0x0, 16), lambda p: p.locatorKind == 0x01000000),
    ]
...
class TransportInfoPacket(EPacket):
    name = 'Transport Info'
    fields_desc = [LEIntField('classID', 0), LEIntField('messageSizeMax', 0)]
