...
from scapy.fields import ByteEnumField, ByteField, ConditionalField, FieldLenField, IPField, PacketListField, ShortField, SignedIntField, StrFixedLenField, StrLenField, XIntField
from scapy.layers.l2 import SNAP
from scapy.compat import chb
from scapy.config import conf

_VTP_VLAN_TYPE = {1: 'Ethernet', 2: 'FDDI', 3: 'TrCRF', 4: 'FDDI-net', 5: 'TrBRF'}
...
class VTPVlanInfoTlv(Packet):
    ...
    fields_desc = [ByteEnumField('type', 0, _VTP_VLANINFO_TLV_TYPE), ByteField('length', 0), StrLenField('value', None, length_from=lambda pkt: pkt.length + 1)]
...
_VTP_Types = {1: 'Summary Advertisement', 2: 'Subset Advertisements', 3: 'Advertisement Request', 4: 'Join'}
...
class VTP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(SignedIntField('rev', 0), lambda pkt: pkt.code == 1 or pkt.code == 2),
        # updater identity
        ConditionalField(IPField('uid', '192.168.0.1'), lambda pkt: pkt.code == 1),
        ConditionalField(VTPTimeStampField('timestamp', '930301000000'), lambda pkt: pkt.code == 1),
        ConditionalField(StrFixedLenField('md5', b'\x00' * 16, 16), lambda pkt: pkt.code == 1),
        ConditionalField(PacketListField('vlaninfo', [], VTPVlanInfo), lambda pkt: pkt.code == 2),
        ConditionalField(ShortField('startvalue', 0), lambda pkt: pkt.code == 3),
    ]
