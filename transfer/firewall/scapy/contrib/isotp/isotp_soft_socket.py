...
from scapy.compat import Optional, Union, List, Tuple, Any, Type, cast, Callable, TYPE_CHECKING
...
from scapy.contrib.isotp.isotp_packet import ISOTP, CAN_MAX_DLEN, N_PCI_SF, N_PCI_CF, N_PCI_FC, N_PCI_FF, ISOTP_MAX_DLEN, ISOTP_MAX_DLEN_2015
...
class ISOTPSoftSocket(SuperSocket):
    ...
    * RX background thread can be stopped by the garbage collector
    ...
    def __init__(
        ...
        impl = ISOTPSocketImplementation(can_socket, tx_id=self.tx_id, rx_id=self.rx_id, padding=padding, ext_address=self.ext_address, rx_ext_address=self.rx_ext_address, bs=bs, stmin=stmin, listen_only=listen_only)
...
class TimeoutScheduler:
    ...
    def _wait(cls, handle):
        ...
        if to_wait > 0:
            log_isotp.debug('TimeoutScheduler Thread going to sleep @ %f ' + 'for %fs', now, to_wait)
            interrupted = cls._event.wait(to_wait)
            new = cls._time()
            log_isotp.debug('TimeoutScheduler Thread awake @ %f, slept for' + ' %f, interrupted=%d', new, new - now, interrupted)
...
class ISOTPSocketImplementation:
    ...
    def _recv_fc(self, data):
        ...
        if (self.txfc_stmin > 0x7F) and ((self.txfc_stmin < 0xF1) or (self.txfc_stmin > 0xF9)):
    ...
    def _recv_ff(self, data, ts):
        ...
        # get the FF_DL
        self.rx_len = (six.indexbytes(data, 0) & 0x0F) * 256 + six.indexbytes(data, 1)
        ff_pci_sz = 2
