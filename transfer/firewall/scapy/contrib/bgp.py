...
class BGPHeader(Packet):
    ...
    name = 'HEADER'
    fields_desc = [XBitField('marker', 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF, 0x80), ShortField('len', None), ByteEnumField('type', 4, _bgp_message_types)]
...
def _bgp_dispatcher(payload):
    ...
    if payload is None:
        ...
        if len(payload) >= _BGP_HEADER_SIZE and payload[:16] == _BGP_HEADER_MARKER:
...
class BGPCapGeneric(BGPCapability):
    ...
    fields_desc = [
        ...
        StrLenField('cap_data', '', length_from=lambda p: p.length, max_length=255),
    ]
...
class BGPCapORFBlock(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField('orf_number', None, count_of='entries', fmt='!B'),
        PacketListField('entries', [], ORFTuple, count_from=lambda p: p.orf_number),
    ]
...
class BGPOptParam(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketField('param_value', None, BGPCapability), lambda p: p.param_type == 2),
        # It"s obsolete, but one can use it provided that
        # param_type == 1.
        ConditionalField(PacketField('authentication_data', None, BGPAuthenticationInformation), lambda p: p.param_type == 1),
    ]
    ...
    def post_build(self, p, pay):
        ...
        if self.param_length is None:
            if self.param_value is None and self.authentication_data is None:
                ...
                length = len(p) - 2  # parameter type (1 byte) - parameter length (1 byte)
            packet = p[:1] + chb(length)
            if (self.param_type == 2 and self.param_value is not None) or (self.param_type == 1 and self.authentication_data is not None):  # noqa: E501
                packet = packet + p[2:]
...
class BGPOpen(BGP):
    ...
    fields_desc = [
        ...
        FieldLenField('opt_param_len', None, length_of='opt_params', fmt='!B'),
        BGPOptParamPacketListField('opt_params', [], BGPOptParam, length_from=lambda p: p.opt_param_len),
    ]
...
class BGPPAExtCommTwoOctetASSpecific(Packet):
    ...
    fields_desc = [ShortField('global_administrator', 0), IntField('local_administrator', 0)]  # noqa: E501
...
class BGPPAExtCommFourOctetASSpecific(Packet):
    ...
    fields_desc = [IntField('global_administrator', 0), ShortField('local_administrator', 0)]  # noqa: E501
...
class BGPPAIPv6AddressSpecificExtComm(Packet):
    ...
    fields_desc = [IP6Field('global_administrator', '::'), ShortField('local_administrator', 0)]  # noqa: E501
...
class BGPPAExtCommunity(Packet):
    ...
    fields_desc = [
        ...
        _TypeLowField('type_low', None, enum_from=lambda x: _get_ext_comm_subtype(x.type_high)),
        _ExtCommValuePacketField('value', None, Packet, type_from=lambda x: (x.type_high, x.type_low)),
    ]
...
class BGPPAMPReachNLRI(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(IPField('nh_v4_addr', '0.0.0.0'), lambda x: x.afi == 1 and x.nh_addr_len == 4),
        ConditionalField(IP6Field('nh_v6_addr', '::'), lambda x: x.afi == 2 and x.nh_addr_len == 16),
        ConditionalField(IP6Field('nh_v6_global', '::'), lambda x: x.afi == 2 and x.nh_addr_len == 32),
        ConditionalField(IP6Field('nh_v6_link_local', '::'), lambda x: x.afi == 2 and x.nh_addr_len == 32),
        ByteField('reserved', 0),
...
class BGPPathAttr(Packet):
    ...
    name = 'BGPPathAttr'
    fields_desc = [
        FlagsField('type_flags', 0x80, 8, ['NA0', 'NA1', 'NA2', 'NA3', 'Extended-Length', 'Partial', 'Transitive', 'Optional']),
        ByteEnumField('type_code', 0, path_attributes),
        ConditionalField(ShortField('attr_ext_len', None), lambda x: x.type_flags is not None and has_extended_length(x.type_flags)),
        ConditionalField(ByteField('attr_len', None), lambda x: x.type_flags is not None and not has_extended_length(x.type_flags)),
...
class BGPUpdate(BGP):
    ...
    name = 'UPDATE'
    fields_desc = [
        FieldLenField('withdrawn_routes_len', None, length_of='withdrawn_routes', fmt='!H'),
        BGPNLRIPacketListField('withdrawn_routes', [], 'IPv4', length_from=lambda p: p.withdrawn_routes_len),
        FieldLenField('path_attr_len', None, length_of='path_attr', fmt='!H'),
        BGPPathAttrPacketListField('path_attr', [], BGPPathAttr, length_from=lambda p: p.path_attr_len),
...
_error_subcodes = {
    ...
    0: {},
    # Header (RFC 4271)
    1: {0: 'Unspecific', 1: 'Connection Not Synchronized', 2: 'Bad Message Length', 3: 'Bad Message Type'},
    ...
    5: {0: 'Unspecified Error', 1: 'Receive Unexpected Message in OpenSent State', 2: 'Receive Unexpected Message in OpenConfirm State', 3: 'Receive Unexpected Message in Established State'},
    # Cease (RFC 4486)
    6: {
        0: 'Unspecified Error',
    ...
    7: {0: 'Reserved', 1: 'Invalid Message Length'},
}
...
_orf_when_to_refresh = {0x01: 'IMMEDIATE', 0x02: 'DEFER'}
...
class BGPRouteRefresh(BGP):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketField('orf_data', '', BGPORF), lambda p: ((p.underlayer and p.underlayer.len or 24) > 23)),
    ]
