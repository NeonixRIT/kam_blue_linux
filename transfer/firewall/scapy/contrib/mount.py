...
from scapy.fields import IntField, StrLenField, IntEnumField, PacketField, ConditionalField, FieldListField
from scapy.contrib.nfs import File_Object

mountstat3 = {0: 'MNT3_OK', 1: 'MNT3ERR_PERM', 2: 'MNT3ERR_NOENT', 5: 'MNT3ERR_IO', 13: 'MNT3ERR_ACCES', 20: 'MNT3ERR_NOTDIR', 22: 'MNT3ERR_INVAL', 63: 'MNT3ERR_NAMETOOLONG', 10004: 'MNT3ERR_NOTSUPP', 10006: 'MNT3ERR_SERVERFAULT'}


class Path(Packet):
    name = 'Path'
    fields_desc = [IntField('length', 0), StrLenField('path', '', length_from=lambda pkt: pkt.length), StrLenField('fill', '', length_from=lambda pkt: (4 - pkt.length) % 4)]
...
class MOUNT_Call(Packet):
    name = 'MOUNT Call'
    fields_desc = [PacketField('path', Path(), Path)]
...
class MOUNT_Reply(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(FieldListField('flavor', None, IntField('', None), count_from=lambda pkt: pkt.flavors), lambda pkt: pkt.status == 0),
    ]
...
bind_layers(RPC_Call, UNMOUNT_Call, program=100005, procedure=3, pversion=3)
