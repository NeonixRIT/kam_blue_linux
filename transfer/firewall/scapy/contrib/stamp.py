...
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, FlagsField, IntField, MultipleTypeField, NBytesField, PacketField, PacketListField, ShortField, StrLenField, UTCTimeField


_sync_types = {0: 'No External Synchronization for the Time Source', 1: 'Clock Synchronized to UTC using an External Source'}

_timestamp_types = {0: 'NTP 64-bit Timestamp Format', 1: 'PTPv2 Truncated Timestamp Format'}
...
class STAMPSessionSenderTestUnauthenticated(Packet):
    ...
    fields_desc = [
        IntField('seq', 0),
        MultipleTypeField([(TimeStampField('ts', 0), lambda pkt: pkt.err_estimate.Z == 0)], UTCTimeField('ts', 0, fmt='Q')),
        PacketField('err_estimate', ErrorEstimate(), ErrorEstimate),
        ShortField('ssid', 1),
        NBytesField('mbz', 0, 28),  # 28 bytes MBZ
        PacketListField('tlv_objects', [], STAMPTestTLV, length_from=lambda pkt: pkt.parent.len - 8 - 44),
    ]
...
class STAMPSessionReflectorTestUnauthenticated(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (TimeStampField('ts', 0), lambda pkt: pkt.err_estimate.Z == 0),
            ],
            UTCTimeField('ts', 0, fmt='Q'),
        ),
        PacketField('err_estimate', ErrorEstimate(), ErrorEstimate),
        ShortField('ssid', 1),
        MultipleTypeField([(TimeStampField('ts_rx', 0), lambda pkt: pkt.err_estimate.Z == 0)], UTCTimeField('ts_rx', 0, fmt='Q')),
        IntField('seq_sender', 0),
        MultipleTypeField([(TimeStampField('ts_sender', 0), lambda pkt: pkt.err_estimate_sender.Z == 0)], UTCTimeField('ts_sender', 0, fmt='Q')),
