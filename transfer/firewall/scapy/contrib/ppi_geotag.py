...
from scapy.fields import ByteField, ConditionalField, Field, FlagsField, LEIntField, LEShortEnumField, LEShortField, StrFixedLenField, UTCTimeField, XLEIntField, SignedByteField, XLEShortField
...
class _RMMLEIntField(LEIntField):
    __slots__ = ['min_i2h', 'max_i2h', 'lambda_i2h', 'min_h2i', 'max_h2i', 'lambda_h2i', 'rname', 'ffmt']
...
class Fixed3_6Field(_RMMLEIntField):
    def __init__(self, name, default, *args, **kargs):
        _RMMLEIntField.__init__(self, name, default, 0, 999999999, -0.5e-6, 999.9999995, lambda x: x * 1e-6, lambda x: int(round(x * 1e6)), '3.6f')


class Fixed3_7Field(_RMMLEIntField):
    def __init__(self, name, default, *args, **kargs):
        _RMMLEIntField.__init__(self, name, default, 0, 3600000000, -180.00000005, 180.00000005, lambda x: (x - 1800000000) * 1e-7, lambda x: int(round((x + 180.0) * 1e7)), '3.7f')


class Fixed6_4Field(_RMMLEIntField):
    def __init__(self, name, default, *args, **kargs):
        _RMMLEIntField.__init__(self, name, default, 0, 3600000000, -180000.00005, 180000.00005, lambda x: (x - 1800000000) * 1e-4, lambda x: int(round((x + 180000.0) * 1e4)), '6.4f')
...
class NSCounter_Field(_RMMLEIntField):
    def __init__(self, name, default):
        _RMMLEIntField.__init__(self, name, default, 0, 2**32, 0, (2**32 - 1) / 1e9, lambda x: (x / 1e9), lambda x: int(round(x * 1e9)), '1.9f')
...
sensor_types = {1: 'Velocity', 2: 'Acceleration', 3: 'Jerk', 100: 'Rotation', 101: 'Magnetic', 1000: 'Temperature', 1001: 'Barometer', 1002: 'Humidity', 2000: 'TDOA_Clock', 2001: 'Phase'}
