...
from scapy.fields import IntField, StrLenField, LongField, PacketField, IntEnumField
from scapy.contrib.nfs import File_Object

nlm4_stats = {0: 'NLM4_GRANTED', 1: 'NLM4_DENIED', 2: 'NLM4_DENIED_NOLOCKS', 3: 'NLM4_BLOCKED', 4: 'NLM4_DENIED_GRACE_PERIOD', 5: 'NLM4_DEADLCK', 6: 'NLM4_ROFS', 7: 'NLM4_STALE_FH', 8: 'NLM4_FBIG', 9: 'NLM4_FAILED'}


class NLM4_Cookie(Packet):
    name = 'Cookie'
    fields_desc = [IntField('length', 0), StrLenField('contents', '', length_from=lambda pkt: pkt.length), StrLenField('fill', b'', length_from=lambda pkt: (4 - pkt.length) % 4)]
...
class SHARE_Reply(Packet):
    name = 'SHARE Reply'
    fields_desc = [PacketField('cookie', NLM4_Cookie(), NLM4_Cookie), IntEnumField('status', 0, nlm4_stats), IntField('sequence', 0)]
...
bind_layers(RPC_Call, UNSHARE_Call, program=100021, pversion=4, procedure=21)
...
class LOCK_Reply(Packet):
    name = 'LOCK Reply'
    fields_desc = [PacketField('cookie', NLM4_Cookie(), NLM4_Cookie), IntEnumField('status', 0, nlm4_stats)]
...
bind_layers(RPC_Call, GRANTED_MSG_Call, program=100021, pversion=4, procedure=10)
...
bind_layers(RPC_Call, GRANTED_RES_Call, program=100021, pversion=4, procedure=15)
