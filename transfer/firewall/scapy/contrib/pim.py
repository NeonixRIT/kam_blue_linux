...
from scapy.fields import BitFieldLenField, BitField, BitEnumField, ByteField, ShortField, XShortField, IPField, PacketListField, IntField, FieldLenField, BoundStrLenField, FlagsField
...
PIM_TYPE = {0: 'Hello', 1: 'Register', 2: 'Register-Stop', 3: 'Join/Prune', 4: 'Bootstrap', 5: 'Assert', 6: 'Graft', 7: 'Graft-Ack', 8: 'Candidate-RP-Advertisement'}


class PIMv2Hdr(Packet):
    name = 'Protocol Independent Multicast Version 2 Header'
    fields_desc = [BitField('version', 2, 4), BitEnumField('type', 0, 4, PIM_TYPE), ByteField('reserved', 0), XShortField('chksum', None)]
...
class _PIMGenericTlvBase(Packet):
    fields_desc = [ByteField('type', 0), FieldLenField('length', None, length_of='value', fmt='B'), BoundStrLenField('value', '', length_from=lambda pkt: pkt.length)]
...
class PIMv2Hello(Packet):
    ...
    fields_desc = [_PIMv2HelloListField()]


class PIMv2HelloHoldtime(_PIMv2GenericHello):
    name = 'PIMv2 Hello Options : Holdtime'
    fields_desc = [ShortField('type', 1), FieldLenField('length', None, length_of='holdtime', fmt='!H'), ShortField('holdtime', 105)]
...
class PIMv2HelloLANPruneDelay(_PIMv2GenericHello):
    ...
    fields_desc = [ShortField('type', 2), FieldLenField('length', None, length_of='value', fmt='!H'), PacketListField('value', PIMv2HelloLANPruneDelayValue(), PIMv2HelloLANPruneDelayValue, length_from=lambda pkt: pkt.length)]


class PIMv2HelloDRPriority(_PIMv2GenericHello):
    name = 'PIMv2 Hello Options : DR Priority'
    fields_desc = [ShortField('type', 19), FieldLenField('length', None, length_of='dr_priority', fmt='!H'), IntField('dr_priority', 1)]


class PIMv2HelloGenerationID(_PIMv2GenericHello):
    name = 'PIMv2 Hello Options : Generation ID'
    fields_desc = [ShortField('type', 20), FieldLenField('length', None, length_of='generation_id', fmt='!H'), IntField('generation_id', RandInt())]


class PIMv2HelloStateRefreshValue(_PIMv2GenericHello):
    name = 'PIMv2 Hello Options : State-Refresh Value'
    fields_desc = [ByteField('version', 1), ByteField('interval', 0), ShortField('reserved', 0)]


class PIMv2HelloStateRefresh(_PIMv2GenericHello):
    name = 'PIMv2 Hello Options : State-Refresh'
    fields_desc = [ShortField('type', 21), FieldLenField('length', None, length_of='value', fmt='!H'), PacketListField('value', PIMv2HelloStateRefreshValue(), PIMv2HelloStateRefreshValue)]
...
class PIMv2JoinPruneAddrsBase(_PIMGenericTlvBase):
    fields_desc = [ByteField('addr_family', 1), ByteField('encoding_type', 0), BitField('rsrvd', 0, 5), BitField('sparse', 0, 1), BitField('wildcard', 0, 1), BitField('rpt', 1, 1), ByteField('mask_len', 32), IPField('src_ip', '0.0.0.0')]
...
class PIMv2GroupAddrs(_PIMGenericTlvBase):
    ...
    fields_desc = [
        ...
        PacketListField('join_ips', [], PIMv2JoinAddrs, count_from=lambda x: x.num_joins),
        PacketListField('prune_ips', [], PIMv2PruneAddrs, count_from=lambda x: x.num_prunes),
    ]
...
class PIMv2JoinPrune(_PIMGenericTlvBase):
    ...
    fields_desc = [
        ...
        PacketListField('jp_ips', [], PIMv2GroupAddrs, count_from=lambda pkt: pkt.num_group),
    ]
