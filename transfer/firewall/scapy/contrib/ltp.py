...
from scapy.fields import BitEnumField, BitField, BitFieldLenField, ByteEnumField, ConditionalField, PacketListField, StrLenField
...
_ltp_extension_tag = {0: 'LTP authentication extension', 1: 'LTP cookie extension'}
...
class LTPReceptionClaim(Packet):
    ...
    fields_desc = [SDNV2('ReceptionClaimOffset', 0), SDNV2('ReceptionClaimLength', 0)]
...
class LTP(Packet):
    ...
    fields_desc = [
        ...
        #
        ConditionalField(SDNV2('DATA_ClientServiceID', 0), lambda x: x.flags in _ltp_data_segment),
        ConditionalField(SDNV2('DATA_PayloadOffset', 0), lambda x: x.flags in _ltp_data_segment),
        ConditionalField(
            SDNV2FieldLenField('DATA_PayloadLength', None, length_of='LTP_Payload'),  # noqa: E501
        ...
        #
        ConditionalField(SDNV2('CheckpointSerialNo', 0), lambda x: x.flags in _ltp_checkpoint_segment),
        #
        # For segments that are checkpoints or reception reports.
        #
        ConditionalField(SDNV2('ReportSerialNo', 0), lambda x: x.flags in _ltp_checkpoint_segment or x.flags == 8),
        #
        ...
        #
        ConditionalField(SDNV2('RA_ReportSerialNo', 0), lambda x: x.flags == 9),
        #
        # Reception reports have the following fields,
        # excluding ReportSerialNo defined above.
        #
        ConditionalField(SDNV2('ReportCheckpointSerialNo', 0), lambda x: x.flags == 8),
        ConditionalField(SDNV2('ReportUpperBound', 0), lambda x: x.flags == 8),
        ConditionalField(SDNV2('ReportLowerBound', 0), lambda x: x.flags == 8),
        ConditionalField(
            SDNV2FieldLenField('ReportReceptionClaimCount', None, count_of='ReportReceptionClaims'),  # noqa: E501
            lambda x: x.flags == 8,
        ),
        ConditionalField(
            PacketListField(
        ...
        #
        ConditionalField(ByteEnumField('CancelFromSenderReason', 15, _ltp_cancel_reasons), lambda x: x.flags == 12),
        ConditionalField(ByteEnumField('CancelFromReceiverReason', 15, _ltp_cancel_reasons), lambda x: x.flags == 14),
        #
        # Cancellation Acknowldgements
        #
        ConditionalField(SDNV2('CancelAckToBlockSender', 0), lambda x: x.flags == 13),
        ConditionalField(SDNV2('CancelAckToBlockReceiver', 0), lambda x: x.flags == 15),
        #
