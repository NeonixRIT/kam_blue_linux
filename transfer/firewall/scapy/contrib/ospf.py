...
_OSPF_types = {1: 'Hello', 2: 'DBDesc', 3: 'LSReq', 4: 'LSUpd', 5: 'LSAck'}
...
class OSPF_Hdr(Packet):
    ...
    def answers(self, other):
        if isinstance(other, OSPF_Hdr) and self.area == other.area and self.type == 5:  # Only acknowledgements answer other packets
...
class LLS_Generic_TLV(Packet):
    name = 'LLS Generic'
    fields_desc = [ShortField('type', 0), FieldLenField('len', None, length_of='val'), StrLenField('val', '', length_from=lambda x: x.len)]
...
class LLS_Extended_Options(LLS_Generic_TLV):
    ...
    fields_desc = [ShortField('type', 1), FieldLenField('len', None, fmt='!H', length_of='options'), StrLenField('options', '', length_from=lambda x: x.len)]
...
_OSPF_LStypes = {1: 'router', 2: 'network', 3: 'summaryIP', 4: 'summaryASBR', 5: 'external', 7: 'NSSAexternal', 9: 'linkScopeOpaque', 10: 'areaScopeOpaque', 11: 'asScopeOpaque'}
...
_OSPF_Router_LSA_types = {1: 'p2p', 2: 'transit', 3: 'stub', 4: 'virtual'}
...
class OSPF_Link_Scope_Opaque_LSA(OSPF_BaseLSA):
    ...
    fields_desc = [
        ...
        XShortField('chksum', None),
        ShortField('len', None),
        StrLenField('data', 'data', length_from=lambda pkt: pkt.len - 20),
    ]
...
class OSPF_DBDesc(Packet):
    ...
    fields_desc = [
        ...
        IntField('ddseq', 1),
        PacketListField('lsaheaders', None, OSPF_LSA_Hdr, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 24 - 8),
    ]  # noqa: E501
...
class OSPF_LSReq_Item(Packet):
    ...
    fields_desc = [IntEnumField('type', 1, _OSPF_LStypes), IPField('id', '1.1.1.1'), IPField('adrouter', '1.1.1.1')]
...
class OSPF_LSReq(Packet):
    ...
    fields_desc = [PacketListField('requests', None, OSPF_LSReq_Item, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 24)]  # noqa: E501
...
class OSPF_LSUpd(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('lsalist', None, _LSAGuessPayloadClass, count_from=lambda pkt: pkt.lsacount, length_from=lambda pkt: pkt.underlayer.len - 24),
    ]  # noqa: E501


class OSPF_LSAck(Packet):
    name = 'OSPF Link State Acknowledgement'
    fields_desc = [PacketListField('lsaheaders', None, OSPF_LSA_Hdr, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 24)]  # noqa: E501
...
class OSPFv3_Hello(Packet):
    ...
    fields_desc = [
        ...
        IPField('router', '0.0.0.0'),
        IPField('backup', '0.0.0.0'),
        FieldListField('neighbors', [], IPField('', '0.0.0.0'), length_from=lambda pkt: (pkt.underlayer.len - 36)),
    ]  # noqa: E501


_OSPFv3_LStypes = {0x2001: 'router', 0x2002: 'network', 0x2003: 'interAreaPrefix', 0x2004: 'interAreaRouter', 0x4005: 'asExternal', 0x2007: 'type7', 0x0008: 'link', 0x2009: 'intraAreaPrefix'}

_OSPFv3_LSclasses = {
    0x2001: 'OSPFv3_Router_LSA',
...
class OSPFv3_LSA_Hdr(Packet):
    ...
    fields_desc = [ShortField('age', 1), ShortEnumField('type', 0x2001, _OSPFv3_LStypes), IPField('id', '0.0.0.0'), IPField('adrouter', '1.1.1.1'), XIntField('seq', 0x80000001), XShortField('chksum', 0), ShortField('len', 36)]
...
_OSPFv3_Router_LSA_types = {1: 'p2p', 2: 'transit', 3: 'reserved', 4: 'virtual'}


class OSPFv3_Link(Packet):
    name = 'OSPFv3 Link'
    fields_desc = [ByteEnumField('type', 1, _OSPFv3_Router_LSA_types), ByteField('reserved', 0), ShortField('metric', 10), IntField('intid', 0), IntField('neighintid', 0), IPField('neighbor', '2.2.2.2')]
...
class OSPFv3_Router_LSA(OSPF_BaseLSA):
    ...
    fields_desc = [
        ...
        OSPFv3OptionsField(),
        PacketListField('linklist', [], OSPFv3_Link, length_from=lambda pkt: pkt.len - 24),
    ]
...
class OSPFv3_Network_LSA(OSPF_BaseLSA):
    ...
    fields_desc = [
        ...
        ByteField('reserved', 0),
        OSPFv3OptionsField(),
        FieldListField('routerlist', [], IPField('', '0.0.0.1'), length_from=lambda pkt: pkt.len - 24),
    ]
...
class OSPFv3_Link_LSA(OSPF_BaseLSA):
    ...
    fields_desc = [
        ...
        PacketListField('prefixlist', None, OSPFv3_Prefix_Item, count_from=lambda pkt: pkt.prefixes),
    ]
...
class OSPFv3_DBDesc(Packet):
    ...
    fields_desc = [
        ...
        IntField('ddseq', 1),
        PacketListField('lsaheaders', None, OSPFv3_LSA_Hdr, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 28),
    ]  # noqa: E501


class OSPFv3_LSReq_Item(Packet):
    name = 'OSPFv3 Link State Request (item)'
    fields_desc = [ShortField('reserved', 0), ShortEnumField('type', 0x2001, _OSPFv3_LStypes), IPField('id', '1.1.1.1'), IPField('adrouter', '1.1.1.1')]
...
class OSPFv3_LSReq(Packet):
    ...
    fields_desc = [PacketListField('requests', None, OSPFv3_LSReq_Item, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 16)]  # noqa: E501
...
class OSPFv3_LSUpd(Packet):
    ...
    fields_desc = [
        FieldLenField('lsacount', None, fmt='!I', count_of='lsalist'),  # noqa: E501
        PacketListField('lsalist', [], _OSPFv3_LSAGuessPayloadClass, count_from=lambda pkt: pkt.lsacount, length_from=lambda pkt: pkt.underlayer.len - 16),
    ]  # noqa: E501


class OSPFv3_LSAck(Packet):
    name = 'OSPFv3 Link State Acknowledgement'
    fields_desc = [PacketListField('lsaheaders', None, OSPFv3_LSA_Hdr, count_from=lambda pkt: None, length_from=lambda pkt: pkt.underlayer.len - 16)]  # noqa: E501
