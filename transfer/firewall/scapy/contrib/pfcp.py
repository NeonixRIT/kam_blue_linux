...
from scapy.packet import bind_layers, bind_bottom_up, Packet, Raw
...
class PFCP(PFCPLengthMixin, Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XLongField('seid', 0), lambda pkt: pkt.S == 1),
        ThreeBytesField('seq', 0),
        ConditionalField(BitField('priority', 0, 4), lambda pkt: pkt.MP == 1),
        ConditionalField(BitField('spare_p', 0, 4), lambda pkt: pkt.MP == 1),
        ConditionalField(ByteField('spare_oct', 0), lambda pkt: pkt.MP == 0),
    ]
...
class IE_Base(PFCPLengthMixin, Packet):
    ...
    fields_desc = [ShortEnumField('ietype', 0, IEType), ShortField('length', None)]


class IE_Compound(IE_Base):
    fields_desc = IE_Base.fields_desc + [PacketListField('IE_list', None, IE_Dispatcher, length_from=lambda pkt: pkt.length)]
...
class IE_Cause(IE_Base):
    ie_type = 19
    name = 'IE Cause'
    fields_desc = IE_Base.fields_desc + [ByteEnumField('cause', None, CauseValues)]
...
class IE_FTEID(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(IPField('ipv4', 0), lambda x: x.V4 == 1 and x.CH == 0),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.V6 == 1 and x.CH == 0),
        ConditionalField(ByteField('choose_id', 0), lambda x: x.CHID == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_NetworkInstance(IE_Base):
    ...
    ie_type = 22
    fields_desc = IE_Base.fields_desc + [APNStrLenField('instance', '', length_from=lambda x: x.length)]
...
class IE_SDF_Filter(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(FieldLenField('flow_description_length', None, length_of='flow_description'), lambda pkt: pkt.FD == 1),
        ConditionalField(StrLenField('flow_description', '', length_from=lambda pkt: pkt.flow_description_length), lambda pkt: pkt.FD == 1),
        ConditionalField(ByteField('tos_traffic_class', 0), lambda pkt: pkt.TTC == 1),
        ConditionalField(ByteField('tos_traffic_mask', 0), lambda pkt: pkt.TTC == 1),
...
class IE_OffendingIE(IE_Base):
    ...
    ie_type = 40
    fields_desc = IE_Base.fields_desc + [ShortEnumField('type', None, IEType)]


class IE_ForwardingPolicy(IE_Base):
    name = 'IE Forwarding Policy'
    ie_type = 41
    fields_desc = IE_Base.fields_desc + [FieldLenField('policy_identifier_length', None, length_of='policy_identifier', fmt='B'), StrLenField('policy_identifier', '', length_from=lambda pkt: pkt.policy_identifier_length)]
...
class IE_DownlinkDataServiceInformation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('QFII', 0, 1),
        BitField('PPI', 0, 1),
        ConditionalField(XBitField('spare_2', None, 2), lambda x: x.PPI == 1),
        ConditionalField(XBitField('ppi_val', None, 6), lambda x: x.PPI == 1),
        ConditionalField(XBitField('spare_3', None, 2), lambda x: x.QFII == 1),
        ConditionalField(XBitField('qfi_val', None, 6), lambda x: x.QFII == 1),
        ExtraDataField('extra_data'),
    ]
    ...
    fields_desc = IE_Base.fields_desc + [
        MultipleTypeField(
            [
                (
                    ByteField('count', 0),
                    (lambda x: x.length == 1, lambda x, val: x.length == 1 or (x.length is None and val < 256)),
                ),
                (ShortField('count', 0), (lambda x: x.length == 2, lambda x, val: x.length == 1 or (x.length is None and val >= 256))),
            ],
            ByteField('count', 0),
        )
    ]
...
class IE_NodeId(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(IPField('ipv4', 0), lambda x: x.id_type == 0),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.id_type == 1),
        ConditionalField(APNStrLenField('id', '', length_from=lambda x: x.length - 1), lambda x: x.id_type == 2),
        ExtraDataField('extra_data'),
    ]
...
class IE_PFDContents(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('FD', 0, 1),
        ByteField('spare_2', 0),
        ConditionalField(FieldLenField('flow_length', None, length_of='flow'), lambda pkt: pkt.FD == 1),
        ConditionalField(StrLenField('flow', '', length_from=lambda pkt: pkt.flow_length), lambda pkt: pkt.FD == 1),
        ConditionalField(FieldLenField('url_length', None, length_of='url'), lambda pkt: pkt.URL == 1),
        ConditionalField(StrLenField('url', '', length_from=lambda pkt: pkt.url_length), lambda pkt: pkt.URL == 1),
        ConditionalField(FieldLenField('domain_length', None, length_of='domain'), lambda pkt: pkt.DN == 1),
        ConditionalField(StrLenField('domain', '', length_from=lambda pkt: pkt.domain_length), lambda pkt: pkt.DN == 1),
        ConditionalField(FieldLenField('custom_length', None, length_of='custom'), lambda pkt: pkt.CP == 1),
        ConditionalField(StrLenField('custom', '', length_from=lambda pkt: pkt.custom_length), lambda pkt: pkt.CP == 1),
        ConditionalField(FieldLenField('dnp_length', None, length_of='dnp'), lambda pkt: pkt.DNP == 1),
        ConditionalField(StrLenField('dnp', '', length_from=lambda pkt: pkt.dnp_length), lambda pkt: pkt.DNP == 1),
        ConditionalField(FieldLenField('additional_flow_length', None, length_of='additional_flow'), lambda pkt: pkt.AFD == 1),
        ConditionalField(StrLenField('additional_flow', '', length_from=lambda pkt: pkt.additional_flow_length), lambda pkt: pkt.AFD == 1),
        ConditionalField(FieldLenField('additional_url_length', None, length_of='additional_url'), lambda pkt: pkt.AURL == 1),
        ConditionalField(StrLenField('additional_url', '', length_from=lambda pkt: pkt.additional_url_length), lambda pkt: pkt.AURL == 1),
        ConditionalField(FieldLenField('additional_dn_dnp_length', None, length_of='additional_dn_dnp'), lambda pkt: pkt.ADNP == 1),
        ConditionalField(StrLenField('additional_dn_dnp', '', length_from=lambda pkt: pkt.additional_dn_dnp_length), lambda pkt: pkt.ADNP == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_FqCSID(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(IPField('ipv4', 0), lambda x: x.node_id_type == 0),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.node_id_type == 1),
        ConditionalField(
        ...
        ConditionalField(BitField('extra_id', 0, 12), lambda x: x.node_id_type == 2),
        FieldListField('csids', None, ShortField('csid', 0), count_from=lambda x: x.num_csids),
        ExtraDataField('extra_data'),
    ]
...
class IE_DroppedDLTrafficThreshold(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('DLBY', 0, 1),
        BitField('DLPA', 0, 1),
        ConditionalField(LongField('packet_count', 0), lambda x: x.DLPA == 1),
        ConditionalField(LongField('byte_count', 0), lambda x: x.DLBY == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_OuterHeaderCreation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ByteField('spare', 0),
        ConditionalField(XIntField('TEID', 0), lambda x: x.GTPUUDPIPV4 == 1 or x.GTPUUDPIPV6 == 1),
        ConditionalField(IPField('ipv4', 0), lambda x: x.IPV4 == 1 or x.UDPIPV4 == 1 or x.GTPUUDPIPV4 == 1),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.IPV6 == 1 or x.UDPIPV6 == 1 or x.GTPUUDPIPV6 == 1),
        ConditionalField(ShortField('port', 0), lambda x: x.UDPIPV4 == 1 or x.UDPIPV6 == 1),
        ConditionalField(ThreeBytesField('ctag', 0), lambda x: x.CTAG == 1),
        ConditionalField(ThreeBytesField('stag', 0), lambda x: x.STAG == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_ApplicationInstanceId(IE_Base):
    ...
    ie_type = 91
    fields_desc = IE_Base.fields_desc + [StrLenField('id', '', length_from=lambda x: x.length)]
...
class IE_PacketRate(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(BitEnumField('ul_time_unit', 'minute', 3, TimeUnit), lambda x: x.ULPR == 1),
        ConditionalField(ShortField('ul_max_packet_rate', 0), lambda x: x.ULPR == 1),
        ConditionalField(BitField('spare_3', 0, 5), lambda x: x.DLPR == 1),
        ConditionalField(BitEnumField('dl_time_unit', 'minute', 3, TimeUnit), lambda x: x.DLPR == 1),
        ConditionalField(ShortField('dl_max_packet_rate', 0), lambda x: x.DLPR == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_DLFlowLevelMarking(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(ByteField('service_class_indicator', 0), lambda x: x.SCI),
...
class IE_RemoteGTP_U_Peer(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(IPField('ipv4', 0), lambda x: x.V4 == 1),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.V6 == 1),
        ConditionalField(ByteField('dest_interface_length', 1), lambda x: x.DI == 1),
        ConditionalField(XBitField('spare_2', 0, 4), lambda x: x.DI == 1),
        ConditionalField(BitEnumField('dest_interface', 'Access', 4, DestinationInterface), lambda x: x.DI == 1),
        ConditionalField(FieldLenField('network_instance_length', 1, length_of='network_instance'), lambda x: x.NI == 1),
        ConditionalField(APNStrLenField('network_instance', '', length_from=lambda x: x.network_instance_length), lambda x: x.NI == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_ActivatePredefinedRules(IE_Base):
    ...
    ie_type = 106
    fields_desc = IE_Base.fields_desc + [StrLenField('name', '', length_from=lambda x: x.length)]
...
class IE_FailedRuleId(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(ShortField('pdr_id', 0), lambda x: x.type == 0),
        ConditionalField(IntField('far_id', 0), lambda x: x.type == 1 or x.type > 4),
        ConditionalField(IntField('qer_id', 0), lambda x: x.type == 2),
        ConditionalField(IntField('urr_id', 0), lambda x: x.type == 3),
        ConditionalField(ByteField('bar_id', 0), lambda x: x.type == 4),
        ExtraDataField('extra_data'),
    ]
...
class IE_UserPlaneIPResourceInformation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(IPField('ipv4', 0), lambda x: x.V4 == 1),
        ConditionalField(IP6Field('ipv6', 0), lambda x: x.V6 == 1),
        ConditionalField(APNStrLenField('network_instance', '', length_from=lambda x: x.length - 1 - (1 if x.TEIDRI != 0 else 0) - (x.V4 * 4) - (x.V6 * 16) - x.ASSOSI), lambda x: x.ASSONI == 1),
        ConditionalField(XBitField('spare2', None, 4), lambda x: x.ASSOSI == 1),
        ConditionalField(BitEnumField('interface', 'Access', 4, SourceInterface), lambda x: x.ASSOSI == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_MACAddress(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('DEST', 0, 1),
        BitField('SOUR', 0, 1),
        ConditionalField(MACField('source_mac', 0), lambda x: x.SOUR == 1),
        ConditionalField(MACField('destination_mac', 0), lambda x: x.DEST == 1),
        ConditionalField(MACField('upper_source_mac', 0), lambda x: x.USOU == 1),
        ConditionalField(MACField('upper_destination_mac', 0), lambda x: x.UDES == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_C_TAG(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('PCP', 0, 1),
        # TODO: fix cvid_value
        ConditionalField(BitField('cvid_value_hi', 0, 4), lambda x: x.VID == 1),
        ...
        ConditionalField(ByteField('cvid_value_low', 0), lambda x: x.VID == 1),
...
class IE_S_TAG(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('PCP', 0, 1),
        # TODO: fix svid_value
        ConditionalField(BitField('svid_value_hi', 0, 4), lambda x: x.VID == 1),
        ConditionalField(BitField('spare_2', 0, 4), lambda x: x.VID == 0),
        ConditionalField(BitField('dei_flag', 0, 1), lambda x: x.DEI == 1),
        ConditionalField(BitField('spare_3', 0, 1), lambda x: x.DEI == 0),
        ConditionalField(BitField('pcp_value', 0, 3), lambda x: x.PCP == 1),
        ConditionalField(BitField('spare_4', 0, 3), lambda x: x.PCP == 0),
        ConditionalField(ByteField('svid_value_low', 0), lambda x: x.VID == 1),
        ConditionalField(ByteField('spare_5', 0), lambda x: x.VID == 0),
        ExtraDataField('extra_data'),
    ]
...
class IE_UserId(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        BitField('IMEIF', 0, 1),
        BitField('IMSIF', 0, 1),
        ConditionalField(FieldLenField('imsi_length', None, length_of='imsi', fmt='B'), lambda x: x.IMSIF == 1),
        ConditionalField(StrLenField('imsi', '', length_from=lambda x: x.imsi_length), lambda x: x.IMSIF == 1),
        ConditionalField(FieldLenField('imei_length', None, length_of='imei', fmt='B'), lambda x: x.IMEIF == 1),
        ConditionalField(StrLenField('imei', '', length_from=lambda x: x.imei_length), lambda x: x.IMEIF == 1),
        ConditionalField(FieldLenField('msisdn_length', None, length_of='msisdn', fmt='B'), lambda x: x.MSISDNF == 1),
        ConditionalField(StrLenField('msisdn', '', length_from=lambda x: x.msisdn_length), lambda x: x.MSISDNF == 1),
        ConditionalField(FieldLenField('nai_length', None, length_of='nai', fmt='B'), lambda x: x.NAIF == 1),
        ConditionalField(StrLenField('nai', '', length_from=lambda x: x.nai_length), lambda x: x.NAIF == 1),
        ExtraDataField('extra_data'),
    ]
...
class IE_MACAddressesDetected(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        FieldListField('macs', None, MACField('mac', 0), count_from=lambda x: x.num_macs),
        ExtraDataField('extra_data'),
    ]
...
class IE_TraceInformation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        FieldLenField('triggering_events_length', None, length_of='triggering_events', fmt='B'),
        StrLenField('triggering_events', '', length_from=lambda x: x.triggering_events_length),
        ByteField('session_trace_depth', 0),
        FieldLenField('list_of_interfaces_length', None, length_of='list_of_interfaces', fmt='B'),
        StrLenField('list_of_interfaces', '', length_from=lambda x: x.list_of_interfaces_length),
        FieldLenField('ip_address_length', None, length_of='ip_address', fmt='B'),
        StrLenField('ip_address', '', length_from=lambda x: x.ip_address_length),
        ExtraDataField('extra_data'),
    ]
...
class IE_FramedRoute(IE_Base):
    ...
    ie_type = 153
    fields_desc = IE_Base.fields_desc + [StrLenField('framed_route', '', length_from=lambda x: x.length)]


class IE_FramedRouting(IE_Base):
    name = 'IE Framed-Routing'
    ie_type = 154
    fields_desc = IE_Base.fields_desc + [StrLenField('framed_routing', '', length_from=lambda x: x.length)]


class IE_FramedIPv6Route(IE_Base):
    name = 'IE Framed-IPv6-Route'
    ie_type = 155
    fields_desc = IE_Base.fields_desc + [StrLenField('framed_ipv6_route', '', length_from=lambda x: x.length)]
...
class IE_APN_DNN(IE_Base):
    name = 'IE APN/DNN'
    ie_type = 159
    fields_desc = IE_Base.fields_desc + [APNStrLenField('apn_dnn', '', length_from=lambda x: x.length)]
...
class IE_NotImplemented(IE_Base):
    ...
    ie_type = 0
    fields_desc = IE_Base.fields_desc + [StrLenField('data', '', length_from=lambda x: x.length)]
...
class PFCPHeartbeatRequest(Packet):
    ...
    fields_desc = [PacketListField('IE_list', [IE_RecoveryTimeStamp()], IE_Dispatcher)]
