...
class VRRP(Packet):
    fields_desc = [
        ...
        FieldListField('addrlist', [], IPField('', '0.0.0.0'), count_from=lambda pkt: pkt.ipcount),
        IntField('auth1', 0),
        IntField('auth2', 0),
    ]
...
class VRRPv3(Packet):
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (FieldListField('addrlist', [], IPField('', '0.0.0.0'), count_from=lambda pkt: pkt.ipcount), lambda p: isinstance(p.underlayer, IP)),
                (FieldListField('addrlist', [], IP6Field('', '::'), count_from=lambda pkt: pkt.ipcount), lambda p: isinstance(p.underlayer, IPv6)),
            ],
            StrField('addrlist', ''),
        ),
    ]
