...
class NTLM_SMB_Server(NTLM_Server, Automaton):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.REAL_HOSTNAME = kwargs.pop('REAL_HOSTNAME', None)  # Compulsory for SMB1 !!!
    ...
    def on_negotiate(self, pkt):
        ...
        if self.SMB2:
            ...
            resp = self.smb_header.copy() / cls(
                ...
                SecurityMode=3 if self.REQUIRE_SIGNATURE else self.get('SecurityMode', bool(self.IDENTITIES)),
            ...
            resp = self.smb_header.copy() / cls(
                ...
                ServerCapabilities=('UNICODE+LARGE_FILES+NT_SMBS+RPC_REMOTE_APIS+STATUS32+' 'LEVEL_II_OPLOCKS+LOCK_AND_READ+NT_FIND+' 'LWIO+INFOLEVEL_PASSTHRU+LARGE_READX+LARGE_WRITEX'),
                SecurityMode=(3 if self.REQUIRE_SIGNATURE else self.get('SecurityMode', bool(self.IDENTITIES))),
        ...
        if self.EXTENDED_SECURITY or self.SMB2:
            ...
            if self.PASS_NEGOEX:  # NEGOEX handling
                ...
                resp.SecurityBlob.innerContextToken.token.mechToken = SPNEGO_Token(value=negoex_token)  # noqa: E501
        else:
            # Non-extended SMB1
    ...
    def on_setup_andx_request(self, pkt):
        ...
        if SMBSession_Setup_AndX_Request_Extended_Security in pkt or SMBSession_Setup_AndX_Request in pkt or SMB2_Session_Setup_Request in pkt:
            ...
            if ntlm_tuple == (None, None, None, None):
                ...
                if SMBSession_Setup_AndX_Request_Extended_Security in pkt or SMB2_Session_Setup_Request in pkt:
                    ...
                    if not ntlm_token:
                        ...
                        if MIC and not self.DROP_MIC:  # Drop the MIC?
                            resp.SecurityBlob.token.mechListMIC = SPNEGO_MechListMIC(value=MIC)  # noqa: E501
                        if negResult == 0:
                ...
                resp.Status = self.get('Status', 0x0 if self.authenticated else 0xC0000016)
        self.send(resp)
