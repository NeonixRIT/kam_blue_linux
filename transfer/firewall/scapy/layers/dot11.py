...
from scapy.data import ETHER_ANY, DLT_IEEE802_11, DLT_PRISM_HEADER, DLT_IEEE802_11_RADIO
...
_rt_flags = ['CFP', 'ShortPreamble', 'wep', 'fragment', 'FCS', 'pad', 'badFCS', 'ShortGI']

_rt_channelflags = ['res1', 'res2', 'res3', 'res4', 'Turbo', 'CCK', 'OFDM', '2GHz', '5GHz', 'Passive', 'Dynamic_CCK_OFDM', 'GFSK', 'GSM', 'StaticTurbo', '10MHz', '5MHz']
...
_rt_tsflags = ['32-bit_counter', 'Accuracy', 'res1', 'res2', 'res3', 'res4', 'res5', 'res6']

_rt_knownmcs = ['MCS_bandwidth', 'MCS_index', 'guard_interval', 'HT_format', 'FEC_type', 'STBC_streams', 'Ness', 'Ness_MSB']

_rt_bandwidth = {0: '20MHz', 1: '40MHz', 2: 'ht40Mhz-', 3: 'ht40MHz+'}

_rt_a_mpdu_flags = ['Report0Subframe', 'Is0Subframe', 'KnownLastSubframe', 'LastSubframe', 'CRCerror', 'EOFsubframe', 'KnownEOF', 'res1', 'res2', 'res3', 'res4', 'res5', 'res6', 'res7', 'res8']
...
_rt_knownvht = ['STBC', 'TXOP_PS_NOT_ALLOWED', 'GuardInterval', 'SGINsysmDis', 'LDPCextraOFDM', 'Beamformed', 'Bandwidth', 'GroupID', 'PartialAID', 'res1', 'res2', 'res3', 'res4', 'res5', 'res6', 'res7']

_rt_presentvht = ['STBC', 'TXOP_PS_NOT_ALLOWED', 'GuardInterval', 'SGINsysmDis', 'LDPCextraOFDM', 'Beamformed', 'res1', 'res2']
...
class RadioTapExtendedPresenceMask(Packet):
    ...
    fields_desc = [FlagsField('present', None, -32, ['b%s' % i for i in range(0, 31)] + ['Ext'])]
...
class RadioTapTLV(Packet):
    fields_desc = [
        ...
        ConditionalField(ByteField('subtype', 0), lambda pkt: pkt.type == 30),
        ConditionalField(LEShortField('presence_type', 0), lambda pkt: pkt.type == 30),
        ConditionalField(LEShortField('reserved', 0), lambda pkt: pkt.type == 30),
...
class RadioTap(Packet):
    ...
    fields_desc = [
        ...
        # to handle padding
        # TSFT
        ConditionalField(ReversePadField(LELongField('mac_timestamp', 0), 8), lambda pkt: pkt.present and pkt.present.TSFT),
        # Flags
        ConditionalField(FlagsField('Flags', None, -8, _rt_flags), lambda pkt: pkt.present and pkt.present.Flags),
        # Rate
        ConditionalField(ScalingField('Rate', 0, scaling=0.5, unit='Mbps', fmt='B'), lambda pkt: pkt.present and pkt.present.Rate),
        # Channel
        ConditionalField(ReversePadField(LEShortField('ChannelFrequency', 0), 2), lambda pkt: pkt.present and pkt.present.Channel),
        ConditionalField(FlagsField('ChannelFlags', None, -16, _rt_channelflags), lambda pkt: pkt.present and pkt.present.Channel),
        # dBm_AntSignal
        ConditionalField(ScalingField('dBm_AntSignal', 0, unit='dBm', fmt='b'), lambda pkt: pkt.present and pkt.present.dBm_AntSignal),
        # dBm_AntNoise
        ConditionalField(ScalingField('dBm_AntNoise', 0, unit='dBm', fmt='b'), lambda pkt: pkt.present and pkt.present.dBm_AntNoise),
        # Lock_Quality
        ConditionalField(ReversePadField(LEShortField('Lock_Quality', 0), 2), lambda pkt: pkt.present and pkt.present.Lock_Quality),
        # Antenna
        ConditionalField(ByteField('Antenna', 0), lambda pkt: pkt.present and pkt.present.Antenna),
        # RX Flags
        ConditionalField(ReversePadField(FlagsField('RXFlags', None, -16, _rt_rxflags), 2), lambda pkt: pkt.present and pkt.present.RXFlags),
        # TX Flags
        ConditionalField(ReversePadField(FlagsField('TXFlags', None, -16, _rt_txflags), 2), lambda pkt: pkt.present and pkt.present.TXFlags),
        # ChannelPlus
        ConditionalField(ReversePadField(FlagsField('ChannelPlusFlags', None, -32, _rt_channelflags2), 4), lambda pkt: pkt.present and pkt.present.ChannelPlus),
        ConditionalField(LEShortField('ChannelPlusFrequency', 0), lambda pkt: pkt.present and pkt.present.ChannelPlus),
        ConditionalField(ByteField('ChannelPlusNumber', 0), lambda pkt: pkt.present and pkt.present.ChannelPlus),
        # MCS
        ConditionalField(ReversePadField(FlagsField('knownMCS', None, -8, _rt_knownmcs), 2), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(BitField('Ness_LSB', 0, 1), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(BitField('STBC_streams', 0, 2), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(BitEnumField('FEC_type', 0, 1, {0: 'BCC', 1: 'LDPC'}), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(BitEnumField('HT_format', 0, 1, {0: 'mixed', 1: 'greenfield'}), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(
            BitEnumField('guard_interval', 0, 1, {0: 'Long_GI', 1: 'Short_GI'}),  # noqa: E501
            lambda pkt: pkt.present and pkt.present.MCS,
        ),
        ConditionalField(BitEnumField('MCS_bandwidth', 0, 2, _rt_bandwidth), lambda pkt: pkt.present and pkt.present.MCS),
        ConditionalField(ByteField('MCS_index', 0), lambda pkt: pkt.present and pkt.present.MCS),
        # A_MPDU
        ConditionalField(ReversePadField(LEIntField('A_MPDU_ref', 0), 4), lambda pkt: pkt.present and pkt.present.A_MPDU),
        ConditionalField(FlagsField('A_MPDU_flags', None, -32, _rt_a_mpdu_flags), lambda pkt: pkt.present and pkt.present.A_MPDU),
        # VHT
        ConditionalField(ReversePadField(FlagsField('KnownVHT', None, -16, _rt_knownvht), 2), lambda pkt: pkt.present and pkt.present.VHT),
        ConditionalField(FlagsField('PresentVHT', None, -8, _rt_presentvht), lambda pkt: pkt.present and pkt.present.VHT),
        ConditionalField(ByteEnumField('VHT_bandwidth', 0, _rt_vhtbandwidth), lambda pkt: pkt.present and pkt.present.VHT),
        ConditionalField(StrFixedLenField('mcs_nss', 0, length=5), lambda pkt: pkt.present and pkt.present.VHT),
        ConditionalField(ByteField('GroupID', 0), lambda pkt: pkt.present and pkt.present.VHT),
        ConditionalField(ShortField('PartialAID', 0), lambda pkt: pkt.present and pkt.present.VHT),
        # timestamp
        ConditionalField(ReversePadField(LELongField('timestamp', 0), 8), lambda pkt: pkt.present and pkt.present.timestamp),
        ConditionalField(LEShortField('ts_accuracy', 0), lambda pkt: pkt.present and pkt.present.timestamp),
        ConditionalField(BitEnumField('ts_unit', 0, 4, {0: 'milliseconds', 1: 'microseconds', 2: 'nanoseconds'}), lambda pkt: pkt.present and pkt.present.timestamp),
        ConditionalField(BitField('ts_position', 0, 4), lambda pkt: pkt.present and pkt.present.timestamp),
        ConditionalField(FlagsField('ts_flags', None, 8, _rt_tsflags), lambda pkt: pkt.present and pkt.present.timestamp),
        # HE - XXX not complete
        ConditionalField(ReversePadField(LEShortField('he_data1', 0), 2), lambda pkt: pkt.present and pkt.present.HE),
        ConditionalField(LEShortField('he_data2', 0), lambda pkt: pkt.present and pkt.present.HE),
        ConditionalField(LEShortField('he_data3', 0), lambda pkt: pkt.present and pkt.present.HE),
        ConditionalField(LEShortField('he_data4', 0), lambda pkt: pkt.present and pkt.present.HE),
        ConditionalField(LEShortField('he_data5', 0), lambda pkt: pkt.present and pkt.present.HE),
        ConditionalField(LEShortField('he_data6', 0), lambda pkt: pkt.present and pkt.present.HE),
        # HE_MU
        ConditionalField(ReversePadField(LEShortField('hemu_flags1', 0), 2), lambda pkt: pkt.present and pkt.present.HE_MU),
        ConditionalField(LEShortField('hemu_flags2', 0), lambda pkt: pkt.present and pkt.present.HE_MU),
        ConditionalField(FieldListField('RU_channel1', [], ByteField('', 0), length_from=lambda x: 4), lambda pkt: pkt.present and pkt.present.HE_MU),
        ConditionalField(FieldListField('RU_channel2', [], ByteField('', 0), length_from=lambda x: 4), lambda pkt: pkt.present and pkt.present.HE_MU),
        # HE_MU_other_user
        ConditionalField(ReversePadField(LEShortField('hemuou_per_user_1', 0x7FFF), 2), lambda pkt: pkt.present and pkt.present.HE_MU_other_user),
        ConditionalField(LEShortField('hemuou_per_user_2', 0x003F), lambda pkt: pkt.present and pkt.present.HE_MU_other_user),
        ConditionalField(ByteField('hemuou_per_user_position', 0), lambda pkt: pkt.present and pkt.present.HE_MU_other_user),
        ConditionalField(FlagsField('hemuou_per_user_known', 0, -16, _rt_hemuother_per_user_known), lambda pkt: pkt.present and pkt.present.HE_MU_other_user),
        # L_SIG
        ConditionalField(ReversePadField(FlagsField('lsig_data1', 0, -16, ['rate', 'length']), 2), lambda pkt: pkt.present and pkt.present.L_SIG),
        ConditionalField(BitField('lsig_length', 0, 12, tot_size=-2), lambda pkt: pkt.present and pkt.present.L_SIG),
        ConditionalField(BitField('lsig_rate', 0, 4, end_tot_size=-2), lambda pkt: pkt.present and pkt.present.L_SIG),
        # TLV fields
        ConditionalField(
            ReversePadField(PacketListField('tlvs', [], RadioTapTLV), 4),
...
class Dot11(Packet):
    name = '802.11'
    fields_desc = [
        BitMultiEnumField('subtype', 0, 4, _dot11_subtypes, lambda pkt: pkt.type),
        BitEnumField('type', 0, 2, ['Management', 'Control', 'Data', 'Extension']),
        BitField('proto', 0, 2),
        ConditionalField(BitEnumField('cfe', 0, 4, _dot11_cfe), lambda pkt: (pkt.type, pkt.subtype) == (1, 6)),
        MultipleTypeField(
            [(FlagsField('FCfield', 0, 4, ['pw-mgt', 'MD', 'protected', 'order']), lambda pkt: (pkt.type, pkt.subtype) == (1, 6))], FlagsField('FCfield', 0, 8, ['to-DS', 'from-DS', 'MF', 'retry', 'pw-mgt', 'MD', 'protected', 'order'])
        ),
        ShortField('ID', 0),
        _Dot11MacField('addr1', ETHER_ANY, 1),
        ConditionalField(
            _Dot11MacField('addr2', ETHER_ANY, 2),
            lambda pkt: (pkt.type != 1 or pkt.subtype in [0x4, 0x5, 0x6, 0x8, 0x9, 0xA, 0xB, 0xE, 0xF]),
        ),
        ConditionalField(
            _Dot11MacField('addr3', ETHER_ANY, 3),
            lambda pkt: (pkt.type in [0, 2] or ((pkt.type, pkt.subtype) == (1, 6) and pkt.cfe == 6)),
        ),
        ConditionalField(LEShortField('SC', 0), lambda pkt: pkt.type != 1),
        ConditionalField(
            _Dot11MacField('addr4', ETHER_ANY, 4),
            lambda pkt: (pkt.type == 2 and pkt.FCfield & 3 == 3),  # from-DS+to-DS
        ),
    ]
...
class Dot11QoS(Packet):
    name = '802.11 QoS'
    fields_desc = [BitField('A_MSDU_Present', 0, 1), BitField('Ack_Policy', 0, 2), BitField('EOSP', 0, 1), BitField('TID', 0, 4), ByteField('TXOP', 0)]
...
capability_list = ['res8', 'res9', 'short-slot', 'res11', 'res12', 'DSSS-OFDM', 'res14', 'res15', 'ESS', 'IBSS', 'CFP', 'CFP-req', 'privacy', 'short-preamble', 'PBCC', 'agility']

reason_code = {0: 'reserved', 1: 'unspec', 2: 'auth-expired', 3: 'deauth-ST-leaving', 4: 'inactivity', 5: 'AP-full', 6: 'class2-from-nonauth', 7: 'class3-from-nonass', 8: 'disas-ST-leaving', 9: 'ST-not-auth'}

status_code = {0: 'success', 1: 'failure', 10: 'cannot-support-all-cap', 11: 'inexist-asso', 12: 'asso-denied', 13: 'algo-unsupported', 14: 'bad-seq-num', 15: 'challenge-failure', 16: 'timeout', 17: 'AP-full', 18: 'rate-unsupported'}
    ...
    def network_stats(self):
        ...
        while isinstance(p, Dot11Elt):
            ...
            if p.ID == 0 and 'ssid' not in summary:
                ...
                rates = [(x & 0x7F) / 2.0 for x in p.rates]
                if 'rates' in summary:
...
_dot11_id_enum = (lambda x: _dot11_info_elts_ids.get(x, x), lambda x: _dot11_info_elts_ids_rev.get(x, x))
...
class Dot11Elt(Packet):
    ...
    fields_desc = [ByteEnumField('ID', 0, _dot11_id_enum), FieldLenField('len', None, 'info', 'B'), StrLenField('info', '', length_from=lambda x: x.len, max_length=255)]
    show_indent = 0
...
class PMKIDListPacket(Packet):
    name = 'PMKIDs'
    fields_desc = [LEFieldLenField('nb_pmkids', None, count_of='pmkid_list'), FieldListField('pmkid_list', None, XStrFixedLenField('', '', length=16), count_from=lambda pkt: pkt.nb_pmkids)]
...
class Dot11EltRSN(Dot11Elt):
    ...
    fields_desc = [
        ...
        LEFieldLenField('nb_pairwise_cipher_suites', None, count_of='pairwise_cipher_suites'),
        PacketListField('pairwise_cipher_suites', [RSNCipherSuite()], RSNCipherSuite, count_from=lambda p: p.nb_pairwise_cipher_suites),
        LEFieldLenField('nb_akm_suites', None, count_of='akm_suites'),
        PacketListField('akm_suites', [AKMSuite()], AKMSuite, count_from=lambda p: p.nb_akm_suites),
        # RSN Capabilities
        ...
        ConditionalField(PacketField('pmkids', PMKIDListPacket(), PMKIDListPacket), lambda pkt: (True if pkt.len is None else pkt.len - (12 + (pkt.nb_pairwise_cipher_suites or 0) * 4 + (pkt.nb_akm_suites or 0) * 4) >= 2)),
        ConditionalField(
            PacketField('group_management_cipher_suite', RSNCipherSuite(cipher=0x6), RSNCipherSuite),
            lambda pkt: (True if pkt.len is None else pkt.len - (12 + (pkt.nb_pairwise_cipher_suites or 0) * 4 + (pkt.nb_akm_suites or 0) * 4 + (2 if pkt.pmkids else 0) + (pkt.pmkids and pkt.pmkids.nb_pmkids or 0) * 16) >= 4),
        ),
    ]
...
class Dot11EltCountryConstraintTriplet(Packet):
    ...
    fields_desc = [ByteField('first_channel_number', 1), ByteField('num_channels', 24), ByteField('mtp', 0)]
...
class Dot11EltCountry(Dot11Elt):
    ...
    fields_desc = [
        ...
        PacketListField('descriptors', [], Dot11EltCountryConstraintTriplet, length_from=lambda pkt: (pkt.len - 3 - (pkt.len % 3))),
        ConditionalField(ByteField('pad', 0), lambda pkt: (len(pkt.descriptors) + 1) % 2),
    ]
...
class Dot11EltRates(Dot11Elt):
    name = '802.11 Rates'
    match_subclass = True
    fields_desc = [ByteEnumField('ID', 1, _dot11_id_enum), ByteField('len', None), FieldListField('rates', [0x82], _RateField('', 0), length_from=lambda p: p.len)]
...
class Dot11EltHTCapabilities(Dot11Elt):
    ...
    fields_desc = [
        ...
        BitEnumField('SM_Power_Save', 0, 2, {0: 'static SM', 1: 'dynamic SM', 3: 'disabled'}),
        BitEnumField('Supported_Channel_Width', 0, 1, {0: '20Mhz', 1: '20Mhz+40Mhz'}),
        ...
        BitField('Noncompressed_Steering_n_Beamformer_Antennas_Supported', 0, 2),
        ...
        BitField('Implicit_Transmit_Beamforming_Receiving', 0, 1, end_tot_size=-4),
...
class Dot11EltVendorSpecific(Dot11Elt):
    ...
    match_subclass = True
    fields_desc = [ByteEnumField('ID', 221, _dot11_id_enum), ByteField('len', None), OUIField('oui', 0x000000), StrLenField('info', '', length_from=lambda x: x.len - 3)]
...
class Dot11EltMicrosoftWPA(Dot11EltVendorSpecific):
    ...
    fields_desc = Dot11EltVendorSpecific.fields_desc[:3] + [XByteField('type', 0x01)] + Dot11EltRSN.fields_desc[2:8]
...
class Dot11Beacon(_Dot11EltUtils):
    name = '802.11 Beacon'
    fields_desc = [LELongField('timestamp', 0), LEShortField('beacon_interval', 0x0064), FlagsField('cap', 0, 16, capability_list)]
...
class Dot11AssoReq(_Dot11EltUtils):
    ...
    fields_desc = [FlagsField('cap', 0, 16, capability_list), LEShortField('listen_interval', 0x00C8)]


class Dot11AssoResp(_Dot11EltUtils):
    name = '802.11 Association Response'
    fields_desc = [FlagsField('cap', 0, 16, capability_list), LEShortField('status', 0), LEShortField('AID', 0)]


class Dot11ReassoReq(_Dot11EltUtils):
    name = '802.11 Reassociation Request'
    fields_desc = [FlagsField('cap', 0, 16, capability_list), LEShortField('listen_interval', 0x00C8), MACField('current_AP', ETHER_ANY)]
...
class Dot11Auth(_Dot11EltUtils):
    ...
    fields_desc = [LEShortEnumField('algo', 0, ['open', 'sharedkey']), LEShortField('seqnum', 0), LEShortEnumField('status', 0, status_code)]
...
class Dot11WEP(Dot11Encrypted):
    ...
    fields_desc = [StrFixedLenField('iv', b'\0\0\0', 3), ByteField('keyid', 0), StrField('wepdata', None, remain=4), IntField('icv', None)]
