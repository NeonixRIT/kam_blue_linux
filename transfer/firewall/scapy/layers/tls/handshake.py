...
from scapy.layers.tls.keyexchange import _TLSSignature, _TLSServerParamsField, _TLSSignatureField, ServerRSAParams, SigAndHashAlgsField, _tls_hash_sig, SigAndHashAlgsLenField
from scapy.layers.tls.session import _GenericTLSSessionInheritance, readConnState, writeConnState
from scapy.layers.tls.keyexchange_tls13 import TLS_Ext_PreSharedKey_CH
from scapy.layers.tls.crypto.compression import _tls_compression_algs, _tls_compression_algs_cls, Comp_NULL, _GenericComp, _GenericCompMetaclass
from scapy.layers.tls.crypto.suites import _tls_cipher_suites, _tls_cipher_suites_cls, _GenericCipherSuite, _GenericCipherSuiteMetaclass
...
class _TLSHandshake(_GenericTLSSessionInheritance):
    ...
    fields_desc = [ByteEnumField('msgtype', None, _tls_handshake_type), ThreeBytesField('msglen', None), StrLenField('msg', '', length_from=lambda pkt: pkt.msglen)]
...
class TLSHelloRequest(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 0, _tls_handshake_type), ThreeBytesField('msglen', None)]
...
class TLSClientHello(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _CipherSuitesField('ciphers', None, _tls_cipher_suites, itemfmt='!H', length_from=lambda pkt: pkt.cipherslen),
        FieldLenField('complen', None, fmt='B', length_of='comp'),
        _CompressionMethodsField('comp', [0], _tls_compression_algs, itemfmt='B', length_from=lambda pkt: pkt.complen),  # noqa: E501
...
class TLSServerHello(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _CompressionMethodsField('comp', [0], _tls_compression_algs, itemfmt='B', length_from=lambda pkt: 1),
        _ExtensionsLenField('extlen', None, length_of='ext'),
        _ExtensionsField('ext', None, length_from=lambda pkt: (pkt.msglen - (pkt.sidlen or 0) - 40)),
    ]
...
_tls_13_server_hello_fields = [
    ...
    _CompressionMethodsField('comp', [0], _tls_compression_algs, itemfmt='B', length_from=lambda pkt: 1),
    _ExtensionsLenField('extlen', None, length_of='ext'),
    _ExtensionsField('ext', None, length_from=lambda pkt: (pkt.msglen - 38)),
]
...
class TLSEncryptedExtensions(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 8, _tls_handshake_type), ThreeBytesField('msglen', None), _ExtensionsLenField('extlen', None, length_of='ext'), _ExtensionsField('ext', None, length_from=lambda pkt: pkt.msglen - 2)]
...
class TLSCertificate(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 11, _tls_handshake_type), ThreeBytesField('msglen', None), _ASN1CertLenField('certslen', None, length_of='certs'), _ASN1CertListField('certs', [], length_from=lambda pkt: pkt.certslen)]
...
class _ASN1CertAndExt(_GenericTLSSessionInheritance):
    ...
    fields_desc = [_ASN1CertField('cert', ''), FieldLenField('extlen', None, length_of='ext'), _ExtensionsField('ext', [], length_from=lambda pkt: pkt.extlen)]
...
class TLS13Certificate(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField('msgtype', 11, _tls_handshake_type),
        ThreeBytesField('msglen', None),
        FieldLenField('cert_req_ctxt_len', None, fmt='B', length_of='cert_req_ctxt'),
        StrLenField('cert_req_ctxt', '', length_from=lambda pkt: pkt.cert_req_ctxt_len),
        _ASN1CertLenField('certslen', None, length_of='certs'),
        _ASN1CertAndExtListField('certs', [], _ASN1CertAndExt, length_from=lambda pkt: pkt.certslen),
    ]  # noqa: E501
...
class TLSServerKeyExchange(_TLSHandshake):
    ...
    fields_desc = [
        ...
        ThreeBytesField('msglen', None),
        _TLSServerParamsField('params', None, length_from=lambda pkt: pkt.msglen),
        _TLSSignatureField('sig', None, length_from=lambda pkt: pkt.msglen - len(pkt.params)),
    ]  # noqa: E501
...
class TLSCertificateRequest(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField('msgtype', 13, _tls_handshake_type),
        ThreeBytesField('msglen', None),
        FieldLenField('ctypeslen', None, fmt='B', length_of='ctypes'),
        _CertTypesField('ctypes', [1, 64], _tls_client_certificate_types, itemfmt='!B', length_from=lambda pkt: pkt.ctypeslen),
        SigAndHashAlgsLenField('sig_algs_len', None, length_of='sig_algs'),
        SigAndHashAlgsField(
            'sig_algs',
            [0x0403, 0x0401, 0x0201],
            ShortEnumField('hash_sig', None, _tls_hash_sig),  # noqa: E501
            length_from=lambda pkt: pkt.sig_algs_len,
        ),  # noqa: E501
        FieldLenField('certauthlen', None, fmt='!H', length_of='certauth'),
        _CertAuthoritiesField('certauth', [], length_from=lambda pkt: pkt.certauthlen),
    ]  # noqa: E501
...
class TLS13CertificateRequest(_TLSHandshake):
    ...
    fields_desc = [
        ...
        StrLenField('cert_req_ctxt', '', length_from=lambda pkt: pkt.cert_req_ctxt_len),
        _ExtensionsLenField('extlen', None, length_of='ext'),
        _ExtensionsField('ext', None, length_from=lambda pkt: pkt.msglen - pkt.cert_req_ctxt_len - 3),
    ]
...
class TLSServerHelloDone(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 14, _tls_handshake_type), ThreeBytesField('msglen', None)]
...
class TLSCertificateVerify(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 15, _tls_handshake_type), ThreeBytesField('msglen', None), _TLSSignatureField('sig', None, length_from=lambda pkt: pkt.msglen)]
...
class TLSClientKeyExchange(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 16, _tls_handshake_type), ThreeBytesField('msglen', None), _TLSCKExchKeysField('exchkeys', length_from=lambda pkt: pkt.msglen)]
...
class TLSFinished(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 20, _tls_handshake_type), ThreeBytesField('msglen', None), _VerifyDataField('vdata', None)]
...
class TLSHelloVerifyRequest(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 21, _tls_handshake_type), ThreeBytesField('msglen', None), FieldLenField('cookielen', None, fmt='B', length_of='cookie'), StrLenField('cookie', '', length_from=lambda pkt: pkt.cookielen)]
...
_tls_cert_chain_types = {0: 'individual_certs', 1: 'pkipath'}


class URLAndOptionalHash(Packet):
    name = 'URLAndOptionHash structure for TLSCertificateURL'
    fields_desc = [
        FieldLenField('urllen', None, length_of='url'),
        StrLenField('url', '', length_from=lambda pkt: pkt.urllen),
        FieldLenField('hash_present', None, fmt='B', length_of='hash', adjust=lambda pkt, x: int(math.ceil(x / 20.0))),  # noqa: E501
        StrLenField('hash', '', length_from=lambda pkt: 20 * pkt.hash_present),
    ]
...
class TLSCertificateURL(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField('msgtype', 21, _tls_handshake_type),
        ThreeBytesField('msglen', None),
        ByteEnumField('certchaintype', None, _tls_cert_chain_types),
        FieldLenField('uahlen', None, length_of='uah'),
        PacketListField('uah', [], URLAndOptionalHash, length_from=lambda pkt: pkt.uahlen),
    ]
...
class TLSCertificateStatus(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField('msgtype', 22, _tls_handshake_type),
        ThreeBytesField('msglen', None),
        ByteEnumField('status_type', 1, _cert_status_type),
        ThreeBytesLenField('responselen', None, length_of='response'),
        _StatusField('response', None, Raw, length_from=lambda pkt: pkt.responselen),
    ]
...
class SupDataEntry(Packet):
    ...
    fields_desc = [ShortField('sdtype', None), FieldLenField('len', None, length_of='data'), StrLenField('data', '', length_from=lambda pkt: pkt.len)]
...
class UserMappingData(Packet):
    ...
    fields_desc = [ByteField('version', None), FieldLenField('len', None, length_of='data'), StrLenField('data', '', length_from=lambda pkt: pkt.len)]
...
class SupDataEntryUM(Packet):
    ...
    fields_desc = [
        ShortField('sdtype', None),
        FieldLenField('len', None, length_of='data', adjust=lambda pkt, x: x + 2),
        FieldLenField('dlen', None, length_of='data'),
        PacketListField('data', [], UserMappingData, length_from=lambda pkt: pkt.dlen),
    ]
...
class TLSSupplementalData(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField('msgtype', 23, _tls_handshake_type),
        ThreeBytesField('msglen', None),
        ThreeBytesLenField('sdatalen', None, length_of='sdata'),
        PacketListField('sdata', [], SupDataEntry, length_from=lambda pkt: pkt.sdatalen),
    ]
...
class TLS13NewSessionTicket(_TLSHandshake):
    ...
    fields_desc = [
        ...
        FieldLenField('noncelen', None, fmt='B', length_of='ticket_nonce'),
        StrLenField('ticket_nonce', '', length_from=lambda pkt: pkt.noncelen),
        FieldLenField('ticketlen', None, length_of='ticket'),
        # TicketField("ticket", "",
        StrLenField('ticket', '', length_from=lambda pkt: pkt.ticketlen),
        _ExtensionsLenField('extlen', None, length_of='ext'),
...
class TLS13EndOfEarlyData(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 5, _tls_handshake_type), ThreeBytesField('msglen', None)]
...
class TLS13KeyUpdate(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 24, _tls_handshake_type), ThreeBytesField('msglen', None), ByteEnumField('request_update', 0, _key_update_request)]
...
_tls_handshake_cls = {
    ...
    1: TLSClientHello,
    2: TLSServerHello,
    3: TLSHelloVerifyRequest,
    4: TLSNewSessionTicket,
    8: TLSEncryptedExtensions,
    11: TLSCertificate,
    12: TLSServerKeyExchange,
    13: TLSCertificateRequest,
    14: TLSServerHelloDone,
    15: TLSCertificateVerify,
    16: TLSClientKeyExchange,
    20: TLSFinished,
    21: TLSCertificateURL,
    22: TLSCertificateStatus,
    23: TLSSupplementalData,
}

_tls13_handshake_cls = {
    1: TLS13ClientHello,
    2: TLS13ServerHello,
