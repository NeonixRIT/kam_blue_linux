...
from scapy.layers.tls.handshake import _tls_handshake_cls, _TLSHandshake, _tls13_handshake_cls, TLS13ServerHello, TLS13ClientHello
from scapy.layers.tls.basefields import _TLSVersionField, _tls_version, _TLSIVField, _TLSMACField, _TLSPadField, _TLSPadLenField, _TLSLengthField, _tls_type
...
class _TLSMsgListField(PacketListField):
    ...
    def __init__(self, name, default, length_from=None):
        ...
        super(_TLSMsgListField, self).__init__(name, default, None, length_from=length_from)
    ...
    def m2i(self, pkt, m):
        ...
        if pkt.type == 22:
            if len(m) >= 1:
                ...
                if (pkt.tls_session.advertised_tls_version == 0x0304 and pkt.tls_session.tls_version is None) or pkt.tls_session.tls_version == 0x0304:
    ...
    def addfield(self, pkt, s, val):
        ...
        if (
            isinstance(pkt, _GenericTLSSessionInheritance)
            and _tls_version_check(pkt.tls_session.tls_version, 0x0304)
            and not isinstance(pkt.msg[0], TLS13ServerHello)
            and not isinstance(pkt.msg[0], TLS13ClientHello)
            and not isinstance(pkt.msg[0], TLSChangeCipherSpec)
        ):
            return s + res
...
class TLS(_GenericTLSSessionInheritance):
        ...
        if _pkt is not None:
            ...
            if plen >= 2:
                ...
                if byte0 not in _tls_type or byte1 != 3:  # Unknown type
                    ...
                    if s and (s.tls_version == 0x0002 or s.advertised_tls_version == 0x0002) or (_ssl_looks_like_sslv2(_pkt) and (not s or s.tls_version is None)):
                ...
                if s and _tls_version_check(s.tls_version, 0x0304):
                    _has_cipher = lambda x: (x and not isinstance(x.cipher, Cipher_NULL))
                    if (_has_cipher(s.rcs) or _has_cipher(s.prcs)) and byte0 == 0x17:
    ...
    def _tls_auth_decrypt(self, hdr, s):
        ...
        try:
            ...
            return self.tls_session.rcs.cipher.auth_decrypt(add_data, s, read_seq_num)
    ...
    def pre_dissect(self, s):
        ...
        if cipher_type == 'block':
            ...
            if conf.crypto_valid_advanced and isinstance(self.tls_session.rcs.cipher, Cipher_CHACHA20_POLY1305):  # noqa: E501
                iv = b''
    ...
    def post_dissect(self, s):
        ...
        """
        if self.tls_session.tls_version and self.tls_session.tls_version <= 0x0303:
    ...
    def do_dissect_payload(self, s):
        ...
        if s:
            try:
                p = TLS(s, _internal=1, _underlayer=self, tls_session=self.tls_session)
    ...
    def _tls_auth_encrypt(self, s):
        ...
        add_data = write_seq_num + pkcs_i2osp(self.type, 1) + pkcs_i2osp(self.version, 2) + pkcs_i2osp(len(s), 2)
        return self.tls_session.wcs.cipher.auth_encrypt(s, add_data, write_seq_num)
...
class TLSAlert(_GenericTLSSessionInheritance):
    name = 'TLS Alert'
    fields_desc = [ByteEnumField('level', None, _tls_alert_level), ByteEnumField('descr', None, _tls_alert_description)]
