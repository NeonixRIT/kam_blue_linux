...
class _TLSSignature(_GenericTLSSessionInheritance):
    ...
    fields_desc = [SigAndHashAlgField('sig_alg', 0x0804, _tls_hash_sig), SigLenField('sig_len', None, fmt='!H', length_of='sig_val'), SigValField('sig_val', None, length_from=lambda pkt: pkt.sig_len)]
...
class ECPentanomialBasis(Packet):
    ...
    val = 1
    fields_desc = [
        FieldLenField('k1len', None, length_of='k1', fmt='B'),
...
class ServerECDHExplicitPrimeParams(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        ByteEnumField('curve_type', 1, _tls_ec_curve_types),
        ...
        StrLenField('base', '', length_from=lambda pkt: pkt.baselen),
        FieldLenField('orderlen', None, length_of='order', fmt='B'),
        StrLenField('order', '', length_from=lambda pkt: pkt.orderlen),
        FieldLenField('cofactorlen', None, length_of='cofactor', fmt='B'),
        StrLenField('cofactor', '', length_from=lambda pkt: pkt.cofactorlen),
        FieldLenField('pointlen', None, length_of='point', fmt='B'),
        StrLenField('point', '', length_from=lambda pkt: pkt.pointlen),
    ]
...
class ServerECDHExplicitChar2Params(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        ByteEnumField('curve_type', 2, _tls_ec_curve_types),
        ShortField('m', None),
        _ECBasisTypeField('basis_type', None, _tls_ec_basis_types, 'basis'),
        _ECBasisField('basis', ECTrinomialBasis(), lambda pkt: pkt.basis_type, _tls_ec_basis_cls),
...
class ServerRSAParams(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        FieldLenField('rsamodlen', None, length_of='rsamod'),
        StrLenField('rsamod', '', length_from=lambda pkt: pkt.rsamodlen),
        FieldLenField('rsaexplen', None, length_of='rsaexp'),
        StrLenField('rsaexp', '', length_from=lambda pkt: pkt.rsaexplen),
    ]
...
class ServerPSKParams(Packet):
    ...
    fields_desc = [FieldLenField('psk_identity_hint_len', None, length_of='psk_identity_hint', fmt='!H'), StrLenField('psk_identity_hint', '', length_from=lambda pkt: pkt.psk_identity_hint_len)]  # noqa: E501
