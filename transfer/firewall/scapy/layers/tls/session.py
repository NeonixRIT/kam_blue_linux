    ...
    try:
        ...
        with open(filename) as fd:
            for line in fd:
                ...
                keys[data[0]] = {'ClientRandom': client_random, 'Secret': secret}
        return keys
...
class connState(object):
    ...
    def __init__(self, connection_end='server', read_or_write='read', seq_num=0, compression_alg=Comp_NULL, ciphersuite=None, tls_version=0x0303):
    ...
    def debug_repr(self, name, secret):
        if conf.debug_tls and secret:
            log_runtime.debug('TLS: %s %s %s: %s', self.connection_end, self.row, name, repr_hex(secret))

    def derive_keys(self, client_random=b'', server_random=b'', master_secret=b''):
        ...
        key_block = self.prf.derive_key_block(master_secret, server_random, client_random, cs.key_block_len)

        # When slicing the key_block, keep the right half of the material
        skip_first = False
        if (self.connection_end == 'client' and self.row == 'read') or (self.connection_end == 'server' and self.row == 'write'):
            skip_first = True
        ...
        if cs.kx_alg.export:
            ...
            cipher_secret = self.prf.postprocess_key_for_export(
                cipher_secret,
                client_random,
                server_random,
        ...
        if cipher_alg.type == 'stream':
            ...
            if cs.kx_alg.export:
                ...
                iv = self.prf.generate_iv_for_export(client_random, server_random, self.connection_end, self.row, reqLen)
    ...
    def snapshot(self):
        ...
        """
        snap = connState(connection_end=self.connection_end, read_or_write=self.row, seq_num=self.seq_num, compression_alg=type(self.compression), ciphersuite=type(self.ciphersuite), tls_version=self.tls_version)
    ...
    def __repr__(self):
        ...
        res += 'Cipher suite   : %s (0x%04x)\n' % (self.ciphersuite.name, self.ciphersuite.val)
        res += 'Compression    : %s (0x%02x)\n' % (self.compression.name, self.compression.val)
        tabsize = 4
...
class tlsSession(object):
    ...
    def __init__(self, ipsrc=None, ipdst=None, sport=None, dport=None, sid=None, connection_end='server', wcs=None, rcs=None):
    ...
    def mirror(self):
        ...
        self.triggered_prcs_commit, self.triggered_pwcs_commit = self.triggered_pwcs_commit, self.triggered_prcs_commit
    ...
    def compute_master_secret(self):
        ...
        ms = self.pwcs.prf.compute_master_secret(self.pre_master_secret, self.client_random, self.server_random, self.extms, self.session_hash)
    ...
    def compute_ms_and_derive_keys(self):
        ...
        if self.nss_keys and self.nss_keys.get('CLIENT_RANDOM', False) and self.nss_keys['CLIENT_RANDOM'].get('Secret', False):
            self.master_secret = self.nss_keys['CLIENT_RANDOM']['Secret']

        if not self.master_secret:
            self.compute_master_secret()

        self.prcs.derive_keys(client_random=self.client_random, server_random=self.server_random, master_secret=self.master_secret)
        self.pwcs.derive_keys(client_random=self.client_random, server_random=self.server_random, master_secret=self.master_secret)
    ...
    def compute_sslv2_key_material(self):
        ...
        km = self.pwcs.prf.derive_key_block(self.master_secret, self.sslv2_challenge, self.sslv2_connection_id, 2 * self.pwcs.cipher.key_len)
    ...
    def compute_tls13_early_secrets(self, external=False):
        ...
        if 'binder_key' not in self.tls13_derived_secrets:
            if external:
                bk = hkdf.derive_secret(self.tls13_early_secret, b'ext binder', b'')
            else:
                bk = hkdf.derive_secret(self.tls13_early_secret, b'res binder', b'')

            self.tls13_derived_secrets['binder_key'] = bk

        cets = hkdf.derive_secret(self.tls13_early_secret, b'c e traffic', b''.join(self.handshake_messages))

        self.tls13_derived_secrets['client_early_traffic_secret'] = cets
        ees = hkdf.derive_secret(self.tls13_early_secret, b'e exp master', b''.join(self.handshake_messages))
    ...
    def compute_tls13_handshake_secrets(self):
        ...
        chts = hkdf.derive_secret(self.tls13_handshake_secret, b'c hs traffic', b''.join(self.handshake_messages))
        self.tls13_derived_secrets['client_handshake_traffic_secret'] = chts

        shts = hkdf.derive_secret(self.tls13_handshake_secret, b's hs traffic', b''.join(self.handshake_messages))
    ...
    def compute_tls13_traffic_secrets(self):
        ...
        tmp = hkdf.derive_secret(self.tls13_handshake_secret, b'derived', b'')
        self.tls13_master_secret = hkdf.extract(tmp, None)

        cts0 = hkdf.derive_secret(self.tls13_master_secret, b'c ap traffic', b''.join(self.handshake_messages))
        self.tls13_derived_secrets['client_traffic_secrets'] = [cts0]

        sts0 = hkdf.derive_secret(self.tls13_master_secret, b's ap traffic', b''.join(self.handshake_messages))
        self.tls13_derived_secrets['server_traffic_secrets'] = [sts0]

        es = hkdf.derive_secret(self.tls13_master_secret, b'exp master', b''.join(self.handshake_messages))
    ...
    def compute_tls13_resumption_secret(self):
        ...
        if self.connection_end == 'server':
            ...
            hkdf = self.pwcs.hkdf
        rs = hkdf.derive_secret(self.tls13_master_secret, b'res master', b''.join(self.handshake_messages))
    ...
    def consider_read_padding(self):
        ...
        return self.rcs.cipher.type == 'block' and not (False in six.itervalues(self.rcs.cipher.ready))
    ...
    def eq(self, other):
        ok = False
        if self.sport == other.sport and self.dport == other.dport and self.ipsrc == other.ipsrc and self.ipdst == other.ipdst:
            ok = True

        if not ok and self.dport == other.sport and self.sport == other.dport and self.ipdst == other.ipsrc and self.ipsrc == other.ipdst:
            ok = True
    ...
    def __repr__(self):
        ...
        if len(sid) > 12:
            sid = sid[:11] + '...'
        return '%s:%s > %s:%s' % (self.ipsrc, str(self.sport), self.ipdst, str(self.dport))
...
class _GenericTLSSessionInheritance(Packet):
    ...
    def __init__(self, _pkt='', post_transform=None, _internal=0, _underlayer=None, tls_session=None, **fields):
        try:
        ...
        if isinstance(_underlayer, TCP):
            ...
            if self.tls_session.connection_end == 'server':
                ...
                if not self.tls_session.server_rsa_key and srk:
                    self.tls_session.server_rsa_key = srk

        Packet.__init__(self, _pkt=_pkt, post_transform=post_transform, _internal=_internal, _underlayer=_underlayer, **fields)
    ...
    def mysummary(self):
        return 'TLS %s / %s' % (repr(self.tls_session), getattr(self, '_name', self.name))
