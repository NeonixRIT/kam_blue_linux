...
from scapy.fields import ByteEnumField, ByteField, EnumField, FieldLenField, FieldListField, IntField, PacketField, PacketListField, ShortEnumField, ShortField, StrFixedLenField, StrLenField, XStrLenField
from scapy.packet import Packet, Raw, Padding
from scapy.layers.x509 import X509_Extensions
from scapy.layers.tls.basefields import _tls_version
from scapy.layers.tls.keyexchange import SigAndHashAlgsLenField, SigAndHashAlgsField, _tls_hash_sig
...
_tls_hello_retry_magic = b'\xcf!\xadt\xe5\x9aa\x11\xbe\x1d\x8c\x02\x1ee\xb8\x91\xc2\xa2\x11' b'\x16z\xbb\x8c^\x07\x9e\t\xe2\xc8\xa83\x9c'
...
class TLS_Ext_Unknown(_GenericTLSSessionInheritance):
    ...
    fields_desc = [ShortEnumField('type', None, _tls_ext), FieldLenField('len', None, fmt='!H', length_of='val'), StrLenField('val', '', length_from=lambda pkt: pkt.len)]
...
class ServerName(Packet):
    name = 'HostName'
    fields_desc = [ByteEnumField('nametype', 0, _tls_server_name_types), FieldLenField('namelen', None, length_of='servername'), StrLenField('servername', '', length_from=lambda pkt: pkt.namelen)]
...
class TLS_Ext_ServerName(TLS_Ext_PrettyPacketList):  # RFC 4366
    ...
    fields_desc = [
        ShortEnumField('type', 0, _tls_ext),
        FieldLenField('len', None, length_of='servernames', adjust=lambda pkt, x: x + 2),
        ServerLenField('servernameslen', None, length_of='servernames'),
        ServerListField('servernames', [], ServerName, length_from=lambda pkt: pkt.servernameslen),
    ]
...
class TLS_Ext_EncryptedServerName(TLS_Ext_PrettyPacketList):
    ...
    fields_desc = [
        ...
        FieldLenField('key_exchange_len', None, length_of='key_exchange', fmt='H'),
        XStrLenField('key_exchange', '', length_from=lambda pkt: pkt.key_exchange_len),
        FieldLenField('record_digest_len', None, length_of='record_digest'),
        XStrLenField('record_digest', '', length_from=lambda pkt: pkt.record_digest_len),
        FieldLenField('encrypted_sni_len', None, length_of='encrypted_sni', fmt='H'),
        XStrLenField('encrypted_sni', '', length_from=lambda pkt: pkt.encrypted_sni_len),
    ]


class TLS_Ext_MaxFragLen(TLS_Ext_Unknown):  # RFC 4366
    name = 'TLS Extension - Max Fragment Length'
    fields_desc = [ShortEnumField('type', 1, _tls_ext), ShortField('len', None), ByteEnumField('maxfraglen', 4, {1: '2^9', 2: '2^10', 3: '2^11', 4: '2^12'})]


class TLS_Ext_ClientCertURL(TLS_Ext_Unknown):  # RFC 4366
    name = 'TLS Extension - Client Certificate URL'
    fields_desc = [ShortEnumField('type', 2, _tls_ext), ShortField('len', None)]


_tls_trusted_authority_types = {0: 'pre_agreed', 1: 'key_sha1_hash', 2: 'x509_name', 3: 'cert_sha1_hash'}
...
class TAKeySHA1Hash(Packet):
    ...
    fields_desc = [ByteEnumField('idtype', 1, _tls_trusted_authority_types), StrFixedLenField('id', None, 20)]
...
class TAX509Name(Packet):
    ...
    fields_desc = [ByteEnumField('idtype', 2, _tls_trusted_authority_types), FieldLenField('dnlen', None, length_of='dn'), StrLenField('dn', '', length_from=lambda pkt: pkt.dnlen)]
...
class TACertSHA1Hash(Packet):
    ...
    fields_desc = [ByteEnumField('idtype', 3, _tls_trusted_authority_types), StrFixedLenField('id', None, 20)]

    def guess_payload_class(self, p):
        return Padding


_tls_trusted_authority_cls = {0: TAPreAgreed, 1: TAKeySHA1Hash, 2: TAX509Name, 3: TACertSHA1Hash}
...
class TLS_Ext_TrustedCAInd(TLS_Ext_Unknown):  # RFC 4366
    ...
    fields_desc = [ShortEnumField('type', 3, _tls_ext), ShortField('len', None), FieldLenField('talen', None, length_of='ta'), _TAListField('ta', [], Raw, length_from=lambda pkt: pkt.talen)]


class TLS_Ext_TruncatedHMAC(TLS_Ext_Unknown):  # RFC 4366
    name = 'TLS Extension - Truncated HMAC'
    fields_desc = [ShortEnumField('type', 4, _tls_ext), ShortField('len', None)]


class ResponderID(Packet):
    name = 'Responder ID structure'
    fields_desc = [FieldLenField('respidlen', None, length_of='respid'), StrLenField('respid', '', length_from=lambda pkt: pkt.respidlen)]
...
class OCSPStatusRequest(Packet):
    ...
    fields_desc = [
        FieldLenField('respidlen', None, length_of='respid'),
        PacketListField('respid', [], ResponderID, length_from=lambda pkt: pkt.respidlen),
...
class TLS_Ext_CSR(TLS_Ext_Unknown):  # RFC 4366
    ...
    fields_desc = [ShortEnumField('type', 5, _tls_ext), ShortField('len', None), ByteEnumField('stype', None, _cert_status_type), _StatusReqField('req', [], Raw, length_from=lambda pkt: pkt.len - 1)]


class TLS_Ext_UserMapping(TLS_Ext_Unknown):  # RFC 4681
    name = 'TLS Extension - User Mapping'
    fields_desc = [ShortEnumField('type', 6, _tls_ext), ShortField('len', None), FieldLenField('umlen', None, fmt='B', length_of='um'), FieldListField('um', [], ByteField('umtype', 0), length_from=lambda pkt: pkt.umlen)]
...
class TLS_Ext_ClientAuthz(TLS_Ext_Unknown):  # RFC 5878
    ...
    fields_desc = [
        ShortEnumField('type', 7, _tls_ext),
        ShortField('len', None),
    ]
...
class TLS_Ext_ServerAuthz(TLS_Ext_Unknown):  # RFC 5878
    ...
    fields_desc = [
        ShortEnumField('type', 8, _tls_ext),
        ShortField('len', None),
    ]
...
class TLS_Ext_ClientCertType(TLS_Ext_Unknown):  # RFC 5081
    ...
    fields_desc = [
        ShortEnumField('type', 9, _tls_ext),
        ShortField('len', None),
        FieldLenField('ctypeslen', None, length_of='ctypes'),
        FieldListField('ctypes', [0, 1], ByteEnumField('certtypes', None, _tls_cert_types), length_from=lambda pkt: pkt.ctypeslen),
    ]


class TLS_Ext_ServerCertType(TLS_Ext_Unknown):  # RFC 5081
    name = 'TLS Extension - Certificate Type (server version)'
    fields_desc = [ShortEnumField('type', 9, _tls_ext), ShortField('len', None), ByteEnumField('ctype', None, _tls_cert_types)]
...
class TLS_Ext_SupportedGroups(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ShortEnumField('type', 10, _tls_ext),
        ShortField('len', None),
        FieldLenField('groupslen', None, length_of='groups'),
        FieldListField('groups', [], ShortEnumField('ng', None, _tls_named_groups), length_from=lambda pkt: pkt.groupslen),
    ]
...
_tls_ecpoint_format = {0: 'uncompressed', 1: 'ansiX962_compressed_prime', 2: 'ansiX962_compressed_char2'}


class TLS_Ext_SupportedPointFormat(TLS_Ext_Unknown):  # RFC 4492
    name = 'TLS Extension - Supported Point Format'
    fields_desc = [
        ShortEnumField('type', 11, _tls_ext),
        ShortField('len', None),
        FieldLenField('ecpllen', None, fmt='B', length_of='ecpl'),
        FieldListField('ecpl', [0], ByteEnumField('nc', None, _tls_ecpoint_format), length_from=lambda pkt: pkt.ecpllen),
    ]
...
class TLS_Ext_SignatureAlgorithms(TLS_Ext_Unknown):  # RFC 5246
    ...
    fields_desc = [
        ShortEnumField('type', 13, _tls_ext),
        ShortField('len', None),
        SigAndHashAlgsLenField('sig_algs_len', None, length_of='sig_algs'),
        SigAndHashAlgsField('sig_algs', [], EnumField('hash_sig', None, _tls_hash_sig), length_from=lambda pkt: pkt.sig_algs_len),
    ]  # noqa: E501


class TLS_Ext_Heartbeat(TLS_Ext_Unknown):  # RFC 6520
    name = 'TLS Extension - Heartbeat'
    fields_desc = [ShortEnumField('type', 0x0F, _tls_ext), ShortField('len', None), ByteEnumField('heartbeat_mode', 2, {1: 'peer_allowed_to_send', 2: 'peer_not_allowed_to_send'})]


class ProtocolName(Packet):
    name = 'Protocol Name'
    fields_desc = [FieldLenField('len', None, fmt='B', length_of='protocol'), StrLenField('protocol', '', length_from=lambda pkt: pkt.len)]
...
class TLS_Ext_ALPN(TLS_Ext_PrettyPacketList):  # RFC 7301
    ...
    fields_desc = [ShortEnumField('type', 0x10, _tls_ext), ShortField('len', None), FieldLenField('protocolslen', None, length_of='protocols'), ProtocolListField('protocols', [], ProtocolName, length_from=lambda pkt: pkt.protocolslen)]


class TLS_Ext_Padding(TLS_Ext_Unknown):  # RFC 7685
    name = 'TLS Extension - Padding'
    fields_desc = [ShortEnumField('type', 0x15, _tls_ext), FieldLenField('len', None, length_of='padding'), StrLenField('padding', '', length_from=lambda pkt: pkt.len)]


class TLS_Ext_EncryptThenMAC(TLS_Ext_Unknown):  # RFC 7366
    name = 'TLS Extension - Encrypt-then-MAC'
    fields_desc = [ShortEnumField('type', 0x16, _tls_ext), ShortField('len', None)]


class TLS_Ext_ExtendedMasterSecret(TLS_Ext_Unknown):  # RFC 7627
    name = 'TLS Extension - Extended Master Secret'
    fields_desc = [ShortEnumField('type', 0x17, _tls_ext), ShortField('len', None)]
...
class TLS_Ext_SessionTicket(TLS_Ext_Unknown):  # RFC 5077
    ...
    fields_desc = [ShortEnumField('type', 0x23, _tls_ext), FieldLenField('len', None, length_of='ticket'), StrLenField('ticket', '', length_from=lambda pkt: pkt.len)]


class TLS_Ext_KeyShare(TLS_Ext_Unknown):
    name = 'TLS Extension - Key Share (dummy class)'
    fields_desc = [ShortEnumField('type', 0x33, _tls_ext), ShortField('len', None)]


class TLS_Ext_PreSharedKey(TLS_Ext_Unknown):
    name = 'TLS Extension - Pre Shared Key (dummy class)'
    fields_desc = [ShortEnumField('type', 0x29, _tls_ext), ShortField('len', None)]


class TLS_Ext_EarlyDataIndication(TLS_Ext_Unknown):
    name = 'TLS Extension - Early Data'
    fields_desc = [ShortEnumField('type', 0x2A, _tls_ext), ShortField('len', None)]


class TLS_Ext_EarlyDataIndicationTicket(TLS_Ext_Unknown):
    name = 'TLS Extension - Ticket Early Data Info'
    fields_desc = [ShortEnumField('type', 0x2A, _tls_ext), ShortField('len', None), IntField('max_early_data_size', 0)]


_tls_ext_early_data_cls = {1: TLS_Ext_EarlyDataIndication, 4: TLS_Ext_EarlyDataIndicationTicket, 8: TLS_Ext_EarlyDataIndication}


class TLS_Ext_SupportedVersions(TLS_Ext_Unknown):
    name = 'TLS Extension - Supported Versions (dummy class)'
    fields_desc = [ShortEnumField('type', 0x2B, _tls_ext), ShortField('len', None)]
...
class TLS_Ext_SupportedVersion_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        FieldLenField('versionslen', None, fmt='B', length_of='versions'),
        FieldListField('versions', [], ShortEnumField('version', None, _tls_version), length_from=lambda pkt: pkt.versionslen),
    ]


class TLS_Ext_SupportedVersion_SH(TLS_Ext_Unknown):
    name = 'TLS Extension - Supported Versions (for ServerHello)'
    fields_desc = [ShortEnumField('type', 0x2B, _tls_ext), ShortField('len', None), ShortEnumField('version', None, _tls_version)]
...
class TLS_Ext_Cookie(TLS_Ext_Unknown):
    ...
    fields_desc = [ShortEnumField('type', 0x2C, _tls_ext), ShortField('len', None), FieldLenField('cookielen', None, length_of='cookie'), XStrLenField('cookie', '', length_from=lambda pkt: pkt.cookielen)]
...
class TLS_Ext_PSKKeyExchangeModes(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ShortEnumField('type', 0x2D, _tls_ext),
        ShortField('len', None),
        FieldLenField('kxmodeslen', None, fmt='B', length_of='kxmodes'),
        FieldListField('kxmodes', [], ByteEnumField('kxmode', None, _tls_psk_kx_modes), length_from=lambda pkt: pkt.kxmodeslen),
    ]


class TLS_Ext_TicketEarlyDataInfo(TLS_Ext_Unknown):
    name = 'TLS Extension - Ticket Early Data Info'
    fields_desc = [ShortEnumField('type', 0x2E, _tls_ext), ShortField('len', None), IntField('max_early_data_size', 0)]
...
class TLS_Ext_NPN(TLS_Ext_PrettyPacketList):
    ...
    fields_desc = [ShortEnumField('type', 0x3374, _tls_ext), FieldLenField('len', None, length_of='protocols'), ProtocolListField('protocols', [], ProtocolName, length_from=lambda pkt: pkt.len)]


class TLS_Ext_PostHandshakeAuth(TLS_Ext_Unknown):  # RFC 8446
    name = 'TLS Extension - Post Handshake Auth'
    fields_desc = [ShortEnumField('type', 0x31, _tls_ext), ShortField('len', None)]


class TLS_Ext_SignatureAlgorithmsCert(TLS_Ext_Unknown):  # RFC 8446
    name = 'TLS Extension - Signature Algorithms Cert'
    fields_desc = [
        ShortEnumField('type', 0x32, _tls_ext),
        ShortField('len', None),
        SigAndHashAlgsLenField('sig_algs_len', None, length_of='sig_algs'),
        SigAndHashAlgsField('sig_algs', [], EnumField('hash_sig', None, _tls_hash_sig), length_from=lambda pkt: pkt.sig_algs_len),
    ]  # noqa: E501
...
class TLS_Ext_RenegotiationInfo(TLS_Ext_Unknown):  # RFC 5746
    ...
    fields_desc = [
        ShortEnumField('type', 0xFF01, _tls_ext),
        ShortField('len', None),
        FieldLenField('reneg_conn_len', None, fmt='B', length_of='renegotiated_connection'),
        StrLenField('renegotiated_connection', '', length_from=lambda pkt: pkt.reneg_conn_len),
    ]


class TLS_Ext_RecordSizeLimit(TLS_Ext_Unknown):  # RFC 8449
    name = 'TLS Extension - Record Size Limit'
    fields_desc = [ShortEnumField('type', 0x1C, _tls_ext), ShortField('len', None), ShortField('record_size_limit', None)]


_tls_ext_cls = {
    0: TLS_Ext_ServerName,
    ...
    def m2i(self, pkt, m):
        ...
        while len(m) >= 4:
            ...
            if cls is TLS_Ext_KeyShare:
                ...
                #    extension
                from scapy.layers.tls.keyexchange_tls13 import _tls_ext_keyshare_cls, _tls_ext_keyshare_hrr_cls

                # If SHA-256("HelloRetryRequest") == server_random,
                # this message is a HelloRetryRequest
                if pkt.random_bytes and pkt.random_bytes == _tls_hello_retry_magic:
