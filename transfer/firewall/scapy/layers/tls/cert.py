...
from scapy.layers.x509 import X509_SubjectPublicKeyInfo, RSAPublicKey, RSAPrivateKey, ECDSAPublicKey, ECDSAPrivateKey, RSAPrivateKey_OpenSSL, ECDSAPrivateKey_OpenSSL, X509_Cert, X509_CRL
from scapy.layers.tls.crypto.pkcs1 import pkcs_os2ip, _get_hash, _EncryptAndVerifyRSA, _DecryptAndSignRSA
...
class PubKeyRSA(PubKey, _EncryptAndVerifyRSA):
    ...
    def fill_and_store(self, modulus=None, modulusLen=None, pubExp=None):
        ...
        if not modulus:
            ...
            private_key = rsa.generate_private_key(public_exponent=pubExp, key_size=real_modulusLen, backend=default_backend())
    ...
    def import_from_tuple(self, tup):
        ...
        self.pem = self.pubkey.public_bytes(encoding=serialization.Encoding.PEM, format=serialization.PublicFormat.SubjectPublicKeyInfo)
...
class PubKeyECDSA(PubKey):
    ...
    def import_from_der(self, pubkey):
        ...
        self.pubkey = serialization.load_der_public_key(pubkey, backend=default_backend())  # noqa: E501
...
class PrivKeyRSA(PrivKey, _EncryptAndVerifyRSA, _DecryptAndSignRSA):
    ...
    @crypto_validator
    def fill_and_store(self, modulus=None, modulusLen=None, pubExp=None, prime1=None, prime2=None, coefficient=None, exponent1=None, exponent2=None, privExp=None):
        pubExp = pubExp or 65537
        if None in [modulus, prime1, prime2, coefficient, privExp, exponent1, exponent2]:
            ...
            real_modulusLen = modulusLen or 2048
            self.key = rsa.generate_private_key(public_exponent=pubExp, key_size=real_modulusLen, backend=default_backend())
            ...
            if modulusLen and real_modulusLen != modulusLen:
                warning('modulus and modulusLen do not match!')
            pubNum = rsa.RSAPublicNumbers(n=modulus, e=pubExp)
            privNum = rsa.RSAPrivateNumbers(p=prime1, q=prime2, dmp1=exponent1, dmq1=exponent2, iqmp=coefficient, d=privExp, public_numbers=pubNum)
    ...
    def import_from_asn1pkt(self, privkey):
        ...
        self.fill_and_store(modulus=modulus, pubExp=pubExp, privExp=privExp, prime1=prime1, prime2=prime2, exponent1=exponent1, exponent2=exponent2, coefficient=coefficient)
...
class PrivKeyECDSA(PrivKey):
    ...
    @crypto_validator
    def import_from_asn1pkt(self, privkey):
        self.key = serialization.load_der_private_key(raw(privkey), None, backend=default_backend())  # noqa: E501
        self.pubkey = self.key.public_key()
