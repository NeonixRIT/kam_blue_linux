...
class KeyShareEntry(Packet):
    ...
    fields_desc = [ShortEnumField('group', None, _tls_named_groups), FieldLenField('kxlen', None, length_of='key_exchange'), XStrLenField('key_exchange', '', length_from=lambda pkt: pkt.kxlen)]
...
class TLS_Ext_KeyShare_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ...
        PacketListField('client_shares', [], KeyShareEntry, length_from=lambda pkt: pkt.client_shares_len),
    ]  # noqa: E501
...
class TLS_Ext_KeyShare_HRR(TLS_Ext_Unknown):
    ...
    fields_desc = [ShortEnumField('type', 0x33, _tls_ext), ShortField('len', None), ShortEnumField('selected_group', None, _tls_named_groups)]


class TLS_Ext_KeyShare_SH(TLS_Ext_Unknown):
    name = 'TLS Extension - Key Share (for ServerHello)'
    fields_desc = [ShortEnumField('type', 0x33, _tls_ext), ShortField('len', None), PacketField('server_share', None, KeyShareEntry)]
...
_tls_ext_keyshare_cls = {1: TLS_Ext_KeyShare_CH, 2: TLS_Ext_KeyShare_SH}
...
class Ticket(Packet):
    ...
    fields_desc = [
        StrFixedLenField('key_name', None, 16),
        StrFixedLenField('iv', None, 16),
        FieldLenField('encstatelen', None, length_of='encstate'),
        StrLenField('encstate', '', length_from=lambda pkt: pkt.encstatelen),
...
class PSKIdentity(Packet):
    name = 'PSK Identity'
    fields_desc = [FieldLenField('identity_len', None, length_of='identity'), TicketField('identity', '', length_from=lambda pkt: pkt.identity_len), IntField('obfuscated_ticket_age', 0)]


class PSKBinderEntry(Packet):
    name = 'PSK Binder Entry'
    fields_desc = [FieldLenField('binder_len', None, fmt='B', length_of='binder'), StrLenField('binder', '', length_from=lambda pkt: pkt.binder_len)]
...
class TLS_Ext_PreSharedKey_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        FieldLenField('identities_len', None, length_of='identities'),
        PacketListField('identities', [], PSKIdentity, length_from=lambda pkt: pkt.identities_len),
        FieldLenField('binders_len', None, length_of='binders'),
        PacketListField('binders', [], PSKBinderEntry, length_from=lambda pkt: pkt.binders_len),
    ]


class TLS_Ext_PreSharedKey_SH(TLS_Ext_Unknown):
    name = 'TLS Extension - Pre Shared Key (for ServerHello)'
    fields_desc = [ShortEnumField('type', 0x29, _tls_ext), ShortField('len', None), ShortField('selected_identity', None)]


_tls_ext_presharedkey_cls = {1: TLS_Ext_PreSharedKey_CH, 2: TLS_Ext_PreSharedKey_SH}
