...
from scapy.layers.tls.extensions import TLS_Ext_SupportedVersion_SH, TLS_Ext_SupportedGroups, TLS_Ext_Cookie, TLS_Ext_SignatureAlgorithms, TLS_Ext_PSKKeyExchangeModes, TLS_Ext_EarlyDataIndicationTicket
from scapy.layers.tls.keyexchange_tls13 import TLS_Ext_KeyShare_SH, KeyShareEntry, TLS_Ext_KeyShare_HRR, TLS_Ext_PreSharedKey_CH, TLS_Ext_PreSharedKey_SH
...
from scapy.layers.tls.handshake_sslv2 import SSLv2ClientCertificate, SSLv2ClientFinished, SSLv2ClientHello, SSLv2ClientMasterKey, SSLv2RequestCertificate, SSLv2ServerFinished, SSLv2ServerHello, SSLv2ServerVerify
from scapy.layers.tls.record import TLSAlert, TLSChangeCipherSpec, TLSApplicationData
from scapy.layers.tls.record_tls13 import TLS13
from scapy.layers.tls.crypto.hkdf import TLS13_HKDF
from scapy.layers.tls.crypto.suites import _tls_cipher_suites_cls, get_usable_ciphersuites
...
class TLSServerAutomaton(_TLSAutomaton):
    ...
    def tls13_should_add_HelloRetryRequest(self):
        ...
        usable_suites = get_usable_ciphersuites(self.cur_pkt.ciphers, kx)
        c = usable_suites[0]
        ext = [TLS_Ext_SupportedVersion_SH(version='TLS 1.3'), TLS_Ext_KeyShare_HRR(selected_group=_tls_named_groups[self.curve])]  # noqa: E501
        if self.cookie:
            ext += TLS_Ext_Cookie()
        ...
        with open(self.session_ticket_file, 'rb') as f:
            for line in f:
                ...
                if psk_identity.__bytes__() == ticket_label:
                    ...
                    computed_binder = hkdf.compute_verify_data(binder_key, handshake_context)  # noqa: E501
        ...
        if self.session_ticket_file:
            ...
            for p in self.buffer_out:
                if isinstance(p, TLS13):
                    ...
                    save_ticket = all(
                        map(
                            lambda x: isinstance(x, TLS13NewSessionTicket),  # noqa: E501
                            p.inner.msg,
                        )
                    )
                    if save_ticket:
    ...
    def sslv2_should_add_ServerHello(self):
        ...
        cert = self.mycert
        ciphers = [0x010080, 0x020080, 0x030080, 0x040080, 0x050080, 0x060040, 0x0700C0]
