...
from scapy.fields import ByteField, ByteEnumField, IntField, StrLenField, XStrLenField, XStrFixedLenField, FieldLenField, PacketLenField, PacketListField, IPField, MultiEnumField
...
class RadiusAttribute(Packet):
    ...
    fields_desc = [ByteEnumField('type', 1, _radius_attribute_types), FieldLenField('len', None, 'value', 'B', adjust=lambda pkt, x: len(pkt.value) + 2), StrLenField('value', '', length_from=lambda pkt: pkt.len - 2)]
...
class _RadiusAttrIntValue(_SpecificRadiusAttr):
    ...
    fields_desc = [ByteEnumField('type', 5, _radius_attribute_types), ByteField('len', 6), IntField('value', 0)]
...
class _RadiusAttrHexStringVal(_SpecificRadiusAttr):
    ...
    fields_desc = [
        ...
        FieldLenField('len', None, 'value', 'B', adjust=lambda p, x: len(p.value) + 2),
...
class _RadiusAttrIPv4AddrVal(_SpecificRadiusAttr):
    ...
    fields_desc = [ByteEnumField('type', 4, _radius_attribute_types), ByteField('len', 6), IPField('value', '0.0.0.0')]
...
_radius_attrs_values = {
    ...
    # Framed-Protocol
    7: {1: 'PPP', 2: 'SLIP', 3: 'AppleTalk Remote Access Protocol (ARAP)', 4: 'Gandalf proprietary SingleLink/MultiLink protocol', 5: 'Xylogics proprietary IPX/SLIP', 6: 'X.75 Synchronous', 7: 'GPRS PDP Context'},
    # Framed-Routing
    10: {0: 'None', 1: 'Send routing packets', 2: 'Listen for routing packets', 3: 'Send and Listen'},
    # Framed-Compression
    13: {0: 'None', 1: 'VJ TCP/IP header compression', 2: 'IPX header compression', 3: 'Stac-LZS compression'},
    # Login-Service
    15: {0: 'Telnet', 1: 'Rlogin', 2: 'TCP Clear', 3: 'PortMaster (proprietary)', 4: 'LAT', 5: 'X25-PAD', 6: 'X25-T3POS', 7: 'Unassigned', 8: 'TCP Clear Quiet (suppresses any NAS-generated connect string)'},
    # Termination-Action
    29: {0: 'Default', 1: 'RADIUS-Request'},
    # Acct-Status-Type
    40: {
        1: 'Start',
        2: 'Stop',
    ...
    # Acct-Authentic
    45: {1: 'RADIUS', 2: 'Local', 3: 'Remote', 4: 'Diameter'},
    ...
    # ARAP-Zone-Access
    72: {1: 'Only allow access to default zone', 2: 'Use zone filter inclusively', 3: 'Not used', 4: 'Use zone filter exclusively'},
    # Prompt
    76: {0: 'No Echo', 1: 'Echo'},
    ...
    126: {0x30: 'TADIG', 0x31: 'REALM', 0x32: 'E212', 0x33: 'ICC', 0xFF: 'Reserved'},
    # Basic-Location-Policy-Rules
    129: {
        0: 'Retransmission allowed',
    },
    # Location-Capable
    131: {1: 'CIVIC_LOCATION', 2: 'GEO_LOCATION', 4: 'USERS_LOCATION', 8: 'NAS_LOCATION'},
    # Framed-Management-Protocol
    133: {1: 'SNMP', 2: 'Web-based', 3: 'NETCONF', 4: 'FTP', 5: 'TFTP', 6: 'SFTP', 7: 'RCP', 8: 'SCP'},
...
class _RadiusAttrIntEnumVal(_SpecificRadiusAttr):
    ...
    fields_desc = [ByteEnumField('type', 6, _radius_attribute_types), ByteField('len', 6), MultiEnumField('value', 0, _radius_attrs_values, depends_on=lambda p: p.type, fmt='I')]
...
class RadiusAttr_EAP_Message(RadiusAttribute):
    ...
    name = 'EAP-Message'
    match_subclass = True
    fields_desc = [ByteEnumField('type', 79, _radius_attribute_types), FieldLenField('len', None, 'value', 'B', adjust=lambda pkt, x: x + 2), _EAPPacketField('value', '', EAP, length_from=lambda p: p.len - 2)]
...
class RadiusAttr_Vendor_Specific(RadiusAttribute):
    ...
    fields_desc = [
        ...
        FieldLenField('len', None, 'value', 'B', adjust=lambda pkt, x: len(pkt.value) + 8),
        IntField('vendor_id', 0),
        ByteField('vendor_type', 0),
        FieldLenField('vendor_len', None, 'value', 'B', adjust=lambda p, x: len(p.value) + 2),
