...
from scapy.asn1fields import ASN1F_OID, ASN1F_PRINTABLE_STRING, ASN1F_SEQUENCE, ASN1F_SEQUENCE_OF
...
class NTLM_Header(Packet):
    ...
    fields_desc = [
        ...
        LEIntEnumField('MessageType', 3, {1: 'NEGOTIATE_MESSAGE', 2: 'CHALLENGE_MESSAGE', 3: 'AUTHENTICATE_MESSAGE'}),
    ]
...
def _NTLMStrField(name, default):
    return MultipleTypeField(
        [(StrFieldUtf16(name, default), lambda pkt: pkt.NegotiateFlags.NTLMSSP_NEGOTIATE_UNICODE)],
...
class NTLM_NEGOTIATE(_NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLM_Version,
        # Payload
        _NTLMPayloadField('Payload', OFFSET, [_NTLMStrField('DomainName', b''), _NTLMStrField('WorkstationName', b'')]),
    ]
...
class AV_PAIR(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (LEIntEnumField('Value', 1, {0x0001: 'constrained', 0x0002: 'MIC integrity', 0x0004: 'SPN from untrusted source'}), lambda pkt: pkt.AvId == 0x0006),
                (UTCTimeField('Value', None, epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7, fmt='<Q'), lambda pkt: pkt.AvId == 0x0007),
                (PacketField('Value', Single_Host_Data(), Single_Host_Data), lambda pkt: pkt.AvId == 0x0008),
                (XStrLenField('Value', b'', length_from=lambda pkt: pkt.AvLen), lambda pkt: pkt.AvId == 0x000A),
            ],
...
class NTLMv2_CLIENT_CHALLENGE(Packet):
    fields_desc = [
        ...
        UTCTimeField('TimeStamp', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
...
class NTLMv2_RESPONSE(Packet):
    fields_desc = [XStrFixedLenField('NTProofStr', b'', length=16), NTLMv2_CLIENT_CHALLENGE]
...
class NTLM_AUTHENTICATE(_NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLM_Version,
        # MIC
        ConditionalField(XStrFixedLenField('MIC', b'', length=16), lambda pkt: pkt.fields.get('MIC', b'') is not None),
        # Payload
        _NTLMPayloadField(
            'Payload',
            OFFSET,
            [
                MultipleTypeField([(PacketField('LmChallengeResponse', LMv2_RESPONSE(), LMv2_RESPONSE), lambda pkt: pkt.NTLM_VERSION == 2)], PacketField('LmChallengeResponse', LM_RESPONSE(), LM_RESPONSE)),
                MultipleTypeField([(PacketField('NtChallengeResponse', NTLMv2_RESPONSE(), NTLMv2_RESPONSE), lambda pkt: pkt.NTLM_VERSION == 2)], PacketField('NtChallengeResponse', NTLM_RESPONSE(), NTLM_RESPONSE)),
        ...
        if self.DROP_MIC_v1 or self.DROP_MIC_v2:
            if isinstance(ntlm, NTLM_AUTHENTICATE):
                ...
                if self.DROP_MIC_v2:
                    ...
                    i = next(i for i, k in enumerate(ChallengeResponse.AvPairs) if k.AvId == 0x0006)
...
class NEGOEX_EXCHANGE_NTLM_ITEM(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(ASN1F_SEQUENCE(ASN1F_SEQUENCE(ASN1F_OID('oid', ''), ASN1F_PRINTABLE_STRING('token', ''), explicit_tag=0x31), explicit_tag=0x80))
...
class NEGOEX_EXCHANGE_NTLM(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(
        ASN1F_SEQUENCE(ASN1F_SEQUENCE_OF('items', [], NEGOEX_EXCHANGE_NTLM_ITEM), implicit_tag=0xA0),
    )
