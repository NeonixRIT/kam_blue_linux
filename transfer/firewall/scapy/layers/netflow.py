...
class NetflowHeaderV1(Packet):
    ...
    fields_desc = [ShortField('count', None), IntField('sysUptime', 0), UTCTimeField('unixSecs', 0), UTCTimeField('unixNanoSeconds', 0, use_nano=True)]
...
class NetflowHeaderV9(Packet):
    ...
    fields_desc = [ShortField('count', None), IntField('sysUptime', 0), UTCTimeField('unixSecs', None), IntField('packageSequence', 0), IntField('SourceID', 0)]

    def post_build(self, pkt, pay):
        if self.count is None:
            count = sum(1 for x in self.layers() if x in [NetflowFlowsetV9, NetflowDataflowsetV9, NetflowOptionsFlowsetV9])
            pkt = struct.pack('!H', count) + pkt[2:]
        return pkt + pay
...
class NetflowHeaderV10(Packet):
    ...
    fields_desc = [ShortField('length', None), UTCTimeField('ExportTime', 0), IntField('flowSequence', 0), IntField('ObservationDomainID', 0)]
...
class NetflowTemplateFieldV9(Packet):
    ...
    fields_desc = [BitField('enterpriseBit', 0, 1), BitEnumField('fieldType', None, 15, NetflowV910TemplateFieldTypes), ShortField('fieldLength', None), ConditionalField(IntField('enterpriseNumber', 0), lambda p: p.enterpriseBit)]
...
class NetflowTemplateV9(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            'template_fields',
            [],
            NetflowTemplateFieldV9,  # noqa: E501
...
class NetflowFlowsetV9(Packet):
    ...
    fields_desc = [ShortField('flowSetID', 0), FieldLenField('length', None, length_of='templates', adjust=lambda pkt, x: x + 4), PacketListField('templates', [], NetflowTemplateV9, length_from=lambda pkt: pkt.length - 4)]
...
class NetflowDataflowsetV9(Packet):
    ...
    fields_desc = [ShortField('templateID', 255), ShortField('length', None), PacketListField('records', [], NetflowRecordV9, length_from=lambda pkt: pkt.length - 4)]
...
class NetflowOptionsFlowsetOptionV9(Packet):
    ...
    fields_desc = [
        BitField('enterpriseBit', 0, 1),
        BitEnumField('optionFieldType', None, 15, NetflowV910TemplateFieldTypes),
        ShortField('optionFieldlength', 0),
        ConditionalField(ShortField('enterpriseNumber', 0), lambda p: p.enterpriseBit),
    ]
...
class NetflowOptionsFlowsetScopeV9(Packet):
    ...
    fields_desc = [ShortEnumField('scopeFieldType', None, ScopeFieldTypes), ShortField('scopeFieldlength', 0)]
...
class NetflowOptionsFlowsetV9(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('scopes', [], NetflowOptionsFlowsetScopeV9, length_from=lambda pkt: pkt.option_scope_length),
        PacketListField('options', [], NetflowOptionsFlowsetOptionV9, length_from=lambda pkt: pkt.option_field_length),
        StrLenField('pad', None, length_from=lambda pkt: (pkt.length - pkt.option_scope_length - pkt.option_field_length - 10)),
    ]
...
class NetflowOptionsFlowset10(NetflowOptionsFlowsetV9):
    ...
    fields_desc = [
        ...
        FieldLenField('field_count', None, count_of='options', adjust=lambda pkt, x: (x + pkt.get_field('scope_field_count').i2m(pkt, None))),
        FieldLenField('scope_field_count', None, count_of='scopes'),
        # We can't use PadField as we have 2 PacketListField
        PacketListField('scopes', [], NetflowOptionsFlowsetScopeV9, count_from=lambda pkt: pkt.scope_field_count),
        PacketListField('options', [], NetflowOptionsFlowsetOptionV9, count_from=lambda pkt: (pkt.field_count - pkt.scope_field_count)),
        StrLenField('pad', None, length_from=lambda pkt: (pkt.length - (pkt.scope_field_count * 4) - 10)),
    ]
