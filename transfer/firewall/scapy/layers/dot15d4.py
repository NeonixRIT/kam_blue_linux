    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                ...
                (XLEIntField('sec_keyid_keysource', 0x00000000), lambda pkt: pkt.getfieldval('sec_sc_keyidmode') == 2),
                # 8 octets when sec_sc_keyidmode == 3
                (LELongField('sec_keyid_keysource', 0x0000000000000000), lambda pkt: pkt.getfieldval('sec_sc_keyidmode') == 3),
            ],
...
class Dot15d4Data(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XLEShortField('src_panid', 0x0), lambda pkt: util_srcpanid_present(pkt)),
        ConditionalField(
            dot15d4AddressField('src_addr', None, length_of='fcf_srcaddrmode'),  # noqa: E501
            lambda pkt: pkt.underlayer.getfieldval('fcf_srcaddrmode') != 0,
        ),  # noqa: E501
        # Security field present if fcf_security == True
        ConditionalField(
            PacketField('aux_sec_header', Dot15d4AuxSecurityHeader(), Dot15d4AuxSecurityHeader),  # noqa: E501
...
class Dot15d4Beacon(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('pa_short_addresses', [], XLEShortField('', 0x0000), count_from=lambda pkt: pkt.pa_num_short),
        FieldListField('pa_long_addresses', [], dot15d4AddressField('', 0, adjust=lambda pkt, x: 8), count_from=lambda pkt: pkt.pa_num_long),
        # TODO beacon payload
    ]
...
class Dot15d4Cmd(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XLEShortField('src_panid', 0x0), lambda pkt: util_srcpanid_present(pkt)),
        ConditionalField(dot15d4AddressField('src_addr', None, length_of='fcf_srcaddrmode'), lambda pkt: pkt.underlayer.getfieldval('fcf_srcaddrmode') != 0),  # noqa: E501
        # Security field present if fcf_security == True
        ConditionalField(
            PacketField('aux_sec_header', Dot15d4AuxSecurityHeader(), Dot15d4AuxSecurityHeader),  # noqa: E501
            lambda pkt: pkt.underlayer.getfieldval('fcf_security') is True,
