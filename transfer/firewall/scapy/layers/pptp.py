...
from scapy.fields import ByteEnumField, FieldLenField, FlagsField, IntField, IntEnumField, LenField, XIntField, ShortField, ShortEnumField, StrFixedLenField, StrLenField, XShortField, XByteField

_PPTP_MAGIC_COOKIE = 0x1A2B3C4D

_PPTP_msg_type = {1: 'Control Message', 2: 'Managemenent Message'}
...
_PPTP_general_error_code = {0: 'None', 1: 'Not-Connected', 2: 'Bad-Format', 3: 'Bad-Value', 4: 'No-Resource', 5: 'Bad-Call ID', 6: 'PAC-Error'}


class PPTP(Packet):
    name = 'PPTP'
    fields_desc = [
        FieldLenField('len', None, fmt='H', length_of='data', adjust=lambda p, x: x + 12),
...
_PPTP_FRAMING_CAPABILITIES_FLAGS = ['Asynchronous Framing supported', 'Synchronous Framing supported']

_PPTP_BEARER_CAPABILITIES_FLAGS = ['Analog access supported', 'Digital access supported']
...
class PPTPStartControlConnectionRequest(PPTP):
    ...
    fields_desc = [
        ...
        FlagsField('framing_capabilities', 0, 32, _PPTP_FRAMING_CAPABILITIES_FLAGS),
        FlagsField('bearer_capabilities', 0, 32, _PPTP_BEARER_CAPABILITIES_FLAGS),
...
_PPTP_start_control_connection_result = {1: 'OK', 2: 'General error', 3: 'Command channel already exists', 4: 'Not authorized', 5: 'Unsupported protocol version'}
...
class PPTPStartControlConnectionReply(PPTP):
    ...
    fields_desc = [
        ...
        XShortField('reserved_0', 0x0000),
        ShortField('protocol_version', 0x0100),
        ByteEnumField('result_code', 1, _PPTP_start_control_connection_result),
...
_PPTP_stop_control_connection_reason = {1: 'None', 2: 'Stop-Protocol', 3: 'Stop-Local-Shutdown'}
...
class PPTPStopControlConnectionRequest(PPTP):
    ...
    fields_desc = [
        ...
        XShortField('reserved_0', 0x0000),
        ByteEnumField('reason', 1, _PPTP_stop_control_connection_reason),
        XByteField('reserved_1', 0x00),
        XShortField('reserved_2', 0x0000),
    ]


_PPTP_stop_control_connection_result = {1: 'OK', 2: 'General error'}
...
class PPTPStopControlConnectionReply(PPTP):
    ...
    fields_desc = [
        ...
        XShortField('reserved_0', 0x0000),
        ByteEnumField('result_code', 1, _PPTP_stop_control_connection_result),
        ByteEnumField('error_code', 0, _PPTP_general_error_code),
        XShortField('reserved_2', 0x0000),
    ]
...
_PPTP_echo_result = {1: 'OK', 2: 'General error'}
...
_PPTP_bearer_type = {1: 'Analog channel', 2: 'Digital channel', 3: 'Any type of channel'}

_PPTP_framing_type = {1: 'Asynchronous framing', 2: 'Synchronous framing', 3: 'Any type of framing'}
...
_PPTP_result_code = {1: 'Connected', 2: 'General error', 3: 'No Carrier', 4: 'Busy', 5: 'No dial tone', 6: 'Time-out', 7: 'Do not accept'}
...
_PPTP_call_disconnect_result = {1: 'Lost Carrier', 2: 'General error', 3: 'Admin Shutdown', 4: 'Request'}
...
class PPTPCallDisconnectNotify(PPTP):
    ...
    fields_desc = [
        ...
        XShortField('reserved_0', 0x0000),
        ShortField('call_id', 1),
        ByteEnumField('result_code', 1, _PPTP_call_disconnect_result),
        ByteEnumField('error_code', 0, _PPTP_general_error_code),
        ShortField('cause_code', 0),
        XShortField('reserved_1', 0x0000),
