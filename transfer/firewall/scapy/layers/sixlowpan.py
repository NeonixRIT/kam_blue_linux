...
class LoWPANUncompressedIPv6(Packet):
    ...
    fields_desc = [BitField('_type', 0x41, 8)]
...
class LoWPANMesh(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField([(XShortField('src', 0x0), lambda pkt: pkt.v == 1)], XLongField('src', 0x0)),
        MultipleTypeField([(XShortField('dst', 0x0), lambda pkt: pkt.v == 1)], XLongField('dst', 0x0)),
    ]
...
class LoWPAN_HC1(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField('nh', 0, 2, {0: 'not compressed', 1: 'UDP', 2: 'ICMP', 3: 'TCP'}),
        BitEnumField('hc2', 0, 1, ['No more header compression bits', 'HC2 Present']),
        # https://tools.ietf.org/html/rfc4944#section-10.2
        ConditionalField(
            MultipleTypeField(
                [
                    (PacketField('hc2Field', LoWPAN_HC2_UDP(), LoWPAN_HC2_UDP), lambda pkt: pkt.nh == 1),
        ...
        IP6FieldLenField('src', '::', lambda pkt: (0 if pkt.sp else 8) + (0 if pkt.si else 8)),
        IP6FieldLenField('dst', '::', lambda pkt: (0 if pkt.dp else 8) + (0 if pkt.di else 8)),
        ConditionalField(ByteField('traffic_class', 0), lambda pkt: not pkt.tc_fl),
        ConditionalField(BitField('flow_label', 0, 20), lambda pkt: not pkt.tc_fl),
        # Other fields
        # https://tools.ietf.org/html/rfc4944#section-10.3.2
        ConditionalField(MultipleTypeField([(BitScalingField('udpSourcePort', 0, 4, offset=0xF0B0), lambda pkt: getattr(pkt.hc2Field, 'sc', 0))], BitField('udpSourcePort', 0, 16)), lambda pkt: pkt.nh == 1 and pkt.hc2),
        ConditionalField(MultipleTypeField([(BitScalingField('udpDestPort', 0, 4, offset=0xF0B0), lambda pkt: getattr(pkt.hc2Field, 'dc', 0))], BitField('udpDestPort', 0, 16)), lambda pkt: pkt.nh == 1 and pkt.hc2),
        ConditionalField(BitField('udpLength', 0, 16), lambda pkt: pkt.nh == 1 and pkt.hc2 and not pkt.hc2Field.lc),
        ConditionalField(XBitField('udpChecksum', 0, 16), lambda pkt: pkt.nh == 1 and pkt.hc2),
        # Out of spec
...
class LoWPANBroadcast(Packet):
    ...
    fields_desc = [ByteField('reserved', 0x50), ByteField('seq', 0)]
...
class LoWPAN_IPHC(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField('hlim', 0x0, 2, {0: 'Inline', 1: 'Compressed/HL1', 2: 'Compressed/HL64', 3: 'Compressed/HL255'}),
        ...
        ConditionalField(BitField('sci', 0, 4), lambda pkt: pkt.cid == 0x1),
        ConditionalField(BitField('dci', 0, 4), lambda pkt: pkt.cid == 0x1),
        # https://tools.ietf.org/html/rfc6282#section-3.2.1
        ConditionalField(BitField('tc_ecn', 0, 2), lambda pkt: pkt.tf in [0, 1, 2]),
        ConditionalField(
        ...
        ConditionalField(BitField('flowlabel', 0, 20), lambda pkt: pkt.tf in [0, 1]),
        # Inline fields https://tools.ietf.org/html/rfc6282#section-3.1.1
        ConditionalField(ByteEnumField('nhField', 0x0, ipv6nh), lambda pkt: pkt.nh == 0x0),
        ConditionalField(ByteField('hopLimit', 0x0), lambda pkt: pkt.hlim == 0x0),
...
class LoWPAN_NHC_UDP(LoWPAN_NHC_Hdr):
    fields_desc = [
        ...
        BitField('P', 0, 2),
        MultipleTypeField(
            [(BitField('udpSourcePort', 0, 16), lambda pkt: pkt.P in [0, 1]), (BitField('udpSourcePort', 0, 8), lambda pkt: pkt.P == 2), (BitField('udpSourcePort', 0, 4), lambda pkt: pkt.P == 3)],
            BitField('udpSourcePort', 0x0, 16),
        ),
        MultipleTypeField(
            [(BitField('udpDestPort', 0, 16), lambda pkt: pkt.P in [0, 2]), (BitField('udpDestPort', 0, 8), lambda pkt: pkt.P == 1), (BitField('udpDestPort', 0, 4), lambda pkt: pkt.P == 3)],
            BitField('udpDestPort', 0x0, 16),
        ),
        ConditionalField(XShortField('udpChecksum', 0x0), lambda pkt: pkt.C == 0),
    ]
