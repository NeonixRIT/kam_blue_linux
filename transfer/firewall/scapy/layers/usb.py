...
from scapy.interfaces import NetworkInterface, InterfaceProvider, network_name, IFACES
...
_usbd_status_codes = {0x00000000: 'Success', 0x40000000: 'Pending', 0xC0000000: 'Halted', 0x80000000: 'Error'}

_transfer_types = {0x0: 'Isochronous', 0x1: 'Interrupt', 0x2: 'Control'}
...
class USBpcapTransferIsochronous(Packet):
    ...
    fields_desc = [LEIntField('offset', 0), LEIntField('length', 0), LEIntEnumField('usbd_status', 0x0, _usbd_status_codes)]
...
def _extcap_call(prog, args, keyword, values):
    ...
    p = subprocess.Popen([prog] + args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    for ifa in data.split('\n'):
        ...
        if not ifa.startswith(keyword):
            continue
        res.append(tuple([re.search(r'{%s=([^}]*)}' % val, ifa).group(1) for val in values]))
    return res
...
if WINDOWS:
    ...
    def get_usbpcap_interfaces():
        ...
        _usbpcap_check()
        return _extcap_call(conf.prog.usbpcapcmd, ['--extcap-interfaces'], 'interface', ['value', 'display'])
    ...
    def get_usbpcap_devices(iface, enabled=True):
        ...
        _usbpcap_check()
        devices = _extcap_call(conf.prog.usbpcapcmd, ['--extcap-interface', iface, '--extcap-config'], 'value', ['value', 'display', 'enabled'])
    ...
    class USBpcapSocket(SuperSocket):
        ...
        def __init__(self, iface=None, *args, **karg):
            _usbpcap_check()
            if iface is None:
                warning('Available interfaces: [%s]', ' '.join(x[0] for x in get_usbpcap_interfaces()))
                raise NameError('No interface specified !' ' See get_usbpcap_interfaces()')
            iface = network_name(iface)
            self.outs = None
            args = ['-d', iface, '-b', '134217728', '-A', '-o', '-']
            self.usbpcap_proc = subprocess.Popen([conf.prog.usbpcapcmd] + args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
