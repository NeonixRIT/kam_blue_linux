...
from scapy.layers.l2 import Ether, Dot3, getmacbyip, CookedLinux, GRE, SNAP, Loopback
...
class IPOption(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField(
            'length',
            None,
            fmt='B',  # Only option 0 and 1 have no length and value  # noqa: E501
            length_of='value',
...
class IPOption_RR(IPOption):
    ...
    fields_desc = [
        _IPOption_HDR,
        FieldLenField('length', None, fmt='B', length_of='routers', adjust=lambda pkt, l: l + 3),  # noqa: E501
        ByteField('pointer', 4),  # 4 is first IP
        FieldListField('routers', [], IPField('', '0.0.0.0'), length_from=lambda pkt: pkt.length - 3),
    ]
...
class IPOption_Timestamp(IPOption):
    ...
    fields_desc = [
        ...
        ByteField('pointer', 9),
        BitField('oflw', 0, 4),
        BitEnumField('flg', 1, 4, {0: 'timestamp_only', 1: 'timestamp_and_ip_addr', 3: 'prespecified_ip_addr'}),
        ConditionalField(IPField('internet_address', '0.0.0.0'), lambda pkt: pkt.flg != 0),
...
class IPOption_SDBM(IPOption):
    ...
    fields_desc = [
        _IPOption_HDR,
        FieldLenField('length', None, fmt='B', length_of='addresses', adjust=lambda pkt, l: l + 2),  # noqa: E501
        FieldListField('addresses', [], IPField('', '0.0.0.0'), length_from=lambda pkt: pkt.length - 2),
    ]
...
class RandTCPOptions(VolatileValue):
    ...
    def _fix(self):
        ...
        # Random ("NAME", fmt)
        rand_patterns = [random.choice(list((opt, fmt) for opt, fmt in six.itervalues(TCPOptions[0]) if opt != 'EOL')) for _ in range(self.size)]
        rand_vals = []
...
class ICMP(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField([(ShortField('unused', 0), lambda pkt: pkt.type in [11, 12]), (IntField('unused', 0), lambda pkt: pkt.type not in [0, 3, 5, 8, 11, 12, 13, 14, 15, 16, 17, 18])], StrFixedLenField('unused', '', length=0)),
    ]
    ...
    def answers(self, other):
        if not isinstance(other, ICMP):
            return 0
        if (
            (other.type, self.type) in [(8, 0), (13, 14), (15, 16), (17, 18), (33, 34), (35, 36), (37, 38)]  # noqa: E501
...
class TracerouteResult(SndRcvList):
    ...
    def show(self):
        return self.make_table(
            lambda s, r: (
                s.sprintf('%IP.dst%:{TCP:tcp%ir,TCP.dport%}{UDP:udp%ir,UDP.dport%}{ICMP:ICMP}'),  # noqa: E501
                s.ttl,
...
Touch screen: pinch/extend to zoom, swipe or two-finger rotate.""",
        ...
        for t in rings:
            ...
            for i in range(tmp_len):
                ...
                s = IPsphere(
                    pos=vpython.vec((tmp_len - 1) * vpython.cos(2 * i * vpython.pi / tmp_len), (tmp_len - 1) * vpython.sin(2 * i * vpython.pi / tmp_len), 2 * t),  # noqa: E501
                    ip=r[i][0],
                    color=col,
                )
...
def traceroute(target, dport=80, minttl=1, maxttl=30, sport=RandShort(), l4=None, filter=None, timeout=2, verbose=None, **kargs):  # noqa: E501
    ...
    if l4 is None:
        a, b = sr(
            IP(dst=target, id=RandShort(), ttl=(minttl, maxttl)) / TCP(seq=RandInt(), sport=sport, dport=dport),  # noqa: E501
            timeout=timeout,
            filter=filter,
            verbose=verbose,
            **kargs,
...
class TCP_client(Automaton):
    ...
    def parse_args(self, ip, port, *args, **kargs):
        ...
        self.l4 = IP(dst=ip) / TCP(sport=self.sport, dport=self.dport, flags=0, seq=random.randrange(0, 2**32))
    ...
    def master_filter(self, pkt):
        return (
            ...
            and self.l4[TCP].seq >= pkt[TCP].ack  # XXX: seq/ack 2^32 wrap up  # noqa: E501
            and ((self.l4[TCP].ack == 0) or (self.sack <= pkt[TCP].seq <= self.l4[TCP].ack + pkt[TCP].window))
        )  # noqa: E501
