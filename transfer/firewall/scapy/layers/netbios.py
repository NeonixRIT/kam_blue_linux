...
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, FieldLenField, FlagsField, IPField, IntField, NetBIOSNameField, PacketListField, ShortEnumField, ShortField, StrFixedLenField, XShortField, XStrFixedLenField
...
_NETBIOS_SUFFIXES = {0x4141: 'workstation', 0x4141 + 0x03: 'messenger service', 0x4141 + 0x200: 'file server service', 0x4141 + 0x10B: 'domain master browser', 0x4141 + 0x10C: 'domain controller', 0x4141 + 0x10E: 'browser election service'}

_NETBIOS_QRTYPES = {0x20: 'NB', 0x21: 'NBSTAT'}

_NETBIOS_QRCLASS = {1: 'INTERNET'}

_NETBIOS_RNAMES = {0xC00C: 'Label String Pointer to QUESTION_NAME'}

_NETBIOS_OWNER_MODE_TYPES = {0: 'B node', 1: 'P node', 2: 'M node', 3: 'H node'}

_NETBIOS_GNAMES = {0: 'Unique name', 1: 'Group name'}
...
class NBNSHeader(Packet):
    ...
    fields_desc = [
        ...
        BitField('OPCODE', 0, 4),
        FlagsField('NM_FLAGS', 0, 7, ['B', 'res1', 'res0', 'RA', 'RD', 'TC', 'AA']),
        BitField('RCODE', 0, 4),
...
class NBNSQueryRequest(Packet):
    ...
    def mysummary(self):
        return "NBNSQueryRequest who has '\\\\%s'" % (self.QUESTION_NAME.strip().decode())


bind_layers(NBNSHeader, NBNSQueryRequest, OPCODE=0x0, NM_FLAGS=0x11, QDCOUNT=1)
...
class NBNS_ADD_ENTRY(Packet):
    fields_desc = [BitEnumField('G', 0, 1, _NETBIOS_GNAMES), BitEnumField('OWNER_NODE_TYPE', 00, 2, _NETBIOS_OWNER_MODE_TYPES), BitEnumField('UNUSED', 0, 13, {0: 'Unused'}), IPField('NB_ADDRESS', '127.0.0.1')]
...
class NBNSQueryResponse(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('ADDR_ENTRY', [NBNS_ADD_ENTRY()], NBNS_ADD_ENTRY, length_from=lambda pkt: pkt.RDLENGTH),
    ]

    def mysummary(self):
        if not self.ADDR_ENTRY:
            return 'NBNSQueryResponse'
        return "NBNSQueryResponse '\\\\%s' is at %s" % (self.RR_NAME.strip().decode(), self.ADDR_ENTRY[0].NB_ADDRESS)


bind_layers(NBNSHeader, NBNSQueryResponse, OPCODE=0x0, NM_FLAGS=0x50, RESPONSE=1, ANCOUNT=1)
...
class NBNSNodeStatusRequest(NBNSQueryRequest):
    ...
    def mysummary(self):
        return "NBNSNodeStatusRequest who has '\\\\%s'" % (self.QUESTION_NAME.strip().decode())
...
class NBNSNodeStatusResponseService(Packet):
    ...
    fields_desc = [
        StrFixedLenField('NETBIOS_NAME', 'WINDOWS         ', 15),
...
class NBNSNodeStatusResponse(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('NODE_NAME', [NBNSNodeStatusResponseService()], NBNSNodeStatusResponseService, count_from=lambda pkt: pkt.NUM_NAMES),
...
bind_layers(NBNSHeader, NBNSNodeStatusResponse, OPCODE=0x0, NM_FLAGS=0x40, RESPONSE=1, ANCOUNT=1)
...
bind_layers(NBNSHeader, NBNSRegistrationRequest, OPCODE=0x5, NM_FLAGS=0x11, QDCOUNT=1, ARCOUNT=1)
...
bind_layers(NBNSHeader, NBNSWackResponse, OPCODE=0x7, NM_FLAGS=0x40, RESPONSE=1, ANCOUNT=1)
...
class NBTSession(Packet):
    ...
    fields_desc = [
        ByteEnumField('TYPE', 0, {0x00: 'Session Message', 0x81: 'Session Request', 0x82: 'Positive Session Response', 0x83: 'Negative Session Response', 0x84: 'Retarget Session Response', 0x85: 'Session Keepalive'}),
...
class NBNS_am(AnsweringMachine):
    ...
    def is_request(self, req):
        if self.from_ip and IP in req and req[IP].src not in self.from_ip:
            return False
        return NBNSQueryRequest in req and (not self.ServerName or req[NBNSQueryRequest].QUESTION_NAME.decode().strip() == self.ServerName)
