...
from scapy.asn1fields import ASN1F_CHOICE, ASN1F_ENUMERATED, ASN1F_FLAGS, ASN1F_OID, ASN1F_PACKET, ASN1F_SEQUENCE, ASN1F_SEQUENCE_OF, ASN1F_STRING, ASN1F_optional
...
_ContextFlags = ['delegFlag', 'mutualFlag', 'replayFlag', 'sequenceFlag', 'superseded', 'anonFlag', 'confFlag', 'integFlag']
...
class SPNEGO_negTokenInit(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(
        ASN1F_SEQUENCE(
            ASN1F_optional(ASN1F_SEQUENCE_OF('mechTypes', None, SPNEGO_MechType, explicit_tag=0xA0)),
            ASN1F_optional(ASN1F_FLAGS('reqFlags', None, _ContextFlags, implicit_tag=0x81)),
            ASN1F_optional(ASN1F_PACKET('mechToken', None, SPNEGO_Token, explicit_tag=0xA2)),
            ASN1F_optional(ASN1F_PACKET('mechListMIC', None, SPNEGO_MechListMIC, implicit_tag=0xA3)),
        )
    )
...
class SPNEGO_negTokenResp(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(
        ASN1F_SEQUENCE(
            ASN1F_optional(
                ASN1F_ENUMERATED('negResult', 0, {0: 'accept-completed', 1: 'accept-incomplete', 2: 'reject', 3: 'request-mic'}, explicit_tag=0xA0),
            ),
            ASN1F_optional(
                ASN1F_PACKET('supportedMech', SPNEGO_MechType(), SPNEGO_MechType, explicit_tag=0xA1),
            ),
            ASN1F_optional(ASN1F_PACKET('responseToken', None, SPNEGO_Token, explicit_tag=0xA2)),
            ASN1F_optional(ASN1F_PACKET('mechListMIC', None, SPNEGO_MechListMIC, implicit_tag=0xA3)),
        )
    )
...
class SPNEGO_negToken(ASN1_Packet):
    ...
    ASN1_root = ASN1F_CHOICE(
        'token', SPNEGO_negTokenInit(), ASN1F_PACKET('negTokenInit', SPNEGO_negTokenInit(), SPNEGO_negTokenInit, implicit_tag=0xA0), ASN1F_PACKET('negTokenResp', SPNEGO_negTokenResp(), SPNEGO_negTokenResp, implicit_tag=0xA1)
    )
...
class NEGOEX_MESSAGE_HEADER(Packet):
    fields_desc = [
        ...
        LEIntEnumField('MessageType', 0, {0x0: 'INITIATOR_NEGO', 0x01: 'ACCEPTOR_NEGO', 0x02: 'INITIATOR_META_DATA', 0x03: 'ACCEPTOR_META_DATA', 0x04: 'CHALENGE', 0x05: 'AP_REQUEST', 0x06: 'VERIFY', 0x07: 'ALERT'}),
...
class NEGOEX_BYTE_VECTOR(Packet):
    fields_desc = [LEIntField('ByteArrayBufferOffset', 0), LEIntField('ByteArrayLength', 0)]
...
class NEGOEX_EXTENSION_VECTOR(Packet):
    fields_desc = [LELongField('ExtensionArrayOffset', 0), LEShortField('ExtensionCount', 0)]
...
class NEGOEX_NEGO_MESSAGE(Packet):
    ...
    fields_desc = [
        ...
        _NTLMPayloadField(
            ...
            OFFSET,
            [
                FieldListField('AuthScheme', [], UUIDEnumField('', None, _NEGOEX_AUTH_SCHEMES), count_from=lambda pkt: pkt.AuthSchemeCount),
                PacketListField('Extension', [], NEGOEX_EXTENSION_VECTOR, count_from=lambda pkt: pkt.ExtensionCount),
            ],
...
class NEGOEX_VERIFY_MESSAGE(Packet):
    show_indent = 0
    fields_desc = [NEGOEX_MESSAGE_HEADER, UUIDEnumField('AuthScheme', None, _NEGOEX_AUTH_SCHEMES), PacketField('Checksum', NEGOEX_CHECKSUM(), NEGOEX_CHECKSUM)]
...
class GSSAPI_BLOB(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_GSSAPI_APPLICATION(ASN1F_OID('MechType', '1.3.6.1.5.5.2'), ASN1F_PACKET('innerContextToken', SPNEGO_negToken(), SPNEGO_negToken, next_cls_cb=lambda pkt: _GSSAPI_OIDS.get(pkt.MechType.val, conf.raw_layer)))
