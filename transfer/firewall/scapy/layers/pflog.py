...
from scapy.fields import ByteEnumField, ByteField, IntField, IPField, IP6Field, MultipleTypeField, PadField, ShortField, SignedIntField, StrFixedLenField, YesNoByteField
...
class PFLog(Packet):
    ...
    fields_desc = [
        ...
        ByteEnumField('addrfamily', 2, {OPENBSD_AF_INET: 'IPv4', OPENBSD_AF_INET6: 'IPv6'}),
        ByteEnumField('action', 1, {0: 'pass', 1: 'drop', 2: 'scrub', 3: 'no-scrub', 4: 'nat', 5: 'no-nat', 6: 'binat', 7: 'no-binat', 8: 'rdr', 9: 'no-rdr', 10: 'syn-proxy-drop'}),
        ByteEnumField(
            'reason',
            0,
            {
        ...
        IntField('rulepid', 0),
        ByteEnumField('direction', 255, {0: 'inout', 1: 'in', 2: 'out', 255: 'unknown'}),
        YesNoByteField('rewritten', 0),
        ByteEnumField('naddrfamily', 2, {OPENBSD_AF_INET: 'IPv4', OPENBSD_AF_INET6: 'IPv6'}),
        StrFixedLenField('pad', b'\x00', 1),
        MultipleTypeField(
            [
                (PadField(IPField('saddr', '127.0.0.1'), 16, padwith=b'\x00'), lambda pkt: pkt.addrfamily == OPENBSD_AF_INET),
                (IP6Field('saddr', '::1'), lambda pkt: pkt.addrfamily == OPENBSD_AF_INET6),
            ],
            PadField(IPField('saddr', '127.0.0.1'), 16, padwith=b'\x00'),
        ),
        MultipleTypeField(
            [
                (PadField(IPField('daddr', '127.0.0.1'), 16, padwith=b'\x00'), lambda pkt: pkt.addrfamily == OPENBSD_AF_INET),
                (IP6Field('daddr', '::1'), lambda pkt: pkt.addrfamily == OPENBSD_AF_INET6),
            ],
