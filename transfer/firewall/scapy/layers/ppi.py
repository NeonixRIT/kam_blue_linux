...
from scapy.fields import ByteField, FieldLenField, LEIntField, PacketListField, LEShortEnumField, LenField
...
class PPI(Packet):
    ...
    fields_desc = [
        ByteField('version', 0),
        ByteField('flags', 0),
        FieldLenField('len', None, length_of='headers', fmt='<H', adjust=lambda p, x: x + 8),  # length of this packet
        LEIntField('dlt', None),
        PacketListField('headers', [], PPI_Hdr, length_from=lambda p: p.len - 8),
    ]
    ...
    def guess_payload_class(self, payload):
        ...
        return conf.l2types.get(self.getfieldval('dlt'), Packet.guess_payload_class(self, payload))
