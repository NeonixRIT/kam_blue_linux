...
class NTLM_SMB_Client(NTLM_Client, Automaton):
    port = 445
    cls = NBTSession
    kwargs_cls = {NTLM_SMB_Server: {'CLIENT_PROVIDES_NEGOEX': True, 'ECHO': True}}
    ...
    def continue_smb2(self):
        ...
        if self.CONTINUE_SMB2:
            self.SMB2 = True
            self.smb_header = NBTSession() / SMB2_Header(PID=0xFEFF)
    ...
    def on_negotiate(self):
        self.smb_header = NBTSession() / SMB_Header(Flags2=('LONG_NAMES+EAS+NT_STATUS+UNICODE+' 'SMB_SECURITY_SIGNATURE+EXTENDED_SECURITY'), TID=0xFFFF, PIDLow=0xFEFF, UID=0, MID=0)
        if self.EXTENDED_SECURITY:
            self.smb_header.Flags2 += 'EXTENDED_SECURITY'
        pkt = self.smb_header.copy() / SMBNegotiate_Request(
            Dialects=[SMB_Dialect(DialectString=x) for x in ['PC NETWORK PROGRAM 1.0', 'LANMAN1.0', 'Windows for Workgroups 3.1a', 'LM1.2X002', 'LANMAN2.1', 'NT LM 0.12'] + (['SMB 2.002', 'SMB 2.???'] if self.ALLOW_SMB2 else [])],
        )
        ...
        if self.SMB2 or self.EXTENDED_SECURITY:
            ...
            if self.SMB2:
                ...
                pkt = self.smb_header.copy() / SMBSession_Setup_AndX_Request_Extended_Security(
                    ServerCapabilities=('UNICODE+NT_SMBS+STATUS32+LEVEL_II_OPLOCKS+' 'DYNAMIC_REAUTH+EXTENDED_SECURITY'), VCNumber=self.get('VCNumber'), NativeOS=b'', NativeLanMan=b''
                )
                    ...
                    pkt.SecurityBlob = GSSAPI_BLOB(
                        innerContextToken=SPNEGO_negToken(
                            token=SPNEGO_negTokenInit(
                                ...
                                mechToken=SPNEGO_Token(value=ntlm_token),
                            )
                        )
                    )
    ...
    def DO_RUN_SCRIPT(self):
        ...
        pkt /= Raw(
            load=b'9\x00\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x9f\x01\x12\x00\x00\x00\x00\x00\x07\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00x\x00\x0c\x00\x00\x00\x00\x00\x00\x00\x00\x00s\x00r\x00v\x00s\x00v\x00c\x00'
        )  # noqa: E501
        self.send(pkt)
        # ... run something?
        self.end()
