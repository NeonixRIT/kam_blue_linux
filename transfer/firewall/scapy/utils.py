...
from scapy.compat import orb, plain_str, chb, bytes_base64, base64_bytes, hex_bytes, lambda_tuple_converter, bytes_encode
...
def zerofree_randstring(length):
    ...
    """
    return b''.join(struct.pack('B', random.randint(1, 255)) for _ in range(length))
...
class ContextManagerSubprocess(object):
    ...
    def __exit__(
        ...
        if isinstance(exc_value, EnvironmentError):
            ...
            msg = '%s: execution failed (%s)' % (self.prog, exc_type.__class__.__name__)
...
def do_graph(
    ...
    if string:
        return graph
    if type is not None:
        warnings.warn('type is deprecated, and was renamed format', DeprecationWarning)
        format = type
    if prog is None:
        prog = conf.prog.dot
    start_viewer = False
    ...
    if isinstance(target, str):
        if target.startswith('|'):
            target = subprocess.Popen(target[1:].lstrip(), shell=True, stdin=subprocess.PIPE).stdin
        elif target.startswith('>'):
            target = open(target[1:].lstrip(), 'wb')
        else:
            target = open(os.path.abspath(target), 'wb')
    target = cast(IO[bytes], target)
    proc = subprocess.Popen('"%s" %s %s' % (prog, options or '', format or ''), shell=True, stdin=subprocess.PIPE, stdout=target, stderr=subprocess.PIPE)
    _, stderr = proc.communicate(bytes_encode(graph))
    if proc.returncode != 0:
        raise OSError('GraphViz call failed (is it installed?):\n' + plain_str(stderr))
    try:
        target.close()
    except Exception:
        pass
...
class PcapReader_metaclass(type):
    ...
    def __call__(cls, filename):  # type: ignore
        ...
        if not magic:
            raise Scapy_Exception('No data could be read!')
        try:
...
class RawPcapReader:
    ...
    def __init__(self, filename, fdesc=None, magic=None):  # type: ignore
        ...
        if magic == b'\xa1\xb2\xc3\xd4':  # big endian
            ...
            raise Scapy_Exception('Not a pcap capture file (bad magic: %r)' % magic)
        hdr = self.f.read(20)
        if len(hdr) < 20:
    ...
    def _read_packet(self, size=MTU):
        ...
        return (self.f.read(caplen)[:size], RawPcapReader.PacketMetadata(sec=sec, usec=usec, wirelen=wirelen, caplen=caplen))

    def read_packet(self, size=MTU):
        # type: (int) -> Packet
        raise Exception('Cannot call read_packet() in RawPcapReader. Use ' '_read_packet()')
...
class PcapReader(RawPcapReader, _SuperSocket):
    def __init__(self, filename, fdesc=None, magic=None):  # type: ignore
        ...
        try:
            self.LLcls = conf.l2types.num2layer[self.linktype]  # type: Type[Packet]
        except KeyError:
...
class RawPcapNgReader(RawPcapReader):
    ...
    PacketMetadata = collections.namedtuple(
        'PacketMetadataNg',  # type: ignore
        ['linktype', 'tsresol', 'tshigh', 'tslow', 'wirelen', 'comment'],
    )
    ...
    def __init__(self, filename, fdesc=None, magic=None):  # type: ignore
        ...
        self.default_options = {'tsresol': 1000000}
        self.blocktypes = {
        ...
        if magic != b'\x0a\x0d\x0d\x0a':  # PcapNg:
            raise Scapy_Exception('Not a pcapng capture file (bad magic: %r)' % magic)

        try:
            self._read_block_shb()
        except EOFError:
            raise Scapy_Exception('The first SHB of the pcapng file is malformed !')
    ...
    def _read_block(self, size=MTU):
        ...
        return self.blocktypes.get(blocktype, lambda block, size: None)(block, size)

    def _read_block_tail(self, blocklen):
        # type: (int) -> None
        if blocklen % 4:
            pad = self.f.read(-blocklen % 4)
            warning('PcapNg: bad blocklen %d (MUST be a multiple of 4. ' 'Ignored padding %r' % (blocklen, pad))
        try:
            if blocklen != struct.unpack(self.endian + 'I', self.f.read(4))[0]:
    ...
    def _read_options(self, options):
        ...
        while len(options) >= 4:
            ...
            if code == 9 and length == 1 and len(options) >= 5:
                ...
                opts['tsresol'] = (2 if tsresol & 128 else 10) ** (tsresol & 127)
    ...
    def _read_block_idb(self, block, _):
        ...
        try:
            interface = (
                struct.unpack(  # type: ignore
                    self.endian + 'HxxI', block[:8]
                )
                + (options['tsresol'],)
    ...
    def _read_block_epb(self, block, size):
        ...
        return (
            block[20 : 20 + caplen][:size],
            RawPcapNgReader.PacketMetadata(
                linktype=self.interfaces[intid][0],  # noqa: E501
...
class GenericPcapWriter(object):
    ...
    def _get_time(
        ...
        if hasattr(packet, 'time'):
            if sec is None:
                ...
                tmp = int(packet_time)
                usec = int(round((packet_time - tmp) * (1000000000 if self.nano else 1000000)))
        ...
        if self.linktype is None:
            try:
                if pkt is None or isinstance(pkt, bytes):
                    # Can't guess LL
                    raise KeyError
                self.linktype = conf.l2types.layer2num[pkt.__class__]
            except KeyError:
    ...
    def write_packet(
        ...
        self._write_packet(rawpkt, sec=f_sec, usec=usec, caplen=caplen, wirelen=wirelen, comment=comment)
    ...
    def write(self, pkt):
        ...
        if isinstance(pkt, bytes):
            ...
            for p in IterSocket(pkt).iter:
                ...
                if not isinstance(p, bytes) and self.linktype != conf.l2types.get(type(p), None):
...
class RawPcapWriter(GenericRawPcapWriter):
    ...
    def __init__(
        ...
        if isinstance(filename, str):
            self.filename = filename
            if gz:
                self.f = cast(_ByteStream, gzip.open(filename, append and 'ab' or 'wb', 9))
            else:
    ...
    def _write_header(self, pkt):
        ...
        if self.linktype is None:
            raise ValueError('linktype could not be guessed. ' 'Please pass a linktype while creating the writer')

        self.f.write(
            struct.pack(
                self.endian + 'IHHIIII',
                0xA1B23C4D if self.nano else 0xA1B2C3D4,  # noqa: E501
                2,
                4,
                0,
                0,
    ...
    def _write_packet(
        ...
        self.f.write(struct.pack(self.endian + 'IIII', int(sec), usec, caplen, wirelen))
        self.f.write(packet)
        if self.sync:
            self.f.flush()
...
class RawPcapNgWriter(GenericRawPcapWriter):
    ...
    def _write_block_epb(
        ...
        self.f.write(self.build_block(block_type, block_epb, options=comment_opt))
    ...
    def _write_packet(
        ...
        self._write_block_epb(packet, timestamp=sec, caplen=caplen, orglen=wirelen, comment=comment)
        if self.sync:
            self.f.flush()
...
class ERFEthernetWriter(PcapWriter):
    ...
    def __init__(
        ...
        :param sync: do not bufferize writes to the capture file
        """
        super(ERFEthernetWriter, self).__init__(filename, gz=gz, append=append, sync=sync)
...
def tcpdump(
    ...
    if prog is None:
        if not conf.prog.tcpdump:
            raise Scapy_Exception('tcpdump is not available')
            ...
            with PcapReader(pktlist) as rd:
                if isinstance(rd, PcapNgReader):
                    ...
                    try:
                        ...
                        raise ValueError('Cannot get linktype from a PcapNg packet.')
                else:
...
def get_terminal_width():
    ...
    if WINDOWS:
        ...
        if res:
            (bufx, bufy, curx, cury, wattr, left, top, right, bottom, maxx, maxy) = struct.unpack('hhhhHhhhhhh', csbi.raw)  # noqa: E501
            sizex = right - left + 1
            # sizey = bottom - top + 1
            return sizex
        return sizex
...
def make_table(*args, **kargs):
    ...
    return __make_table(lambda l: '%%-%is' % l, lambda l: '%%-%is' % l, '', *args, **kargs)


def make_lined_table(*args, **kargs):
    # type: (*Any, **Any) -> Optional[str]
    return __make_table(  # type: ignore
        lambda l: '%%-%is |' % l, lambda l: '%%-%is |' % l, '', *args, seplinefunc=lambda a, x: '+'.join('-' * (y + 2) for y in [a - 1] + x + [-2]), **kargs
    )
...
def make_tex_table(*args, **kargs):
    # type: (*Any, **Any) -> Optional[str]
    return __make_table(  # type: ignore
        lambda l: '%s', lambda l: '& %s', '\\\\', *args, seplinefunc=lambda a, x: '\\hline', **kargs
    )
