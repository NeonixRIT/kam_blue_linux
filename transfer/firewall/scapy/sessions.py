...
# Typing imports
from scapy.compat import Any, Callable, DefaultDict, Dict, List, Optional, Tuple, cast
            ...
            if not ip.flags.MF:  # end of frag
                try:
                    if self.fragments[uniq][0].frag == 0:
                        ...
                        defragmented_packet = defragmented_packet.__class__(raw(defragmented_packet))
...
class TCPSession(IPSession):
    ...
    def __init__(self, app=False, *args, **kwargs):
        ...
        if app:
            ...
            self.tcp_frags = defaultdict(lambda: (StringBuffer(), {}))  # type: DefaultDict[bytes, Tuple[StringBuffer, Dict[str, Any]]]
            self.tcp_sessions = defaultdict(dict)  # type: DefaultDict[bytes, Dict[str, Any]]
