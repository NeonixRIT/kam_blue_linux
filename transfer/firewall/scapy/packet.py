...
from scapy.base_classes import BasePacket, Gen, SetGen, Packet_metaclass, _CanvasDumpExtended
from scapy.interfaces import _GlobInterfaceType
from scapy.volatile import RandField, VolatileValue
from scapy.utils import import_hexcap, tex_escape, colgen, issubtype, pretty_list, EDecimal
...
class Packet(
        ...
        **fields,  # type: Any
        ...
        for field in self.fields_desc:
            ...
            try:
                ...
                continue
            self.fields[fname] = value if isinstance(value, RawVal) else self.get_field(fname).any2i(self, value)
    ...
    def __reduce__(self):
        ...
        return (self.__class__, (self.build(),), (self.time, self.sent_time, self.direction, self.sniffed_on, self.wirelen, self.comment))
    ...
    def copy(self):
        ...
        clone.raw_packet_cache_fields = self.copy_fields_dict(self.raw_packet_cache_fields)
    ...
    def _resolve_alias(self, attr):
        ...
        warnings.warn('%s has been deprecated in favor of %s since %s !' % (attr, new_attr, version), DeprecationWarning)
        return new_attr
    ...
    def __repr__(self):
        ...
        return '%s%s %s %s%s%s' % (ct.punct('<'), ct.layer_name(self.__class__.__name__), s, ct.punct('|'), repr(self.payload), ct.punct('>'))
    ...
    def copy_fields_dict(self, fields):
        ...
        if fields is None:
            return None
        return {fname: self.copy_field_value(fname, fval) for fname, fval in six.iteritems(fields)}
        ...
        for f in self.fields_desc:
            ...
            if isinstance(val, RawVal):
                ...
                try:
                    ...
                    try:
                        ex.args = ("While dissecting field '%s': " % f.name + ex.args[0],) + ex.args[1:]
    ...
    def canvas_dump(self, layer_shift=0, rebuild=1):
        ...
        while t:
            ...
            proto, fields = t.pop()
            y += 0.5
            pt = pyx.text.text(XSTART, (YTXT - y) * YMUL, r'\font\cmssfont=cmss10\cmssfont{%s}' % tex_escape(str(proto.name)), [pyx.text.size.Large])
            y += 1
            ptbb = pt.bbox()
            ...
            for field, fval, fdump in fields:
                ...
                if isinstance(field, BitField):
                    ...
                    fsize = '%sB' % len(fdump)
                if hasattr(field, 'field') and 'LE' in field.field.__class__.__name__[:3] or 'LE' in field.__class__.__name__[:3]:
    ...
    def guess_payload_class(self, payload):
        ...
        for t in self.aliastypes:
            for fval, cls in t.payload_guess:
                try:
                    if all(v == self.getfieldval(k) for k, v in six.iteritems(fval)):
    ...
    def clone_with(self, payload=None, **kargs):
        ...
        pkt.raw_packet_cache_fields = self.copy_fields_dict(self.raw_packet_cache_fields)
        ...
        if self.explicit or self.raw_packet_cache is not None:
            ...
            todo = [
                ...
                for (k, v) in itertools.chain(
                    six.iteritems(self.default_fields),  # noqa: E501
    ...
    def haslayer(self, cls, _subclass=None):
        ...
        if cls is None or match(self.__class__, cls) or cls in [self.__class__.__name__, self._name]:
            return True
    ...
    def getlayer(
        ...
        **flt,  # type: Any
        ...
        if not class_name or match(self.__class__, class_name) or class_name in [self.__class__.__name__, self._name]:
            if all(self.getfieldval(fldname) == fldvalue for fldname, fldvalue in six.iteritems(flt)):
                if nb == 1:
                    if fld is None:
                        return self
                    else:
        ...
        for f in self.packetfields:
            ...
            for fvalue in fvalue_gen:
                if isinstance(fvalue, Packet):
                    track = []  # type: List[int]
                    ret = fvalue.getlayer(class_name, nb=nb, _track=track, _subclass=_subclass, **flt)
                    if ret is not None:
                        return ret
                    nb = track[0]
        return self.payload.getlayer(class_name, nb=nb, _track=_track, _subclass=_subclass, **flt)
    ...
    def _show_or_dump(
        ...
        if self.payload:
            s += self.payload._show_or_dump(  # type: ignore
                dump=dump, indent=indent, lvl=lvl + (' ' * indent * self.show_indent), label_lvl=label_lvl, first_call=False
            )
    ...
    def sprintf(self, fmt, relax=1):
        ...
        while '%' in fmt:
            ...
            try:
                ...
                if fld == 'time':
                    val = time.strftime('%H:%M:%S.%%06i', time.localtime(float(self.time))) % int((self.time - int(self.time)) * 1000000)
    ...
    def command(self):
        ...
        for fn, fv in six.iteritems(self.fields):
            ...
            if isinstance(fv, Packet):
                ...
                fv = '[%s]' % ', '.join(getattr(x, 'command', lambda: repr(x))() for x in fv)
...
class Padding(Raw):
    ...
    def build_padding(self):
        # type: () -> bytes
        return (bytes_encode(self.load) if self.raw_packet_cache is None else self.raw_packet_cache) + self.payload.build_padding()
...
def split_bottom_up(
    ...
    **fval,  # type: Any
    ...
    def do_filter(params, cls):
        ...
        params_is_invalid = any(k not in params or params[k] != v for k, v in six.iteritems(fval))
...
def explore(layer=None):
    ...
    if layer is None:  # GUI MODE
        ...
        btn_diag = button_dialog(
            ...
            buttons=[(six.text_type('Layers'), 'layers'), (six.text_type('Contribs'), 'contribs'), (six.text_type('Cancel'), 'cancel')],
        )
        ...
        if action == 'layers':
            ...
            values = [x for x in lvalues if ('layers' in x[0] or 'packet' in x[0] or 'asn1' in x[0])]
        ...
        # Python 2 compat
        if six.PY2:
            values = [(six.text_type(x), six.text_type(y)) for x, y in values]
        # Build tree
        if action == 'contribs':
        ...
        while True:
            ...
            _radio_values = [('$' + name, six.text_type('[+] ' + name.capitalize())) for name in folders if not name.startswith('_')] + current.get('_l', [])  # type: List[str]
            cur_path = ''
            if previous:
                cur_path = '.'.join(
                    itertools.chain(
            ...
            extra_text = ('\n<style bg="white" fg="green">> scapy.%s</style>') % (action + ('.' + cur_path if cur_path else ''))
            # Show popup
            rd_diag = radiolist_dialog(
                values=_radio_values,
                title=six.text_type('Scapy v%s' % conf.version),
                text=HTML(six.text_type(('<style bg="white" fg="red">Please select a file' 'among the following, to see all layers contained in' ' it:</style>') + extra_text)),
...
def _pkt_ls(
    ...
    for f in obj.fields_desc:
        ...
        if verbose and isinstance(cur_fld, EnumField) and hasattr(cur_fld, 'i2s'):
            if len(cur_fld.i2s or []) < 50:
                long_attrs.extend('%s: %d' % (strval, numval) for numval, strval in sorted(six.iteritems(cur_fld.i2s)))
        elif isinstance(cur_fld, MultiEnumField):
            fld_depend = cur_fld.depends_on(cast(Packet, obj if is_pkt else obj()))
            attrs.append('Depends on %s' % fld_depend)
            if verbose:
                cur_i2s = cur_fld.i2s_multi.get(cur_fld.depends_on(cast(Packet, obj if is_pkt else obj())), {})
                if len(cur_i2s) < 50:
                    long_attrs.extend('%s: %d' % (strval, numval) for numval, strval in sorted(six.iteritems(cur_i2s)))
            ...
            attrs.append(', '.join(x[0].__class__.__name__ for x in itertools.chain(cur_fld.flds, [(cur_fld.dflt,)])))

        cls = cur_fld.__class__
        class_name_extras = '(%s)' % (', '.join(attrs)) if attrs else ''
        if isinstance(cur_fld, BitField):
            class_name_extras += ' (%d bit%s)' % (cur_fld.size, 's' if cur_fld.size > 1 else '')
...
def ls(
    ...
    if obj is None or is_string:
        ...
        if obj is None:
            ...
            pattern = re.compile(cast(str, obj), 0 if case_sensitive else re.I)
            ...
            all_layers = sorted((layer for layer in conf.layers if (isinstance(layer.__name__, str) and pattern.search(layer.__name__)) or (isinstance(layer.name, str) and pattern.search(layer.name))), key=sorter)
...
def fuzz(
    ...
    while not isinstance(q, NoPayload):
        ...
        if multiple_type_fields:
            ...
            new_default_fields = {key: (val._fix() if isinstance(val, VolatileValue) else val) for key, val in six.iteritems(new_default_fields)}
