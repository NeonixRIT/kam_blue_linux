    ...
    def __new__(
        ...
        if resolved_fld:  # perform default value replacements
            ...
            for f in resolved_fld:
                if f.name in names:
                    war_msg = "Packet '%s' has a duplicated '%s' field ! " 'If you are using several ConditionalFields, have ' 'a look at MultipleTypeField instead ! This will ' 'become a SyntaxError in a future version of ' 'Scapy !' % (
                        name,
                        f.name,
                    )
        ...
        try:
            ...
            dct['__signature__'] = inspect.Signature(
                ...
                + [inspect.Parameter(f.name, inspect.Parameter.KEYWORD_ONLY, default=f.default) for f in dct['fields_desc']]
            )
        except (ImportError, AttributeError, KeyError):
            pass
        newcls = cast('Type[scapy.packet.Packet]', type.__new__(cls, name, bases, dct))
