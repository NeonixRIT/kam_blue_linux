...
class NmapKnowledgeBase(KnowledgeBase):
    ...
    def lazy_init(self):
        # type: () -> None
        try:
            fdesc = open(conf.nmap_base if self.filename is None else self.filename, 'rb')
        ...
        for line in fdesc:
            ...
            sig[test] = dict(val.split('=', 1) for val in (values.split('%') if values else []))
        if name is not None:
            self.base.append((name, sig))
        fdesc.close()
...
def nmap_udppacket_sig(snd, rcv):
    ...
    if rcv is None:
        ...
        res['RIPCK'] = 'E' if snd.chksum == rcv[IPerror].chksum else ('0' if rcv[IPerror].chksum == 0 else 'F')
        res['UCK'] = 'E' if snd.payload.chksum == rcv[UDPerror].chksum else ('0' if rcv[UDPerror].chksum == 0 else 'F')
        res['ULEN'] = '%X' % rcv[UDPerror].len
        res['DAT'] = 'E' if (isinstance(rcv[UDPerror].payload, NoPayload) or raw(rcv[UDPerror].payload) == raw(snd[UDP].payload)) else 'F'
    return res


def nmap_match_one_sig(seen, ref):
    # type: (Dict, Dict) -> float
    cnt = sum(val in ref.get(key, '').split('|') for key, val in six.iteritems(seen))
...
def nmap_sig(target, oport=80, cport=81, ucport=1):
    ...
    tcpopt = [('WScale', 10), ('NOP', None), ('MSS', 256), ('Timestamp', (123, 0))]
    tests = [IP(dst=target, id=1) / TCP(seq=1, sport=5001 + i, dport=oport if i < 4 else cport, options=tcpopt, flags=flags) for i, flags in enumerate(['CS', '', 'SFUP', 'A', 'S', 'A', 'FPU'])]
...
def nmap_sig2txt(sig):
    ...
    korder = ['Class', 'gcd', 'SI', 'IPID', 'TS', 'Resp', 'DF', 'W', 'ACK', 'Flags', 'Ops', 'TOS', 'IPLEN', 'RIPTL', 'RID', 'RIPCK', 'UCK', 'ULEN', 'DAT']
    txt = []
    for i in sig:
        if i not in torder:
            torder.append(i)
    for test in torder:
        ...
        if testsig is None:
            continue
        txt.append('%s(%s)' % (test, '%'.join('%s=%s' % (key, testsig[key]) for key in korder if key in testsig)))
    return '\n'.join(txt)
