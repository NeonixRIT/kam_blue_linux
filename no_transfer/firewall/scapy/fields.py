...
from scapy.volatile import RandBin, RandByte, RandEnumKeys, RandInt, RandIP, RandIP6, RandLong, RandMAC, RandNum, RandShort, RandSInt, RandSByte, RandTermString, RandUUID, VolatileValue, RandSShort, RandSLong, RandFloat
...
from scapy.utils6 import in6_6to4ExtractAddr, in6_isaddr6to4, in6_isaddrTeredo, in6_ptop, Net6, teredoAddrExtractInfo
...
class Field(Generic[I, M]):
    ...
    def addfield(self, pkt, s, val):
        ...
        try:
            ...
            raise ValueError('Incorrect type of value for field %s:\n' % self.name + "struct.error('%s')\n" % ex + 'To inject bytes into the field regardless of the type, ' + 'use RawVal. See help(RawVal)')
    ...
    def __repr__(self):
        # type: () -> str
        return '<%s (%s).%s>' % (self.__class__.__name__, ','.join(x.__name__ for x in self.owners), self.name)
    ...
    def randval(self):
        ...
        fmtt = self.fmt[-1]
        if fmtt in 'BbHhIiQq':
            return {'B': RandByte, 'b': RandSByte, 'H': RandShort, 'h': RandSShort, 'I': RandInt, 'i': RandSInt, 'Q': RandLong, 'q': RandSLong}[fmtt]()
        elif fmtt == 's':
            ...
            raise ValueError('no random class for [%s] (fmt=%s).' % (self.name, self.fmt))
...
class MultipleTypeField(_FieldContainer):
    ...
    def __init__(
        ...
        if any(x[0].name != self.name for x in self.flds):
            warnings.warn(('All fields should have the same name in a ' 'MultipleTypeField (%s)') % self.name, SyntaxWarning)
...
class PadField(_FieldContainer):
    ...
    def addfield(
        ...
        return s + sval + struct.pack('%is' % (self.padlen(len(sval), pkt)), self._padwith)
...
class DestField(Field[str, bytes]):
    ...
    def dst_from_pkt(self, pkt):
        ...
        for addr, condition in self.bindings.get(pkt.payload.__class__, []):
            try:
                if all(pkt.payload.getfieldval(field) == value for field, value in six.iteritems(condition)):
...
class SourceIPField(IPField):
    ...
    def __findaddr(self, pkt):
        ...
        dst = '0.0.0.0' if self.dstname is None else getattr(pkt, self.dstname) or '0.0.0.0'
        if isinstance(dst, (Gen, list)):
            r = {conf.route.route(str(daddr)) for daddr in dst}  # type:  Set[Tuple[str, str, str]]
            if len(r) > 1:
...
class SourceIP6Field(IP6Field):
    ...
    def i2m(self, pkt, x):
        # type: (Optional[Packet], Optional[Union[str, Net6]]) -> bytes
        if x is None:
            dst = '::' if self.dstname is None else getattr(pkt, self.dstname) or '::'
...
class BitFieldLenField(BitField):
    ...
    def __init__(
        ...
        # type: (...) -> None
        super(BitFieldLenField, self).__init__(name, default, size, tot_size, end_tot_size)
        self.length_of = length_of
        self.count_of = count_of
        self.adjust = adjust
...
class LEFieldLenField(FieldLenField):
    def __init__(
        ...
        # type: (...) -> None
        FieldLenField.__init__(self, name, default, length_of=length_of, fmt=fmt, count_of=count_of, adjust=adjust)
...
class FlagValue(object):
    ...
    def __sub__(self, other):
        # type: (int) -> FlagValue
        return self.__class__(self.value & (2 ** len(self.names) - 1 - self._fixvalue(other)), self.names)
    ...
    def flagrepr(self):
        # type: () -> str
        warnings.warn('obj.flagrepr() is obsolete. Use str(obj) instead.', DeprecationWarning)
        return str(self)
...
class FlagsField(_BitField[Optional[Union[int, FlagValue]]]):
    ...
    def i2repr(
        ...
        # type: (...) -> str
        if isinstance(x, (list, tuple)):
            return repr(type(x)('None' if v is None else str(self._fixup_val(v)) for v in x))
...
class UTCTimeField(Field[float, int]):
    __slots__ = ['epoch', 'delta', 'strf', 'use_msec', 'use_micro', 'use_nano', 'custom_scaling']
    ...
    * Given a tuple:
    ...
    def _check_uuid_fmt(self):
        ...
        if self.uuid_fmt not in UUIDField.FORMATS:
            raise FieldValueRangeException('Unsupported uuid_fmt ({})'.format(self.uuid_fmt))
