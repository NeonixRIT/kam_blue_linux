...
class RPC_UNICODE_STRING(NDRPacket):
    ...
    fields_desc = [
        ...
        NDRFullPointerField(
            NDRConfVarStrLenFieldUtf16('Buffer', '', length_from=lambda pkt: (pkt.Length // 2)),
            deferred=True,
        ),
    ]
...
class KERB_VALIDATION_INFO(NDRPacket):
    ...
    fields_desc = [
        ...
        NDRFullPointerField(NDRPacketField('LogonDomainId', PSID(), PSID), deferred=True),
        ...
        NDRFullPointerField(NDRPacketField('ResourceGroupDomainSid', PSID(), PSID), deferred=True),
...
class u_sub1(NDRPacket):
    ...
    fields_desc = [
        NDRIntField('ValueCount', 0),
        NDRFullPointerField(
            NDRConfFieldListField('Uint64Values', [], NDRLongField, count_from=lambda pkt: pkt.ValueCount),
            deferred=True,
        ),
    ]
...
class u_sub3(NDRPacket):
    ...
    fields_desc = [
        NDRIntField('ValueCount', 0),
        NDRFullPointerField(
            NDRConfFieldListField('BooleanValues', [], NDRLongField, count_from=lambda pkt: pkt.ValueCount),
            deferred=True,
        ),
    ]
        ...
        NDRUnionField(
            [
                (
                    ...
                    (
                        (lambda pkt: getattr(pkt, 'Type', None) == CLAIM_TYPE.CLAIM_TYPE_INT64),
                ...
                (
                    ...
                    (
                        (lambda pkt: getattr(pkt, 'Type', None) == CLAIM_TYPE.CLAIM_TYPE_UINT64),
                ...
                (
                    ...
                    (
                        (lambda pkt: getattr(pkt, 'Type', None) == CLAIM_TYPE.CLAIM_TYPE_STRING),
...
class CLAIMS_SET(NDRPacket):
    ...
    fields_desc = [
        ...
        NDRFullPointerField(
            NDRConfStrLenField('ReservedField', '', length_from=lambda pkt: pkt.ulReservedFieldSize),
            deferred=True,
        ),
    ]
...
class CLAIMS_SET_METADATA(NDRPacket):
    ...
    fields_desc = [
        ...
        NDRFullPointerField(
            _CLAIMSClaimSet('ClaimsSet', None, None, length_from=lambda pkt: pkt.ulClaimsSetSize),
            deferred=True,
        ),
        NDRInt3264EnumField(
            'usCompressionFormat',
        ...
        NDRIntField('ulReservedFieldSize', 0),
        NDRFullPointerField(
            NDRConfVarStrLenField('ReservedField', '', length_from=lambda pkt: pkt.ulReservedFieldSize),
            deferred=True,
        ),
    ]
...
class PAC_DEVICE_INFO(NDRPacket):
    ...
    fields_desc = [
        NDRIntField('UserId', 0),
        NDRIntField('PrimaryGroupId', 0),
        NDRFullPointerField(NDRPacketField('AccountDomainId', PSID(), PSID), deferred=True),
