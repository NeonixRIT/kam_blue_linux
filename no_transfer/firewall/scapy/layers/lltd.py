...
class LLTD(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ShortField('xid', 0), lambda pkt: pkt.function in [0, 8]),
        ConditionalField(ShortField('seq', 0), lambda pkt: pkt.function not in [0, 8]),
    ]
...
class LLTDDiscover(Packet):
    name = 'LLTD - Discover'
    fields_desc = [ShortField('gen_number', 0), FieldLenField('stations_count', None, count_of='stations_list', fmt='H'), FieldListField('stations_list', [], MACField('', ETHER_ANY), count_from=lambda pkt: pkt.stations_count)]
...
class LLTDEmit(Packet):
    name = 'LLTD - Emit'
    fields_desc = [
        FieldLenField('descs_count', None, count_of='descs_list', fmt='H'),
        PacketListField('descs_list', [], LLTDEmiteeDesc, count_from=lambda pkt: pkt.descs_count),
    ]
...
class LLTDQueryResp(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('descs_list', [], LLTDRecveeDesc, count_from=lambda pkt: pkt.descs_count),
    ]
...
class LLTDQueryLargeTlvResp(Packet):
    ...
    fields_desc = [FlagsField('flags', 0, 2, 'RM'), BitField('len', None, 14), StrLenField('value', '', length_from=lambda pkt: pkt.len)]
...
class LLTDAttributeCharacteristics(LLTDAttribute):
    ...
    fields_desc = [
        ...
        # ByteField("len", 2),
        FieldLenField('len', None, length_of='reserved2', fmt='B', adjust=lambda _, x: x + 2),
...
class LLTDAttributeMachineName(LLTDAttribute):
    ...
    def mysummary(self):
        return (self.sprintf('Hostname: %r' % self.hostname), [LLTD, LLTDAttributeHostID])
...
class LargeTlvBuilder(object):
    ...
    def get_data(self):
        ...
        """
        return {key: ''.join(chr(byte) for byte in data) for key, data in six.iteritems(self.data)}
