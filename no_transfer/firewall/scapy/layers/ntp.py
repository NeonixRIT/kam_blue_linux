...
class NTPHeader(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(StrFixedLenEnumField('ref_id', '', length=4, enum=_reference_identifiers), lambda p: p.stratum < 2),
...
class NTPControl(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(NTPControlStatusField('status_word', '', Packet), lambda p: p.response == 1),
        ConditionalField(ShortField('status', 0), lambda p: p.response == 0),
        ShortField('association_id', 0),
        ShortField('offset', 0),
        ShortField('count', None),
        NTPControlDataPacketLenField('data', '', Packet, length_from=lambda p: p.count),
...
class NTPInfoPeer(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('filtdelay', [0.0 for i in range(0, _NTP_SHIFT)], FixedPointField('', 0, size=32, frac_bits=16), count_from=lambda p: _NTP_SHIFT),
        FieldListField('filtoffset', [0.0 for i in range(0, _NTP_SHIFT)], TimeStampField('', 0), count_from=lambda p: _NTP_SHIFT),
        FieldListField('order', [0 for i in range(0, _NTP_SHIFT)], ByteField('', 0), count_from=lambda p: _NTP_SHIFT),
        FixedPointField('delay', 0, size=32, frac_bits=16),
        FixedPointField('dispersion', 0, size=32, frac_bits=16),
        TimeStampField('offset', 0),
...
class NTPInfoMemStats(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('hashcount', [0.0 for i in range(0, _NTP_HASH_SIZE)], ByteField('', 0), count_from=lambda p: _NTP_HASH_SIZE),
    ]
...
class NTPPrivate(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(NTPPrivateReqPacketListField('req_data', [], Packet, length_from=lambda p: p.data_item_size, count_from=lambda p: p.nb_items), lambda p: p.response == 0),
        # Responses
        ConditionalField(NTPPrivateRespPacketListField('data', [], Packet, length_from=lambda p: p.data_item_size, count_from=lambda p: p.nb_items), lambda p: p.response == 1),
        # Responses are not supposed to be authenticated
        ConditionalField(PacketField('authenticator', '', NTPPrivatePktTail), lambda p: p.response == 0 and p.auth == 1),
    ]
