...
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, ConditionalField, Field, FieldLenField, FlagsField, IntField, PacketListField, ShortEnumField, ShortField, StrField, StrLenField, MultipleTypeField, UTCTimeField, I
...
def _is_ptr(x):
    return b'.' not in x and ((x and orb(x[-1]) == 0) or (len(x) >= 2 and (orb(x[-2]) & 0xC0) == 0xC0))
    ...
    for current, name, dat in field_gen(dns_pkt):
        for part in possible_shortens(dat):
            ...
            if part not in data:
                ...
                _in = data[part][0][3]
                build_pkt = build_pkt[:_in] + build_pkt[_in:].replace(encoded, b'\0\0', 1)
                break
...
class DNSQR(InheritOriginDNSStrPacket):
    ...
    show_indent = 0
    fields_desc = [DNSStrField('qname', 'www.example.com'), ShortEnumField('qtype', 1, dnsqtypes), ShortEnumField('qclass', 1, dnsclasses)]


class DNS(Packet):
    name = 'DNS'
    fields_desc = [
        ConditionalField(ShortField('length', None), lambda p: isinstance(p.underlayer, TCP)),
        ShortField('id', 0),
        BitField('qr', 0, 1),
...
edns0types = {0: 'Reserved', 1: 'LLQ', 2: 'UL', 3: 'NSID', 4: 'Reserved', 5: 'PING', 8: 'edns-client-subnet'}


class EDNS0TLV(Packet):
    name = 'DNS EDNS0 TLV'
    fields_desc = [ShortEnumField('optcode', 0, edns0types), FieldLenField('optlen', None, 'optdata', fmt='H'), StrLenField('optdata', '', length_from=lambda pkt: pkt.optlen)]
...
class EDNS0ClientSubnet(Packet):
    ...
    fields_desc = [
        ShortEnumField('optcode', 8, edns0types),
        FieldLenField('optlen', None, 'address', fmt='H', adjust=lambda pkt, x: x + 4),
        ShortField('family', 1),
        FieldLenField('source_plen', None, length_of='address', fmt='B', adjust=lambda pkt, x: x * 8),
        ByteField('scope_plen', 0),
        MultipleTypeField(
            [(ClientSubnetv4('address', '192.168.0.0', length_from=lambda p: p.source_plen), lambda pkt: pkt.family == 1), (ClientSubnetv6('address', '2001:db8::', length_from=lambda p: p.source_plen), lambda pkt: pkt.family == 2)],
            ClientSubnetv4('address', '192.168.0.0', length_from=lambda p: p.source_plen),
        ),
    ]
...
dnssecalgotypes = {
    ...
    14: 'ECDSA Curve P-384 with SHA-384',  # noqa: E501
    252: 'Reserved for Indirect Keys',
    253: 'Private algorithms - domain name',  # noqa: E501
...
def RRlist2bitmap(lst):
    ...
    for wb in range(min_window_blocks, max_window_blocks + 1):
        ...
        bitmap += b''.join(
            struct.pack(
                b'B',
                sum(2 ** (7 - (x - 256 * wb) + (tmp * 8)) for x in rrlist if 256 * wb + 8 * tmp <= x < 256 * wb + 8 * tmp + 8),
            )
...
class DNSRRNSEC(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 47, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), DNSStrField('nextname', ''), RRlistField('typebitmaps', '')]
...
tsig_algo_sizes = {'HMAC-MD5.SIG-ALG.REG.INT': 16, 'hmac-sha1': 20}
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                ...
                # NS, MD, MF, CNAME, PTR
                (DNSStrField('rdata', '', length_from=lambda pkt: pkt.rdlen), lambda pkt: pkt.type in [2, 3, 4, 5, 12]),
                # TEXT
                (DNSTextField('rdata', [], length_from=lambda pkt: pkt.rdlen), lambda pkt: pkt.type == 16),
            ],
...
class DNS_am(AnsweringMachine):
    ...
    def parse_options(self, joker=None, match=None, joker6=None, from_ip=None):
        """
    ...
    def is_request(self, req):
        ...
        return req.haslayer(self.cls) and req.getlayer(self.cls).qr == 0 and (not self.from_ip or (req[IPv6].src in req if IPv6 in req else req[IP].src) in self.from_ip)
    ...
    def make_reply(self, req):
        ...
        if req.qd.qtype == 28:
            ...
            if self.joker6 is False:
                return
            rdata = self.match.get(dns.qd.qname, self.joker or get_if_addr6(self.optsniff.get('iface', conf.iface)))
            if isinstance(rdata, (tuple, list)):
                rdata = rdata[1]
            resp /= self.cls(id=dns.id, qr=1, qd=dns.qd, an=DNSRR(rrname=dns.qd.qname, ttl=10, rdata=rdata, type=28))
        else:
            if self.joker is False:
                return
            rdata = self.match.get(dns.qd.qname, self.joker or get_if_addr(self.optsniff.get('iface', conf.iface)))
            if isinstance(rdata, (tuple, list)):
                # Fallback
                rdata = rdata[0]
            resp /= self.cls(id=dns.id, qr=1, qd=dns.qd, an=DNSRR(rrname=dns.qd.qname, ttl=10, rdata=rdata))
        return resp
