...
GENERAL_HEADERS = ['Cache-Control', 'Connection', 'Permanent', 'Content-Length', 'Content-MD5', 'Content-Type', 'Date', 'Keep-Alive', 'Pragma', 'Upgrade', 'Via', 'Warning']
    ...
    def _get_encodings(self):
        ...
        if isinstance(self, HTTPResponse):
            if self.Transfer_Encoding:
                encodings += [plain_str(x).strip().lower() for x in plain_str(self.Transfer_Encoding).split(',')]
            if self.Content_Encoding:
                encodings += [plain_str(x).strip().lower() for x in plain_str(self.Content_Encoding).split(',')]
        return encodings
        ...
        try:
            if 'deflate' in encodings:
                ...
                if _is_brotli_available:
                    ...
                    log_loading.info("Can't import brotli. brotli decompression " 'will be ignored !')
                ...
                if _is_zstd_available:
                    ...
                    log_loading.info("Can't import zstandard. zstd decompression " 'will be ignored !')
        except Exception:
    ...
    def post_build(self, pkt, pay):
        ...
        if 'deflate' in encodings:
            ...
            if _is_brotli_available:
                ...
                log_loading.info("Can't import brotli. brotli compression will " 'be ignored !')
        elif 'zstd' in encodings:
            if _is_zstd_available:
                pay = zstandard.ZstdCompressor().compress(pay)
            else:
                log_loading.info("Can't import zstandard. zstd compression will " 'be ignored !')
        return pkt + pay
    ...
    def self_build(self, **kwargs):
        ...
        for f in self.fields_desc:
            ...
            if not val:
                # Not specified. Skip
                continue
            if f.name not in ['Method', 'Path', 'Reason_Phrase', 'Http_Version', 'Status_Code']:
...
class HTTPRequest(_HTTPContent):
    ...
    fields_desc = (
        ...
        + (_generate_headers(GENERAL_HEADERS, REQUEST_HEADERS, COMMON_UNSTANDARD_GENERAL_HEADERS, COMMON_UNSTANDARD_REQUEST_HEADERS))
        + [
    ...
    def do_dissect(self, s):
        ...
        try:
            Method, Path, HTTPVersion = re.split(rb'\s+', first_line, 2)
    ...
    def mysummary(self):
        return self.sprintf('%HTTPRequest.Method% %HTTPRequest.Path% ' '%HTTPRequest.Http_Version%')
...
class HTTPResponse(_HTTPContent):
    ...
    fields_desc = (
        ...
        + (_generate_headers(GENERAL_HEADERS, RESPONSE_HEADERS, COMMON_UNSTANDARD_GENERAL_HEADERS, COMMON_UNSTANDARD_RESPONSE_HEADERS))
        + [
            _HTTPHeaderField('Unknown-Headers', None),
        ]
    )
    ...
    def do_dissect(self, s):
        ...
        first_line, body = _dissect_headers(self, s)
        try:
            HTTPVersion, Status, Reason = re.split(rb'\s+', first_line, 2)
            self.setfieldval('Http_Version', HTTPVersion)
    ...
    def mysummary(self):
        return self.sprintf('%HTTPResponse.Http_Version% %HTTPResponse.Status_Code% ' '%HTTPResponse.Reason_Phrase%')
...
class HTTP(Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        """
        try:
            prog = re.compile(rb'^(?:OPTIONS|GET|HEAD|POST|PUT|DELETE|TRACE|CONNECT) ' rb'(?:.+?) ' rb'HTTP/\d\.\d$')
            ...
            if result:
                ...
                prog = re.compile(rb'^HTTP/\d\.\d \d\d\d .*$')
                result = prog.match(req)
                if result:
                    return HTTPResponse
        except ValueError:
...
def http_request(host, path='/', port=80, timeout=3, display=False, verbose=0, raw=False, iptables=False, iface=None, **headers):
    ...
    try:
        ans = sock.sr1(req, session=TCPSession(app=True), timeout=timeout, verbose=verbose)
    finally:
        sock.close()
        if raw and iptables:
            host = str(Net(host))
