...
class FileNetworkOpenInformation(Packet):
    fields_desc = [
        UTCTimeField('CreationTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
        UTCTimeField('LastAccessTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
        UTCTimeField('LastWriteTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
        UTCTimeField('ChangeTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
...
class FileStreamInformation(Packet):
    fields_desc = [
        ...
        StrLenFieldUtf16('StreamName', b'::$DATA', length_from=lambda pkt: pkt.StreamNameLength),
    ]
...
class FileFsAttributeInformation(Packet):
    fields_desc = [
        ...
        FieldLenField('FileSystemNameLength', None, length_of='FileSystemName', fmt='<I'),
        StrLenFieldUtf16('FileSystemName', b'NTFS', length_from=lambda pkt: pkt.FileSystemNameLength),
    ]
...
class FileFsVolumeInformation(Packet):
    fields_desc = [
        UTCTimeField('VolumeCreationTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
...
class SMB2_Header(Packet):
    ...
    fields_desc = [
        ...
        # ASYNC
        ConditionalField(LELongField('AsyncId', 0), lambda pkt: pkt.Flags.SMB2_FLAGS_ASYNC_COMMAND),
        # SYNC
        ConditionalField(
...
class SMB2_Negotiate_Protocol_Request(_SMB2_Payload):
    ...
    fields_desc = [
        ...
        FieldLenField('NegotiateCount', None, fmt='<H', count_of='NegotiateContexts'),
        ShortField('Reserved2', 0),
        FieldListField('Dialects', [0x0202], LEShortEnumField('', 0x0, SMB_DIALECTS), count_from=lambda pkt: pkt.DialectCount),
        # Field only exists if Dialects contains 0x0311
        # Each negotiate context must be 8-byte aligned
        ConditionalField(FieldListField('NegotiateContexts', [], ReversePadField(PacketField('Context', None, SMB2_Negotiate_Context), 8), count_from=lambda pkt: pkt.NegotiateCount), lambda x: 0x0311 in x.Dialects),
    ]
...
class SMB2_Preauth_Integrity_Capabilities(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField('HashAlgorithmCount', 1, fmt='<H', count_of='HashAlgorithms'),
...
bind_layers(SMB2_Negotiate_Context, SMB2_Preauth_Integrity_Capabilities, ContextType=0x0001)
...
class SMB2_Compression_Capabilities(Packet):
    ...
    fields_desc = [
        FieldLenField('CompressionAlgorithmCount', 0, fmt='<H', count_of='CompressionAlgorithms'),
...
bind_layers(SMB2_Negotiate_Context, SMB2_Netname_Negotiate_Context_ID, ContextType=0x0005)
...
class SMB2_Negotiate_Protocol_Response(_SMB2_Payload):
    ...
    fields_desc = [
        ...
        UTCTimeField('ServerTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
        UTCTimeField('ServerStartTime', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=1e7),
        FieldLenField('SecurityBlobOffset', None, fmt='<H', length_of='SecurityBlobPad', adjust=lambda pkt, x: x + 0x80),
        FieldLenField('SecurityBlobLength', None, fmt='<H', length_of='SecurityBlob'),
        XLEIntField('NegotiateContextOffset', 0),
        XStrLenField('SecurityBlobPad', '', length_from=lambda pkt: pkt.SecurityBlobOffset - 0x80),
        PacketLenField('SecurityBlob', None, GSSAPI_BLOB, length_from=lambda x: x.SecurityBlobLength),
        # Field only exists if Dialect is 0x0311
        # Each negotiate context must be 8-byte aligned
        ConditionalField(FieldListField('NegotiateContexts', [], ReversePadField(PacketField('Context', None, SMB2_Negotiate_Context), 8), count_from=lambda pkt: pkt.NegotiateCount), lambda x: x.DialectRevision == 0x0311),
    ]
...
class SMB2_Tree_Connect_Request(_SMB2_Payload, _NTLMPayloadPacket):
    ...
    fields_desc = [
        XLEShortField('StructureSize', 0x9),
        FlagsField('Flags', 0, -16, ['CLUSTER_RECONNECT', 'REDIRECT_TO_OWNER', 'EXTENSION_PRESENT']),
    ...
    fields_desc = [
        ...
        _NTLMPayloadField(
            ...
            [
                ...
                PacketLenField('Data', None, conf.raw_layer, length_from=lambda pkt: pkt.DataLen),
            ],
        ),
        StrLenField('pad', b'', length_from=lambda x: (x.Next - max(x.DataBufferOffset + x.DataLen, x.NameBufferOffset + x.NameLen)) if x.Next else 0),
    ]
    ...
    fields_desc = [
        ...
        _NTLMPayloadField(
            ...
            [
                ...
                _NextPacketListField('CreateContexts', [], SMB2_Create_Context, length_from=lambda pkt: pkt.CreateContextsLen),
            ],
        ),
    ]
...
class SMB2_Close_Request(_SMB2_Payload):
    ...
    fields_desc = [XLEShortField('StructureSize', 0x18), FlagsField('Flags', 0, -16, ['SMB2_CLOSE_FLAG_POSTQUERY_ATTRIB']), LEIntField('Reserved', 0), PacketField('FileId', SMB2_FILEID(), SMB2_FILEID)]
...
class SMB2_Read_Request(_SMB2_Payload, _NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLMPayloadField('Buffer', OFFSET, [StrLenField('ReadChannelInfo', b'', length_from=lambda pkt: pkt.ReadChannelInfoLen)]),
    ]
...
class SMB2_Read_Response(_SMB2_Payload, _NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLMPayloadField('Buffer', OFFSET, [StrLenField('Data', b'', length_from=lambda pkt: pkt.DataLen)]),
    ]
...
class SMB2_IOCTL_Request(_SMB2_Payload, _NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLMPayloadField(
            ...
            OFFSET,
            [
                _SMB2_IOCTL_Request_PacketLenField('Input', None, conf.raw_layer, length_from=lambda pkt: pkt.InputLen),
                _SMB2_IOCTL_Request_PacketLenField('Output', None, conf.raw_layer, length_from=lambda pkt: pkt.OutputLen),
            ],
        ),
    ]
    ...
    fields_desc = (
        ...
        + [
            _NTLMPayloadField(
                ...
                OFFSET,
                [
                    _SMB2_IOCTL_Response_PacketLenField('Input', None, conf.raw_layer, length_from=lambda pkt: pkt.InputLen),
                    _SMB2_IOCTL_Response_PacketLenField('Output', None, conf.raw_layer, length_from=lambda pkt: pkt.OutputLen),
                ],
            ),
        ]
    )
...
class FILE_GET_QUOTA_INFORMATION(Packet):
    fields_desc = [
        ...
        StrLenField('pad', b'', length_from=lambda x: ((x.NextEntryOffset - x.SidLength) if x.NextEntryOffset else 0)),
    ]
...
class SMB2_Query_Quota_Info(Packet):
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (PacketListField('SidBuffer', [], FILE_GET_QUOTA_INFORMATION, length_from=lambda x: x.SidListLength), lambda x: x.SidListLength),
                (StrLenField('SidBuffer', b'', length_from=lambda x: x.StartSidLength), lambda x: x.StartSidLength),
            ],
...
class SMB2_Query_Info_Request(_SMB2_Payload, _NTLMPayloadPacket):
    ...
    fields_desc = [
        ...
        _NTLMPayloadField(
            ...
            OFFSET,
            [
                PacketListField('Input', None, SMB2_Query_Quota_Info, length_from=lambda pkt: pkt.InputLen),
            ],
        ),
    ]
