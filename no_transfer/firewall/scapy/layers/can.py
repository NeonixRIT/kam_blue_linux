...
from scapy.compat import Tuple, Optional, Type, List, Union, Callable, IO, Any, cast, hex_bytes
...
class CAN(Packet):
    ...
    def dispatch_hook(
        ...
        **kargs,  # type: Any
        ...
        if _pkt:
            fdf_set = len(_pkt) > 5 and orb(_pkt[5]) & 0x04 and not orb(_pkt[5]) & 0xF8
            if fdf_set:
                return CANFD
            elif len(_pkt) > 16:
                return CANFD
...
class CANFD(CAN):
    ...
    fields_desc = [
        ...
        XBitField('identifier', 0, 29),
        FieldLenField('length', None, length_of='data', fmt='B'),
        FlagsField('fd_flags', 4, 8, ['bit_rate_switch', 'error_state_indicator', 'fd_frame']),
...
class SignalField(ScalingField):
    ...
    def addfield(self, pkt, s, val):
        ...
        if self._is_float_number():
            int_val = struct.unpack(self.fmt[0] + 'I', struct.pack(self.fmt, val))[0]  # type: int
...
class SignalHeader(CAN):
    ...
    fields_desc = [
        ...
        FlagsField('fd_flags', 0, 8, ['bit_rate_switch', 'error_state_indicator', 'fd_frame']),
        ShortField('reserved', 0),
    ]
