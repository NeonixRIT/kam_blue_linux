...
class EAPOL(Packet):
    ...
    name = 'EAPOL'
    fields_desc = [ByteEnumField('version', 1, eapol_versions), ByteEnumField('type', 0, eapol_types), LenField('len', None, 'H')]
...
eap_codes = {1: 'Request', 2: 'Response', 3: 'Success', 4: 'Failure', 5: 'Initiate', 6: 'Finish'}
...
class EAP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(FieldListField('desired_auth_types', [], ByteEnumField('auth_type', 0, eap_types), length_from=lambda pkt: pkt.len - 4), lambda pkt: pkt.code == EAP.RESPONSE and pkt.type == 3),
        ConditionalField(StrLenField('identity', '', length_from=lambda pkt: pkt.len - 5), lambda pkt: pkt.code == EAP.RESPONSE and hasattr(pkt, 'type') and pkt.type == 1),  # noqa: E501
        ConditionalField(StrLenField('message', '', length_from=lambda pkt: pkt.len - 5), lambda pkt: pkt.code == EAP.REQUEST and hasattr(pkt, 'type') and pkt.type == 1),  # noqa: E501
    ]
...
class EAP_MD5(EAP):
    ...
    fields_desc = [
        ...
        ByteField('id', 0),
        FieldLenField('len', None, fmt='H', length_of='optional_name', adjust=lambda p, x: x + 6 + (p.value_size or 0)),
...
class EAP_TLS(EAP):
    ...
    fields_desc = [
        ByteEnumField('code', 1, eap_codes),
        ByteField('id', 0),
        FieldLenField('len', None, fmt='H', length_of='tls_data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6),
...
class EAP_TTLS(EAP):
    ...
    fields_desc = [
        ByteEnumField('code', 1, eap_codes),
        ByteField('id', 0),
        FieldLenField('len', None, fmt='H', length_of='data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6),
...
_parameter_set_types = {1: 'Live Peer List', 2: 'Potential Peer List', 3: 'MACsec SAK Use', 4: 'Distributed SAK', 5: 'Distributed CAK', 6: 'KMD', 7: 'Announcement', 255: 'ICV Indicator'}
...
class MACsecSCI(Packet):
    ...
    name = 'SCI'
    fields_desc = [SourceMACField('system_identifier'), ShortField('port_identifier', 0)]
...
class MKABasicParamSet(Packet):
    ...
    fields_desc = [
        ...
        PadField(XStrLenField('cak_name', '', length_from=lambda pkt: (pkt.param_set_body_len - 28)), 4, padwith=b'\x00'),
    ]
...
class MKALivePeerListParamSet(MKAParamSet):
    ...
    fields_desc = [PadField(ByteEnumField('param_set_type', 1, _parameter_set_types), 2, padwith=b'\x00'), ShortField('param_set_body_len', 0), PacketListField('member_id_message_num', [], MKAPeerListTuple)]
...
class MKAPotentialPeerListParamSet(MKAParamSet):
    ...
    fields_desc = [PadField(ByteEnumField('param_set_type', 2, _parameter_set_types), 2, padwith=b'\x00'), ShortField('param_set_body_len', 0), PacketListField('member_id_message_num', [], MKAPeerListTuple)]
...
class MKADistributedSAKParamSet(MKAParamSet):
    ...
    fields_desc = [
        ...
        ConditionalField(XStrFixedLenField('macsec_cipher_suite', '', length=8), lambda pkt: pkt.param_set_body_len > 28),
        XStrFixedLenField('sak_aes_key_wrap', '', length=MKAParamSet.EAPOL_MKA_DEFAULT_KEY_WRAP_LEN),
    ]
...
class MKADistributedCAKParamSet(MKAParamSet):
    ...
    fields_desc = [
        PadField(ByteEnumField('param_set_type', 5, _parameter_set_types), 2, padwith=b'\x00'),
        ShortField('param_set_body_len', 0),
        XStrFixedLenField('cak_aes_key_wrap', '', length=MKAParamSet.EAPOL_MKA_DEFAULT_KEY_WRAP_LEN),
...
class MKAICVSet(MKAParamSet):
    ...
    name = 'ICV'
    fields_desc = [PadField(ByteEnumField('param_set_type', 255, _parameter_set_types), 2, padwith=b'\x00'), ShortField('param_set_body_len', 0), XStrFixedLenField('icv', '', length=MKAParamSet.MACSEC_DEFAULT_ICV_LEN)]
