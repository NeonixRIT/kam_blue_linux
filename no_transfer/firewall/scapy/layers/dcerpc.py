...
class DceRpc4(Packet):
    ...
    fields_desc = (
        [
            ByteEnumField('rpc_vers', 4, {4: '4 (connection-less)', 5: '5 (connection-oriented)'}),
...
class NL_AUTH_SIGNATURE(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (StrFixedLenField('Reserved2', b'', length=24), lambda pkt: pkt.SignatureAlgorithm == 0x0013),
            ],
        ...
        MultipleTypeField(
            [
                ...
                (
                    ...
                    lambda pkt: pkt.auth_type == 0x44 and pkt.parent and pkt.parent.ptype in [11, 12, 13, 14, 15],
                ),
                (
                    PacketLenField(
                        'auth_value',
                    ...
                    lambda pkt: pkt.auth_type == 0x44 and (not pkt.parent or pkt.parent.ptype not in [11, 12, 13, 14, 15]),
                ),
            ],
            PacketLenField(
                'auth_value',
...
class DceRpc5(Packet):
    ...
    fields_desc = (
        [
            ByteEnumField('rpc_vers', 5, {4: '4 (connection-less)', 5: '5 (connection-oriented)'}),
...
class DceRpc5Result(EPacket):
    ...
    fields_desc = [
        _EField(ShortEnumField('result', 0, ['acceptance', 'user_rejection', 'provider_rejection'])),
        _EField(
            ShortEnumField(
                'reason',
                0,
...
class DceRpc5Bind(_DceRpcPayload):
    ...
    fields_desc = [
        ...
        # p_cont_list_t
        _EField(FieldLenField('n_context_elem', None, count_of='context_elem', fmt='B')),
...
class _NDRVarField(object):
    ...
    def addfield(self, pkt, s, val):
        ...
        s = NDRAlign(Field('', 0, fmt=fmt), align=(4, 8)).addfield(
            pkt,
            s,
            val.actual_count is None and super(_NDRVarField, self).i2len(pkt, val.value) or val.actual_count,
        )
...
class NDRConformantArray(_NDRPacket):
    fields_desc = [
        ...
        PacketListField('value', [], conf.raw_layer, count_from=lambda pkt: pkt.max_count),
    ]
...
class _NDRConfField(object):
    ...
    def addfield(self, pkt, s, val):
        ...
        s = NDRAlign(Field('', 0, fmt=fmt), align=(4, 8)).addfield(
            pkt,
            s,
            val.max_count is None and super(_NDRConfField, self).i2len(pkt, value) or val.max_count,
        )
...
class DceRpcSession(DefaultSession):
    ...
    def on_packet_received(self, pkt):
        if DceRpc5 in pkt:
            return super(DceRpcSession, self).on_packet_received(self._process_dcerpc_packet(pkt))
