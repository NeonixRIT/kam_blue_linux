    ...
    fields_desc = [
        ByteEnumField(
            ...
            8,
            {
                ...
                4: 'conf_req',
                5: 'conf_resp',
                6: 'disconn_req',  # noqa: E501
                7: 'disconn_resp',
                8: 'echo_req',
                9: 'echo_resp',  # noqa: E501
                10: 'info_req',
                11: 'info_resp',
                18: 'conn_param_update_req',  # noqa: E501
...
class L2CAP_ConnReq(Packet):
    name = 'L2CAP Conn Req'
    fields_desc = [
        LEShortEnumField('psm', 0, {1: 'SDP', 3: 'RFCOMM', 5: 'telephony control'}),  # noqa: E501
        LEShortField('scid', 0),
    ]
...
class ATT_Find_Information_Response(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (PacketListField('handles', [], ATT_Handle), lambda pkt: pkt.format == 1),
                (PacketListField('handles', [], ATT_Handle_UUID128), lambda pkt: pkt.format == 2),
            ],
...
class ATT_Read_By_Type_Request_128bit(Packet):
    ...
    fields_desc = [XLEShortField('start', 0x0001), XLEShortField('end', 0xFFFF), XLELongField('uuid1', None), XLELongField('uuid2', None)]
...
class ATT_Read_By_Type_Request(Packet):
    name = 'Read By Type Request'
    fields_desc = [XLEShortField('start', 0x0001), XLEShortField('end', 0xFFFF), XLEShortField('uuid', None)]
...
class SM_Pairing_Request(Packet):
    ...
    fields_desc = [
        ByteEnumField('iocap', 3, {0: 'DisplayOnly', 1: 'DisplayYesNo', 2: 'KeyboardOnly', 3: 'NoInputNoOutput', 4: 'KeyboardDisplay'}),  # noqa: E501
...
class SM_Public_Key(Packet):
    name = 'Public Key'
    fields_desc = [
        StrFixedLenField('key_x', b'\x00' * 32, 32),
...
class SM_DHKey_Check(Packet):
    name = 'DHKey Check'
    fields_desc = [
        StrFixedLenField('dhkey_check', b'\x00' * 16, 16),
    ]
...
class EIR_Flags(EIR_Element):
    name = 'Flags'
    fields_desc = [FlagsField('flags', 0x2, 8, ['limited_disc_mode', 'general_disc_mode', 'br_edr_not_supported', 'simul_le_br_edr_ctrl', 'simul_le_br_edr_host'] + 3 * ['reserved'])]
...
class EIR_CompleteList16BitServiceUUIDs(EIR_Element):
    ...
    fields_desc = [
        ...
        FieldListField('svc_uuids', None, XLEShortField('uuid', 0), length_from=EIR_Element.length_from)
    ]
...
class EIR_Manufacturer_Specific_Data(EIR_Element):
    ...
    def default_payload_class(self, payload):
        for cls, check in six.iteritems(EIR_Manufacturer_Specific_Data.registered_magic_payloads):
            if check(payload):
                return cls
...
class HCI_Cmd_LE_Set_Scan_Parameters(Packet):
    ...
    fields_desc = [ByteEnumField('type', 1, {1: 'active'}), XLEShortField('interval', 16), XLEShortField('window', 16), ByteEnumField('atype', 0, {0: 'public'}), ByteEnumField('policy', 0, {0: 'all', 1: 'whitelist'})]
...
class HCI_Cmd_LE_Add_Device_To_White_List(Packet):
    ...
    fields_desc = [ByteEnumField('atype', 0, {0: 'public', 1: 'random'}), LEMACField('address', None)]
...
class HCI_Cmd_LE_Set_Advertising_Parameters(Packet):
    ...
    fields_desc = [
        LEShortField('interval_min', 0x0800),
...
class HCI_Cmd_LE_Set_Advertising_Data(Packet):
    ...
    fields_desc = [
        ...
        PadField(PacketListField('data', [], EIR_Hdr, length_from=lambda pkt: pkt.len), align=31, padwith=b'\0'),
    ]
...
class HCI_Event_Command_Status(Packet):
    name = 'Command Status'
    fields_desc = [
        ByteEnumField('status', 0, {0: 'pending'}),
        ByteField('number', 0),
