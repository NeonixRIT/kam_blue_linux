...
from scapy.layers.tls.basefields import _TLSVersionField, _tls_version, _TLSMACField, _TLSLengthField, _tls_type
...
class TLSInnerPlaintext(_GenericTLSSessionInheritance):
    ...
    fields_desc = [_TLSMsgListField('msg', []), ByteEnumField('type', None, _tls_type), XStrField('pad', '')]
...
class TLS13(_GenericTLSSessionInheritance):
    ...
    name = 'TLS 1.3'
    fields_desc = [ByteEnumField('type', 0x17, _tls_type), _TLSVersionField('version', 0x0303, _tls_version), _TLSLengthField('len', None), _TLSInnerPlaintextField('inner', TLSInnerPlaintext()), _TLSMACField('auth_tag', None)]
    ...
    def pre_dissect(self, s):
        ...
        self.type = orb(s[0])
        if isinstance(self.tls_session.rcs.cipher, Cipher_NULL) or self.type == 0x14:
