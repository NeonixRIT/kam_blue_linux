...
class _FFDHParamsMetaclass(type):
    def __new__(cls, ffdh_name, bases, dct):
        the_class = super(_FFDHParamsMetaclass, cls).__new__(cls, ffdh_name, bases, dct)
...
_tls_named_ffdh_groups = {256: 'ffdhe2048', 257: 'ffdhe3072', 258: 'ffdhe4096', 259: 'ffdhe6144', 260: 'ffdhe8192'}
    ...
    if group in _tls_named_ffdh_groups:
        ...
        if _tls_named_curves[group] in ['x25519', 'x448']:
            ...
            try:  # cryptography >= 2.5
                return ec.EllipticCurvePublicKey.from_encoded_point(curve, pubbytes)
            except AttributeError:
                pub_num = ec.EllipticCurvePublicNumbers.from_encoded_point(curve, pubbytes).public_numbers()
...
def _tls_named_groups_pubbytes(privkey):
    if isinstance(privkey, dh.DHPrivateKey):
    ...
    elif isinstance(privkey, (x25519.X25519PrivateKey, x448.X448PrivateKey)):
        # https://datatracker.ietf.org/doc/html/rfc8446#section-4.2.8.2
        pubkey = privkey.public_key()
        ...
        try:
            # cryptography >= 2.5
            return pubkey.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)
        except TypeError:
            # older versions
    ...
    if group in _tls_named_ffdh_groups:
        ...
        if group_name in ['x25519', 'x448']:
            if conf.crypto_valid_advanced:
                ...
                warning("Your cryptography version doesn't support " + group_name)
        else:
            curve = ec._CURVE_TYPES[_tls_named_curves[group]]()
