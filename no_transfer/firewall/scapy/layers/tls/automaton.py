...
class _TLSAutomaton(Automaton):
    ...
    def get_next_msg(self, socket_timeout=2, retry=2):
        ...
        while retry and (still_getting_len or len(self.remain_in) < grablen):
            ...
            final = False
            try:
                tmp, _, _ = select.select([self.socket], [], [], socket_timeout)
                if not tmp:
                    retry -= 1
                else:
        ...
        if byte0 == 0x17 and (self.cur_session.advertised_tls_version >= 0x0304 or self.cur_session.tls_version >= 0x0304):
            ...
            if isinstance(p, SSLv2) and not p.msg:
                p.msg = Raw('')
            if self.cur_session.tls_version is None or self.cur_session.tls_version < 0x0304:
    ...
    def raise_on_packet(self, pkt_cls, state, get_next_msg=True):
        ...
        if not self.buffer_in or (not isinstance(self.buffer_in[0], pkt_cls) and not (isinstance(self.buffer_in[0], TLSClientHello) and self.cur_session.advertised_tls_version == 0x0304)):
            return
    ...
    def add_record(self, is_sslv2=None, is_tls13=None, is_tls12=None):
        ...
        if is_sslv2 is None and is_tls13 is None and is_tls12 is None:
            v = self.cur_session.tls_version or self.cur_session.advertised_tls_version
        ...
        # be 0x0303
            ...
            self.buffer_out.append(TLS(version='TLS 1.2', tls_session=self.cur_session))
        else:
