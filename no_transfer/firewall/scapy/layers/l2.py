...
class Ether(Packet):
    name = 'Ethernet'
    fields_desc = [DestMACField('dst'), SourceMACField('src'), XShortEnumField('type', 0x9000, ETHER_TYPES)]
...
class LLC(Packet):
    name = 'LLC'
    fields_desc = [XByteField('dsap', 0x00), XByteField('ssap', 0x00), ByteField('ctrl', 0)]
...
class CookedLinux(Packet):
    ...
    fields_desc = [ShortEnumField('pkttype', 0, COOKED_LINUX_PACKET_TYPES), XShortField('lladdrtype', 512), ShortField('lladdrlen', 0), StrFixedLenField('src', b'', 8), XShortEnumField('proto', 0x800, ETHER_TYPES)]
...
class SNAP(Packet):
    name = 'SNAP'
    fields_desc = [OUIField('OUI', 0x000000), XShortEnumField('code', 0x000, ETHER_TYPES)]
...
class Dot1Q(Packet):
    name = '802.1Q'
    aliastypes = [Ether]
    fields_desc = [BitField('prio', 0, 3), BitField('id', 0, 1), BitField('vlan', 1, 12), XShortEnumField('type', 0x0000, ETHER_TYPES)]
...
class ARP(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (SourceMACField('hwsrc'), (lambda pkt: pkt.hwtype == 1 and pkt.hwlen == 6, lambda pkt, val: pkt.hwtype == 1 and (pkt.hwlen == 6 or (pkt.hwlen is None and (val is None or len(val) == 6 or valid_mac(val)))))),
            ],
            StrFixedLenField('hwsrc', None, length_from=lambda pkt: pkt.hwlen),
        ),
        MultipleTypeField(
            [
                (SourceIPField('psrc', 'pdst'), (lambda pkt: pkt.ptype == 0x0800 and pkt.plen == 4, lambda pkt, val: pkt.ptype == 0x0800 and (pkt.plen == 4 or (pkt.plen is None and (val is None or valid_net(val)))))),
                (SourceIP6Field('psrc', 'pdst'), (lambda pkt: pkt.ptype == 0x86DD and pkt.plen == 16, lambda pkt, val: pkt.ptype == 0x86DD and (pkt.plen == 16 or (pkt.plen is None and (val is None or valid_net6(val)))))),
            ],
            StrFixedLenField('psrc', None, length_from=lambda pkt: pkt.plen),
        ),
        MultipleTypeField(
            [
                (MACField('hwdst', ETHER_ANY), (lambda pkt: pkt.hwtype == 1 and pkt.hwlen == 6, lambda pkt, val: pkt.hwtype == 1 and (pkt.hwlen == 6 or (pkt.hwlen is None and (val is None or len(val) == 6 or valid_mac(val)))))),
            ],
            StrFixedLenField('hwdst', None, length_from=lambda pkt: pkt.hwlen),
        ),
        MultipleTypeField(
            [
                (IPField('pdst', '0.0.0.0'), (lambda pkt: pkt.ptype == 0x0800 and pkt.plen == 4, lambda pkt, val: pkt.ptype == 0x0800 and (pkt.plen == 4 or (pkt.plen is None and (val is None or valid_net(val)))))),
                (IP6Field('pdst', '::'), (lambda pkt: pkt.ptype == 0x86DD and pkt.plen == 16, lambda pkt, val: pkt.ptype == 0x86DD and (pkt.plen == 16 or (pkt.plen is None and (val is None or valid_net6(val)))))),
            ],
...
def l2_register_l3_arp(l2, l3):
    ...
    log_runtime.warning('Unable to guess L2 MAC address from an ARP packet with a ' 'non-IPv4 pdst. Provide it manually !')
    return None
...
class GRErouting(Packet):
    ...
    fields_desc = [
        ShortField('address_family', 0),
        ByteField('SRE_offset', 0),
        FieldLenField('SRE_len', None, 'routing_info', 'B'),
        StrLenField('routing_info', b'', length_from=lambda pkt: pkt.SRE_len),
    ]
...
LOOPBACK_TYPES = {0x2: 'IPv4', 0x7: 'OSI', 0x10: 'Appletalk', 0x17: 'Netware IPX/SPX', 0x18: 'IPv6', 0x1C: 'IPv6', 0x1E: 'IPv6'}
...
def arpcachepoison(
    ...
    if isinstance(addresses, str):
        ...
        couple_list = addresses
    p = [Ether(src=y) / ARP(op='who-has', psrc=x, pdst=targets, hwsrc=y, hwdst='ff:ff:ff:ff:ff:ff') for x, y in couple_list]
    try:
        while True:
...
def arp_mitm(
    ...
    srploop(
        [
            Ether(dst=mac1, src=target_mac) / ARP(op='who-has', psrc=ip2, pdst=ip1, hwsrc=target_mac, hwdst='ff:ff:ff:ff:ff:ff'),
            Ether(dst=mac2, src=target_mac) / ARP(op='who-has', psrc=ip1, pdst=ip2, hwsrc=target_mac, hwdst='ff:ff:ff:ff:ff:ff'),
        ],
    ...
    print('Restoring...')
    sendp(
        [Ether(dst=mac1, src=target_mac) / ARP(op='who-has', psrc=ip2, pdst=ip1, hwsrc=mac2, hwdst='ff:ff:ff:ff:ff:ff'), Ether(dst=mac2, src=target_mac) / ARP(op='who-has', psrc=ip1, pdst=ip2, hwsrc=mac1, hwdst='ff:ff:ff:ff:ff:ff')],
        iface=iface,
    )
...
def promiscping(net, timeout=2, fake_bcast='ff:ff:ff:ff:ff:fe', **kargs):
    ...
    ans, unans = srp(Ether(dst=fake_bcast) / ARP(pdst=net), filter='arp and arp[7] = 2', timeout=timeout, iface_hint=net, **kargs)  # noqa: E501
