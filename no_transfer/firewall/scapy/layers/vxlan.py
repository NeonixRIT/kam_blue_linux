...
from scapy.fields import FlagsField, XByteField, ThreeBytesField, ConditionalField, ShortField, ByteEnumField, X3BytesField
...
class VXLAN(Packet):
    ...
    fields_desc = [
        FlagsField('flags', 0x8, 8, ['OAM', 'R', 'NextProtocol', 'Instance', 'V1', 'V2', 'R', 'G']),
        ConditionalField(
            ShortField('reserved0', 0),
            lambda pkt: pkt.flags.NextProtocol,
        ),
        ConditionalField(
            ByteEnumField('NextProtocol', 0, {0: 'NotDefined', 1: 'IPv4', 2: 'IPv6', 3: 'Ethernet', 4: 'NSH'}),
            lambda pkt: pkt.flags.NextProtocol,
        ),
        ConditionalField(
