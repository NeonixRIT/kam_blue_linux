...
class SMB_Header(Packet):
    ...
    fields_desc = [
        ...
        FlagsField('Flags', 0x18, 8, ['LOCK_AND_READ_OK', 'BUF_AVAIL', 'res', 'CASE_INSENSITIVE', 'CANONICALIZED_PATHS', 'OPLOCK', 'OPBATCH', 'REPLY']),
        FlagsField(
            'Flags2',
            0x0000,
            -16,
            ['LONG_NAMES', 'EAS', 'SMB_SECURITY_SIGNATURE', 'COMPRESSED', 'SMB_SECURITY_SIGNATURE_REQUIRED', 'res', 'IS_LONG_NAME', 'res', 'res', 'res', 'REPARSE_PATH', 'EXTENDED_SECURITY', 'DFS', 'PAGING_IO', 'NT_STATUS', 'UNICODE'],
        ),
...
class SMB_Dialect(Packet):
    name = 'SMB Dialect'
    fields_desc = [ByteField('BufferFormat', 0x02), StrNullField('DialectString', 'NT LM 0.12')]
...
class SMBNegotiate_Response_NoSecurity(_SMBNegotiate_Response):
    ...
    fields_desc = [
        ...
        FlagsField('SecurityMode', 0x03, 8, ['USER_SECURITY', 'ENCRYPT_PASSWORDS', 'SECURITY_SIGNATURES_ENABLED', 'SECURITY_SIGNATURES_REQUIRED']),
        ...
        LEFieldLenField('ByteCount', None, length_of='DomainName', adjust=lambda pkt, x: x + len(pkt.Challenge)),
        XStrLenField(
            'Challenge',
            b'',  # aka EncryptionKey
...
class SMBNegotiate_Response_Extended_Security(_SMBNegotiate_Response):
    ...
    fields_desc = SMBNegotiate_Response_NoSecurity.fields_desc[:12] + [
        LEFieldLenField('ByteCount', None, length_of='SecurityBlob', adjust=lambda _, x: x + 16),
        SMBNegotiate_Response_NoSecurity.fields_desc[13],
        UUIDField('GUID', None, uuid_fmt=UUIDField.FORMAT_LE),
        PacketLenField('SecurityBlob', None, GSSAPI_BLOB, length_from=lambda x: x.ByteCount - 16),
    ]


bind_top_down(SMB_Header, SMBNegotiate_Response_Extended_Security, Command=0x72, Flags=0x80, Flags2=0x800)
...
class SMBNegotiate_Response_Security(_SMBNegotiate_Response):
    ...
    WordCount = 0x11
    fields_desc = SMBNegotiate_Response_NoSecurity.fields_desc[:12] + [
        LEFieldLenField('ByteCount', None, length_of='DomainName', adjust=lambda pkt, x: x + 2 + _len(pkt, 'Challenge') + _len(pkt, 'ServerName')),
        XStrLenField(
            'Challenge',
            b'',  # aka EncryptionKey
            length_from=lambda pkt: pkt.ChallengeLength,
...
class SMBSession_Setup_AndX_Request(Packet):
    ...
    fields_desc = [
        ...
        XStrLenField('UnicodePassword', 'Pass', length_from=lambda x: x.UnicodePasswordLength),
...
class SMBTree_Connect_AndX(Packet):
    ...
    fields_desc = SMBSession_Setup_AndX_Request.fields_desc[:4] + [
        FlagsField('Flags', '', -16, ['DISCONNECT_TID', 'r2', 'EXTENDED_SIGNATURES', 'EXTENDED_RESPONSE']),
...
bind_top_down(SMB_Header, SMBSession_Setup_AndX_Request_Extended_Security, Command=0x73, Flags2=0x800)
...
bind_top_down(SMB_Header, SMBSession_Setup_AndX_Response_Extended_Security, Command=0x73, Flags=0x80, Flags2=0x800)
...
bind_top_down(SMB_Header, SMBNetlogon_Protocol_Response_Header, Command=0x25)
...
class SMBNetlogon_Protocol_Response_Tail_SAM(Packet):
    ...
    fields_desc = [
        ByteEnumField('Command', 0x17, {0x12: 'SAM logon request', 0x17: 'SAM Active directory Response'}),
        ByteField('unused', 0),
        ShortField('Data1', 0),
...
class SMBNetlogon_Protocol_Response_Tail_LM20(Packet):
    ...
    fields_desc = [
        ByteEnumField('Command', 0x06, {0x06: 'LM 2.0 Response to logon request'}),  # noqa: E501
        ByteField('unused', 0),
