...
class ZigbeeNWK(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField('frametype', 0, 2, {0: 'data', 1: 'command', 3: 'Inter-PAN'}),
...
class ZigbeeNWKCommandPayload(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(BitField('res1', 0, 1), lambda pkt: pkt.cmd_identifier in [1, 2]),
        ConditionalField(BitField('multicast', 0, 1), lambda pkt: pkt.cmd_identifier in [1, 2]),
        ...
        ConditionalField(ByteField('route_request_identifier', 0), lambda pkt: pkt.cmd_identifier in [1, 2]),  # noqa: E501
        ...
        ConditionalField(XLEShortField('destination_address', 0x0000), lambda pkt: pkt.cmd_identifier in [1, 3]),
        # Path cost (1 octet)
        ConditionalField(ByteField('path_cost', 0), lambda pkt: pkt.cmd_identifier in [1, 2]),  # noqa: E501
        # Destination IEEE Address (0/8 octets), only present when dest_addr_bit has a value of 1  # noqa: E501
        ConditionalField(
            dot15d4AddressField('ext_dst', 0, adjust=lambda pkt, x: 8),  # noqa: E501
            lambda pkt: (pkt.cmd_identifier == 1 and pkt.dest_addr_bit == 1),
        ),  # noqa: E501
        # Originator IEEE address (0/8 octets)
        ConditionalField(
            dot15d4AddressField('originator_addr', 0, adjust=lambda pkt, x: 8),  # noqa: E501
            lambda pkt: (pkt.cmd_identifier == 2 and pkt.originator_addr_bit == 1),
        ),  # noqa: E501
        # Responder IEEE address (0/8 octets)
        ConditionalField(
            dot15d4AddressField('responder_addr', 0, adjust=lambda pkt, x: 8),  # noqa: E501
        ...
        ConditionalField(dot15d4AddressField('epid', 0, adjust=lambda pkt, x: 8), lambda pkt: pkt.cmd_identifier in [9, 10]),
        ...
        ConditionalField(
            FieldListField(
                ...
                [],
                XLEShortField('', 0x0000),  # noqa: E501
        ...
        # New PAN ID (2 octets)
        ConditionalField(XLEShortField('new_PAN_ID', 0x0000), lambda pkt: (pkt.cmd_identifier == 10 and pkt.update_command_identifier == 0)),  # noqa: E501
        ...
        ConditionalField(ByteField('ed_conf', 0), lambda pkt: pkt.cmd_identifier == 11),
        # - End Device Timeout Response Command - #
        # Status (1 octet)
        ConditionalField(ByteEnumField('status', 0, {0: 'Success', 1: 'Incorrect Value'}), lambda pkt: pkt.cmd_identifier == 12),
        # Parent Information (1 octet)
        ConditionalField(BitField('res6', 0, 6), lambda pkt: pkt.cmd_identifier == 12),
        ConditionalField(BitField('ed_timeout_req_keepalive', 0, 1), lambda pkt: pkt.cmd_identifier == 12),
        ConditionalField(BitField('mac_data_poll_keepalive', 0, 1), lambda pkt: pkt.cmd_identifier == 12),
        # StrField("data", ""),
    ]
...
class ZigbeeSecurityHeader(Packet):
    ...
    fields_desc = [
        ...
        # Key identifier
        BitEnumField('key_type', 1, 2, {0: 'data_key', 1: 'network_key', 2: 'key_transport_key', 3: 'key_load_key'}),
        # Security level (3 bits)
        BitEnumField('nwk_seclevel', 0, 3, {0: 'None', 1: 'MIC-32', 2: 'MIC-64', 3: 'MIC-128', 4: 'ENC', 5: 'ENC-MIC-32', 6: 'ENC-MIC-64', 7: 'ENC-MIC-128'}),
...
class ZigbeeAppDataPayload(Packet):
    ...
    fields_desc = [
        ...
        FlagsField('frame_control', 2, 4, ['ack_format', 'security', 'ack_req', 'extended_hdr']),
        ...
        ConditionalField(ByteField('dst_endpoint', 10), lambda pkt: ((pkt.aps_frametype == 0 and pkt.delivery_mode in [0, 2]) or (pkt.aps_frametype == 2 and not pkt.frame_control.ack_format))),
        # Group address (0/2 octets)
        ConditionalField(XLEShortField('group_addr', 0x0000), lambda pkt: (pkt.aps_frametype == 0 and pkt.delivery_mode == 3)),
        # Cluster identifier (0/2 octets)
        ConditionalField(
            # unsigned short (little-endian)
            XLEShortField('cluster', 0x0000),
            lambda pkt: ((pkt.aps_frametype == 0) or (pkt.aps_frametype == 2 and not pkt.frame_control.ack_format)),
        ),
        # Profile identifier (0/2 octets)
        ConditionalField(EnumField('profile', 0, _aps_profile_identifiers, fmt='<H'), lambda pkt: ((pkt.aps_frametype == 0) or (pkt.aps_frametype == 2 and not pkt.frame_control.ack_format))),
        # Source endpoint (0/1 octets)
        ConditionalField(ByteField('src_endpoint', 10), lambda pkt: ((pkt.aps_frametype == 0) or (pkt.aps_frametype == 2 and not pkt.frame_control.ack_format))),
        # APS counter (1 octet)
        ByteField('counter', 0),
        ...
        ConditionalField(ByteEnumField('fragmentation', 0, {0: 'none', 1: 'first_block', 2: 'middle_block'}), lambda pkt: (pkt.aps_frametype in [0, 2] and pkt.frame_control.extended_hdr)),
        ConditionalField(ByteField('block_number', 0), lambda pkt: (pkt.aps_frametype in [0, 2] and pkt.fragmentation in [1, 2])),
        ConditionalField(ByteField('ack_bitfield', 0), lambda pkt: (pkt.aps_frametype == 2 and pkt.fragmentation in [1, 2])),
...
class ZigbeeAppCommandPayload(Packet):
    ...
    fields_desc = [
        ...
        # SKKE Commands
        ConditionalField(dot15d4AddressField('initiator', 0, adjust=lambda pkt, x: 8), lambda pkt: pkt.cmd_identifier in [1, 2, 3, 4]),
        ConditionalField(dot15d4AddressField('responder', 0, adjust=lambda pkt, x: 8), lambda pkt: pkt.cmd_identifier in [1, 2, 3, 4]),
        ConditionalField(StrFixedLenField('data', 0, length=16), lambda pkt: pkt.cmd_identifier in [1, 2, 3, 4]),
        # Confirm-key command
        ConditionalField(ByteEnumField('status', 0, _ApsStatusValues), lambda pkt: pkt.cmd_identifier == 16),
        # Common fields
        ConditionalField(ByteEnumField('key_type', 0, _TransportKeyKeyTypes), lambda pkt: pkt.cmd_identifier in [5, 8, 15, 16]),
        ConditionalField(dot15d4AddressField('address', 0, adjust=lambda pkt, x: 8), lambda pkt: pkt.cmd_identifier in [6, 7, 15, 16]),
        # Transport-key Command
        ConditionalField(StrFixedLenField('key', None, 16), lambda pkt: pkt.cmd_identifier == 5),
        ConditionalField(ByteField('key_seqnum', 0), lambda pkt: (pkt.cmd_identifier == 5 and pkt.key_type in [0x01, 0x05])),
        ConditionalField(dot15d4AddressField('dest_addr', 0, adjust=lambda pkt, x: 8), lambda pkt: ((pkt.cmd_identifier == 5 and pkt.key_type not in [0x02, 0x03]) or pkt.cmd_identifier == 14)),
        ConditionalField(dot15d4AddressField('src_addr', 0, adjust=lambda pkt, x: 8), lambda pkt: (pkt.cmd_identifier == 5 and pkt.key_type not in [0x02, 0x03])),
        ConditionalField(dot15d4AddressField('partner_addr', 0, adjust=lambda pkt, x: 8), lambda pkt: ((pkt.cmd_identifier == 5 and pkt.key_type in [0x02, 0x03]) or (pkt.cmd_identifier == 8 and pkt.key_type == 0x02))),
        ConditionalField(ByteField('initiator_flag', 0), lambda pkt: (pkt.cmd_identifier == 5 and pkt.key_type in [0x02, 0x03])),
        # Update-Device Command
        ConditionalField(XLEShortField('short_address', 0), lambda pkt: pkt.cmd_identifier == 6),
        ConditionalField(ByteField('update_status', 0), lambda pkt: pkt.cmd_identifier == 6),
        # Switch-Key Command
        ConditionalField(StrFixedLenField('seqnum', None, 8), lambda pkt: pkt.cmd_identifier == 9),
        # Un-implemented: 10-13 (+?)
        ConditionalField(StrField('unimplemented', ''), lambda pkt: (pkt.cmd_identifier >= 10 and pkt.cmd_identifier <= 13)),
        # Tunnel Command
        ConditionalField(FlagsField('frame_control', 2, 4, ['ack_format', 'security', 'ack_req', 'extended_hdr']), lambda pkt: pkt.cmd_identifier == 14),
        ConditionalField(BitEnumField('delivery_mode', 0, 2, {0: 'unicast', 1: 'indirect', 2: 'broadcast', 3: 'group_addressing'}), lambda pkt: pkt.cmd_identifier == 14),
        ConditionalField(BitEnumField('aps_frametype', 1, 2, {0: 'data', 1: 'command', 2: 'ack'}), lambda pkt: pkt.cmd_identifier == 14),
        ConditionalField(ByteField('counter', 0), lambda pkt: pkt.cmd_identifier == 14),
        # Verify-Key Command
        ConditionalField(StrFixedLenField('key_hash', None, 16), lambda pkt: pkt.cmd_identifier == 15),
    ]
...
class ZCLReadAttributeStatusRecord(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteEnumField('attribute_data_type', 0, _zcl_attribute_data_types), lambda pkt: pkt.status == 0x00),
        ConditionalField(_DiscreteString('attribute_value', ''), lambda pkt: pkt.status == 0x00),
    ]
...
class ZCLWriteAttributeStatusRecord(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(XLEShortField('attribute_identifier', 0), lambda pkt: pkt.status != 0x00),
    ]
...
class ZCLConfigureReportingRecord(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteEnumField('attribute_data_type', 0, _zcl_attribute_data_types), lambda pkt: pkt.attribute_direction == 0x00),
        # Minimum Reporting Interval (0/2 octets)
        ConditionalField(XLEShortField('min_reporting_interval', 0), lambda pkt: pkt.attribute_direction == 0x00),
        # Maximum Reporting Interval (0/2 octets)
        ConditionalField(XLEShortField('max_reporting_interval', 0), lambda pkt: pkt.attribute_direction == 0x00),
        # Reportable Change (variable)
        ConditionalField(_DiscreteString('reportable_change', ''), lambda pkt: pkt.attribute_direction == 0x00),
        # Timeout Period (0/2 octets)
        ConditionalField(XLEShortField('timeout_period', 0), lambda pkt: pkt.attribute_direction == 0x01),
    ]
...
class ZCLConfigureReportingResponseRecord(Packet):
    ...
    fields_desc = [
        ...
        ByteEnumField('status', 0, _zcl_enumerated_status_values),
        # Direction (0/1 octet)
        ConditionalField(ByteField('attribute_direction', 0), lambda pkt: pkt.status != 0x00),
        # Attribute Identifier (0/2 octets)
        ConditionalField(XLEShortField('attribute_identifier', 0), lambda pkt: pkt.status != 0x00),
    ]
...
class ZigbeeClusterLibrary(Packet):
    ...
    def guess_payload_class(self, payload):
        if self.zcl_frametype == 0x00:
            ...
            if self.command_identifier in {0x00, 0x01, 0x02, 0x04, 0x06, 0x07, 0x0A, 0x0B}:
                # done in bind_layers
                pass
...
bind_layers(ZigbeeClusterLibrary, ZCLGeneralReadAttributes, zcl_frametype=0x00, command_identifier=0x00)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralReadAttributesResponse, zcl_frametype=0x00, command_identifier=0x01)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralWriteAttributes, zcl_frametype=0x00, command_identifier=0x02)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralWriteAttributesResponse, zcl_frametype=0x00, command_identifier=0x04)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralConfigureReporting, zcl_frametype=0x00, command_identifier=0x06)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralConfigureReportingResponse, zcl_frametype=0x00, command_identifier=0x07)
bind_layers(ZigbeeClusterLibrary, ZCLGeneralReportAttributes, zcl_frametype=0x00, command_identifier=0x0A)
