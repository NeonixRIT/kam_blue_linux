...
class SCTP(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ShortEnumField('sport', 0, SCTP_SERVICES),
...
class SCTPChunkParamHeartbeatInfo(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 1, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='data', adjust=lambda pkt, x: x + 4),
        PadField(StrLenField('data', '', length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]


class SCTPChunkParamIPv4Addr(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 5, sctpchunkparamtypes),
        ShortField('len', 8),
...
class SCTPChunkParamStateCookie(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 7, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='cookie', adjust=lambda pkt, x: x + 4),
        PadField(StrLenField('cookie', '', length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]


class SCTPChunkParamUnrocognizedParam(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 8, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='param', adjust=lambda pkt, x: x + 4),
        PadField(StrLenField('param', '', length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]


class SCTPChunkParamCookiePreservative(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 9, sctpchunkparamtypes),
        ShortField('len', 8),
...
class SCTPChunkParamHostname(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 11, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='hostname', adjust=lambda pkt, x: x + 4),
        PadField(StrLenField('hostname', '', length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]


class SCTPChunkParamSupportedAddrTypes(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 12, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='addr_type_list', adjust=lambda pkt, x: x + 4),
        PadField(
            FieldListField(
                'addr_type_list',
                ['IPv4'],
...
class SCTPChunkParamECNCapable(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0x8000, sctpchunkparamtypes),
        ShortField('len', 4),
    ]
...
class SCTPChunkParamRandom(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0x8002, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='random', adjust=lambda pkt, x: x + 4),
        PadField(StrLenField('random', RandBin(32), length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]


class SCTPChunkParamChunkList(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0x8003, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='chunk_list', adjust=lambda pkt, x: x + 4),
        PadField(
            FieldListField(
                'chunk_list',
                None,
...
class SCTPChunkParamRequestedHMACFunctions(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0x8004, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='HMAC_functions_list', adjust=lambda pkt, x: x + 4),
        PadField(
            FieldListField(
                'HMAC_functions_list',
                ['SHA-1'],
...
class SCTPChunkParamSupportedExtensions(_SCTPChunkParam, Packet):
    fields_desc = [
        ...
        FieldLenField('len', None, length_of='supported_extensions', adjust=lambda pkt, x: x + 4),
        PadField(
            FieldListField(
                'supported_extensions',
                ['authentication', 'address-configuration', 'address-configuration-ack'],
                ByteEnumField(
...
class SCTPChunkParamFwdTSN(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0xC000, sctpchunkparamtypes),
        ShortField('len', 4),
    ]
...
class SCTPChunkParamAddIPAddr(_SCTPChunkParam, Packet):
    fields_desc = [
        ...
        FieldLenField('len', None, length_of='addr', adjust=lambda pkt, x: x + 12),
        XIntField('correlation_id', None),
        ShortEnumField('addr_type', 5, sctpchunkparamtypes),
        FieldLenField('addr_len', None, length_of='addr', adjust=lambda pkt, x: x + 4),
        MultipleTypeField(
            [
                (IPField('addr', '127.0.0.1'), lambda p: p.addr_type == 5),
...
class SCTPChunkParamErrorIndication(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0xC003, sctpchunkparamtypes),
        FieldLenField('len', None, length_of='error_causes', adjust=lambda pkt, x: x + 8),
        XIntField('correlation_id', None),
        PadField(StrLenField('error_causes', '', length_from=lambda pkt: pkt.len - 4), 4, padwith=b'\x00'),
    ]
...
class SCTPChunkParamSuccessIndication(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0xC005, sctpchunkparamtypes),
        ShortField('len', 8),
        XIntField('correlation_id', None),
    ]
...
class SCTPChunkParamAdaptationLayer(_SCTPChunkParam, Packet):
    fields_desc = [
        ShortEnumField('type', 0xC006, sctpchunkparamtypes),
        ShortField('len', 8),
...
class SCTPChunkData(_SCTPChunkGuessPayload, Packet):
    ...
    fields_desc = [
        ...
        PadField(
            StrLenField('data', None, length_from=lambda pkt: pkt.len - 16),  # noqa: E501
            4,
            padwith=b'\x00',
        ),
    ]
...
class SCTPChunkInit(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 1, sctpchunktypes),
...
class SCTPChunkHeartbeatReq(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 4, sctpchunktypes),
        XByteField('flags', None),
...
class SCTPChunkShutdown(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 7, sctpchunktypes),
        XByteField('flags', None),
        ShortField('len', 8),
        XIntField('cumul_tsn_ack', None),
    ]
...
class SCTPChunkShutdownAck(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 8, sctpchunktypes),
        XByteField('flags', None),
        ShortField('len', 4),
    ]
...
class SCTPChunkError(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 9, sctpchunktypes),
        XByteField('flags', None),
        FieldLenField('len', None, length_of='error_causes', adjust=lambda pkt, x: x + 4),  # noqa: E501
        PadField(
            StrLenField('error_causes', '', length_from=lambda pkt: pkt.len - 4),  # noqa: E501
...
class SCTPChunkCookieEcho(SCTPChunkError):
    fields_desc = [
        ByteEnumField('type', 10, sctpchunktypes),
        XByteField('flags', None),
        FieldLenField('len', None, length_of='cookie', adjust=lambda pkt, x: x + 4),  # noqa: E501
        PadField(
            StrLenField('cookie', '', length_from=lambda pkt: pkt.len - 4),  # noqa: E501
            4,
            padwith=b'\x00',
        ),
    ]
...
class SCTPChunkCookieAck(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 11, sctpchunktypes),
        XByteField('flags', None),
        ShortField('len', 4),
    ]
...
class SCTPChunkShutdownComplete(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 14, sctpchunktypes),
        BitField('reserved', None, 7),
        BitField('TCB', 0, 1),
        ShortField('len', 4),
    ]
...
class SCTPChunkAuthentication(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 15, sctpchunktypes),
        XByteField('flags', None),
        FieldLenField('len', None, length_of='HMAC', adjust=lambda pkt, x: x + 8),
        ShortField('shared_key_id', None),
        ShortField('HMAC_function', None),
        PadField(
            StrLenField('HMAC', '', length_from=lambda pkt: pkt.len - 8),  # noqa: E501
            4,
            padwith=b'\x00',
        ),
    ]
...
class SCTPChunkAddressConf(_SCTPChunkGuessPayload, Packet):
    fields_desc = [
        ByteEnumField('type', 0xC1, sctpchunktypes),
        XByteField('flags', None),
