...
from scapy.layers.inet import IP, IPTools, TCP, TCPerror, TracerouteResult, UDP, UDPerror
...
from scapy.utils6 import in6_getnsma, in6_getnsmac, in6_isaddr6to4, in6_isaddrllallnodes, in6_isaddrllallservers, in6_isaddrTeredo, in6_isllsnmaddr, in6_ismaddr, Net6, teredoAddrExtractInfo
...
ipv6nhcls = {0: 'IPv6ExtHdrHopByHop', 4: 'IP', 6: 'TCP', 17: 'UDP', 43: 'IPv6ExtHdrRouting', 44: 'IPv6ExtHdrFragment', 50: 'ESP', 51: 'AH', 58: 'ICMPv6Unknown', 59: 'Raw', 60: 'IPv6ExtHdrDestOpt'}
        ...
        if conf.checkIPaddr:
            ...
            if in6_ismaddr(other.dst):
                if in6_ismaddr(self.dst):
                    if (od == sd) or (in6_isaddrllallnodes(self.dst) and in6_isaddrllallservers(other.dst)):  # noqa: E501
...
class IPerror6(IPv6):
    ...
    def answers(self, other):
        ...
        if isinstance(self.underlayer, _ICMPv6) and self.underlayer.type < 128:
            ...
            if (
                (ss == os and sd == od)  # < Basic case
                or (ss == os and request_has_rh)
            ):
                # ^ Request has a RH : don't check dst address

                # Let's deal with possible MSS Clamping
                if isinstance(selfup, TCP) and isinstance(otherup, TCP) and selfup.options != otherup.options:  # seems clamped
...
class PseudoIPv6(Packet):  # IPv6 Pseudo-header for checksum computation
    ...
    fields_desc = [IP6Field('src', '::'), IP6Field('dst', '::'), IntField('uplen', None), BitField('zero', 0, 24), ByteField('nh', 0)]
...
_hbhopts = {0x00: 'Pad1', 0x01: 'PadN', 0x04: 'Tunnel Encapsulation Limit', 0x05: 'Router Alert', 0x06: 'Quick-Start', 0xC2: 'Jumbo Payload', 0xC9: 'Home Address Option'}
...
class _OTypeField(ByteEnumField):
    ...
    pol = {0x00: '00: skip', 0x40: '01: discard', 0x80: '10: discard+ICMP', 0xC0: '11: discard+ICMP not mcast'}

    enroutechange = {0x00: "0: Don't change en-route", 0x20: '1: May change en-route'}
...
class HBHOptUnknown(Packet):  # IPv6 Hop-By-Hop Option
    ...
    fields_desc = [_OTypeField('otype', 0x01, _hbhopts), FieldLenField('optlen', None, length_of='optdata', fmt='B'), StrLenField('optdata', '', length_from=lambda pkt: pkt.optlen)]
...
class RouterAlert(Packet):  # RFC 2711 - IPv6 Hop-By-Hop Option
    name = 'Router Alert'
    fields_desc = [
        _OTypeField('otype', 0x05, _hbhopts),
        ByteField('optlen', 2),
        ShortEnumField(
            'value',
            None,
...
class Jumbo(Packet):  # IPv6 Hop-By-Hop Option
    name = 'Jumbo Payload'
    fields_desc = [_OTypeField('otype', 0xC2, _hbhopts), ByteField('optlen', 4), IntField('jumboplen', None)]
...
class HAO(Packet):  # IPv6 Destination Options Header Option
    ...
    fields_desc = [_OTypeField('otype', 0xC9, _hbhopts), ByteField('optlen', 16), IP6Field('hoa', '::')]
...
_hbhoptcls = {0x00: Pad1, 0x01: PadN, 0x05: RouterAlert, 0xC2: Jumbo, 0xC9: HAO}
...
class IPv6ExtHdrHopByHop(_IPv6ExtHdr):
    ...
    fields_desc = [
        ByteEnumField('nh', 59, ipv6nh),
        FieldLenField('len', None, length_of='options', fmt='B', adjust=lambda pkt, x: (x + 2 + 7) // 8 - 1),
        _PhantomAutoPadField('autopad', 1),  # autopad activated by default  # noqa: E501
        _OptionsField('options', [], HBHOptUnknown, 2, length_from=lambda pkt: (8 * (pkt.len + 1)) - 2),
    ]  # noqa: E501
...
class IPv6ExtHdrRouting(_IPv6ExtHdr):
    ...
    fields_desc = [
        ByteEnumField('nh', 59, ipv6nh),
        FieldLenField('len', None, count_of='addresses', fmt='B', adjust=lambda pkt, x: 2 * x),  # in 8 bytes blocks  # noqa: E501
        ByteField('type', 0),
        ByteField('segleft', None),
        BitField('reserved', 0, 32),  # There is meaning in this field ...  # noqa: E501
        IP6ListField('addresses', [], length_from=lambda pkt: 8 * pkt.len),
    ]
...
class IPv6ExtHdrSegmentRoutingTLV(Packet):
    ...
    # RFC 8754 sect 2.1
    fields_desc = [ByteEnumField('type', None, _segment_routing_header_tlvs), ByteField('len', 0), StrLenField('value', '', length_from=lambda pkt: pkt.len)]
...
class IPv6ExtHdrSegmentRoutingTLVIngressNode(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [ByteEnumField('type', 1, _segment_routing_header_tlvs), ByteField('len', 18), ByteField('reserved', 0), ByteField('flags', 0), IP6Field('ingress_node', '::1')]
...
class IPv6ExtHdrSegmentRoutingTLVEgressNode(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [ByteEnumField('type', 2, _segment_routing_header_tlvs), ByteField('len', 18), ByteField('reserved', 0), ByteField('flags', 0), IP6Field('egress_node', '::1')]
...
class IPv6ExtHdrSegmentRoutingTLVPad1(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [ByteEnumField('type', 0, _segment_routing_header_tlvs), FieldLenField('len', None, length_of='padding', fmt='B'), StrLenField('padding', b'\x00', length_from=lambda pkt: pkt.len)]  # noqa: E501
...
class IPv6ExtHdrSegmentRoutingTLVPadN(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [ByteEnumField('type', 4, _segment_routing_header_tlvs), FieldLenField('len', None, length_of='padding', fmt='B'), StrLenField('padding', b'\x00', length_from=lambda pkt: pkt.len)]  # noqa: E501
...
class IPv6ExtHdrSegmentRoutingTLVHMAC(IPv6ExtHdrSegmentRoutingTLV):
    ...
    # RFC8754 sect 2.1.2
    fields_desc = [
        ByteEnumField('type', 5, _segment_routing_header_tlvs),
        FieldLenField('len', None, length_of='hmac', adjust=lambda _, x: x + 48),
        BitField('D', 0, 1),
        BitField('reserved', 0, 15),
        IntField('hmackeyid', 0),
        StrLenField('hmac', '', length_from=lambda pkt: pkt.len - 48),
    ]
...
class IPv6ExtHdrSegmentRouting(_IPv6ExtHdr):
    ...
    fields_desc = [
        ...
        ShortField('tag', 0),
        IP6ListField('addresses', ['::1'], count_from=lambda pkt: (pkt.lastentry + 1)),
        PacketListField('tlv_objects', [], IPv6ExtHdrSegmentRoutingTLV, length_from=lambda pkt: 8 * pkt.len - 16 * (pkt.lastentry + 1)),
    ]
...
class IPv6ExtHdrFragment(_IPv6ExtHdr):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), BitField('res1', 0, 8), BitField('offset', 0, 13), BitField('res2', 0, 2), BitField('m', 0, 1), IntField('id', None)]
...
class ICMPv6Unknown(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 1, icmp6types), ByteField('code', 0), XShortField('cksum', None), StrField('msgbody', '')]
...
class ICMPv6DestUnreach(_ICMPv6Error):
    ...
    fields_desc = [
        ByteEnumField('type', 1, icmp6types),
        ByteEnumField(
            'code',
            0,
            {
...
class ICMPv6PacketTooBig(_ICMPv6Error):
    ...
    fields_desc = [ByteEnumField('type', 2, icmp6types), ByteField('code', 0), XShortField('cksum', None), IntField('mtu', 1280)]


class ICMPv6TimeExceeded(_ICMPv6Error):
    name = 'ICMPv6 Time Exceeded'
    fields_desc = [
        ByteEnumField('type', 3, icmp6types),
        ByteEnumField(
            'code',
            0,
            {
...
class ICMPv6ParamProblem(_ICMPv6Error):
    ...
    fields_desc = [
        ByteEnumField('type', 4, icmp6types),
        ByteEnumField('code', 0, {0: 'erroneous header field encountered', 1: 'unrecognized Next Header type encountered', 2: 'unrecognized IPv6 option encountered', 3: 'first fragment has incomplete header chain'}),
        XShortField('cksum', None),
        IntField('ptr', 6),
    ]
...
class ICMPv6EchoRequest(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 128, icmp6types), ByteField('code', 0), XShortField('cksum', None), XShortField('id', 0), XShortField('seq', 0), StrField('data', '')]
...
class _ICMPv6ML(_ICMPv6):
    fields_desc = [ByteEnumField('type', 130, icmp6types), ByteField('code', 0), XShortField('cksum', None), ShortField('mrd', 0), ShortField('reserved', 0), IP6Field('mladdr', '::')]
...
class ICMPv6MLQuery2(_ICMPv6):  # RFC 3810
    ...
    fields_desc = [
        ...
        IP6ListField('sources', [], count_from=lambda pkt: pkt.sources_number),
    ]
...
class ICMPv6MLDMultAddrRec(Packet):
    ...
    fields_desc = [
        ByteField('rtype', 4),
        FieldLenField('auxdata_len', None, length_of='auxdata', fmt='B'),
        FieldLenField('sources_number', None, length_of='sources', adjust=lambda p, num: num // 16),
        IP6Field('dst', '::'),
        IP6ListField('sources', [], length_from=lambda p: 16 * p.sources_number),
        StrLenField('auxdata', '', length_from=lambda p: p.auxdata_len),
    ]
...
class ICMPv6MLReport2(_ICMPv6):  # RFC 3810
    ...
    fields_desc = [
        ...
        PacketListField('records', [], ICMPv6MLDMultAddrRec, count_from=lambda p: p.records_number),
    ]
...
class ICMPv6MRD_Advertisement(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 151, icmp6types), ByteField('advinter', 20), XShortField('cksum', None), ShortField('queryint', 0), ShortField('robustness', 0)]
...
class ICMPv6MRD_Solicitation(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 152, icmp6types), ByteField('res', 0), XShortField('cksum', None)]
    overload_fields = {IPv6: {'nh': 58, 'hlim': 1, 'dst': 'ff02::2'}}
    # IPv6 Router Alert requires manual inclusion
...
class ICMPv6MRD_Termination(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 153, icmp6types), ByteField('res', 0), XShortField('cksum', None)]
...
icmp6ndraprefs = {0: 'Medium (default)', 1: 'High', 2: 'Reserved', 3: 'Low'}  # RFC 4191
...
class ICMPv6NDOptUnknown(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', None), FieldLenField('len', None, length_of='data', fmt='B', adjust=lambda pkt, x: x + 2), StrLenField('data', '', length_from=lambda pkt: pkt.len - 2)]
...
class ICMPv6NDOptSrcLLAddr(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 1), ByteField('len', 1), MACField('lladdr', ETHER_ANY)]
...
class ICMPv6NDOptRedirectedHdr(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField('type', 4),
        FieldLenField('len', None, length_of='pkt', fmt='B', adjust=lambda pkt, x: (x + 8) // 8),
        StrFixedLenField('res', b'\x00' * 6, 6),
        TruncPktLenField('pkt', b'', IPv6, 8, length_from=lambda pkt: 8 * pkt.len - 8),
    ]
...
class ICMPv6NDOptMTU(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 5), ByteField('len', 1), XShortField('res', 0), IntField('mtu', 1280)]
...
class ICMPv6NDOptAdvInterval(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 7), ByteField('len', 1), ShortField('res', 0), IntField('advint', 0)]
...
class ICMPv6NDOptHAInfo(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 8), ByteField('len', 1), ShortField('res', 0), ShortField('pref', 0), ShortField('lifetime', 1)]
...
class ICMPv6NDOptIPAddr(_ICMPv6NDGuessPayload, Packet):  # RFC 4068
    ...
    fields_desc = [ByteField('type', 17), ByteField('len', 3), ByteEnumField('optcode', 1, {1: 'Old Care-Of Address', 2: 'New Care-Of Address', 3: "NAR's IP address"}), ByteField('plen', 64), IntField('res', 0), IP6Field('addr', '::')]
...
class ICMPv6NDOptNewRtrPrefix(_ICMPv6NDGuessPayload, Packet):  # RFC 4068
    ...
    fields_desc = [ByteField('type', 18), ByteField('len', 3), ByteField('optcode', 0), ByteField('plen', 64), IntField('res', 0), IP6Field('prefix', '::')]


_rfc4068_lla_optcode = {
    0: 'Wildcard requesting resolution for all nearby AP',
...
class ICMPv6NDOptLLA(_ICMPv6NDGuessPayload, Packet):  # RFC 4068
    ...
    fields_desc = [ByteField('type', 19), ByteField('len', 1), ByteEnumField('optcode', 0, _rfc4068_lla_optcode), MACField('lla', ETHER_ANY)]  # We only support ethernet
...
class ICMPv6NDOptRouteInfo(_ICMPv6NDGuessPayload, Packet):  # RFC 4191
    ...
    fields_desc = [
        ByteField('type', 24),
        FieldLenField('len', None, length_of='prefix', fmt='B', adjust=lambda pkt, x: x // 8 + 1),
        ByteField('plen', None),
        BitField('res1', 0, 3),
...
class ICMPv6NDOptRDNSS(_ICMPv6NDGuessPayload, Packet):  # RFC 5006
    ...
    fields_desc = [
        ByteField('type', 25),
        FieldLenField('len', None, count_of='dns', fmt='B', adjust=lambda pkt, x: 2 * x + 1),
        ShortField('res', None),
        IntField('lifetime', 0xFFFFFFFF),
        IP6ListField('dns', [], length_from=lambda pkt: 8 * (pkt.len - 1)),
    ]
...
class ICMPv6NDOptEFA(_ICMPv6NDGuessPayload, Packet):  # RFC 5175 (prev. 5075)
    ...
    fields_desc = [ByteField('type', 26), ByteField('len', 1), BitField('res', 0, 48)]
...
class ICMPv6NDOptDNSSL(_ICMPv6NDGuessPayload, Packet):  # RFC 6106
    ...
    fields_desc = [
        ByteField('type', 31),
        FieldLenField('len', None, length_of='searchlist', fmt='B', adjust=lambda pkt, x: 1 + x // 8),
        ShortField('res', None),
        IntField('lifetime', 0xFFFFFFFF),
        DomainNameListField('searchlist', [], length_from=lambda pkt: 8 * pkt.len - 8, padded=True),
    ]
...
class ICMPv6ND_RS(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 133, icmp6types), ByteField('code', 0), XShortField('cksum', None), IntField('res', 0)]
...
class ICMPv6ND_RA(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField('type', 134, icmp6types),
        ByteField('code', 0),
        XShortField('cksum', None),
        ByteField('chlim', 0),
        BitField('M', 0, 1),
...
class ICMPv6ND_NS(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [ByteEnumField('type', 135, icmp6types), ByteField('code', 0), XShortField('cksum', None), IntField('res', 0), IP6Field('tgt', '::')]
    overload_fields = {IPv6: {'nh': 58, 'dst': 'ff02::1', 'hlim': 255}}
...
class ICMPv6ND_NA(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [ByteEnumField('type', 136, icmp6types), ByteField('code', 0), XShortField('cksum', None), BitField('R', 1, 1), BitField('S', 0, 1), BitField('O', 1, 1), XBitField('res', 0, 29), IP6Field('tgt', '::')]
    overload_fields = {IPv6: {'nh': 58, 'dst': 'ff02::1', 'hlim': 255}}
...
class ICMPv6ND_Redirect(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [ByteEnumField('type', 137, icmp6types), ByteField('code', 0), XShortField('cksum', None), XIntField('res', 0), IP6Field('tgt', '::'), IP6Field('dst', '::')]
    overload_fields = {IPv6: {'nh': 58, 'dst': 'ff02::1', 'hlim': 255}}
...
class ICMPv6NDOptSrcAddrList(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField('type', 9),
        FieldLenField('len', None, count_of='addrlist', fmt='B', adjust=lambda pkt, x: 2 * x + 1),
        StrFixedLenField('res', b'\x00' * 6, 6),
        IP6ListField('addrlist', [], length_from=lambda pkt: 8 * (pkt.len - 1)),
    ]
...
class ICMPv6ND_INDSol(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 141, icmp6types), ByteField('code', 0), XShortField('cksum', None), XIntField('reserved', 0)]
    overload_fields = {IPv6: {'nh': 58, 'dst': 'ff02::1', 'hlim': 255}}
...
class ICMPv6ND_INDAdv(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 142, icmp6types), ByteField('code', 0), XShortField('cksum', None), XIntField('reserved', 0)]
    overload_fields = {IPv6: {'nh': 58, 'dst': 'ff02::1', 'hlim': 255}}
...
icmp6_niqtypes = {0: 'NOOP', 2: 'Node Name', 3: 'IPv6 Address', 4: 'IPv4 Address'}
...
class ICMPv6NIQueryNOOP(_ICMPv6NIHashret, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField('type', 139, icmp6types),
...
_nireply_code = {0: 'Successful Reply', 1: 'Response Refusal', 3: 'Unknown query type'}

_nireply_flags = {1: 'Reply set incomplete', 2: 'All unicast addresses', 4: 'IPv4 addresses', 8: 'Link-local addresses', 16: 'Site-local addresses', 32: 'Global addresses'}
...
class ICMPv6NIReplyNOOP(_ICMPv6NIAnswers, _ICMPv6NIHashret, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField('type', 140, icmp6types),
...
class ICMPv6RPL(_ICMPv6):  # RFC 6550
    name = 'RPL'
    fields_desc = [ByteEnumField('type', 155, icmp6types), ByteEnumField('code', 0, rplcodes), XShortField('cksum', None)]
...
class ICMPv6HAADRequest(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 144, icmp6types), ByteField('code', 0), XShortField('cksum', None), XShortField('id', None), BitEnumField('R', 1, 1, {1: 'MR'}), XBitField('res', 0, 15)]
...
class ICMPv6HAADReply(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 145, icmp6types), ByteField('code', 0), XShortField('cksum', None), XShortField('id', None), BitEnumField('R', 1, 1, {1: 'MR'}), XBitField('res', 0, 15), IP6ListField('addresses', None)]
...
class ICMPv6MPSol(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 146, icmp6types), ByteField('code', 0), XShortField('cksum', None), XShortField('id', None), XShortField('res', 0)]
...
class ICMPv6MPAdv(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 147, icmp6types), ByteField('code', 0), XShortField('cksum', None), XShortField('id', None), BitEnumField('flags', 2, 2, {2: 'M', 1: 'O'}), XBitField('res', 0, 14)]
...
class MIP6OptBRAdvice(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 2, _mobopttypes), ByteField('olen', 2), ShortField('rinter', 0)]
    x = 2
...
class MIP6OptAltCoA(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 3, _mobopttypes), ByteField('olen', 16), IP6Field('acoa', '::')]
    x = 8
...
class MIP6OptNonceIndices(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 4, _mobopttypes), ByteField('olen', 16), ShortField('hni', 0), ShortField('coni', 0)]
    x = 2
    y = 0  # alignment requirement: 2n
...
class MIP6OptBindingAuthData(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 5, _mobopttypes), ByteField('olen', 16), BitField('authenticator', 0, 96)]
    x = 8
...
class MIP6OptMobNetPrefix(_MIP6OptAlign):  # NEMO - RFC 3963
    ...
    fields_desc = [ByteEnumField('otype', 6, _mobopttypes), ByteField('olen', 18), ByteField('reserved', 0), ByteField('plen', 64), IP6Field('prefix', '::')]
    x = 8
...
class MIP6OptLLAddr(_MIP6OptAlign):  # Sect 6.4.4 of RFC 4068
    ...
    fields_desc = [ByteEnumField('otype', 7, _mobopttypes), ByteField('olen', 7), ByteEnumField('ocode', 2, _rfc4068_lla_optcode), ByteField('pad', 0), MACField('lla', ETHER_ANY)]  # Only support ethernet
    x = 0
...
class MIP6OptMNID(_MIP6OptAlign):  # RFC 4283
    ...
    fields_desc = [
        ByteEnumField('otype', 8, _mobopttypes),
        FieldLenField('olen', None, length_of='id', fmt='B', adjust=lambda pkt, x: x + 1),
        ByteEnumField('subtype', 1, {1: 'NAI'}),
        StrLenField('id', '', length_from=lambda pkt: pkt.olen - 1),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptMsgAuth(_MIP6OptAlign):  # RFC 4285 (Sect. 5)
    ...
    fields_desc = [
        ByteEnumField('otype', 9, _mobopttypes),
        FieldLenField('olen', None, length_of='authdata', fmt='B', adjust=lambda pkt, x: x + 5),
        ByteEnumField(
            'subtype',
            1,
            {
                1: 'MN-HA authentication mobility option',  # noqa: E501
                2: 'MN-AAA authentication mobility option',
            },
        ),  # noqa: E501
        IntField('mspi', None),
        StrLenField('authdata', 'A' * 12, length_from=lambda pkt: pkt.olen - 5),
    ]
    x = 4
...
class MIP6OptReplayProtection(_MIP6OptAlign):  # RFC 4285 (Sect. 6)
    ...
    fields_desc = [ByteEnumField('otype', 10, _mobopttypes), ByteField('olen', 8), NTPTimestampField('timestamp', 0)]
    x = 8
    y = 2  # alignment requirement: 8n+2
...
class MIP6OptCGAParamsReq(_MIP6OptAlign):  # RFC 4866 (Sect. 5.6)
    ...
    fields_desc = [ByteEnumField('otype', 11, _mobopttypes), ByteField('olen', 0)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptCGAParams(_MIP6OptAlign):  # RFC 4866 (Sect. 5.1)
    ...
    fields_desc = [ByteEnumField('otype', 12, _mobopttypes), FieldLenField('olen', None, length_of='cgaparams', fmt='B'), StrLenField('cgaparams', '', length_from=lambda pkt: pkt.olen)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptSignature(_MIP6OptAlign):  # RFC 4866 (Sect. 5.2)
    ...
    fields_desc = [ByteEnumField('otype', 13, _mobopttypes), FieldLenField('olen', None, length_of='sig', fmt='B'), StrLenField('sig', '', length_from=lambda pkt: pkt.olen)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptHomeKeygenToken(_MIP6OptAlign):  # RFC 4866 (Sect. 5.3)
    ...
    fields_desc = [ByteEnumField('otype', 14, _mobopttypes), FieldLenField('olen', None, length_of='hkt', fmt='B'), StrLenField('hkt', '', length_from=lambda pkt: pkt.olen)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptCareOfTestInit(_MIP6OptAlign):  # RFC 4866 (Sect. 5.4)
    ...
    fields_desc = [ByteEnumField('otype', 15, _mobopttypes), ByteField('olen', 0)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptCareOfTest(_MIP6OptAlign):  # RFC 4866 (Sect. 5.5)
    ...
    fields_desc = [ByteEnumField('otype', 16, _mobopttypes), FieldLenField('olen', None, length_of='cokt', fmt='B'), StrLenField('cokt', b'\x00' * 8, length_from=lambda pkt: pkt.olen)]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptUnknown(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 6, _mobopttypes), FieldLenField('olen', None, length_of='odata', fmt='B'), StrLenField('odata', '', length_from=lambda pkt: pkt.olen)]
    x = 0
    y = 0  # alignment requirement: none
...
mhtypes = {0: 'BRR', 1: 'HoTI', 2: 'CoTI', 3: 'HoT', 4: 'CoT', 5: 'BU', 6: 'BA', 7: 'BE', 8: 'Fast BU', 9: 'Fast BA', 10: 'Fast NA'}
...
class MIP6MH_Generic(_MobilityHeader):  # Mainly for decoding of unknown msg
    ...
    fields_desc = [
        ...
        ByteField('res', None),
        XShortField('cksum', None),
        StrLenField('msg', b'\x00' * 2, length_from=lambda pkt: 8 * pkt.len - 6),
    ]
...
class MIP6MH_BRR(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _PhantomAutoPadField('autopad', 1),  # autopad activated by default  # noqa: E501
        _OptionsField('options', [], MIP6OptUnknown, 8, length_from=lambda pkt: 8 * pkt.len),
    ]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_HoTI(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _PhantomAutoPadField('autopad', 1),  # autopad activated by default  # noqa: E501
        _OptionsField('options', [], MIP6OptUnknown, 16, length_from=lambda pkt: 8 * (pkt.len - 1)),
    ]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_HoT(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _PhantomAutoPadField('autopad', 1),  # autopad activated by default  # noqa: E501
        _OptionsField('options', [], MIP6OptUnknown, 24, length_from=lambda pkt: 8 * (pkt.len - 2)),
    ]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_BU(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _PhantomAutoPadField('autopad', 1),  # autopad activated by default  # noqa: E501
        _OptionsField('options', [], MIP6OptUnknown, 12, length_from=lambda pkt: 8 * pkt.len - 4),
    ]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_BA(_MobilityHeader):
    ...
    def answers(self, other):
        if (
            isinstance(other, MIP6MH_BU)
            and other.mhtype == 5
            and self.mhtype == 6
            and other.flags & 0x1  # Ack request flags is set
...
_bestatus = {1: 'Unknown binding for Home Address destination option', 2: 'Unrecognized MH Type value'}
...
_mip6_mhtype2cls = {0: MIP6MH_BRR, 1: MIP6MH_HoTI, 2: MIP6MH_CoTI, 3: MIP6MH_HoT, 4: MIP6MH_CoT, 5: MIP6MH_BU, 6: MIP6MH_BA, 7: MIP6MH_BE}
...
class TracerouteResult6(TracerouteResult):
    ...
    def show(self):
        return self.make_table(
            lambda s, r: (
                s.sprintf('%-42s,IPv6.dst%:{TCP:tcp%TCP.dport%}{UDP:udp%UDP.dport%}{ICMPv6EchoRequest:IER}'),  # TODO: ICMPv6 !  # noqa: E501
                s.hlim,
                r.sprintf(
                    '%-42s,IPv6.src% {TCP:%TCP.flags%}'  # noqa: E501
...
def traceroute6(target, dport=80, minttl=1, maxttl=30, sport=RandShort(), l4=None, timeout=2, verbose=None, **kargs):
    ...
    if l4 is None:
        a, b = sr(
            IPv6(dst=target, hlim=(minttl, maxttl)) / TCP(seq=RandInt(), sport=sport, dport=dport),  # noqa: E501
            timeout=timeout,
            filter='icmp6 or tcp',
            verbose=verbose,
            **kargs,
