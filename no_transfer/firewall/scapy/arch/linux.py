...
from scapy.data import MTU, ETH_P_ALL, SOL_PACKET, SO_ATTACH_FILTER, SO_TIMESTAMPNS
...
def attach_filter(sock, bpf_filter, iface):
    ...
    if conf.use_pypy and sys.pypy_version_info <= (7, 3, 2):  # type: ignore
        ...
        bp = struct.pack('HL', bp.bf_len, ctypes.addressof(bp.bf_insns.contents))
    else:
...
def get_alias_address(
    ...
    ifreq = ioctl(sck.fileno(), SIOCGIFCONF, struct.pack('iL', len(names_ar), names_ar.buffer_info()[0]))
    ...
    for ifname_b in names:
        ...
        if (ifaddr & msk) == ip_mask:
            sck.close()
            return (ifaddr & msk, msk, gw_str, ifname, scapy.utils.ltoa(ifaddr), metric)
...
def in6_getifaddr():
    ...
    for line in fdesc:
        ...
        addr = scapy.utils6.in6_ptop(b':'.join(struct.unpack('4s4s4s4s4s4s4s4s', tmp[0].encode())).decode())
        # (addr, scope, iface)
    ...
    def load(self):
        ...
        for i in _get_if_list():
            try:
                ...
                mac = scapy.utils.str2mac(get_if_raw_hwaddr(i, siocgifhwaddr=SIOCGIFHWADDR)[1])
            ...
            if_data = {'name': i, 'network_name': i, 'description': i, 'flags': ifflags, 'index': index, 'ip': ip, 'ips': [x[0] for x in ips if x[2] == i] + [ip] if ip else [], 'mac': mac}
...
class L2Socket(SuperSocket):
    ...
    def __init__(
        ...
        if not isinstance(self, L2ListenSocket):
            ...
            self.outs.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, conf.bufsize)
        else:
...
class L3PacketSocket(L2Socket):
    ...
    def send(self, x):
        ...
        if self.lvl == 3 and type_x != self.LL:
            warning('Incompatible L3 types detected using %s instead of %s !', type_x, self.LL)
            self.LL = type_x
        sx = raw(ll(x))
