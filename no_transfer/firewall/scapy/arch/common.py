...
# From if.h
_iff_flags = ['UP', 'BROADCAST', 'DEBUG', 'LOOPBACK', 'POINTTOPOINT', 'NOTRAILERS', 'RUNNING', 'NOARP', 'PROMISC', 'NOTRAILERS', 'ALLMULTI', 'MASTER', 'SLAVE', 'MULTICAST', 'PORTSEL', 'AUTOMEDIA', 'DYNAMIC', 'LOWER_UP', 'DORMANT', 'ECHO']
...
def compile_filter(
    ...
    """
    try:
        from scapy.libs.winpcapy import PCAP_ERRBUF_SIZE, pcap_open_live, pcap_compile, pcap_compile_nopcap, pcap_close
    except OSError:
    ...
    if linktype is not None:
        ret = pcap_compile_nopcap(MTU, linktype, ctypes.byref(bpf), bpf_filter, 1, -1)
    elif iface:
        ...
        if error:
            raise OSError(error)
        ret = pcap_compile(pcap, ctypes.byref(bpf), bpf_filter, 1, -1)
        pcap_close(pcap)
    if ret == -1:
        raise Scapy_Exception('Failed to compile filter expression %s (%s)' % (filter_exp, ret))
    return bpf
