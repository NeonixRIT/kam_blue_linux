...
def get_service_status(service):
    ...
    service = OpenServiceW(schSCManager, service, SERVICE_QUERY_STATUS)
...
class sockaddr_in(Structure):
    _fields_ = [('sin_family', SHORT), ('sin_port', USHORT), ('sin_addr', in_addr), ('sin_zero', 8 * CHAR)]


class sockaddr_in6(Structure):
    _fields_ = [('sin6_family', SHORT), ('sin6_port', USHORT), ('sin6_flowinfo', ULONG), ('sin6_addr', in6_addr), ('sin6_scope_id', ULONG)]


class SOCKADDR_INET(ctypes.Union):
    _fields_ = [('Ipv4', sockaddr_in), ('Ipv6', sockaddr_in6), ('si_family', USHORT)]
...
_GetIcmpStatistics = WINFUNCTYPE(ULONG, PMIB_ICMP)(('GetIcmpStatistics', iphlpapi))
...
class SOCKET_ADDRESS(ctypes.Structure):
    _fields_ = [('address', POINTER(SOCKADDR_INET)), ('length', INT)]


class _IP_ADAPTER_ADDRESSES_METRIC(Structure):
    _fields_ = [('length', ULONG), ('interface_index', DWORD)]
...
IP_ADAPTER_PREFIX._fields_ = [('alignment', ULONGLONG), ('next', PIP_ADAPTER_PREFIX), ('address', SOCKET_ADDRESS), ('prefix_length', ULONG)]
...
_GetAdaptersAddresses = WINFUNCTYPE(ULONG, ULONG, ULONG, POINTER(VOID), LP_IP_ADAPTER_ADDRESSES, POINTER(ULONG))(('GetAdaptersAddresses', iphlpapi))
...
class MIB_IPFORWARDTABLE(Structure):
    _fields_ = [('NumEntries', DWORD), ('Table', MIB_IPFORWARDROW * ANY_SIZE)]
...
_GetIpForwardTable = WINFUNCTYPE(DWORD, PMIB_IPFORWARDTABLE, POINTER(ULONG), BOOL)(('GetIpForwardTable', iphlpapi))
...
class IP_ADDRESS_PREFIX(Structure):
    _fields_ = [('Prefix', SOCKADDR_INET), ('PrefixLength', UINT8)]
...
class MIB_IPFORWARD_TABLE2(Structure):
    _fields_ = [('NumEntries', ULONG), ('Table', MIB_IPFORWARD_ROW2 * ANY_SIZE)]
...
if not WINDOWS_XP:
    ...
    _GetIpForwardTable2 = WINFUNCTYPE(ULONG, USHORT, POINTER(PMIB_IPFORWARD_TABLE2))(('GetIpForwardTable2', iphlpapi))
    _FreeMibTable = WINFUNCTYPE(None, PMIB_IPFORWARD_TABLE2)(('FreeMibTable', iphlpapi))
