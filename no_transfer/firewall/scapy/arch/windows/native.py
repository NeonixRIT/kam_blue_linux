...
class L3WinSocket(SuperSocket):
    ...
    def __init__(
        ...
        **kwargs,  # type: Any
        ...
        try:
            ...
            if e.errno == 10013:
                raise OSError('Windows native L3 Raw sockets are only ' 'usable as administrator ! ' 'Install Winpcap/Npcap to workaround !')
            raise
    ...
    def send(self, x):
        ...
        if self.cls not in x:
            raise Scapy_Exception('L3WinSocket can only send IP/IPv6 packets !' ' Install Npcap/Winpcap to send more')
        if not self.outs:
...
def open_icmp_firewall(host):
    ...
    with open(os.devnull, 'wb') as DEVNULL:
        return subprocess.Popen('ping -4 -w 1 -n 1 %s' % host, shell=True, stdout=DEVNULL, stderr=DEVNULL).wait()
