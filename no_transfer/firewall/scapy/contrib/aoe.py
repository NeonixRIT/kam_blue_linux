...
from scapy.fields import FlagsField, XByteField, ByteField, XShortField, ShortField, StrLenField, BitField, BitEnumField, ByteEnumField, FieldLenField, PacketListField, FieldListField, MACField, PacketField, ConditionalField, XIntField
...
class Directive(Packet):
    name = 'Directive'
    fields_desc = [ByteField('reserved', 0), ByteEnumField('dcmd', 0, {0: 'No directive', 1: 'Add mac address to mask list', 2: 'Delete mac address from mask list'}), MACField('mac_addr', ETHER_ANY)]
...
class MacMaskList(Packet):
    ...
    fields_desc = [
        ...
        ByteEnumField('mcmd', 0, {0: 'Read Mac Mask List', 1: 'Edit Mac Mask List'}),
        ByteEnumField('merror', 0, {0: '', 1: 'Unspecified error', 2: 'Bad dcmd directive', 3: 'Mask List Full'}),
        FieldLenField('dir_count', None, count_of='directives'),
        PacketListField('directives', None, Directive, count_from=lambda pkt: pkt.dir_count),
    ]
...
class ReserveRelease(Packet):
    ...
    fields_desc = [
        ByteEnumField('rcmd', 0, {0: 'Read Reserve List', 1: 'Set Reserve List', 2: 'Force Set Reserve List'}),
        FieldLenField('nb_mac', None, count_of='mac_addrs'),
        FieldListField('mac_addrs', None, MACField('', ETHER_ANY), count_from=lambda pkt: pkt.nb_mac),
    ]
