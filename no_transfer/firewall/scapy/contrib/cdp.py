...
class CDPAddrRecord(Packet):
    ...
    fields_desc = [
        ...
        StrLenField('proto', None, length_from=lambda x: x.plen, max_length=255),
        FieldLenField('addrlen', None, length_of=lambda x: x.addr),
        StrLenField('addr', None, length_from=lambda x: x.addrlen, max_length=65535),
    ]
...
class CDPAddrRecordIPv4(CDPAddrRecord):
    ...
    fields_desc = [
        ...
        StrLenField('proto', _cdp_addrrecord_proto_ip, length_from=lambda x: x.plen, max_length=255),
...
class CDPAddrRecordIPv6(CDPAddrRecord):
    ...
    fields_desc = [
        ...
        StrLenField('proto', _cdp_addrrecord_proto_ipv6, length_from=lambda x: x.plen, max_length=255),
...
class CDPMsgAddr(CDPMsgGeneric):
    name = 'Addresses'
    fields_desc = [XShortEnumField('type', 0x0002, _cdp_tlv_types), ShortField('len', None), FieldLenField('naddr', None, fmt='!I', count_of='addr'), PacketListField('addr', [], _CDPGuessAddrRecord, length_from=lambda x: x.len - 8)]
...
class CDPMsgPortID(CDPMsgGeneric):
    name = 'Port ID'
    fields_desc = [XShortEnumField('type', 0x0003, _cdp_tlv_types), FieldLenField('len', None, 'iface', '!H', adjust=lambda pkt, x: x + 4), StrLenField('iface', 'Port 1', length_from=lambda x: x.len - 4)]  # noqa: E501
...
class CDPMsgCapabilities(CDPMsgGeneric):
    name = 'Capabilities'
    fields_desc = [XShortEnumField('type', 0x0004, _cdp_tlv_types), ShortField('len', 8), FlagsField('cap', 0, 32, _cdp_capabilities)]
...
class CDPMsgIPGateway(CDPMsgGeneric):
    name = 'IP Gateway'
    type = 0x0007
    fields_desc = [XShortEnumField('type', 0x0007, _cdp_tlv_types), ShortField('len', 8), IPField('defaultgw', '192.168.0.1')]


class CDPMsgIPPrefix(CDPMsgGeneric):
    name = 'IP Prefix'
    type = 0x0007
    fields_desc = [XShortEnumField('type', 0x0007, _cdp_tlv_types), ShortField('len', 9), IPField('prefix', '192.168.0.1'), ByteField('plen', 24)]
...
class CDPMsgNativeVLAN(CDPMsgGeneric):
    name = 'Native VLAN'
    fields_desc = [XShortEnumField('type', 0x000A, _cdp_tlv_types), ShortField('len', 6), ShortField('vlan', 1)]


class CDPMsgDuplex(CDPMsgGeneric):
    name = 'Duplex'
    fields_desc = [XShortEnumField('type', 0x000B, _cdp_tlv_types), ShortField('len', 5), ByteEnumField('duplex', 0x00, _cdp_duplex)]


class CDPMsgVoIPVLANReply(CDPMsgGeneric):
    name = 'VoIP VLAN Reply'
    fields_desc = [XShortEnumField('type', 0x000E, _cdp_tlv_types), ShortField('len', 7), ByteField('status', 1), ShortField('vlan', 1)]
...
class CDPMsgVoIPVLANQuery(CDPMsgGeneric):
    ...
    fields_desc = [
        ...
        FieldLenField('len', None, 'unknown2', fmt='!H', adjust=lambda pkt, x: x + 7),
        XByteField('unknown1', 0),
        ShortField('vlan', 1),
        # TLV length (len) - 2 (type) - 2 (len) - 1 (unknown1) - 2 (vlan)  # noqa: E501
        StrLenField('unknown2', '', length_from=lambda p: p.len - 7, max_length=65528),
    ]
...
class CDPMsgPower(CDPMsgGeneric):
    ...
    fields_desc = [XShortEnumField('type', 0x0010, _cdp_tlv_types), ShortField('len', 6), _CDPPowerField('power', 1337)]


class CDPMsgMTU(CDPMsgGeneric):
    name = 'MTU'
    # Check if field length is fixed (2 bytes)
    fields_desc = [XShortEnumField('type', 0x0011, _cdp_tlv_types), ShortField('len', 6), ShortField('mtu', 1500)]


class CDPMsgTrustBitmap(CDPMsgGeneric):
    name = 'Trust Bitmap'
    fields_desc = [XShortEnumField('type', 0x0012, _cdp_tlv_types), ShortField('len', 5), XByteField('trust_bitmap', 0x0)]


class CDPMsgUntrustedPortCoS(CDPMsgGeneric):
    name = 'Untrusted Port CoS'
    fields_desc = [XShortEnumField('type', 0x0013, _cdp_tlv_types), ShortField('len', 5), XByteField('untrusted_port_cos', 0x0)]
