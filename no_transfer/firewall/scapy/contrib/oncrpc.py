...
from scapy.fields import XIntField, IntField, IntEnumField, StrLenField, FieldListField, ConditionalField, PacketField, FieldLenField
...
class Object_Name(Packet):
    name = 'Object Name'
    fields_desc = [IntField('length', 0), StrLenField('_name', '', length_from=lambda pkt: pkt.length), StrLenField('fill', '', length_from=lambda pkt: (4 - pkt.length) % 4)]
...
class Auth_Unix(Packet):
    ...
    fields_desc = [
        ...
        FieldListField('auxgids', [], IntField('', None), count_from=lambda pkt: pkt.num_auxgids),
    ]
...
class Auth_RPCSEC_GSS(Packet):
    ...
    fields_desc = [IntField('gss_version', 0), IntField('gss_procedure', 0), IntField('gss_seq_num', 0), IntField('gss_service', 0), PacketField('gss_context', Object_Name(), Object_Name)]
...
class RPC_Call(Packet):
    ...
    fields_desc = [
        ...
        IntEnumField('aflavor', 1, {0: 'AUTH_NULL', 1: 'AUTH_UNIX', 6: 'RPCSEC_GSS'}),
        IntField('alength', None),
        ConditionalField(PacketField('a_unix', Auth_Unix(), Auth_Unix), lambda pkt: pkt.aflavor == 1),
        ConditionalField(PacketField('a_rpcsec_gss', Auth_RPCSEC_GSS(), Auth_RPCSEC_GSS), lambda pkt: pkt.aflavor == 6),
        IntEnumField('vflavor', 0, {0: 'AUTH_NULL', 1: 'AUTH_UNIX', 6: 'RPCSEC_GSS'}),
        ConditionalField(IntField('vlength', None), lambda pkt: pkt.vflavor != 6),
        ConditionalField(PacketField('v_unix', Auth_Unix(), Auth_Unix), lambda pkt: pkt.vflavor == 1),
        ConditionalField(PacketField('v_rpcsec_gss', Verifier_RPCSEC_GSS(), Verifier_RPCSEC_GSS), lambda pkt: pkt.vflavor == 6),
    ]
...
class RPC_Reply(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketField('a_unix', Auth_Unix(), Auth_Unix), lambda pkt: pkt.flavor == 1),
        IntField('length', 0),
