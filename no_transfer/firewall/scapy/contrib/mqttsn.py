...
from scapy.fields import BitField, BitEnumField, ByteField, ByteEnumField, ConditionalField, FieldLenField, ShortField, StrFixedLenField, StrLenField, XByteEnumField
...
class MQTTSN(Packet):
    ...
    fields_desc = [
        ...
        VariableFieldLenField('len', None, length_of='len', adjust=_mqttsn_len_adjust),
...
class MQTTSNWillMsg(Packet):
    ...
    fields_desc = [StrLenField('will_msg', '', length_from=_mqttsn_length_from(0))]
...
class MQTTSNSubscribe(Packet):
    ...
    fields_desc = FLAG_FIELDS + [
        ShortField('mid', 0),
        ConditionalField(ShortField('tid', None), lambda pkt: pkt.tid_type == 0b01),
        ConditionalField(StrFixedLenField('short_topic', None, length=2), lambda pkt: pkt.tid_type == 0b10),
        ConditionalField(StrLenField('topic_name', None, length_from=_mqttsn_length_from(3)), lambda pkt: pkt.tid_type not in [0b01, 0b10]),
    ]
...
class MQTTSNDisconnect(Packet):
    ...
    fields_desc = [
        ConditionalField(ShortField('duration', None), lambda pkt: hasattr(pkt.underlayer, 'len') and ((pkt.underlayer.len is None) or (pkt.underlayer.len > 2))),
    ]
...
class MQTTSNEncaps(Packet):
    ...
    fields_desc = [
        BitField('resvd', 0, 6),
        BitField('radius', 0, 2),
        StrLenField('w_node_id', '', length_from=_mqttsn_length_from(1)),
    ]
