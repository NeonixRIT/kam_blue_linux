...
from scapy.fields import ByteField, BitField, ByteEnumField, PacketListField, BitEnumField, XByteField, FieldLenField, LEShortField, BitFieldLenField
...
CAUSE_OF_TRANSMISSIONS = {
    ...
    IEC104_COT_UNKNOWN_COMMON_ADDRESS_OF_ASDU: 'unknown common address of ASDU',
    IEC104_COT_UNKNOWN_ADDRESS_OF_INFORMATION_OBJECT: 'unknown address of information object',
...
class IEC104_APDU(Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        if payload_len < apdu_length:
            warning('invalid len of APDU. given len: {} available len: {}'.format(apdu_length, payload_len))
            return self.default_payload_class(payload)

        apdu_type = _iec104_apci_type_from_packet(payload)

        return IEC104_APDU_CLASSES.get(apdu_type, self.default_payload_class(payload))
...
def _i_msg_io_dispatcher_single(pkt, next_layer_data):
    ...
    """
    next_layer_class_type = IEC104_IO_WITH_IOA_CLASSES.get(pkt.type_id, conf.raw_layer)
...
class IEC104_I_Message(IEC104_APDU):
    ...
    SQ_FLAGS = {SQ_FLAG_SINGLE: 'single', SQ_FLAG_SEQUENCE: 'sequence'}

    TEST_DISABLED = 0
    TEST_ENABLED = 1

    TEST_FLAGS = {TEST_DISABLED: 'disabled', TEST_ENABLED: 'enabled'}

    ACK_POSITIVE = 0
    ACK_NEGATIVE = 1

    ACK_FLAGS = {ACK_POSITIVE: 'positive', ACK_NEGATIVE: 'negative'}

    fields_desc = []

    def __init__(self, _pkt=b'', post_transform=None, _internal=0, _underlayer=None, **fields):
        super(IEC104_I_Message, self).__init__(_pkt=_pkt, post_transform=post_transform, _internal=_internal, _underlayer=_underlayer, **fields)
    ...
    def _information_object_update(self, io_instances):
        ...
        if not issubclass(first_io_class, IEC104_IO_Packet):
            raise IEC104_I_Message_StructureException('information object payload must be a subclass of ' 'IEC104_IO_Packet')
        ...
        for io_inst in io_instances[1:]:
            if io_inst.__class__ != first_io_class:
                raise IEC104_I_Message_StructureException('each information object within the ASDU must be of ' 'the same class type (first io: {}, ' 'current io: {})'.format(first_io_class._name, io_inst._name))
...
class IEC104_I_Message_SeqIOA(IEC104_I_Message):
    ...
    fields_desc = [
        ...
        FieldLenField('apdu_length', None, fmt='!B', length_of='io', adjust=lambda pkt, x: x + 13),
        IEC104SequenceNumber('tx_seq_num', 0),
        IEC104SequenceNumber('rx_seq_num', 0),
        # ASDU
        ByteEnumField('type_id', 0, IEC104_IO_NAMES),
        BitEnumField('sq', IEC104_I_Message.SQ_FLAG_SEQUENCE, 1, IEC104_I_Message.SQ_FLAGS),
        ...
        IEC104ASDUPacketListField('io', conf.raw_layer(), _i_msg_io_dispatcher_sequence, length_from=lambda pkt: pkt.apdu_length - 13),
    ]
...
class IEC104_I_Message_SingleIOA(IEC104_I_Message):
    ...
    fields_desc = [
        # APCI
        XByteField('start', IEC104_I_Message.IEC_104_MAGIC),
        FieldLenField('apdu_length', None, fmt='!B', length_of='io', adjust=lambda pkt, x: x + 10),
        IEC104SequenceNumber('tx_seq_num', 0),
        IEC104SequenceNumber('rx_seq_num', 0),
        # ASDU
        ByteEnumField('type_id', 0, IEC104_IO_NAMES),
        BitEnumField('sq', IEC104_I_Message.SQ_FLAG_SINGLE, 1, IEC104_I_Message.SQ_FLAGS),
        BitFieldLenField('num_io', None, 7, count_of='io'),
        BitEnumField('test', 0, 1, IEC104_I_Message.TEST_FLAGS),
        BitEnumField('ack', 0, 1, IEC104_I_Message.ACK_FLAGS),
        BitEnumField('cot', 0, 6, CAUSE_OF_TRANSMISSIONS),
        ByteField('origin_address', 0),
        LEShortField('common_asdu_address', 0),
        IEC104ASDUPacketListField('io', conf.raw_layer(), _i_msg_io_dispatcher_single, length_from=lambda pkt: pkt.apdu_length - 10),
    ]
...
def _iec104_is_i_apdu_seq_ioa(payload):
    ...
    if orb(payload[0]) != 0x68 or (orb(payload[1]) + 2) > len_payload or len_payload < 8:
        return False
...
def _iec104_is_i_apdu_single_ioa(payload):
    ...
    return IEC104_APDU_TYPE_I_SINGLE_IOA == _iec104_apci_type_from_packet(payload)
