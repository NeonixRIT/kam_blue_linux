...
from scapy.fields import BitField, ByteField, ConditionalField, FieldLenField, IPField, IntField, PacketListField, ShortField, StrLenField
...
class ICMPExtensionObject(Packet):
    ...
    fields_desc = [ShortField('len', None), ByteField('classnum', 0), ByteField('classtype', 0)]
...
class ICMPExtensionHeader(Packet):
    ...
    fields_desc = [BitField('version', 2, 4), BitField('reserved', 0, 12), BitField('chksum', None, 16)]
    ...
    def guess_payload_class(self, payload):
        ...
        for fval, cls in self.payload_guess:
            if all(hasattr(ieo, k) and v == ieo.getfieldval(k) for k, v in six.iteritems(fval)):
                return cls
...
class ICMPExtensionMPLS(ICMPExtensionObject):
    ...
    fields_desc = [ShortField('len', None), ByteField('classnum', 1), ByteField('classtype', 1), PacketListField('stack', [], MPLS, length_from=lambda pkt: pkt.len - 4)]
...
class ICMPExtensionInterfaceInformation(ICMPExtensionObject):
    ...
    fields_desc = [
        ...
        ConditionalField(FieldLenField('ifname_len', None, fmt='B', length_of='ifname'), lambda pkt: pkt.has_ifname == 1),
        ConditionalField(StrLenField('ifname', None, length_from=lambda pkt: pkt.ifname_len), lambda pkt: pkt.has_ifname == 1),
