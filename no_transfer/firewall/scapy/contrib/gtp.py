...
from scapy.packet import bind_layers, bind_bottom_up, bind_top_down, Packet, Raw
from scapy.volatile import RandInt, RandIP, RandNum, RandString


# GTP Data types

RATType = {1: 'UTRAN', 2: 'GETRAN', 3: 'WLAN', 4: 'GAN', 5: 'HSPA'}
...
Selection_Mode = {11111100: 'MS or APN', 11111101: 'MS', 11111110: 'NET', 11111111: 'FutureUse'}

TrueFalse_value = {254: 'False', 255: 'True'}

# http://www.arib.or.jp/IMT-2000/V720Mar09/5_Appendix/Rel8/29/29281-800.pdf
ExtensionHeadersTypes = {0: 'No more extension headers', 1: 'Reserved', 2: 'Reserved', 64: 'UDP Port', 133: 'PDU Session Container', 192: 'PDCP PDU Number', 193: 'Reserved', 194: 'Reserved'}
...
GTPforcedTypes = {16: GTPHeader, 17: GTPHeader, 18: GTPHeader, 19: GTPHeader, 20: GTPHeader, 21: GTPHeader, 26: GTP_U_Header, 27: GTPHeader, 254: GTP_U_Header, 255: GTP_U_Header}
...
class GTPPDUSessionContainer(Packet):
    ...
    fields_desc = [
        ByteField('ExtHdrLen', None),
        BitEnumField('type', 0, 4, {0: 'DL PDU SESSION INFORMATION', 1: 'UL PDU SESSION INFORMATION'}),
        BitField('QMP', 0, 1),
        # UL (type 1)
        ConditionalField(BitField('dlDelayInd', 0, 1), lambda pkt: pkt.type == 1),
        ConditionalField(BitField('ulDelayInd', 0, 1), lambda pkt: pkt.type == 1),
        # Common
        BitField('SNP', 0, 1),
        # UL (type 1)
        ConditionalField(BitField('N3N9DelayInd', 0, 1), lambda pkt: pkt.type == 1),
        ConditionalField(XBitField('spareUl1', 0, 1), lambda pkt: pkt.type == 1),
        # DL (type 0)
        ConditionalField(XBitField('spareDl1', 0, 2), lambda pkt: pkt.type == 0),
        ConditionalField(BitField('PPP', 0, 1), lambda pkt: pkt.type == 0),
        ConditionalField(BitField('RQI', 0, 1), lambda pkt: pkt.type == 0),
        # Common
        BitField('QFI', 0, 6),  # QoS Flow Identifier
        # DL (type 0)
        ConditionalField(XBitField('PPI', 0, 3), lambda pkt: pkt.type == 0 and pkt.PPP == 1),
        ConditionalField(XBitField('spareDl2', 0, 5), lambda pkt: pkt.type == 0 and pkt.PPP == 1),
        ConditionalField(XBitField('dlSendTime', 0, 64), lambda pkt: pkt.type == 0 and pkt.QMP == 1),
        ConditionalField(X3BytesField('dlQFISeqNum', 0), lambda pkt: pkt.type == 0 and pkt.SNP == 1),
        # UL (type 1)
        ConditionalField(XBitField('dlSendTimeRpt', 0, 64), lambda pkt: pkt.type == 1 and pkt.QMP == 1),
        ConditionalField(XBitField('dlRecvTime', 0, 64), lambda pkt: pkt.type == 1 and pkt.QMP == 1),
        ConditionalField(XBitField('ulSendTime', 0, 64), lambda pkt: pkt.type == 1 and pkt.QMP == 1),
        ConditionalField(XBitField('dlDelayRslt', 0, 32), lambda pkt: pkt.type == 1 and pkt.dlDelayInd == 1),
        ConditionalField(XBitField('ulDelayRslt', 0, 32), lambda pkt: pkt.type == 1 and pkt.ulDelayInd == 1),
        ConditionalField(XBitField('UlQFISeqNum', 0, 24), lambda pkt: pkt.type == 1 and pkt.SNP == 1),
        ConditionalField(XBitField('N3N9DelayRslt', 0, 32), lambda pkt: pkt.type == 1 and pkt.N3N9DelayInd == 1),
        # Common
        ByteEnumField('NextExtHdr', 0, ExtensionHeadersTypes),
        ConditionalField(StrLenField('padding', b'', length_from=lambda p: 0), lambda pkt: pkt.NextExtHdr == 0),
    ]
...
class IE_Cause(IE_Base):
    name = 'Cause'
    fields_desc = [ByteEnumField('ietype', 1, IEType), ByteEnumField('CauseValue', None, CauseValues)]


class IE_IMSI(IE_Base):
    name = 'IMSI - Subscriber identity of the MS'
    fields_desc = [ByteEnumField('ietype', 2, IEType), TBCDByteField('imsi', str(RandNum(0, 999999999999999)), 8)]


class IE_Routing(IE_Base):
    name = 'Routing Area Identity'
    fields_desc = [
        ByteEnumField('ietype', 3, IEType),
        TBCDByteField('MCC', '', 2),
...
class IE_ReorderingRequired(IE_Base):
    name = 'Recovery'
    fields_desc = [ByteEnumField('ietype', 8, IEType), ByteEnumField('reordering_required', 254, TrueFalse_value)]


class IE_Recovery(IE_Base):
    name = 'Recovery'
    fields_desc = [ByteEnumField('ietype', 14, IEType), ByteField('restart_counter', 24)]


class IE_SelectionMode(IE_Base):
    # Indicates the origin of the APN in the message
    name = 'Selection Mode'
    fields_desc = [ByteEnumField('ietype', 15, IEType), BitEnumField('SelectionMode', 'MS or APN', 8, Selection_Mode)]


class IE_TEIDI(IE_Base):
    name = 'Tunnel Endpoint Identifier Data'
    fields_desc = [ByteEnumField('ietype', 16, IEType), XIntField('TEIDI', RandInt())]


class IE_TEICP(IE_Base):
    name = 'Tunnel Endpoint Identifier Control Plane'
    fields_desc = [ByteEnumField('ietype', 17, IEType), XIntField('TEICI', RandInt())]


class IE_Teardown(IE_Base):
    name = 'Teardown Indicator'
    fields_desc = [ByteEnumField('ietype', 19, IEType), ByteEnumField('indicator', 'True', TrueFalse_value)]


class IE_NSAPI(IE_Base):
    # Identifies a PDP context in a mobility management context specified by TEICP  # noqa: E501
    name = 'NSAPI'
    fields_desc = [ByteEnumField('ietype', 20, IEType), XBitField('sparebits', 0x0000, 4), XBitField('NSAPI', RandNum(0, 15), 4)]
...
class IE_ChargingCharacteristics(IE_Base):
    ...
    fields_desc = [
        ByteEnumField('ietype', 26, IEType),
...
class IE_TraceReference(IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 27, IEType), XBitField('Trace_reference', None, 16)]


class IE_TraceType(IE_Base):
    # Indicates the type of the trace
    name = 'Trace Type'
    fields_desc = [ByteEnumField('ietype', 28, IEType), XBitField('Trace_type', None, 16)]


class IE_ChargingId(IE_Base):
    name = 'Charging ID'
    fields_desc = [ByteEnumField('ietype', 127, IEType), XIntField('Charging_id', RandInt())]
...
class IE_EndUserAddress(IE_Base):
    ...
    fields_desc = [
        ...
        ConditionalField(IPField('PDPAddress', RandIP()), lambda pkt: pkt.length == 6 or pkt.length == 22),  # noqa: E501
        ConditionalField(IP6Field('IPv6_PDPAddress', '::1'), lambda pkt: pkt.length == 18 or pkt.length == 22),
    ]  # noqa: E501
...
class IE_AccessPointName(IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 131, IEType), ShortField('length', None), APNStrLenField('APN', 'nternet', length_from=lambda x: x.length)]  # noqa: E501
...
class IE_ProtocolConfigurationOptions(IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 132, IEType), ShortField('length', 4), StrLenField('Protocol_Configuration', '', length_from=lambda x: x.length)]


class IE_GSNAddress(IE_Base):
    name = 'GSN Address'
    fields_desc = [
        ByteEnumField('ietype', 133, IEType),
        ShortField('length', None),
        ConditionalField(IPField('ipv4_address', RandIP()), lambda pkt: pkt.length == 4),
        ConditionalField(IP6Field('ipv6_address', '::1'), lambda pkt: pkt.length == 16),
    ]
...
class IE_QoS(IE_Base):
    ...
    fields_desc = [
        ...
        # 3GPP TS 24.008 10.5.6.5
        ConditionalField(XBitField('spare1', 0x00, 2), lambda p: p.length and p.length > 1),
        ConditionalField(XBitField('delay_class', 0x000, 3), lambda p: p.length and p.length > 1),
        ConditionalField(XBitField('reliability_class', 0x000, 3), lambda p: p.length and p.length > 1),
        ConditionalField(XBitField('peak_troughput', 0x0000, 4), lambda p: p.length and p.length > 2),
        ConditionalField(BitField('spare2', 0, 1), lambda p: p.length and p.length > 2),
        ConditionalField(XBitField('precedence_class', 0x000, 3), lambda p: p.length and p.length > 2),
        ConditionalField(XBitField('spare3', 0x000, 3), lambda p: p.length and p.length > 3),
        ConditionalField(XBitField('mean_troughput', 0x00000, 5), lambda p: p.length and p.length > 3),
        ConditionalField(XBitField('traffic_class', 0x000, 3), lambda p: p.length and p.length > 4),
        ConditionalField(XBitField('delivery_order', 0x00, 2), lambda p: p.length and p.length > 4),
        ConditionalField(XBitField('delivery_of_err_sdu', 0x000, 3), lambda p: p.length and p.length > 4),
        ConditionalField(ByteField('max_sdu_size', None), lambda p: p.length and p.length > 5),
        ConditionalField(ByteField('max_bitrate_up', None), lambda p: p.length and p.length > 6),
        ConditionalField(ByteField('max_bitrate_down', None), lambda p: p.length and p.length > 7),
        ConditionalField(XBitField('redidual_ber', 0x0000, 4), lambda p: p.length and p.length > 8),
        ConditionalField(XBitField('sdu_err_ratio', 0x0000, 4), lambda p: p.length and p.length > 8),
        ConditionalField(XBitField('transfer_delay', 0x00000, 6), lambda p: p.length and p.length > 9),
        ConditionalField(XBitField('traffic_handling_prio', 0x000, 2), lambda p: p.length and p.length > 9),
        ConditionalField(ByteField('guaranteed_bit_rate_up', None), lambda p: p.length and p.length > 10),
        ConditionalField(ByteField('guaranteed_bit_rate_down', None), lambda p: p.length and p.length > 11),
        ConditionalField(XBitField('spare4', 0x000, 3), lambda p: p.length and p.length > 12),
        ConditionalField(BitField('signaling_indication', 0, 1), lambda p: p.length and p.length > 12),
        ConditionalField(XBitField('source_stats_desc', 0x0000, 4), lambda p: p.length and p.length > 12),
        ConditionalField(ByteField('max_bitrate_down_ext', None), lambda p: p.length and p.length > 13),
        ConditionalField(ByteField('guaranteed_bitrate_down_ext', None), lambda p: p.length and p.length > 14),
        ConditionalField(ByteField('max_bitrate_up_ext', None), lambda p: p.length and p.length > 15),
        ConditionalField(ByteField('guaranteed_bitrate_up_ext', None), lambda p: p.length and p.length > 16),
        ConditionalField(ByteField('max_bitrate_down_ext2', None), lambda p: p.length and p.length > 17),
        ConditionalField(ByteField('guaranteed_bitrate_down_ext2', None), lambda p: p.length and p.length > 18),
        ConditionalField(ByteField('max_bitrate_up_ext2', None), lambda p: p.length and p.length > 19),
        ConditionalField(ByteField('guaranteed_bitrate_up_ext2', None), lambda p: p.length and p.length > 20),
    ]
...
class IE_CommonFlags(IE_Base):
    name = 'Common Flags'
    fields_desc = [
        ByteEnumField('ietype', 148, IEType),
        ShortField('length', None),
...
class IE_APNRestriction(IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 149, IEType), ShortField('length', 1), ByteField('restriction_type_value', 0)]


class IE_RATType(IE_Base):
    name = 'Rat Type'
    fields_desc = [ByteEnumField('ietype', 151, IEType), ShortField('length', 1), ByteEnumField('RAT_Type', None, RATType)]


class IE_UserLocationInformation(IE_Base):
    name = 'User Location Information'
    fields_desc = [
        ByteEnumField('ietype', 152, IEType),
        ShortField('length', None),
        ByteField('type', 1),
...
class IE_MSTimeZone(IE_Base):
    name = 'MS Time Zone'
    fields_desc = [ByteEnumField('ietype', 153, IEType), ShortField('length', None), ByteField('timezone', 0), BitField('spare', 0, 6), XBitField('daylight_saving_time', 0x00, 2)]


class IE_IMEI(IE_Base):
    name = 'IMEI'
    fields_desc = [ByteEnumField('ietype', 154, IEType), ShortField('length', None), TBCDByteField('IMEI', '', length_from=lambda x: x.length)]


class IE_MSInfoChangeReportingAction(IE_Base):
    name = 'MS Info Change Reporting Action'
    fields_desc = [ByteEnumField('ietype', 181, IEType), ShortField('length', 1), ByteField('Action', 0)]


class IE_DirectTunnelFlags(IE_Base):
    name = 'Direct Tunnel Flags'
    # 29.060 7.7.81
    fields_desc = [ByteEnumField('ietype', 182, IEType), ShortField('length', 1), BitField('spare', 0, 5), BitField('EI', 0, 1), BitField('GCSI', 0, 1), BitField('DTI', 0, 1)]


class IE_BearerControlMode(IE_Base):
    name = 'Bearer Control Mode'
    fields_desc = [ByteEnumField('ietype', 184, IEType), ShortField('length', 1), ByteField('bearer_control_mode', 0)]


class IE_EvolvedAllocationRetentionPriority(IE_Base):
    name = 'Evolved Allocation/Retention Priority'
    # 29.060 7.7.91
    fields_desc = [ByteEnumField('ietype', 191, IEType), ShortField('length', 1), BitField('spare1', 0, 1), BitField('PCI', 0, 1), XBitField('PL', 0x0000, 4), BitField('spare2', 0, 1), BitField('PVI', 0, 1)]


class IE_CharginGatewayAddress(IE_Base):
    name = 'Chargin Gateway Address'
    fields_desc = [
        ByteEnumField('ietype', 251, IEType),
        ShortField('length', 4),
        ConditionalField(IPField('ipv4_address', '127.0.0.1'), lambda pkt: pkt.length == 4),
        ConditionalField(IP6Field('ipv6_address', '::1'), lambda pkt: pkt.length == 16),
    ]
...
class IE_PrivateExtension(IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 255, IEType), ShortField('length', 1), ByteField('extension_identifier', 0), StrLenField('extention_value', '', length_from=lambda x: x.length)]
...
class IE_NotImplementedTLV(Packet):
    ...
    fields_desc = [ByteEnumField('ietype', 0, IEType), ShortField('length', None), StrLenField('data', '', length_from=lambda x: x.length)]
