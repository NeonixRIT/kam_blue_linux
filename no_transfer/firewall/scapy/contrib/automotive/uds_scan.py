...
from scapy.compat import Dict, Optional, List, Type, Any, Iterable, cast, Union, NamedTuple, orb, Set
from scapy.contrib.automotive import log_automotive
from scapy.packet import Raw, Packet
import scapy.libs.six as six
from scapy.error import Scapy_Exception
from scapy.contrib.automotive.uds import UDS, UDS_NR, UDS_DSC, UDS_TP, UDS_RDBI, UDS_WDBI, UDS_SA, UDS_RC, UDS_IOCBI, UDS_RMBA, UDS_ER, UDS_TesterPresentSender, UDS_CC, UDS_RDBPI, UDS_RD, UDS_TD

from scapy.contrib.automotive.ecu import EcuState
from scapy.contrib.automotive.scanner.enumerator import ServiceEnumerator, _AutomotiveTestCaseScanResult, _AutomotiveTestCaseFilteredScanResult, StateGeneratingServiceEnumerator
from scapy.contrib.automotive.scanner.test_case import AutomotiveTestCaseABC, _SocketUnion, _TransitionTuple, StateGenerator
...
class UDS_DSCEnumerator(UDS_Enumerator, StateGeneratingServiceEnumerator):
    ...
    _supported_kwargs.update({'delay_state_change': (int, lambda x: x >= 0), 'overwrite_timeout': (bool, None)})
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 0x100 and min(x) >= 0)

    _supported_kwargs_doc = (
        ServiceEnumerator._supported_kwargs_doc
        + """
...
class UDS_ServiceEnumerator(UDS_Enumerator):
    ...
    def _get_initial_requests(self, **kwargs):
        ...
        # default scan_range
        scan_range = kwargs.pop('scan_range', (x for x in range(0x100) if not x & 0x40))
...
class UDS_RDBIEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 0x10000 and min(x) >= 0)
...
class UDS_RDBIRandomEnumerator(UDS_RDBIEnumerator):
    ...
    _supported_kwargs.update({'probe_start': (int, lambda x: 0 <= x <= 0xFFFF), 'probe_end': (int, lambda x: 0 <= x <= 0xFFFF)})
    block_size = 2**6

    _supported_kwargs_doc = (
        UDS_RDBIEnumerator._supported_kwargs_doc
        + """
...
class UDS_WDBIEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'rdbi_enumerator': (UDS_RDBIEnumerator, None)})
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 0x100 and min(x) >= 0)
...
class UDS_WDBISelectiveEnumerator(StagedAutomotiveTestCase):
    @staticmethod
    def __connector_rdbi_to_wdbi(
        rdbi,  # type: AutomotiveTestCaseABC
        _,  # type: AutomotiveTestCaseABC
    ):  # type: (...) -> Dict[str, Any]
...
class UDS_SAEnumerator(UDS_Enumerator):
    ...
    def _evaluate_retry(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        if response.service == 0x7F and self._get_negative_response_code(response) in [0x24, 0x37]:
    ...
    def _evaluate_response(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        if response is not None and response.service == 0x67 and response.securityAccessType % 2 == 1:
    ...
    def get_seed_pkt(sock, level=1, record=b''):
        ...
        for _ in range(10):
            ...
            if seed is None:
                return None
            elif seed.service == 0x7F and UDS_Enumerator._get_negative_response_code(seed) != 0x37:
...
class UDS_SA_XOR_Enumerator(UDS_SAEnumerator, StateGenerator):
    ...
    def get_new_edge(self, socket, config):
        ...
        try:
            if last_resp.service != 0x67 or last_resp.securityAccessType % 2 != 1:
                return None
...
class UDS_RCEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'type_list': (list, lambda x: max(x) < 0x100 and min(x) >= 0)})
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 0x10000 and min(x) >= 0)
...
class UDS_IOCBIEnumerator(UDS_Enumerator):
    _description = 'Available Input Output Controls By Identifier ' 'and negative response per state'
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 0x10000 and min(x) >= 0)
...
class UDS_RMBARandomEnumerator(UDS_RMBAEnumeratorABC):
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'unittest': (bool, None)})
    ...
    def _random_memory_addr_pkt(addr_len=None, size_len=None, size=None):
        ...
        UDS_RMBARandomEnumerator.set_addr(pkt, random.randint(0, (2 ** (8 * pkt.memoryAddressLen) - 1)) & 0xFFFFFFF0)
        return pkt
...
class UDS_RMBASequentialEnumerator(UDS_RMBAEnumeratorABC):
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'points_of_interest': (list, None)})
...
class UDS_RMBAEnumerator(StagedAutomotiveTestCase):
    ...
    def __connector_rand_to_seq(rand, _):
        ...
        for tup in rand.results_with_positive_response:
            points_of_interest += [
                _PointOfInterest(UDS_RMBAEnumeratorABC.get_addr(tup.req), True, tup.req.memorySizeLen, tup.req.memoryAddressLen, 0x80),
                _PointOfInterest(UDS_RMBAEnumeratorABC.get_addr(tup.req), False, tup.req.memorySizeLen, tup.req.memoryAddressLen, 0x80),
            ]
...
class UDS_RDEnumerator(UDS_Enumerator):
    ...
    def _random_memory_addr_pkt(addr_len=None):  # noqa: E501
        ...
        addr = random.randint(0, 2 ** (8 * pkt.memoryAddressLen) - 1) & (0xFFFFFFFF << (4 * pkt.memoryAddressLen))
