...
from scapy.contrib.automotive.xcp.cto_commands_master import TransportLayerCmd, TransportLayerCmdGetSlaveId, Connect
from scapy.contrib.automotive.xcp.cto_commands_slave import ConnectPositiveResponse, TransportLayerCmdGetSlaveIdResponse
...
class XCPOnCANScanner:
    ...
    def __init__(self, can_socket, id_range=None, sniff_time=0.1, add_padding=False, verbose=False):
    ...
    def _scan(self, identifier, body, pid, answer_type):
        ...
        cto_request = XCPOnCAN(identifier=identifier, flags=flags) / CTORequest(pid=pid) / body

        req_and_res_list, _unanswered = self.__socket.sr(cto_request, timeout=self.__sniff_time, verbose=False, multi=True)

        if len(req_and_res_list) == 0:
            log_automotive.info('No answer for identifier: ' + str(identifier))
            return []

        valid_req_and_res_list = filter(lambda req_and_res: req_and_res[1].haslayer(answer_type), req_and_res_list)
    ...
    def _send_connect(self, identifier):
        ...
        xcp_req_and_res_list = self._scan(identifier, body, 0xFF, ConnectPositiveResponse)

        for req_and_res in xcp_req_and_res_list:
            result = XCPScannerResult(response_id=req_and_res[1].identifier, request_id=identifier)
            all_slaves.append(result)
            log_automotive.info('Detected XCP slave for broadcast identifier: ' + str(identifier) + '\nResponse: ' + str(result))

        if len(all_slaves) == 0:
            log_automotive.info('No XCP slave detected for identifier: ' + str(identifier))
        return all_slaves
    ...
    def _send_get_slave_id(self, identifier):
        ...
        xcp_req_and_res_list = self._scan(identifier, body, 0xF2, TransportLayerCmdGetSlaveIdResponse)
        ...
        for req_and_res in xcp_req_and_res_list:
            ...
            if response.position_1 != 0x58 or response.position_2 != 0x43 or response.position_3 != 0x50:
                continue

            # Identifier that the master must use to send packets to the slave
            # and the slave will answer with
            request_id = response['TransportLayerCmdGetSlaveIdResponse'].can_identifier

            result = XCPScannerResult(request_id=request_id, response_id=response.identifier)
            all_slaves.append(result)
            log_automotive.info('Detected XCP slave for broadcast identifier: ' + str(identifier) + '\nResponse: ' + str(result))
    ...
    def scan_with_get_slave_id(self):
        ...
        GetSlaveId Message"""
        log_automotive.info('Start scan with GetSlaveId id in range: ' + str(self.id_range))
    ...
    def scan_with_connect(self):
        # type: () -> List[XCPScannerResult]
        log_automotive.info('Start scan with CONNECT id in range: ' + str(self.id_range))
        results = []
        for identifier in self.id_range:
