...
from scapy.contrib.automotive.xcp.utils import get_max_cto, get_ag, XCPEndiannessField, StrVarLenField
from scapy.fields import ByteEnumField, ByteField, ShortField, StrLenField, FlagsField, IntField, ThreeBytesField, ConditionalField, XByteField, StrField, LEShortField, XIntField, FieldLenField
...
class NegativeResponse(Packet):
    ...
    fields_desc = [ByteEnumField('error_code', 0, error_code_enum), StrField('error_info', '')]


class GenericResponse(Packet):
    """Command Response packet"""

    fields_desc = [StrField('command_response_data', '')]


class ConnectPositiveResponse(Packet):
    fields_desc = [
        FlagsField('resource', 0, 8, ['cal_pag', 'x1', 'daq', 'stim', 'pgm', 'x5', 'x6', 'x7']),
        FlagsField('comm_mode_basic', 0, 8, ['byte_order', 'address_granularity_0', 'address_granularity_1', 'x3', 'x4', 'x5', 'slave_block_mode', 'optional']),
        ByteField('max_cto', 0),
        ConditionalField(ShortField('max_dto', 0), lambda p: p.comm_mode_basic.byte_order),
        ConditionalField(LEShortField('max_dto_le', 0), lambda p: not p.comm_mode_basic.byte_order),
...
class StatusPositiveResponse(Packet):
    fields_desc = [
        FlagsField('current_session_status', 0, 8, ['store_cal_req', 'x1', 'store_daq_req', 'clear_daq_request', 'x4', 'x5', 'daq_running', 'resume']),
        FlagsField('current_resource_protection_status', 0, 8, ['cal_pag', 'x1', 'daq', 'stim', 'pgm', 'x5', 'x6', 'x7']),
...
class CommonModeInfoPositiveResponse(Packet):
    fields_desc = [
        ...
        FlagsField('comm_mode_optional', 0, 8, ['master_block_mode', 'interleaved_mode', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7']),
...
class IdPositiveResponse(Packet):
    fields_desc = [
        ...
        XCPEndiannessField(FieldLenField('length', None, length_of='element', fmt='I')),
        StrVarLenField('element', b'', length_from=lambda p: p.length, max_length=lambda pkt: get_ag()),
    ]


class SeedPositiveResponse(Packet):
    fields_desc = [FieldLenField('seed_length', None, length_of='seed', fmt='B'), StrVarLenField('seed', b'', length_from=lambda p: p.seed_length, max_length=lambda: get_max_cto() - 2)]


class UnlockPositiveResponse(Packet):
    fields_desc = [FlagsField('current_resource_protection_status', 0, 8, ['cal_pag', 'x1', 'daq', 'stim', 'pgm', 'x5', 'x6', 'x7'])]


class UploadPositiveResponse(Packet):
    fields_desc = [
        ConditionalField(StrLenField('alignment', b'', length_from=lambda pkt: get_ag() - 1), lambda _: get_ag() > 1),
        StrLenField('element', b'', length_from=lambda pkt: get_max_cto() - get_ag()),
    ]
...
class ChecksumPositiveResponse(Packet):
    checksum_type_dict = {0x01: 'XCP_ADD_11', 0x02: 'XCP_ADD_12', 0x03: 'XCP_ADD_14', 0x04: 'XCP_ADD_22', 0x05: 'XCP_ADD_24', 0x06: 'XCP_ADD_44', 0x07: 'XCP_CRC_16', 0x08: 'XCP_CRC_16_CITT', 0x09: 'XCP_CRC_32', 0xFF: 'XCP_USER_DEFINED'}
    fields_desc = [
...
class TransportLayerCmdGetDAQIdResponse(Packet):
    can_id_fixed_enum = {0x00: 'configurable', 0x01: 'fixed'}
    fields_desc = [ByteEnumField('can_id_fixed', 0xFE, can_id_fixed_enum), XCPEndiannessField(ShortField('reserved', 0)), XCPEndiannessField(IntField('can_identifier', 0))]
...
class PagProcessorInfoPositiveResponse(Packet):
    fields_desc = [
        ...
        FlagsField('pag_properties', 0, 8, ['freeze_supported', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7']),
    ]
...
class PageInfoPositiveResponse(Packet):
    fields_desc = [
        FlagsField('page_properties', 0, 8, ['ecu_access_without_xcp', 'ecu_access_with_xcp', 'xcp_read_access_without_ecu', 'xcp_read_access_with_ecu', 'xcp_write_access_without_ecu', 'xcp_write_access_with_ecu', 'x6', 'x7']),
...
class SegmentModePositiveResponse(Packet):
    fields_desc = [
        ByteField('reserved', 0),
        FlagsField('mode', 0, 8, ['freeze', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7']),
    ]


class DAQListModePositiveResponse(Packet):
    fields_desc = [
        FlagsField('current_mode', 0, 8, ['selected', 'direction', 'x2', 'x3', 'timestamp', 'pid_off', 'running', 'resume']),
        XCPEndiannessField(ShortField('reserved', 0)),
...
class ReadDAQPositiveResponse(Packet):
    fields_desc = [ByteField('bit_offset', 0), ByteField('size_daq_element', 0), ByteField('address_extension_daq_element', 0), XCPEndiannessField(IntField('daq_element_address', 0))]


class DAQProcessorInfoPositiveResponse(Packet):
    fields_desc = [
        FlagsField('daq_properties', 0, 8, ['daq_config_type', 'prescaler_supported', 'resume_supported', 'bit_stim_supported', 'timestamp_supported', 'pid_off_supported', 'overload_msb', 'overload_event']),
        XCPEndiannessField(ShortField('max_daq', 0)),
        XCPEndiannessField(ShortField('max_event_channel', 0)),
        ByteField('min_daq', 0),
        FlagsField(
            'daq_key_byte', 0, 8, ['optimisation_type_0', 'optimisation_type_1', 'optimisation_type_2', 'optimisation_type_3', 'address_extension_odt', 'address_extension_daq', 'identification_field_type_0', 'identification_field_type_1']
        ),
    ]
...
class DAQResolutionInfoPositiveResponse(Packet):
    fields_desc = [
        ...
        FlagsField('timestamp_mode', 0, 8, ['size_0', 'size_1', 'size_2', 'timestamp_fixed', 'unit_0', 'unit_1', 'unit_2', 'unit_3']),
...
class DAQListInfoPositiveResponse(Packet):
    fields_desc = [
        FlagsField('daq_list_properties', 0, 8, ['predefined', 'event_fixed', 'daq', 'stim', 'x4', 'x5', 'x6', 'x7']),
        ByteField('max_odt', 0),
...
class DAQEventInfoPositiveResponse(Packet):
    fields_desc = [
        FlagsField('daq_event_properties', 0, 8, ['x0', 'x1', 'daq', 'stim', 'x4', 'x5', 'x6', 'x7']),
...
class ProgramStartPositiveResponse(Packet):
    fields_desc = [
        ByteField('reserved', 0),
        FlagsField('comm_mode_pgm', 0, 8, ['master_block_mode', 'interleaved_mode', 'x2', 'x3', 'x4', 'x5', 'slave_block_mode', 'x7']),
...
class PgmProcessorPositiveResponse(Packet):
    fields_desc = [
        FlagsField('pgm_properties', 0, 8, ['absolute_mode', 'functional_mode', 'compression_supported', 'compression_required', 'encryption_supported', 'encryption_required', 'non_seq_pgm_supported', 'non_seq_pgm_required']),
        ByteField('max_sector', 0),
    ]


class SectorInfoPositiveResponse(Packet):
    fields_desc = [ByteField('clear_sequence_number', 0), ByteField('program_sequence_number', 0), ByteField('programming_method', 0), XCPEndiannessField(IntField('sector_info', 0))]
...
class EvPacket(Packet):
    ...
    fields_desc = [ByteEnumField('event_code', 0, event_code_enum), StrLenField('event_information_data', b'', max_length=lambda _: get_max_cto() - 2)]
...
class ServPacket(Packet):
    ...
    fields_desc = [ByteEnumField('service_request_code', 0, service_request_code_enum), StrLenField('command_response_data', b'', max_length=lambda _: get_max_cto() - 2)]
