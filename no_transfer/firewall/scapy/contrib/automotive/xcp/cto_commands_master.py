...
from scapy.contrib.automotive.xcp.utils import get_ag, get_max_cto, XCPEndiannessField, StrVarLenField
from scapy.fields import ByteEnumField, ByteField, ShortField, StrLenField, IntField, ThreeBytesField, FlagsField, ConditionalField, XByteField, XIntField, FieldLenField
...
class GetId(Packet):
    ...
    types = {0x00: 'ASCII', 0x01: 'file_name_without_path_and_extension', 0x02: 'file_name_with_path_and_extension', 0x03: 'URL', 0x04: 'File'}
...
class SetRequest(Packet):
    ...
    fields_desc = [FlagsField('mode', 0, 8, ['store_cal_req', 'store_daq_req', 'clear_daq_req', 'x3', 'x4', 'x5', 'x6', 'x7']), XCPEndiannessField(ShortField('session_configuration_id', 0x00))]
...
class GetSeed(Packet):
    ...
    fields_desc = [ByteEnumField('mode', 0, seed_mode), ByteEnumField('resource', 0, res)]


class Unlock(Packet):
    # Send key for unlocking a protected resource
    fields_desc = [FieldLenField('len', None, length_of='seed', fmt='B'), StrVarLenField('seed', b'', length_from=lambda p: p.len, max_length=lambda: get_max_cto() - 2)]
...
class ShortUpload(Packet):
    ...
    fields_desc = [ByteField('nr_of_data_elements', 0), ByteField('reserved', 0), XByteField('address_extension', 0), XCPEndiannessField(IntField('address', 0))]
...
bind_layers(TransportLayerCmd, TransportLayerCmdGetSlaveId, sub_command_code=0xFF)
...
bind_layers(TransportLayerCmd, TransportLayerCmdGetDAQId, sub_command_code=0xFE)
...
class TransportLayerCmdSetDAQId(Packet):
    ...
    fields_desc = [XCPEndiannessField(ShortField('daq_list_number', 0)), XCPEndiannessField(IntField('can_identifier', 0))]


bind_layers(TransportLayerCmd, TransportLayerCmdSetDAQId, sub_command_code=0xFD)
...
class Download(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(StrLenField('alignment', b'', length_from=lambda pkt: get_ag() - 2), lambda pkt: get_ag() > 2),
        StrLenField('data_elements', b'', length_from=lambda pkt: get_max_cto() - 2 if get_ag() == 1 else get_max_cto() - get_ag()),
    ]
...
class DownloadMax(Packet):
    ...
    fields_desc = [ConditionalField(StrLenField('alignment', b'', length_from=lambda _: get_ag() - 1), lambda _: get_ag() > 1), StrLenField('data_elements', b'', length_from=lambda _: get_max_cto() - (get_ag() * 2 - 1))]
...
class ShortDownload(Packet):
    ...
    fields_desc = [
        ...
        StrVarLenField('data_elements', b'', length_from=lambda p: p.len, max_length=lambda: get_max_cto() - 8),
    ]


class ModifyBits(Packet):
    # Modify bits
    fields_desc = [ByteField('shift_value', 0), XCPEndiannessField(ShortField('and_mask', 0)), XCPEndiannessField(ShortField('xor_mask', 0))]
...
class SetCalPage(Packet):
    ...
    fields_desc = [FlagsField('mode', 0, 8, ['ecu', 'xcp', 'x2', 'x3', 'x4', 'x5', 'x6', 'all']), ByteField('data_segment_num', 0), ByteField('data_page_num', 0)]


class GetCalPage(Packet):
    """Get calibration page"""

    fields_desc = [ByteField('access_mode', 0), ByteField('data_segment_num', 0)]
...
class GetSegmentInfo(Packet):
    ...
    info_mode = {0x00: 'get_basic_address_info', 0x01: 'get_standard_info', 0x02: 'get_address_mapping_info'}

    fields_desc = [ByteEnumField('mode', 0x00, info_mode), ByteField('segment_number', 0), ByteField('segment_info', 0), ByteField('mapping_index', 0)]


class GetPageInfo(Packet):
    """Get specific information for a PAGE"""

    fields_desc = [ByteField('reserved', 0), ByteField('segment_number', 0), ByteField('page_number', 0)]


class SetSegmentMode(Packet):
    """Set mode for a SEGMENT"""

    fields_desc = [FlagsField('mode', 0, 8, ['freeze', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7']), ByteField('segment_number', 0)]


class GetSegmentMode(Packet):
    """Get mode for a SEGMENT"""

    fields_desc = [ByteField('reserved', 0), ByteField('segment_number', 0)]
...
class CopyCalPage(Packet):
    ...
    fields_desc = [ByteField('segment_num_src', 0), ByteField('page_num_src', 0), ByteField('segment_num_dst', 0), ByteField('page_num_dst', 0)]


class SetDaqPtr(Packet):
    """Data acquisition and stimulation, static, mandatory"""

    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_list_num', 0)), ByteField('odt_num', 0), ByteField('odt_entry_num', 0)]


class WriteDaq(Packet):
    """Data acquisition and stimulation, static, mandatory"""

    fields_desc = [ByteField('bit_offset', 0), ByteField('size_of_daq_element', 0), ByteField('address_extension', 0), XCPEndiannessField(IntField('address', 0))]
...
class SetDaqListMode(Packet):
    ...
    fields_desc = [
        FlagsField('mode', 0, 8, ['x0', 'direction', 'x2', 'x3', 'timestamp', 'pid_off', 'x6', 'x7']),
...
class GetDaqListMode(Packet):
    ...
    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_list_number', 0))]
...
class StartStopDaqList(Packet):
    ...
    fields_desc = [ByteEnumField('mode', 0, mode_enum), XCPEndiannessField(ShortField('daq_list_number', 0))]
...
class StartStopSynch(Packet):
    ...
    mode_enum = {0x00: 'stop', 0x01: 'start', 0x02: 'select'}
    fields_desc = [ByteEnumField('mode', 0x00, mode_enum)]
...
class GetDaqListInfo(Packet):
    ...
    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_list_num', 0))]


class GetDaqEventInfo(Packet):
    """Get specific information for an event channel"""

    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('event_channel_num', 0))]
...
class AllocDaq(Packet):
    ...
    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_count', 0))]


class AllocOdt(Packet):
    """Allocate ODTs to a DAQ list"""

    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_list_num', 0)), ByteField('odt_count', 0)]


class AllocOdtEntry(Packet):
    """Allocate ODT entries to an ODT"""

    fields_desc = [ByteField('reserved', 0), XCPEndiannessField(ShortField('daq_list_num', 0)), ByteField('odt_num', 0), ByteField('odt_entries_count', 0)]
...
class ProgramClear(Packet):
    ...
    fields_desc = [ByteEnumField('mode', 0, access_mode), XCPEndiannessField(ShortField('reserved', 0)), XCPEndiannessField(IntField('clear_range', 0))]
...
class GetSectorInfo(Packet):
    ...
    fields_desc = [ByteEnumField('mode', 0, address_mode), ByteField('sector_number', 0)]


class ProgramPrepare(Packet):
    """Prepare non-volatile memory programming"""

    fields_desc = [ByteField('not_used', 0), XCPEndiannessField(ShortField('code_size', 0))]


class ProgramFormat(Packet):
    """Set data format before programming"""

    fields_desc = [ByteField('compression_method', 0), ByteField('encryption_mode', 0), ByteField('programming_method', 0), ByteField('access_method', 0)]
...
class ProgramVerify(Packet):
    ...
    start_mode = {0x00: 'request_to_start_internal_routine', 0x01: 'sending_verification_value'}
    fields_desc = [ByteEnumField('verification_mode', 0, start_mode), XCPEndiannessField(ShortField('verification_type', 0)), XCPEndiannessField(IntField('verification_value', 0))]
