...
from scapy.compat import Any, Union, Iterable, Callable, List, Optional, Tuple, Type, cast, Dict, orb, ValuesView
...
class EcuState(object):
    ...
    def _expand(self):
        # type: () -> List[EcuState]
        if self.__cache__ is None or self.__cache__[1] != self.__dict__.values():
            expanded = list()
            for x in itertools.product(*[self._flatten(v) for v in self.__dict__.values()]):
                kwargs = {}
...
class Ecu(object):
    ...
    def sort_key_func(resp):
        ...
        return (service == 0x7F, service, 0xFFFFFFFF - len(resp.states or []), 0xFFFFFFFF - len(resp.key_response))
...
class EcuSession(DefaultSession):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ecu = Ecu(logging=kwargs.pop('logging', True), verbose=kwargs.pop('verbose', True), store_supported_responses=kwargs.pop('store_supported_responses', True))  # noqa: E501
