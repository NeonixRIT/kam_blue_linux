...
from scapy.compat import Any, Union, List, Optional, Iterable, Dict, Tuple, Set, Callable, cast, NamedTuple, orb
...
from scapy.contrib.automotive.scanner.test_case import AutomotiveTestCase, StateGenerator, _SocketUnion, _TransitionTuple
...
_AutomotiveTestCaseScanResult = NamedTuple('_AutomotiveTestCaseScanResult', [('state', EcuState), ('req', Packet), ('resp', Optional[Packet]), ('req_ts', Union[EDecimal, float]), ('resp_ts', Optional[Union[EDecimal, float]])])

_AutomotiveTestCaseFilteredScanResult = NamedTuple('_AutomotiveTestCaseFilteredScanResult', [('state', EcuState), ('req', Packet), ('resp', Packet), ('req_ts', Union[EDecimal, float]), ('resp_ts', Union[EDecimal, float])])
...
class ServiceEnumerator(AutomotiveTestCase):
    ...
    _supported_kwargs_doc = (
        AutomotiveTestCase._supported_kwargs_doc
        + """
    ...
    def execute(self, socket, state, **kwargs):
        ...
        log_automotive.debug('Start execution of enumerator: %s', time.ctime(start_time))
    ...
    def _evaluate_ecu_state_modifications(
        self,
        state,  # type: EcuState
        request,  # type: Packet
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]  # noqa: E501
        ...
        if exit_if_service_not_supported and response.service == 0x7F:
            ...
            if response_code in [0x11, 0x7F]:
                names = {0x11: 'serviceNotSupported', 0x7F: 'serviceNotSupportedInActiveSession'}
    ...
    def _evaluate_retry(
        ...
        **kwargs,  # type: Optional[Dict[str, Any]]
        ...
        if retry_if_busy_returncode and response.service == 0x7F and self._get_negative_response_code(response) == 0x21:
    ...
    def _compute_statistics(self):
        ...
        for desc, data in data_sets:
            ...
            answertimes_nr = [float(x.resp_ts) - float(x.req_ts) for x in answered if x.resp.service == 0x7F]
            answertimes_pr = [float(x.resp_ts) - float(x.req_ts) for x in answered if x.resp.service != 0x7F]
    ...
    def results_with_negative_response(self):
        ...
        """
        return [r for r in self.results_with_response if r.resp and r.resp.service == 0x7F]
    ...
    def results_with_positive_response(self):
        ...
        return [
            r
            for r in self.results_with_response  # noqa: E501
    ...
    def supported_responses(self):
        ...
        all_responses = [p for p in self._result_packets.values() if orb(bytes(p)[0]) & 0x40]
