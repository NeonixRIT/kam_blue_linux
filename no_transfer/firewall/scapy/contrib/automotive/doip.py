...
from scapy.fields import ByteEnumField, ConditionalField, XByteField, XShortField, XIntField, XShortEnumField, XByteEnumField, IntField, StrFixedLenField, XStrField
...
class DoIP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteEnumField('nack', 0, {0: 'Incorrect pattern format', 1: 'Unknown payload type', 2: 'Message too large', 3: 'Out of memory', 4: 'Invalid payload length'}), lambda p: p.payload_type in [0x0]),
        ConditionalField(StrFixedLenField('vin', b'', 17), lambda p: p.payload_type in [3, 4]),
        ConditionalField(XShortField('logical_address', 0), lambda p: p.payload_type in [4]),
        ConditionalField(StrFixedLenField('eid', b'', 6), lambda p: p.payload_type in [2, 4]),
        ConditionalField(StrFixedLenField('gid', b'', 6), lambda p: p.payload_type in [4]),
        ConditionalField(
            XByteEnumField(
                'further_action',
                0,
        ...
        ConditionalField(XShortField('source_address', 0), lambda p: p.payload_type in [5, 8, 0x8001, 0x8002, 0x8003]),  # noqa: E501
        ConditionalField(XByteEnumField('activation_type', 0, {0: 'Default', 1: 'WWH-OBD', 0xE0: 'Central security', 0x16: 'Default', 0x116: 'Diagnostic', 0xE016: 'Central security'}), lambda p: p.payload_type in [5]),
        ConditionalField(XShortField('logical_address_tester', 0), lambda p: p.payload_type in [6]),
        ConditionalField(XShortField('logical_address_doip_entity', 0), lambda p: p.payload_type in [6]),
        ConditionalField(
            XByteEnumField(
        ...
        ConditionalField(XIntField('reserved_iso', 0), lambda p: p.payload_type in [5, 6]),
        ConditionalField(XStrField('reserved_oem', b''), lambda p: p.payload_type in [5, 6]),
        ConditionalField(XByteEnumField('diagnostic_power_mode', 0, {0: 'not ready', 1: 'ready', 2: 'not supported'}), lambda p: p.payload_type in [0x4004]),
        ConditionalField(ByteEnumField('node_type', 0, {0: 'DoIP gateway', 1: 'DoIP node'}), lambda p: p.payload_type in [0x4002]),
        ConditionalField(XByteField('max_open_sockets', 1), lambda p: p.payload_type in [0x4002]),
        ConditionalField(XByteField('cur_open_sockets', 0), lambda p: p.payload_type in [0x4002]),
        ConditionalField(IntField('max_data_size', 0), lambda p: p.payload_type in [0x4002]),
        ConditionalField(XShortField('target_address', 0), lambda p: p.payload_type in [0x8001, 0x8002, 0x8003]),  # noqa: E501
        ConditionalField(XByteEnumField('ack_code', 0, {0: 'ACK'}), lambda p: p.payload_type in [0x8002]),
        ConditionalField(
            ByteEnumField(
                'nack_code',
                0,
        ...
        ConditionalField(XStrField('previous_msg', b''), lambda p: p.payload_type in [0x8002, 0x8003]),
    ]
    ...
    def answers(self, other):
        ...
        if isinstance(other, type(self)):
            ...
            matches = [(4, 1), (4, 2), (4, 3), (6, 5), (8, 7), (0x4002, 0x4001), (0x4004, 0x4003), (0x8001, 0x8001), (0x8003, 0x8001)]
...
class DoIPSocket(StreamSocket):
    ...
    def __init__(self, ip='127.0.0.1', port=13400, activate_routing=True, source_address=0xE80, target_address=0, activation_type=0, reserved_oem=b''):
    ...
    def _activate_routing(
        ...
        resp = self.sr1(DoIP(payload_type=0x5, activation_type=activation_type, source_address=source_address, reserved_oem=reserved_oem), verbose=False, timeout=1)
        if resp and resp.payload_type == 0x6 and resp.routing_activation_response == 0x10:
...
class DoIPSocket6(DoIPSocket):
    ...
    def __init__(self, ip='::1', port=13400, activate_routing=True, source_address=0xE80, target_address=0, activation_type=0, reserved_oem=b''):
        # type: (str, int, bool, int, int, int, bytes) -> None
        self.ip = ip
        self.port = port
        self.source_address = source_address
...
class UDS_DoIPSocket(DoIPSocket):
    ...
    def send(self, x):
        ...
        if isinstance(x, UDS):
            pkt = DoIP(payload_type=0x8001, source_address=self.source_address, target_address=self.target_address) / x
        else:
            pkt = x
