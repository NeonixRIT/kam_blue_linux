...
from scapy.fields import ByteField, XByteField, BitEnumField, PacketListField, XBitField, XByteEnumField, FieldListField, FieldLenField
...
class OBD_NR(Packet):
    ...
    fields_desc = [XByteField('request_service_id', 0), XByteEnumField('response_code', 0, responses)]

    def answers(self, other):
        return self.request_service_id == other.service and (self.response_code != 0x78 or conf.contribs['OBD']['treat-response-pending-as-answer'])


class OBD_S01(Packet):
    name = 'S1_CurrentData'
    fields_desc = [FieldListField('pid', [], XByteField('', 0))]


class OBD_S02_Record(OBD_Packet):
    fields_desc = [XByteField('pid', 0), ByteField('frame_no', 0)]


class OBD_S02(Packet):
    name = 'S2_FreezeFrameData'
    fields_desc = [PacketListField('requests', [], OBD_S02_Record)]
...
class OBD_S03_PR(Packet):
    ...
    fields_desc = [FieldLenField('count', None, count_of='dtcs', fmt='B'), PacketListField('dtcs', [], OBD_DTC, count_from=lambda pkt: pkt.count)]
...
class OBD_S06(Packet):
    ...
    fields_desc = [FieldListField('mid', [], XByteField('', 0))]
...
class OBD_S08(Packet):
    ...
    fields_desc = [FieldListField('tid', [], XByteField('', 0))]


class OBD_S09(Packet):
    name = 'S9_VehicleInformation'
    fields_desc = [FieldListField('iid', [], XByteField('', 0))]
