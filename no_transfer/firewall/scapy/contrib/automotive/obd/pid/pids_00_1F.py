...
from scapy.fields import BitEnumField, BitField, ScalingField, FlagsField, XByteEnumField, PacketField
...
class OBD_PID01(OBD_Packet):
    ...
    fields_desc = [
        ...
        FlagsField('continuous_tests_ready', 0, 3, ['misfire', 'fuelSystem', 'components']),
        BitField('reserved2', 0, 1),
        FlagsField('continuous_tests_supported', 0, 3, ['misfire', 'fuel_system', 'components']),
        FlagsField('once_per_trip_tests_supported', 0, 8, ['egr', 'oxygenSensorHeater', 'oxygenSensor', 'acSystemRefrigerant', 'secondaryAirSystem', 'evaporativeSystem', 'heatedCatalyst', 'catalyst']),
        FlagsField('once_per_trip_tests_ready', 0, 8, ['egr', 'oxygenSensorHeater', 'oxygenSensor', 'acSystemRefrigerant', 'secondaryAirSystem', 'evaporativeSystem', 'heatedCatalyst', 'catalyst']),
    ]


class OBD_PID02(OBD_Packet):
    name = 'PID_02_FreezeDtc'
    fields_desc = [PacketField('dtc', b'', OBD_DTC)]


class OBD_PID03(OBD_Packet):
    name = 'PID_03_FuelSystemStatus'

    loopStates = {0x00: 'OpenLoopInsufficientEngineTemperature', 0x02: 'ClosedLoop', 0x04: 'OpenLoopEngineLoadOrFuelCut', 0x08: 'OpenLoopDueSystemFailure', 0x10: 'ClosedLoopWithFault'}

    fields_desc = [XByteEnumField('fuel_system1', 0, loopStates), XByteEnumField('fuel_system2', 0, loopStates)]


class OBD_PID04(OBD_Packet):
    name = 'PID_04_CalculatedEngineLoad'
    fields_desc = [ScalingField('data', 0, scaling=100 / 255.0, unit='%')]


class OBD_PID05(OBD_Packet):
    name = 'PID_05_EngineCoolantTemperature'
    fields_desc = [ScalingField('data', 0, unit='deg. C', offset=-40.0)]


class OBD_PID06(OBD_Packet):
    name = 'PID_06_ShortTermFuelTrimBank1'
    fields_desc = [ScalingField('data', 0, scaling=100 / 128.0, unit='%', offset=-100.0)]
...
class OBD_PID0A(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=3, unit='kPa')]


class OBD_PID0B(OBD_Packet):
    name = 'PID_0B_IntakeManifoldAbsolutePressure'
    fields_desc = [ScalingField('data', 0, unit='kPa')]


class OBD_PID0C(OBD_Packet):
    name = 'PID_0C_EngineRpm'
    fields_desc = [ScalingField('data', 0, scaling=1 / 4.0, unit='min-1', fmt='H')]


class OBD_PID0D(OBD_Packet):
    name = 'PID_0D_VehicleSpeed'
    fields_desc = [ScalingField('data', 0, unit='km/h')]


class OBD_PID0E(OBD_Packet):
    name = 'PID_0E_TimingAdvance'
    fields_desc = [ScalingField('data', 0, scaling=1 / 2.0, unit='deg.', offset=-64.0)]
...
class OBD_PID10(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=1 / 100.0, unit='g/s')]
...
class OBD_PID12(OBD_Packet):
    ...
    states = {0x00: 'upstream', 0x02: 'downstreamCatalyticConverter', 0x04: 'outsideAtmosphereOrOff', 0x08: 'pumpCommanded'}
...
class OBD_PID13(OBD_Packet):
    ...
    fields_desc = [FlagsField('sensors_present', b'', 8, ['Bank1Sensor1', 'Bank1Sensor2', 'Bank1Sensor3', 'Bank1Sensor4', 'Bank2Sensor1', 'Bank2Sensor2', 'Bank2Sensor3', 'Bank2Sensor4'])]


class _OBD_PID14_1B(OBD_Packet):
    fields_desc = [ScalingField('outputVoltage', 0, scaling=0.005, unit='V'), ScalingField('trim', 0, scaling=100 / 128.0, unit='%', offset=-100)]
...
class OBD_PID1C(OBD_Packet):
    ...
    fields_desc = [XByteEnumField('data', 0, obdStandards)]


class OBD_PID1D(OBD_Packet):
    name = 'PID_1D_OxygenSensorsPresent'
    fields_desc = [FlagsField('sensors_present', 0, 8, ['Bank1Sensor1', 'Bank1Sensor2', 'Bank2Sensor1', 'Bank2Sensor2', 'Bank3Sensor1', 'Bank3Sensor2', 'Bank4Sensor1', 'Bank4Sensor2'])]


class OBD_PID1E(OBD_Packet):
    name = 'PID_1E_AuxiliaryInputStatus'
    fields_desc = [BitField('reserved', 0, 7), BitEnumField('pto_status', 0, 1, OBD_PID01.onOff)]


class OBD_PID1F(OBD_Packet):
    name = 'PID_1F_RunTimeSinceEngineStart'
    fields_desc = [ScalingField('data', 0, unit='s', fmt='H')]
