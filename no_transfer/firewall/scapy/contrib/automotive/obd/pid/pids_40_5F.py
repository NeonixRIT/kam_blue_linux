...
from scapy.fields import ByteEnumField, BitField, FlagsField, XByteField, ScalingField, ThreeBytesField
...
class OBD_PID41(OBD_Packet):
    ...
    fields_desc = [
        ...
        FlagsField('continuous_tests_supported', 0, 3, ['misfire', 'fuelSystem', 'components']),
...
class OBD_PID42(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=0.001, unit='V', fmt='H')]


class OBD_PID43(OBD_Packet):
    name = 'PID_43_AbsoluteLoadValue'
    fields_desc = [ScalingField('data', 0, scaling=100 / 255.0, unit='%', fmt='H')]


class OBD_PID44(OBD_Packet):
    name = 'PID_44_FuelAirCommandedEquivalenceRatio'
    fields_desc = [ScalingField('data', 0, scaling=0.0000305, fmt='H')]
...
class OBD_PID4D(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, unit='min', fmt='H')]
...
class OBD_PID4F(OBD_Packet):
    ...
    fields_desc = [ScalingField('equivalence_ratio', 0), ScalingField('sensor_voltage', 0, unit='V'), ScalingField('sensor_current', 0, unit='mA'), ScalingField('intake_manifold_absolute_pressure', 0, scaling=10, unit='kPa')]


class OBD_PID50(OBD_Packet):
    name = 'PID_50_MaximumValueForAirFlowRateFromMassAirFlowSensor'
    fields_desc = [ScalingField('data', 0, scaling=10, unit='g/s'), ThreeBytesField('reserved', 0)]
...
class OBD_PID51(OBD_Packet):
    ...
    fields_desc = [ByteEnumField('data', 0, fuelTypes)]
...
class OBD_PID53(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=1 / 200.0, unit='kPa', fmt='H')]


class OBD_PID54(OBD_Packet):
    name = 'PID_54_EvapSystemVaporPressure'
    fields_desc = [ScalingField('data', 0, unit='Pa', fmt='h')]


class _OBD_SensorTrimPacket1(OBD_Packet):
    fields_desc = [ScalingField('bank1', 0, scaling=100 / 128.0, offset=-100, unit='%'), ScalingField('bank3', 0, scaling=100 / 128.0, offset=-100, unit='%')]


class _OBD_SensorTrimPacket2(OBD_Packet):
    fields_desc = [ScalingField('bank2', 0, scaling=100 / 128.0, offset=-100, unit='%'), ScalingField('bank4', 0, scaling=100 / 128.0, offset=-100, unit='%')]
...
class OBD_PID59(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=10, unit='kPa', fmt='H')]
...
class OBD_PID5D(OBD_Packet):
    ...
    fields_desc = [ScalingField('data', 0, scaling=1 / 128.0, offset=-210, unit='deg.', fmt='H')]


class OBD_PID5E(OBD_Packet):
    name = 'PID_5E_EngineFuelRate'
    fields_desc = [ScalingField('data', 0, scaling=0.05, unit='L/h', fmt='H')]
...
class OBD_PID5F(OBD_Packet):
    ...
    fields_desc = [ByteEnumField('data', 0, emissionRequirementTypes)]
