...
from scapy.fields import LEShortField, LEShortEnumField, LEIntEnumField, LEIntField, LELongField, FieldLenField, PacketListField, ByteField, PacketField, MultipleTypeField, StrLenField, StrFixedLenField, XLEIntField, XLEStrLenField
...
class EncapsulatedPacket(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('item', None, ItemData, count_from=lambda pkt: pkt.itemCount),
    ]
...
class ENIPTCP(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                # List Services Reply
                (PacketField('commandSpecificData', ENIPListServicesReply, ENIPListServicesReply), lambda pkt: pkt.commandId == 0x4),
                # List Identity Reply
                (PacketField('commandSpecificData', ENIPListIdentityReply, ENIPListIdentityReply), lambda pkt: pkt.commandId == 0x63),
                # List Interfaces Reply
                (PacketField('commandSpecificData', ENIPListInterfacesReply, ENIPListInterfacesReply), lambda pkt: pkt.commandId == 0x64),
                # Register Session
                (PacketField('commandSpecificData', ENIPRegisterSession, ENIPRegisterSession), lambda pkt: pkt.commandId == 0x65),
                # Send RR Data
                (PacketField('commandSpecificData', ENIPSendRRData, ENIPSendRRData), lambda pkt: pkt.commandId == 0x6F),
                # Send Unit Data
                (PacketField('commandSpecificData', ENIPSendUnitData, ENIPSendUnitData), lambda pkt: pkt.commandId == 0x70),
            ],
            PacketField('commandSpecificData', None, CommandSpecificData),  # By default
        ),
    ]
