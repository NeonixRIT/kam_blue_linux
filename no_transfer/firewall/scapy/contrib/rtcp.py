...
_rtcp_packet_types = {200: 'Sender report', 201: 'Receiver report', 202: 'Source description', 203: 'BYE', 204: 'APP'}


class SenderInfo(Packet):
    name = 'Sender info'
    fields_desc = [LongField('ntp_timestamp', None), IntField('rtp_timestamp', None), IntField('sender_packet_count', None), IntField('sender_octet_count', None)]
...
_sdes_chunk_types = {0: 'END', 1: 'CNAME', 2: 'NAME', 3: 'EMAIL', 4: 'PHONE', 5: 'LOC', 6: 'TOOL', 7: 'NOTE', 8: 'PRIV'}


class SDESItem(Packet):
    name = 'SDES item'
    fields_desc = [ByteEnumField('chunk_type', None, _sdes_chunk_types), FieldLenField('length', None, fmt='!b', length_of='value'), StrLenField('value', None, length_from=lambda pkt: pkt.length)]
...
class SDESChunk(Packet):
    name = 'SDES chunk'
    fields_desc = [IntField('sourcesync', None), PacketListField('items', None, pkt_cls=SDESItem)]
