...
from scapy.fields import ConditionalField, EnumField, Field, FieldLenField, FlagsField, IEEEDoubleField, IEEEFloatField, IntEnumField, IntField, LongField, PacketListField, SignedIntField, StrLenField, X3BytesField, XByteField, XIntField
...
def GuessAvpType(p, **kargs):
    if len(p) > AVP_Code_length + AVP_Flag_length:
        ...
        vnd = bool(struct.unpack('!B', p[AVP_Code_length : AVP_Code_length + AVP_Flag_length])[0] & 128)  # noqa: E501
        ...
        try:
            for vnd in AvpDefDict:
                for code in AvpDefDict[vnd]:
                    ...
                    if val[0][: len(avpId)] == avpId:  # A prefix of the full name is considered valid  # noqa: E501
                        raise
            found = False
        except BaseException:
            found = True
...
class AVP_FL_NV(AVP_Generic):
    ...
    fields_desc = [IntField('avpCode', None), AVPFlags('avpFlags', None, 8, AVP_Flags_List), X3BytesField('avpLen', None)]


class AVP_FL_V(AVP_Generic):
    """Defines the AVP of Fixed Length with Vendor field."""

    fields_desc = [IntField('avpCode', None), AVPFlags('avpFlags', None, 8, AVP_Flags_List), X3BytesField('avpLen', None), AVPVendor('avpVnd', 0)]


class AVP_VL_NV(AVP_Generic):
    """Defines the AVP of Variable Length with No Vendor field."""

    fields_desc = [IntField('avpCode', None), AVPFlags('avpFlags', None, 8, AVP_Flags_List), I3FieldLenField('avpLen', None, length_of='val', adjust=lambda pkt, x: x + 8)]


class AVP_VL_V(AVP_Generic):
    """Defines the AVP of Variable Length with Vendor field."""

    fields_desc = [IntField('avpCode', None), AVPFlags('avpFlags', None, 8, AVP_Flags_List), I3FieldLenField('avpLen', None, length_of='val', adjust=lambda pkt, x: x + 12), AVPVendor('avpVnd', 0)]
...
class AVP_Unknown(AVP_Generic):
    ...
    fields_desc = [
        ...
        I3FieldLenField('avpLen', None, length_of='val', adjust=lambda pkt, x: x + 8 + ((pkt.avpFlags & 0x80) >> 5)),  # noqa: E501
        ConditionalField(AVPVendor('avpVnd', 0), lambda pkt: pkt.avpFlags & 0x80),  # noqa: E501
        StrLenField('val', None, length_from=lambda pkt: pkt.avpLen - 8 - ((pkt.avpFlags & 0x80) >> 5)),  # noqa: E501
    ]
...
class AVPV_StrLenField(AVP_VL_V):
    fields_desc = [AVP_VL_V, StrLenField('val', None, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_StrLenField(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, StrLenField('val', None, length_from=lambda pkt: pkt.avpLen - 8)]


class AVPV_OctetString(AVP_VL_V):
    fields_desc = [AVP_VL_V, OctetString('val', None, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_OctetString(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, OctetString('val', None, length_from=lambda pkt: pkt.avpLen - 8)]


class AVPV_Grouped(AVP_VL_V):
    fields_desc = [AVP_VL_V, PacketListField('val', [], GuessAvpType, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_Grouped(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, PacketListField('val', [], GuessAvpType, length_from=lambda pkt: pkt.avpLen - 8)]
...
class AVPV_Address(AVP_VL_V):
    fields_desc = [AVP_VL_V, Address('val', None, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_Address(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, Address('val', None, length_from=lambda pkt: pkt.avpLen - 8)]


class AVPV_IPFilterRule(AVP_VL_V):
    fields_desc = [AVP_VL_V, IPFilterRule('val', None, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_IPFilterRule(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, IPFilterRule('val', None, length_from=lambda pkt: pkt.avpLen - 8)]


class AVPV_QoSFilterRule(AVP_VL_V):
    fields_desc = [AVP_VL_V, QoSFilterRule('val', None, length_from=lambda pkt: pkt.avpLen - 12)]


class AVPNV_QoSFilterRule(AVP_VL_NV):
    fields_desc = [AVP_VL_NV, QoSFilterRule('val', None, length_from=lambda pkt: pkt.avpLen - 8)]
...
class AVP_10415_701(AVP_VL_V):
    name = 'AVP MSISDN'
    fields_desc = [AVP_VL_V, ISDN('val', None, length_from=lambda pkt: pkt.avpLen - 12)]
...
class AVP_10415_652(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'NOT_PRIVILEDGED_SENDER',
                1: 'PRIVILEDGED_SENDER',
            },
        ),
    ]
...
class AVP_10415_710(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'USER_DATA_NOT_REQUESTED',
                1: 'USER_DATA_REQUESTED',
            },
        ),
    ]  # noqa: E501
...
class AVP_10415_712(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'ONE_TIME_NOTIFICATION_REQUESTED',
            },
        ),
    ]  # noqa: E501
...
class AVP_10415_717(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'PREPAGING_NOT_SUPPORTED',
                1: 'PREPAGING_SUPPORTED',
            },
        ),
    ]  # noqa: E501
...
class AVP_10415_718(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'ONLY_LOCAL_TIME_ZONE_REQUESTED',
                1: 'LOCAL_TIME_ZONE_WITH_LOCATION_INFO_REQUESTED',
            },
        ),
    ]  # noqa: E501
...
class AVP_10415_1650(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            {
                0: 'NO_ADJUSTMENT',
                1: 'PLUS_ONE_HOUR_ADJUSTMENT',
                2: 'PLUS_TWO_HOURS_ADJUSTMENT',
            },
        ),
    ]  # noqa: E501
...
class AVP_10415_2507(AVP_FL_V):
    ...
    fields_desc = [
        ...
        Enumerated(
            ...
            None,
            {
                0: 'VERTICAL_COORDINATE_IS_NOT REQUESTED',
...
class DiamG(Packet):
    ...
    fields_desc = [
        ...
        I3FieldLenField('drLen', None, length_of='avpList', adjust=lambda p, x: x + DR_Header_Length),
        ...
        XIntField('drEtEId', 0),
        PacketListField('avpList', [], GuessAvpType, length_from=lambda pkt: pkt.drLen - DR_Header_Length),
    ]
...
def getCmdParams(cmd, request, **fields):
    ...
    if isinstance(cmd, int):
        ...
        for k, f in DR_cmd_def.items():
            if f[fpos][: len(cmd)] == cmd:  # Accept only a prefix of the full name
                drCode = k
                params = f
                break
        if not drCode:
