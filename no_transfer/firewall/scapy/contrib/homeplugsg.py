...
HomePlugSGTypes = {0xA400: 'VS_UART_CMD_Req', 0xA401: 'VS_UART_CMD_Cnf'}
...
class VS_UART_CMD_REQ(Packet):
    ...
    fields_desc = [FieldLenField('UDataLen', None, count_of='UData', fmt='H'), StrLenField('UData', 'UartCommand\x00', length_from=lambda pkt: pkt.UDataLen)]


class VS_UART_CMD_CNF(Packet):
    name = 'VS_UART_CMD_CNF'
    fields_desc = [StrFixedLenField('reserved', b'\x00', 6), FieldLenField('UDataLen', None, count_of='UData', fmt='H'), StrLenField('UData', 'UartCommand\x00', length_from=lambda pkt: pkt.UDataLen)]
