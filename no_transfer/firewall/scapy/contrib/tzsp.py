...
class TZSP(Packet):
    ...
    ENCAPSULATED_PROTOCOLS = {ENCAPSULATED_ETHERNET: 'ETHERNET', ENCAPSULATED_IEEE_802_11: 'IEEE 802.11', ENCAPSULATED_PRISM_HEADER: 'PRISM HEADER', ENCAPSULATED_WLAN_AVS: 'WLAN AVS'}

    ENCAPSULATED_PROTOCOL_CLASSES = {ENCAPSULATED_ETHERNET: Ether, ENCAPSULATED_IEEE_802_11: Dot11, ENCAPSULATED_PRISM_HEADER: PrismHeader, ENCAPSULATED_WLAN_AVS: AVSWLANHeader}
    ...
    def get_encapsulated_payload_class(self):
        ...
        try:
            ...
            warning('unknown or invalid encapsulation type (%i) - returning payload as raw()' % self.encapsulated_protocol)  # noqa: E501
            return Raw
...
class TZSPTagRawRSSIByte(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_RAW_RSSI, _TZSPTag.TAG_TYPES), ByteField('len', 1), ByteField('raw_rssi', 0)]
...
class TZSPTagRawRSSIShort(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_RAW_RSSI, _TZSPTag.TAG_TYPES), ByteField('len', 2), ShortField('raw_rssi', 0)]
...
class TZSPTagSNRByte(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_SNR, _TZSPTag.TAG_TYPES), ByteField('len', 1), ByteField('snr', 0)]
...
class TZSPTagSNRShort(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_SNR, _TZSPTag.TAG_TYPES), ByteField('len', 2), ShortField('snr', 0)]
...
class TZSPTagDataRate(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_DATA_RATE, _TZSPTag.TAG_TYPES), ByteField('len', 1), ByteEnumField('data_rate', DATA_RATE_UNKNOWN, DATA_RATES)]
...
class TZSPTagTimestamp(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_TIMESTAMP, _TZSPTag.TAG_TYPES), ByteField('len', 4), IntField('timestamp', 0)]
...
class TZSPTagDecrypted(_TZSPTag):
    ...
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_DECRYPTED, _TZSPTag.TAG_TYPES), ByteField('len', 1), YesNoByteField('decrypted', NO, config={'yes': YES, 'no': (NO, 0xFF)})]
...
class TZSPTagUnknown(_TZSPTag):
    ...
    fields_desc = [ByteField('type', 0xFF), FieldLenField('len', None, length_of='data', fmt='b'), XStrLenField('data', '', length_from=lambda pkt: pkt.len)]
