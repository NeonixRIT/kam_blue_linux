...
from scapy.fields import BitField, ByteField, FieldLenField, PacketField, PacketLenField, ShortField, StrFixedLenField, StrLenField, UTCTimeField
...
class ICMPv6NDOptNonce(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField('type', 14),
        FieldLenField('len', None, length_of='nonce', fmt='B', adjust=lambda pkt, x: int(round((x + 2) / 8.0))),  # noqa: E501
...
class ICMPv6NDOptTmstp(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 13), ByteField('len', 2), BitField('reserved', 0, 48), UTCTimeField('timestamp', None)]
...
class CGA_Params(Packet):
    ...
    fields_desc = [StrFixedLenField('modifier', RandBin(size=16), length=16), StrFixedLenField('subprefix', '', length=8), ByteField('cc', 0), PacketField('pubkey', X509_SubjectPublicKeyInfo(), X509_SubjectPublicKeyInfo)]
...
send_icmp6ndoptscls = {11: ICMPv6NDOptCGA, 12: ICMPv6NDOptRsaSig, 13: ICMPv6NDOptTmstp, 14: ICMPv6NDOptNonce}
