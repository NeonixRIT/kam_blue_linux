...
class Uberlogger_honeypot_caract(Packet):
    ...
    fields_desc = [ByteField('honeypot_id', 0), ByteField('reserved', 0), ByteField('os_type_and_version', 0)]
...
class Uberlogger_capset_data(Packet):
    ...
    fields_desc = [IntField('target_pid', 0), IntField('effective_cap', 0), IntField('permitted_cap', 0), IntField('inheritable_cap', 0)]
...
class Uberlogger_chown_data(Packet):
    ...
    fields_desc = [IntField('uid', 0), IntField('gid', 0)]


class Uberlogger_open_data(Packet):
    name = 'Uberlogger open_data'
    fields_desc = [IntField('flags', 0), IntField('mode', 0)]


class Uberlogger_read_data(Packet):
    name = 'Uberlogger read_data'
    fields_desc = [IntField('fd', 0), IntField('count', 0)]
