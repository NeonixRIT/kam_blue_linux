...
class GTPHeader(gtp.GTPHeader):
    ...
    fields_desc = [
        ...
        ConditionalField(XIntField('teid', 0), lambda pkt: pkt.T == 1),
        ThreeBytesField('seq', RandShort()),
        ConditionalField(BitField('msg_priority', 0, 4), lambda pkt: pkt.MP == 1),
        ConditionalField(MultipleTypeField([(BitField('SPARE3', 0, 4), lambda pkt: pkt.MP == 1)], ByteField('SPARE3', 0)), lambda pkt: pkt.MP in [0, 1]),
    ]
...
class IE_IP_Address(gtp.IE_Base):
    name = 'IE IP Address'
    fields_desc = [
        ByteEnumField('ietype', 74, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        ConditionalField(IPField('address', RandIP()), lambda pkt: pkt.length == 4),
        ConditionalField(IP6Field('address6', None), lambda pkt: pkt.length == 16),
    ]
...
class IE_MEI(gtp.IE_Base):
    name = 'IE MEI'
    fields_desc = [ByteEnumField('ietype', 75, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), gtp.TBCDByteField('MEI', '175675478970685', length_from=lambda x: x.length)]
...
class IE_EPSBearerID(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 73, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteField('EBI', 0)]


class IE_RAT(gtp.IE_Base):
    name = 'IE RAT'
    fields_desc = [ByteEnumField('ietype', 82, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteEnumField('RAT_type', None, RATType)]


class IE_ServingNetwork(gtp.IE_Base):
    name = 'IE Serving Network'
    fields_desc = [ByteEnumField('ietype', 83, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), gtp.TBCDByteField('MCC', '', 2), gtp.TBCDByteField('MNC', '', 1)]
...
class IE_ULI(gtp.IE_Base):
    ...
    fields_desc = [
        ...
        ConditionalField(PacketField('CGI', 0, ULI_CGI), lambda pkt: bool(pkt.CGI_Present)),
        ConditionalField(PacketField('SAI', 0, ULI_SAI), lambda pkt: bool(pkt.SAI_Present)),
        ConditionalField(PacketField('RAI', 0, ULI_RAI), lambda pkt: bool(pkt.RAI_Present)),
        ConditionalField(PacketField('TAI', 0, ULI_TAI), lambda pkt: bool(pkt.TAI_Present)),
        ConditionalField(PacketField('ECGI', 0, ULI_ECGI), lambda pkt: bool(pkt.ECGI_Present)),
        ConditionalField(PacketField('LAI', 0, ULI_LAI), lambda pkt: bool(pkt.LAI_Present)),
    ]


class IE_ULI_Timestamp(gtp.IE_Base):
    name = 'IE ULI Timestamp'
    fields_desc = [ByteEnumField('ietype', 170, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), XIntField('timestamp', 0)]
...
class IE_UCI(gtp.IE_Base):
    name = 'IE UCI'
    fields_desc = [
        ByteEnumField('ietype', 145, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        gtp.TBCDByteField('MCC', '', 2),
...
class IE_FTEID(gtp.IE_Base):
    name = 'IE F-TEID'
    fields_desc = [
        ByteEnumField('ietype', 87, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        ...
        XIntField('GRE_Key', 0),
        ConditionalField(IPField('ipv4', RandIP()), lambda pkt: pkt.ipv4_present),
...
class IE_BearerContext(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 93, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), PacketListField('IE_list', None, IE_Dispatcher, length_from=lambda pkt: pkt.length)]


class IE_BearerFlags(gtp.IE_Base):
    name = 'IE Bearer Flags'
    fields_desc = [
        ByteEnumField('ietype', 97, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        BitField('SPARE', 0, 4),
...
class IE_MMContext_EPS(gtp.IE_Base):
    ...
    fields_desc = [
        ByteEnumField('ietype', 107, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        ...
        ConditionalField(StrLenField('fields', '', length_from=lambda x: x.length - 41), lambda pkt: pkt.length > 40),
    ]


class IE_PDNConnection(gtp.IE_Base):
    name = 'IE PDN Connection'
    fields_desc = [ByteEnumField('ietype', 109, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), PacketListField('IE_list', None, IE_Dispatcher, length_from=lambda pkt: pkt.length)]


class IE_FQDN(gtp.IE_Base):
    name = 'IE FQDN'
    fields_desc = [ByteEnumField('ietype', 136, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteField('fqdn_tr_bit', 0), StrLenField('fqdn', '', length_from=lambda x: x.length - 1)]


class IE_NotImplementedTLV(gtp.IE_Base):
    name = 'IE not implemented'
    fields_desc = [ByteEnumField('ietype', 0, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), StrLenField('data', '', length_from=lambda x: x.length)]


class IE_IMSI(gtp.IE_Base):
    name = 'IE IMSI'
    fields_desc = [ByteEnumField('ietype', 1, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), gtp.TBCDByteField('IMSI', '33607080910', length_from=lambda x: x.length)]
...
class IE_RecoveryRestart(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 3, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteField('restart_counter', 0)]


class IE_APN(gtp.IE_Base):
    name = 'IE APN'
    fields_desc = [ByteEnumField('ietype', 71, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), gtp.APNStrLenField('APN', 'internet', length_from=lambda x: x.length)]


class IE_BearerTFT(gtp.IE_Base):
    name = 'IE Bearer TFT'
    fields_desc = [ByteEnumField('ietype', 84, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), StrLenField('Bearer_TFT', '', length_from=lambda x: x.length)]


class IE_AMBR(gtp.IE_Base):
    name = 'IE AMBR'
    fields_desc = [ByteEnumField('ietype', 72, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), IntField('AMBR_Uplink', 0), IntField('AMBR_Downlink', 0)]


class IE_MSISDN(gtp.IE_Base):
    name = 'IE MSISDN'
    fields_desc = [ByteEnumField('ietype', 76, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), gtp.TBCDByteField('digits', '33123456789', length_from=lambda x: x.length)]
...
class IE_Indication(gtp.IE_Base):
    ...
    fields_desc = [
        ...
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        ConditionalField(BitField('DAF', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('DTF', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('HI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('DFI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('OI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('ISRSI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('ISRAI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('SGWCI', 0, 1), lambda pkt: pkt.length > 0),
        ConditionalField(BitField('SQCI', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('UIMSI', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('CFSI', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('CRSI', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('PS', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('PT', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('SI', 0, 1), lambda pkt: pkt.length > 1),
        ConditionalField(BitField('MSV', 0, 1), lambda pkt: pkt.length > 1),
        ...
        ConditionalField(BitField('NSI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('UASI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('DTCI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('BDWI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('PSCI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('PCRI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('AOSI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('AOPI', 0, 1), lambda pkt: pkt.length > 4),
        ConditionalField(BitField('ROAAI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('EPCOSI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('CPOPCI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('PMTSMI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('S11TF', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('PNSI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('UNACCSI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('WPMSI', 0, 1), lambda pkt: pkt.length > 5),
        ConditionalField(BitField('_5GSNN26', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('REPREFI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('_5GSIWKI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('EEVRSI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('LTEMUI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('LTEMPI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('ENBCRSI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('TSPCMI', 0, 1), lambda pkt: pkt.length > 6),
        ConditionalField(BitField('SPARE1', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('SPARE2', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('SPARE3', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('N5GNMI', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('_5GCNRS', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('_5GCNRI', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('_5SRHOI', 0, 1), lambda pkt: pkt.length > 7),
        ConditionalField(BitField('ETHPDN', 0, 1), lambda pkt: pkt.length > 7),
    ]
...
class PCO_IPv4(PCO_Option):
    name = 'IPv4'
    fields_desc = [ByteEnumField('type', None, PCO_OPTION_TYPES), ByteField('length', None), IPField('address', RandIP())]
...
class PCO_P_CSCF_IPv6_Address_Request(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), ConditionalField(XBitField('address', '2001:db8:0:42::', 128), lambda pkt: pkt.length)]


class PCO_IM_CN_Subsystem_Signaling_Flag(PCO_Protocol):
    name = 'PCO IM CN Subsystem Signaling Flag'
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), PacketListField('Options', None, PCO_option_dispatcher, length_from=len_options)]
...
class PCO_PPP(PCO_Protocol):
    ...
    fields_desc = [ByteField('Code', 0), ByteField('Identifier', 0), ShortField('length', None), PacketListField('Options', None, PCO_option_dispatcher, length_from=len_options)]
...
class PCO_P_CSCF_IPv4_Address_Request(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), ConditionalField(IPField('address', RandIP()), lambda pkt: pkt.length)]
...
class PCO_IPv4_Link_MTU_Request(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), ConditionalField(ShortField('MTU_size', 1500), lambda pkt: pkt.length)]
...
class PCO_PDU_Session_Id(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', 1), ByteField('PduSessionId', 1)]
...
class PCO_QoS_Rules_With_Support_Indicator(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), PacketListField('Options', None, PCO_option_dispatcher, length_from=lambda pkt: pkt.length)]
...
class PCO_S_Nssai(PCO_Protocol):
    ...
    fields_desc = [
        ShortEnumField('type', None, PCO_PROTOCOL_TYPES),
        ByteField('length', None),
        ConditionalField(ByteField('SST', 0), lambda pkt: pkt.length > 0),
        ConditionalField(ShortField('SD', 0), lambda pkt: pkt.length > 1),
        ConditionalField(ByteField('Hplmn_Sst', 0), lambda pkt: pkt.length >= 4),
        ConditionalField(ShortField('Hplmn_Sd', 0), lambda pkt: pkt.length > 4),
    ]
...
class PCO_Session_AMBR(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', 6), ByteField('dlunit', 0), ShortField('dlambr', 0), ByteField('ulunit', 0), ShortField('ulambr', 0)]
...
class PCO_IPCP(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), PacketField('PPP', None, PCO_PPP)]
...
class PCO_PPP_Auth(PCO_Protocol):
    ...
    fields_desc = [
        ...
        ConditionalField(StrFixedLenField('PeerID', '', length_from=lambda pkt: pkt.PeerID_length), lambda pkt: pkt.PeerID_length),
...
class PCO_PasswordAuthentificationProtocol(PCO_Protocol):
    ...
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), PacketField('PPP', None, PCO_PPP_Auth)]
...
class PCO_PPP_Challenge(PCO_Protocol):
    ...
    fields_desc = [
        ...
        ConditionalField(StrFixedLenField('value', '', length_from=lambda pkt: pkt.value_size), lambda pkt: pkt.value_size),
        ConditionalField(
            StrFixedLenField('name', '', length_from=lambda pkt: pkt.length - pkt.value_size - 5),  # noqa: E501
...
class PCO_ChallengeHandshakeAuthenticationProtocol(PCO_Protocol):
    name = 'PCO Password Authentication Protocol'
    fields_desc = [ShortEnumField('type', None, PCO_PROTOCOL_TYPES), ByteField('length', None), PacketField('PPP', None, PCO_PPP_Challenge)]
...
class IE_PCO(gtp.IE_Base):
    ...
    fields_desc = [
        ByteEnumField('ietype', 78, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        ...
        BitField('SPARE', 0, 4),
        BitField('PPP', 0, 3),
        PacketListField('Protocols', None, PCO_protocol_dispatcher, length_from=lambda pkt: pkt.length - 1),
    ]
...
class IE_EPCO(gtp.IE_Base):
    ...
    fields_desc = [
        ByteEnumField('ietype', 197, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        BitField('Extension', 0, 1),
...
class IE_APCO(gtp.IE_Base):
    ...
    fields_desc = [
        ByteEnumField('ietype', 163, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        BitField('extension', 0, 1),
...
class IE_PAA(gtp.IE_Base):
    name = 'IE PAA'
    fields_desc = [
        ByteEnumField('ietype', 79, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        BitField('SPARE', 0, 5),
        BitEnumField('PDN_type', None, 3, PDN_TYPES),
        ConditionalField(ByteField('ipv6_prefix_length', 8), lambda pkt: pkt.PDN_type in (2, 3)),
        ConditionalField(XBitField('ipv6', '2001:db8:0:42::', 128), lambda pkt: pkt.PDN_type in (2, 3)),
        ConditionalField(IPField('ipv4', 0), lambda pkt: pkt.PDN_type in (1, 3)),
    ]
...
class IE_ChargingID(gtp.IE_Base):
    name = 'IE Charging ID'
    fields_desc = [ByteEnumField('ietype', 94, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), IntField('ChargingID', 0)]


class IE_ChargingCharacteristics(gtp.IE_Base):
    name = 'IE Charging Characteristics'
    fields_desc = [ByteEnumField('ietype', 95, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), XShortField('ChargingCharacteristric', 0)]


class IE_PDN_type(gtp.IE_Base):
    name = 'IE PDN Type'
    fields_desc = [ByteEnumField('ietype', 99, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), BitField('SPARE', 0, 5), BitEnumField('PDN_type', None, 3, PDN_TYPES)]


class IE_UE_Timezone(gtp.IE_Base):
    name = 'IE UE Time zone'
    fields_desc = [ByteEnumField('ietype', 114, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteField('Timezone', 0), ByteField('DST', 0)]


class IE_Port_Number(gtp.IE_Base):
    name = 'IE Port Number'
    fields_desc = [ByteEnumField('ietype', 126, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ShortField('PortNumber', RandShort())]


class IE_APN_Restriction(gtp.IE_Base):
    name = 'IE APN Restriction'
    fields_desc = [ByteEnumField('ietype', 127, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), ByteField('APN_Restriction', 0)]


class IE_SelectionMode(gtp.IE_Base):
    name = 'IE Selection Mode'
    fields_desc = [ByteEnumField('ietype', 128, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), BitField('SPARE', 0, 6), BitField('SelectionMode', 0, 2)]


class IE_MMBR(gtp.IE_Base):
    name = 'IE Max MBR/APN-AMBR (MMBR)'
    fields_desc = [ByteEnumField('ietype', 161, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), IntField('uplink_rate', 0), IntField('downlink_rate', 0)]


class IE_UPF_SelInd_Flags(gtp.IE_Base):
    name = 'IE UP Function Selection Indication Flags'
    fields_desc = [ByteEnumField('ietype', 202, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), BitField('SPARE', 0, 7), BitField('DCNR', 0, 1)]


class IE_FQCSID(gtp.IE_Base):
    name = 'IE FQ-CSID'
    fields_desc = [
        ByteEnumField('ietype', 132, IEType),
        ShortField('length', None),
        BitField('CR_flag', 0, 4),
        BitField('instance', 0, 4),
        BitField('nodeid_type', 0, 4),
        BitField('num_csid', 0, 4),
        ConditionalField(IPField('nodeid_v4', 0), lambda pkt: pkt.nodeid_type == 0),
        ConditionalField(XBitField('nodeid_v6', '2001:db8:0:42::', 128), lambda pkt: pkt.nodeid_type == 1),
        ConditionalField(BitField('nodeid_nonip', 0, 32), lambda pkt: pkt.nodeid_type == 2),
        ShortField('csid', 0),
    ]
...
class IE_Ran_Nas_Cause(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField('ietype', 172, IEType), ShortField('length', None), BitField('CR_flag', 0, 4), BitField('instance', 0, 4), BitField('protocol_type', 0, 4), BitField('cause_type', 0, 4), ByteField('cause_value', 0)]
...
bind_layers(GTPHeader, GTPV2CreateIndirectDataForwardingTunnelRequest, gtp_type=166)
bind_layers(GTPHeader, GTPV2CreateIndirectDataForwardingTunnelResponse, gtp_type=167)
bind_layers(GTPHeader, GTPV2DeleteIndirectDataForwardingTunnelRequest, gtp_type=168)
bind_layers(GTPHeader, GTPV2DeleteIndirectDataForwardingTunnelResponse, gtp_type=169)
