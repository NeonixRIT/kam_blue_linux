...
def s2i_frameid(x):
    ...
    try:
        ...
        pass
    try:
        return next(key for key, value in six.iteritems(PNIO_FRAME_IDS) if value == x)
...
class ProfinetIO(Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        if self.frameID in [0xFEFE, 0xFEFF, 0xFEFD]:
            ...
            return Alarm_High
        elif (0x0100 <= self.frameID < 0x1000) or (0x8000 <= self.frameID < 0xFC00):
...
class PNIORealTimeCyclicPDU(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('data', [], next_cls_cb=lambda pkt, lst, p, remain: pkt.next_cls_cb(lst, p, remain)),
