...
from scapy.compat import List, Type, Tuple, Dict, Any, Optional, cast, orb
...
class PriotizedCanMessage(object):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, PriotizedCanMessage):
            return False
        return self.msg.timestamp == other.msg.timestamp and self.count == other.count

    def __lt__(self, other):
        # type: (Any) -> bool
        if not isinstance(other, PriotizedCanMessage):
            return False
        return self.msg.timestamp < other.msg.timestamp or (self.msg.timestamp == other.msg.timestamp and self.count < other.count)
...
class PythonCANSocket(SuperSocket):
    ...
    def select(sockets, remain=conf.recv_poll_rate):
        ...
        """
        SocketsPool.multiplex_rx_packets()
        return [s for s in sockets if isinstance(s, PythonCANSocket) and not s.can_iface.rx_queue.empty()]
