...
class NSH(Packet):
    ...
    fields_desc = [
        ...
        BitField('ttl', 63, 6),
        BitFieldLenField('length', None, 6, count_of='vlch', adjust=lambda pkt, x: 6 if pkt.mdtype == 1 else x + 2),
        BitField('unused2', 0, 4),
        BitEnumField('mdtype', 1, 4, {0: 'Reserved MDType', 1: 'Fixed Length', 2: 'Variable Length', 0xF: 'Experimental MDType'}),
        ByteEnumField('nextproto', 3, {1: 'IPv4', 2: 'IPv6', 3: 'Ethernet', 4: 'NSH', 5: 'MPLS', 0xFE: 'Experiment 1', 0xFF: 'Experiment 2'}),
        X3BytesField('spi', 0),
        ByteField('si', 0xFF),
        ConditionalField(XStrFixedLenField('context_header', '', 16), lambda pkt: pkt.mdtype == 1),
        ConditionalField(PacketListField('vlch', None, NSHTLV, count_from='length'), lambda pkt: pkt.mdtype == 2),
    ]
