...
ofp_port_no = {0xFFF8: 'IN_PORT', 0xFFF9: 'TABLE', 0xFFFA: 'NORMAL', 0xFFFB: 'FLOOD', 0xFFFC: 'ALL', 0xFFFD: 'CONTROLLER', 0xFFFE: 'LOCAL', 0xFFFF: 'NONE'}
...
ofp_port_config = ['PORT_DOWN', 'NO_STP', 'NO_RECV', 'NO_RECV_STP', 'NO_FLOOD', 'NO_FWD', 'NO_PACKET_IN']

ofp_port_state = ['LINK_DOWN']

ofp_port_state_stp = {0: 'OFPPS_STP_LISTEN', 1: 'OFPPS_STP_LEARN', 2: 'OFPPS_STP_FORWARD', 3: 'OFPPS_STP_BLOCK'}

ofp_port_features = ['10MB_HD', '10MB_FD', '100MB_HD', '100MB_FD', '1GB_HD', '1GB_FD', '10GB_FD', 'COPPER', 'FIBER', 'AUTONEG', 'PAUSE', 'PAUSE_ASYM']
...
class OFPMatch(Packet):
    ...
    fields_desc = [
        ...
        FlagsField('wildcards2', None, 8, ['IN_PORT', 'DL_VLAN', 'DL_SRC', 'DL_DST', 'DL_TYPE', 'NW_PROTO', 'TP_SRC', 'TP_DST']),
...
class OFPATOutput(OpenFlow):
    name = 'OFPAT_OUTPUT'
    fields_desc = [ShortEnumField('type', 0, ofp_action_types), ShortField('len', 8), ShortEnumField('port', 0, ofp_port_no), ShortEnumField('max_len', 'NO_BUFFER', ofp_max_len)]
...
class OFPATEnqueue(OpenFlow):
    name = 'OFPAT_ENQUEUE'
    fields_desc = [ShortEnumField('type', 11, ofp_action_types), ShortField('len', 16), ShortEnumField('port', 0, ofp_port_no), XBitField('pad', 0, 48), IntField('queue_id', 0)]


class OFPATVendor(OpenFlow):
    name = 'OFPAT_VENDOR'
    fields_desc = [ShortEnumField('type', 65535, ofp_action_types), ShortField('len', 8), IntField('vendor', 0)]
...
class OFPQTMinRate(_ofp_header):
    name = 'OFPQT_MIN_RATE'
    fields_desc = [ShortEnumField('type', 1, ofp_queue_property_types), ShortField('len', 16), XIntField('pad', 0), ShortField('rate', 0), XBitField('pad2', 0, 48)]
...
class OFPPacketQueue(Packet):
    ...
    fields_desc = [IntField('queue_id', 0), ShortField('len', None), XShortField('pad', 0), PacketListField('properties', [], OFPQT, length_from=lambda pkt: pkt.len - 8)]
...
class OFPTHello(_ofp_header):
    name = 'OFPT_HELLO'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 0, ofp_type), ShortField('len', None), IntField('xid', 0)]
...
ofp_error_type = {0: 'OFPET_HELLO_FAILED', 1: 'OFPET_BAD_REQUEST', 2: 'OFPET_BAD_ACTION', 3: 'OFPET_FLOW_MOD_FAILED', 4: 'OFPET_PORT_MOD_FAILED', 5: 'OFPET_QUEUE_OP_FAILED'}
...
class OFPETBadRequest(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField(
            'errcode', 0, {0: 'OFPBRC_BAD_VERSION', 1: 'OFPBRC_BAD_TYPE', 2: 'OFPBRC_BAD_STAT', 3: 'OFPBRC_BAD_VENDOR', 4: 'OFPBRC_BAD_SUBTYPE', 5: 'OFPBRC_EPERM', 6: 'OFPBRC_BAD_LEN', 7: 'OFPBRC_BUFFER_EMPTY', 8: 'OFPBRC_BUFFER_UNKNOWN'}
        ),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETBadAction(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField(
            'errcode', 0, {0: 'OFPBAC_BAD_TYPE', 1: 'OFPBAC_BAD_LEN', 2: 'OFPBAC_BAD_VENDOR', 3: 'OFPBAC_BAD_VENDOR_TYPE', 4: 'OFPBAC_BAD_OUT_PORT', 5: 'OFPBAC_BAD_ARGUMENT', 6: 'OFPBAC_EPERM', 7: 'OFPBAC_TOO_MANY', 8: 'OFPBAC_BAD_QUEUE'}
        ),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETPortModFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPPMFC_BAD_PORT', 1: 'OFPPMFC_BAD_HW_ADDR'}),
        OFPacketField('data', '', Raw),
    ]
...
ofp_error_cls = {0: OFPETHelloFailed, 1: OFPETBadRequest, 2: OFPETBadAction, 3: OFPETFlowModFailed, 4: OFPETPortModFailed, 5: OFPETQueueOpFailed}
...
class OFPTEchoRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 2, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTEchoReply(_ofp_header):
    name = 'OFPT_ECHO_REPLY'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 3, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTVendor(_ofp_header):
    name = 'OFPT_VENDOR'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 4, ofp_type), ShortField('len', None), IntField('xid', 0), IntField('vendor', 0)]


class OFPTFeaturesRequest(_ofp_header):
    name = 'OFPT_FEATURES_REQUEST'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 5, ofp_type), ShortField('len', None), IntField('xid', 0)]


ofp_action_types_flags = [v for v in six.itervalues(ofp_action_types) if v != 'OFPAT_VENDOR']
...
class OFPTFeaturesReply(_ofp_header):
    ...
    fields_desc = [
        ...
        X3BytesField('pad', 0),
        FlagsField('capabilities', 0, 32, ['FLOW_STATS', 'TABLE_STATS', 'PORT_STATS', 'STP', 'RESERVED', 'IP_REASM', 'QUEUE_STATS', 'ARP_MATCH_IP']),
        FlagsField('actions', 0, 32, ofp_action_types_flags),
        PacketListField('ports', [], OFPPhyPort, length_from=lambda pkt: pkt.len - 32),
    ]


class OFPTGetConfigRequest(_ofp_header):
    name = 'OFPT_GET_CONFIG_REQUEST'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 7, ofp_type), ShortField('len', None), IntField('xid', 0)]
...
class OFPTPacketIn(_ofp_header):
    ...
    fields_desc = [
        ...
        ByteEnumField('reason', 0, {0: 'OFPR_NO_MATCH', 1: 'OFPR_ACTION'}),
        XByteField('pad', 0),
...
class OFPTFlowRemoved(_ofp_header):
    ...
    fields_desc = [
        ...
        ByteEnumField('reason', 0, {0: 'OFPRR_IDLE_TIMEOUT', 1: 'OFPRR_HARD_TIMEOUT', 2: 'OFPRR_DELETE'}),
        XByteField('pad1', 0),
...
class OFPTPacketOut(_ofp_header):
    ...
    fields_desc = [
        ...
        PacketListField('actions', [], OFPAT, ofp_action_cls, length_from=lambda pkt: pkt.actions_len),  # noqa: E501
        PacketField('data', None, Ether),
    ]
...
class OFPTFlowMod(_ofp_header):
    ...
    fields_desc = [
        ...
        PacketField('match', OFPMatch(), OFPMatch),
        LongField('cookie', 0),
        ShortEnumField('cmd', 0, {0: 'OFPFC_ADD', 1: 'OFPFC_MODIFY', 2: 'OFPFC_MODIFY_STRICT', 3: 'OFPFC_DELETE', 4: 'OFPFC_DELETE_STRICT'}),
        ShortField('idle_timeout', 0),
        ShortField('hard_timeout', 0),
        ShortField('priority', 0),
        IntEnumField('buffer_id', 'NO_BUFFER', ofp_buffer),
        ShortEnumField('out_port', 'NONE', ofp_port_no),
        FlagsField('flags', 0, 16, ['SEND_FLOW_REM', 'CHECK_OVERLAP', 'EMERG']),
        PacketListField('actions', [], OFPAT, ofp_action_cls, length_from=lambda pkt: pkt.len - 72),
    ]
...
ofp_stats_types = {0: 'OFPST_DESC', 1: 'OFPST_FLOW', 2: 'OFPST_AGGREGATE', 3: 'OFPST_TABLE', 4: 'OFPST_PORT', 5: 'OFPST_QUEUE', 65535: 'OFPST_VENDOR'}


class OFPTStatsRequestDesc(_ofp_header):
    name = 'OFPST_STATS_REQUEST_DESC'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 16, ofp_type), ShortField('len', None), IntField('xid', 0), ShortEnumField('stats_type', 0, ofp_stats_types), FlagsField('flags', 0, 16, [])]
...
class OFPFlowStats(Packet):
    ...
    fields_desc = [
        ...
        LongField('packet_count', 0),
        LongField('byte_count', 0),
        PacketListField('actions', [], OFPAT, ofp_action_cls, length_from=lambda pkt: pkt.length - 88),
    ]
...
class OFPTStatsReplyFlow(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('stats_type', 1, ofp_stats_types),
        FlagsField('flags', 0, 16, []),
        PacketListField('flow_stats', [], OFPFlowStats, length_from=lambda pkt: pkt.len - 12),
    ]  # noqa: E501
...
class OFPTStatsRequestTable(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 16, ofp_type), ShortField('len', None), IntField('xid', 0), ShortEnumField('stats_type', 3, ofp_stats_types), FlagsField('flags', 0, 16, [])]
...
class OFPTableStats(Packet):
    ...
    fields_desc = [
        ...
        FlagsField('wildcards2', 0xFF, 8, ['IN_PORT', 'DL_VLAN', 'DL_SRC', 'DL_DST', 'DL_TYPE', 'NW_PROTO', 'TP_SRC', 'TP_DST']),
...
class OFPTStatsReplyTable(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, []),
        PacketListField('table_stats', [], OFPTableStats, length_from=lambda pkt: pkt.len - 12),
    ]
...
class OFPTStatsReplyPort(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('stats_type', 4, ofp_stats_types),
        FlagsField('flags', 0, 16, []),
        PacketListField('port_stats', [], OFPPortStats, length_from=lambda pkt: pkt.len - 12),
    ]
...
ofp_stats_request_cls = {0: OFPTStatsRequestDesc, 1: OFPTStatsRequestFlow, 2: OFPTStatsRequestAggregate, 3: OFPTStatsRequestTable, 4: OFPTStatsRequestPort, 5: OFPTStatsRequestQueue, 65535: OFPTStatsRequestVendor}

ofp_stats_reply_cls = {0: OFPTStatsReplyDesc, 1: OFPTStatsReplyFlow, 2: OFPTStatsReplyAggregate, 3: OFPTStatsReplyTable, 4: OFPTStatsReplyPort, 5: OFPTStatsReplyQueue, 65535: OFPTStatsReplyVendor}
...
class OFPTBarrierRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 18, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTBarrierReply(_ofp_header):
    name = 'OFPT_BARRIER_REPLY'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 19, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTQueueGetConfigRequest(_ofp_header):
    name = 'OFPT_QUEUE_GET_CONFIG_REQUEST'
    fields_desc = [ByteEnumField('version', 0x01, ofp_version), ByteEnumField('type', 20, ofp_type), ShortField('len', None), IntField('xid', 0), ShortEnumField('port', 0, ofp_port_no), XShortField('pad', 0)]
