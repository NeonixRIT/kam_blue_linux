...
class SyntaxId(Packet):
    name = 'syntax Id'
    fields_desc = [
        UUIDField('interfaceUUID', str('0001' * 8), uuid_fmt=UUIDField.FORMAT_BE),
...
class ResultElement(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ShortEnumField('reason', 0, _defReason), lambda pkt: pkt.resultContextNegotiation != 0),
...
class ResultList(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('resultList', None, ResultElement, count_from=lambda pkt: pkt.nbResult),
    ]
...
class ContextElement(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('transferSyntax', None, SyntaxId, count_from=lambda pkt: pkt.nbTransferSyn),
    ]
...
IRemoteSCMActivator = {
    ...
    4: [IRemoteSCMActivator_RemoteCreateInstance, IRemoteSCMActivator_RemoteCreateInstanceLE],
}
...
class AV_PAIR(Packet):
    ...
    fields_desc = [
        ...
        ShortField('avLen', 0),
        StrLenField('value', '', length_from=lambda pkt: pkt.avLen),
    ]
...
class OpcDaRequest(Packet):
    ...
    fields_desc = [
        ...
        ShortField('opNum', 0),
        ConditionalField(UUIDField('uuid', str('0001' * 8), uuid_fmt=UUIDField.FORMAT_BE), lambda pkt: OpcDaHeaderMessage in pkt.firstlayer() and pkt.firstlayer()[OpcDaHeaderMessage].pfc_flags & 'objectUuid'),
        PacketLenField('stubData', None, RequestStubData, length_from=lambda pkt: _opc_stubdata_length(pkt)),
...
class OpcDaResponse(Packet):
    ...
    fields_desc = [
        ...
        ByteField('reserved', 0),
        StrLenField('stubData', None, length_from=lambda pkt: pkt.allocHint - 32),
        PacketField('authentication', None, AuthentificationProtocol),
    ]
...
class OpcDaFack(Packet):
    ...
    fields_desc = [
        ...
        PacketListField('selack', None, IntField, count_from=lambda pkt: pkt.selackLen),
    ]
...
class OpcDaBind(Packet):
    ...
    fields_desc = [
        ...
        ByteField('reserved', 0),
        ShortField('reserved2', 0),
        PacketListField('contextItem', None, ContextElement, count_from=lambda pkt: pkt.nbContextElement),
        PacketField('authentication', None, AuthentificationProtocol),
    ]
...
class OpcDaBind_nak(Packet):
    ...
    name = 'OpcDaBind_nak'
    fields_desc = [ShortEnumField('providerRejectReason', 0, _rejectBindNack)]  # To complete
...
class OpcDaHeaderMessage(Packet):
    ...
    fields_desc = [
        ...
        # DCE 1.1 RPC - 14.1
        BitEnumField('integerRepresentation', 1, 4, {0: 'bigEndian', 1: 'littleEndian'}),
        BitEnumField('characterRepresentation', 0, 4, {0: 'ascii', 1: 'ebcdic'}),
        ByteEnumField('floatingPointRepresentation', 0, {0: 'ieee', 1: 'vax', 2: 'cray', 3: 'ibm'}),
        ShortField('res', 0),
    ]
