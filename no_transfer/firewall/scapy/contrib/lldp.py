...
def _ldp_id_adjustlen(pkt, x):
    ...
    if (isinstance(pkt, LLDPDUPortID) and pkt.subtype == 0x4) or (isinstance(pkt, LLDPDUChassisID) and pkt.subtype == 0x5):
...
class LLDPDUChassisID(LLDPDU):
    ...
    fields_desc = [
        ...
        BitFieldLenField('_length', None, 9, length_of='id', adjust=lambda pkt, x: _ldp_id_adjustlen(pkt, x)),
        ByteEnumField('subtype', 0x00, LLDP_CHASSIS_ID_TLV_SUBTYPES),
        ConditionalField(ByteEnumField('family', 0, LLDPDU.IANA_ADDRESS_FAMILY_NUMBERS), lambda pkt: pkt.subtype == 0x05),
        MultipleTypeField(
            [
                (MACField('id', None), lambda pkt: pkt.subtype == 0x04),
                (IPField('id', None), lambda pkt: pkt.subtype == 0x05 and pkt.family == 0x01),
                (IP6Field('id', None), lambda pkt: pkt.subtype == 0x05 and pkt.family == 0x02),
            ],
...
class LLDPDUPortID(LLDPDU):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteEnumField('family', 0, LLDPDU.IANA_ADDRESS_FAMILY_NUMBERS), lambda pkt: pkt.subtype == 0x04),
        MultipleTypeField(
            [
                (MACField('id', None), lambda pkt: pkt.subtype == 0x03),
                (IPField('id', None), lambda pkt: pkt.subtype == 0x04 and pkt.family == 0x01),
                (IP6Field('id', None), lambda pkt: pkt.subtype == 0x04 and pkt.family == 0x02),
            ],
            StrLenField('id', '', length_from=_ldp_id_lengthfrom),
        ),
    ]
...
class LLDPDUTimeToLive(LLDPDU):
    ...
    fields_desc = [BitEnumField('_type', 0x03, 7, LLDPDU.TYPES), BitField('_length', 0x02, 9), ShortField('ttl', 20)]
...
class LLDPDUPortDescription(LLDPDU):
    ...
    fields_desc = [BitEnumField('_type', 0x04, 7, LLDPDU.TYPES), BitFieldLenField('_length', None, 9, length_of='description'), StrLenField('description', '', length_from=lambda pkt: pkt._length)]
...
class LLDPDUSystemName(LLDPDU):
    ...
    fields_desc = [BitEnumField('_type', 0x05, 7, LLDPDU.TYPES), BitFieldLenField('_length', None, 9, length_of='system_name'), StrLenField('system_name', '', length_from=lambda pkt: pkt._length)]
...
class LLDPDUSystemDescription(LLDPDU):
    ...
    fields_desc = [BitEnumField('_type', 0x06, 7, LLDPDU.TYPES), BitFieldLenField('_length', None, 9, length_of='description'), StrLenField('description', '', length_from=lambda pkt: pkt._length)]
...
class LLDPDUManagementAddress(LLDPDU):
    ...
    INTERFACE_NUMBERING_SUBTYPES = {0x01: 'unknown', 0x02: 'ifIndex', 0x03: 'system port number'}
    ...
    fields_desc = [
        ...
        BitFieldLenField('_length', None, 9, length_of='management_address', adjust=lambda pkt, x: 8 + len(pkt.management_address) + len(pkt.object_id)),
        BitFieldLenField('_management_address_string_length', None, 8, length_of='management_address', adjust=lambda pkt, x: len(pkt.management_address) + 1),  # noqa: E501
        ByteEnumField('management_address_subtype', 0x00, LLDPDU.IANA_ADDRESS_FAMILY_NUMBERS),
        XStrLenField('management_address', '', length_from=lambda pkt: 0 if pkt._management_address_string_length is None else pkt._management_address_string_length - 1),
        ByteEnumField('interface_numbering_subtype', SUBTYPE_INTERFACE_NUMBER_UNKNOWN, INTERFACE_NUMBERING_SUBTYPES),
        BitField('interface_number', 0, 32),
        BitFieldLenField('_oid_string_length', None, 8, length_of='object_id'),
        XStrLenField('object_id', '', length_from=lambda pkt: pkt._oid_string_length),
    ]
...
class LLDPDUGenericOrganisationSpecific(LLDPDU):
    ...
    fields_desc = [
        ...
        XStrLenField('data', '', length_from=lambda pkt: 0 if pkt._length is None else pkt._length - 4),
    ]
