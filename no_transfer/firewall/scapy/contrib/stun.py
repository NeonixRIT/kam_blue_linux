...
_stun_class = {'request': 0b00, 'indication': 0b01, 'success response': 0b10, 'error response': 0b11}

_stun_method = {'Binding': 0b000000000001}
...
class STUNGenericTlv(Packet):
    ...
    fields_desc = [XShortField('type', 0x0000), FieldLenField('length', None, length_of='value'), PadField(StrLenField('value', '', length_from=lambda pkt: pkt.length), align=4)]
...
class STUNUsername(STUNGenericTlv):
    ...
    fields_desc = [XShortField('type', 0x0006), FieldLenField('length', None, length_of='username'), PadField(StrLenField('username', '', length_from=lambda pkt: pkt.length), align=4, padwith=b'\x20')]


class STUNMessageIntegrity(STUNGenericTlv):
    name = 'STUN Message Integrity'

    fields_desc = [XShortField('type', 0x0008), ShortField('length', 20), XNBytesField('hmac_sha1', 0, 20)]
...
class STUNPriority(STUNGenericTlv):
    ...
    fields_desc = [XShortField('type', 0x0024), ShortField('length', 4), IntField('priority', 0)]


_xor_mapped_address_family = {'IPv4': 0x01, 'IPv6': 0x02}
...
class STUNUseCandidate(STUNGenericTlv):
    ...
    fields_desc = [XShortField('type', 0x0025), FieldLenField('length', 0, length_of='value'), PadField(StrLenField('value', '', length_from=lambda pkt: pkt.length), align=4)]


class STUNFingerprint(STUNGenericTlv):
    name = 'STUN Fingerprint'

    fields_desc = [XShortField('type', 0x8028), ShortField('length', 4), XIntField('crc_32', None)]


class STUNIceControlling(STUNGenericTlv):
    name = 'STUN ICE-controlling'

    fields_desc = [XShortField('type', 0x802A), ShortField('length', 8), XLongField('tie_breaker', None)]


class STUNGoogNetworkInfo(STUNGenericTlv):
    name = 'STUN Google Network Info'

    fields_desc = [XShortField('type', 0xC057), ShortField('length', 4), ShortField('network_id', 0), ShortField('network_cost', 999)]


_stun_tlv_class = {0x0006: STUNUsername, 0x0008: STUNMessageIntegrity, 0x0020: STUNXorMappedAddress, 0x0025: STUNUseCandidate, 0x0024: STUNPriority, 0x8028: STUNFingerprint, 0x802A: STUNIceControlling, 0xC057: STUNGoogNetworkInfo}
