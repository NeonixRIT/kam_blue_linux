...
from scapy.compat import Optional, List, Union, Callable, Any, Tuple, Sized, Pattern  # noqa: F401
...
class HPackMagicBitField(fields.BitField):
    ...
    def getfield(self, pkt, s):
        ...
        assert isinstance(r, tuple) and len(r) == 2 and isinstance(r[1], six.integer_types), 'Second element of BitField.getfield return value expected to be an int or a long; API change detected'  # noqa: E501
    ...
    def h2i(self, pkt, x):
        ...
        :raises: AssertionError
        """
        assert x == self._magic, 'EINVAL: x: This field is magic. Do not attempt to modify it. Expected value: {}'.format(self._magic)  # noqa: E501
...
class FieldUVarLenField(AbstractUVarIntField):
    ...
    def addfield(self, pkt, s, val):
        ...
        """
        if val is None:
            assert isinstance(pkt, packet.Packet), 'EINVAL: pkt: Packet expected when val is None; received {}'.format(type(pkt))  # noqa: E501
    ...
    def i2m(self, pkt, x):
        ...
        """
        if x is None:
            assert isinstance(pkt, packet.Packet), 'EINVAL: pkt: Packet expected when x is None; received {}'.format(type(pkt))  # noqa: E501
...
class HPackHdrString(packet.Packet):
    ...
    fields_desc = [
        ...
        HPackStrLenField('data', HPackLiteralString(''), length_from=lambda pkt: pkt.getfieldval('len'), type_from='type'),
    ]
...
class HPackLitHdrFldWithIncrIndexing(HPackHeaders):
    ...
    fields_desc = [
        ...
        fields.ConditionalField(fields.PacketField('hdr_name', None, HPackHdrString), lambda pkt: pkt.getfieldval('index') == 0),
...
class HPackLitHdrFldWithoutIndexing(HPackHeaders):
    ...
    fields_desc = [
        ...
        fields.BitEnumField('never_index', 0, 1, {0: "Don't Index", 1: 'Never Index'}),
...
class H2PaddedDataFrame(H2DataFrame):
    ...
    fields_desc = [
        ...
        fields.StrLenField('data', '', length_from=lambda pkt: pkt.get_data_len()),
        fields.StrLenField('padding', '', length_from=lambda pkt: pkt.getfieldval('padlen')),
    ]
...
class H2PaddedHeadersFrame(H2AbstractHeadersFrame):
    ...
    fields_desc = [
        fields.FieldLenField('padlen', None, length_of='padding', fmt='B'),
        fields.PacketListField('hdrs', [], HPackHeaders, length_from=lambda pkt: pkt.get_hdrs_len()),
        fields.StrLenField('padding', '', length_from=lambda pkt: pkt.getfieldval('padlen')),
    ]
...
class HPackHdrTable(Sized):
    ...
    def __init__(self, dynamic_table_max_size=4096, dynamic_table_cap_size=4096):  # noqa: E501
        ...
        assert dynamic_table_max_size <= dynamic_table_cap_size, 'EINVAL: dynamic_table_max_size too large; expected value is less or equal to dynamic_table_cap_size'  # noqa: E501
    ...
    def resize(self, ns):
        ...
        :raises: AssertionError
        """
        assert 0 <= ns <= self._dynamic_table_cap_size, 'EINVAL: ns: out-of-range value; expected value is in the range [0;{}['.format(self._dynamic_table_cap_size)  # noqa: E501
    ...
    def _parse_header_line(self, line):
        ...
        if self._regexp is None:
            self._regexp = re.compile(rb'^(?::([a-z\-0-9]+)|([a-z\-0-9]+):)\s+(.+)$')  # noqa: E501
