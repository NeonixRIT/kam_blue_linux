...
from scapy.contrib.openflow import _ofp_header, _ofp_header_item, OFPacketField, OpenFlow, _UnknownOpenFlow

#####################################################
#                 Predefined values                 #
#####################################################

ofp_port_no = {0xFFFFFFF8: 'IN_PORT', 0xFFFFFFF9: 'TABLE', 0xFFFFFFFA: 'NORMAL', 0xFFFFFFFB: 'FLOOD', 0xFFFFFFFC: 'ALL', 0xFFFFFFFD: 'CONTROLLER', 0xFFFFFFFE: 'LOCAL', 0xFFFFFFFF: 'ANY'}

ofp_group = {0xFFFFFF00: 'MAX', 0xFFFFFFFC: 'ALL', 0xFFFFFFFF: 'ANY'}

ofp_table = {0xFE: 'MAX', 0xFF: 'ALL'}

ofp_queue = {0xFFFFFFFF: 'ALL'}

ofp_meter = {0xFFFF0000: 'MAX', 0xFFFFFFFD: 'SLOWPATH', 0xFFFFFFFE: 'CONTROLLER', 0xFFFFFFFF: 'ALL'}
...
class OFPHETVersionBitmap(_ofp_header):
    ...
    fields_desc = [ShortEnumField('type', 1, ofp_hello_elem_types), ShortField('len', 8), FlagsField('bitmap', 0, 32, ['Type 0', 'OFv1.0', 'OFv1.1', 'OFv1.2', 'OFv1.3', 'OFv1.4', 'OFv1.5'])]
...
ofp_port_state = ['LINK_DOWN', 'BLOCKED', 'LIVE']

ofp_port_features = ['10MB_HD', '10MB_FD', '100MB_HD', '100MB_FD', '1GB_HD', '1GB_FD', '10GB_FD', '40GB_FD', '100GB_FD', '1TB_FD', 'OTHER', 'COPPER', 'FIBER', 'AUTONEG', 'PAUSE', 'PAUSE_ASYM']
...
ofp_oxm_classes = {0: 'OFPXMC_NXM_0', 1: 'OFPXMC_NXM_1', 0x8000: 'OFPXMC_OPENFLOW_BASIC', 0xFFFF: 'OFPXMC_EXPERIMENTER'}
...
ipv6flags = ['NONEXT', 'ESP', 'AUTH', 'DEST', 'FRAG', 'ROUTER', 'HOP', 'UNREP', 'UNSEQ']
...
def add_ofp_oxm_fields(i, org):
    ofp_oxm_fields[i] = [
        ShortEnumField('class_', 'OFPXMC_OPENFLOW_BASIC', ofp_oxm_classes),  # noqa: E501
...
class OFPMatch(Packet):
    name = 'OFP_MATCH'
    fields_desc = [ShortEnumField('type', 1, {0: 'OFPMT_STANDARD', 1: 'OFPMT_OXM'}), ShortField('len', None), OXMPacketListField('oxm_fields', [], Packet, length_from=lambda pkt: pkt.len - 4)]
...
class OFPATOutput(OFPAT):
    name = 'OFPAT_OUTPUT'
    fields_desc = [ShortEnumField('type', 0, ofp_action_types), ShortField('len', 16), IntEnumField('port', 0, ofp_port_no), ShortEnumField('max_len', 'NO_BUFFER', ofp_max_len), XBitField('pad', 0, 48)]
...
class OFPATSetVLANVID(OFPAT):
    ...
    fields_desc = [ShortEnumField('type', 1, ofp_action_types), ShortField('len', 8), ShortField('vlan_vid', 0), XShortField('pad', 0)]


class OFPATSetVLANPCP(OFPAT):
    name = 'OFPAT_SET_VLAN_PCP'
    fields_desc = [ShortEnumField('type', 2, ofp_action_types), ShortField('len', 8), ByteField('vlan_pcp', 0), X3BytesField('pad', 0)]


class OFPATStripVLAN(OFPAT):
    name = 'OFPAT_STRIP_VLAN'
    fields_desc = [ShortEnumField('type', 3, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATSetDlSrc(OFPAT):
    name = 'OFPAT_SET_DL_SRC'
    fields_desc = [ShortEnumField('type', 4, ofp_action_types), ShortField('len', 16), MACField('dl_addr', '0'), XBitField('pad', 0, 48)]


class OFPATSetDlDst(OFPAT):
    name = 'OFPAT_SET_DL_DST'
    fields_desc = [ShortEnumField('type', 5, ofp_action_types), ShortField('len', 16), MACField('dl_addr', '0'), XBitField('pad', 0, 48)]


class OFPATSetNwSrc(OFPAT):
    name = 'OFPAT_SET_NW_SRC'
    fields_desc = [ShortEnumField('type', 6, ofp_action_types), ShortField('len', 8), IPField('nw_addr', '0')]


class OFPATSetNwDst(OFPAT):
    name = 'OFPAT_SET_NW_DST'
    fields_desc = [ShortEnumField('type', 7, ofp_action_types), ShortField('len', 8), IPField('nw_addr', '0')]


class OFPATSetNwToS(OFPAT):
    name = 'OFPAT_SET_TP_TOS'
    fields_desc = [ShortEnumField('type', 8, ofp_action_types), ShortField('len', 8), ByteField('nw_tos', 0), X3BytesField('pad', 0)]


class OFPATSetTpSrc(OFPAT):
    name = 'OFPAT_SET_TP_SRC'
    fields_desc = [ShortEnumField('type', 9, ofp_action_types), ShortField('len', 8), ShortField('tp_port', 0), XShortField('pad', 0)]


class OFPATSetTpDst(OFPAT):
    name = 'OFPAT_SET_TP_DST'
    fields_desc = [ShortEnumField('type', 10, ofp_action_types), ShortField('len', 8), ShortField('tp_port', 0), XShortField('pad', 0)]
...
class OFPATSetMPLSLabel(OFPAT):
    ...
    fields_desc = [ShortEnumField('type', 13, ofp_action_types), ShortField('len', 8), IntField('mpls_label', 0)]


class OFPATSetMPLSTC(OFPAT):
    name = 'OFPAT_SET_MPLS_TC'
    fields_desc = [ShortEnumField('type', 14, ofp_action_types), ShortField('len', 8), ByteField('mpls_tc', 0), X3BytesField('pad', 0)]
...
class OFPATCopyTTLOut(OFPAT):
    ...
    fields_desc = [ShortEnumField('type', 11, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATCopyTTLIn(OFPAT):
    name = 'OFPAT_COPY_TTL_IN'
    fields_desc = [ShortEnumField('type', 12, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATSetMPLSTTL(OFPAT):
    name = 'OFPAT_SET_MPLS_TTL'
    fields_desc = [ShortEnumField('type', 15, ofp_action_types), ShortField('len', 8), ByteField('mpls_ttl', 0), X3BytesField('pad', 0)]


class OFPATDecMPLSTTL(OFPAT):
    name = 'OFPAT_DEC_MPLS_TTL'
    fields_desc = [ShortEnumField('type', 16, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATPushVLAN(OFPAT):
    name = 'OFPAT_PUSH_VLAN'
    fields_desc = [
        ShortEnumField('type', 17, ofp_action_types),
        ShortField('len', 8),
...
class OFPATPopVLAN(OFPAT):
    name = 'OFPAT_POP_VLAN'
    fields_desc = [ShortEnumField('type', 18, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]
...
class OFPATPopMPLS(OFPAT):
    name = 'OFPAT_POP_MPLS'
    fields_desc = [
        ShortEnumField('type', 20, ofp_action_types),
        ShortField('len', 8),
        ShortField('ethertype', 0x8847),  # or 0x8848
        XShortField('pad', 0),
    ]
...
class OFPATSetQueue(OFPAT):
    ...
    fields_desc = [ShortEnumField('type', 21, ofp_action_types), ShortField('len', 8), IntEnumField('queue_id', 0, ofp_queue)]


class OFPATGroup(OFPAT):
    name = 'OFPAT_GROUP'
    fields_desc = [ShortEnumField('type', 22, ofp_action_types), ShortField('len', 8), IntEnumField('group_id', 0, ofp_group)]


class OFPATSetNwTTL(OFPAT):
    name = 'OFPAT_SET_NW_TTL'
    fields_desc = [ShortEnumField('type', 23, ofp_action_types), ShortField('len', 8), ByteField('nw_ttl', 0), X3BytesField('pad', 0)]


class OFPATDecNwTTL(OFPAT):
    name = 'OFPAT_DEC_NW_TTL'
    fields_desc = [ShortEnumField('type', 24, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATSetField(OFPAT):
    name = 'OFPAT_SET_FIELD'
    fields_desc = [
        ShortEnumField('type', 25, ofp_action_types),
        ShortField('len', None),
...
class OFPATPushPBB(OFPAT):
    name = 'OFPAT_PUSH_PBB'
    fields_desc = [ShortEnumField('type', 26, ofp_action_types), ShortField('len', 8), ShortField('ethertype', 0x88E7), XShortField('pad', 0)]


class OFPATPopPBB(OFPAT):
    name = 'OFPAT_POP_PBB'
    fields_desc = [ShortEnumField('type', 27, ofp_action_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPATExperimenter(OFPAT):
    name = 'OFPAT_EXPERIMENTER'
    fields_desc = [ShortEnumField('type', 65535, ofp_action_types), ShortField('len', 8), IntField('experimenter', 0)]
...
class OFPATOutputID(OFPATID):
    name = 'OFPAT_OUTPUT'
    fields_desc = [ShortEnumField('type', 0, ofp_action_types), ShortField('len', 4)]
...
class OFPATSetVLANVIDID(OFPATID):
    name = 'OFPAT_SET_VLAN_VID'
    fields_desc = [ShortEnumField('type', 1, ofp_action_types), ShortField('len', 4)]


class OFPATSetVLANPCPID(OFPATID):
    name = 'OFPAT_SET_VLAN_PCP'
    fields_desc = [ShortEnumField('type', 2, ofp_action_types), ShortField('len', 4)]


class OFPATStripVLANID(OFPATID):
    name = 'OFPAT_STRIP_VLAN'
    fields_desc = [ShortEnumField('type', 3, ofp_action_types), ShortField('len', 4)]


class OFPATSetDlSrcID(OFPATID):
    name = 'OFPAT_SET_DL_SRC'
    fields_desc = [ShortEnumField('type', 4, ofp_action_types), ShortField('len', 4)]


class OFPATSetDlDstID(OFPATID):
    name = 'OFPAT_SET_DL_DST'
    fields_desc = [ShortEnumField('type', 5, ofp_action_types), ShortField('len', 4)]


class OFPATSetNwSrcID(OFPATID):
    name = 'OFPAT_SET_NW_SRC'
    fields_desc = [ShortEnumField('type', 6, ofp_action_types), ShortField('len', 4)]


class OFPATSetNwDstID(OFPATID):
    name = 'OFPAT_SET_NW_DST'
    fields_desc = [ShortEnumField('type', 7, ofp_action_types), ShortField('len', 4)]


class OFPATSetNwToSID(OFPATID):
    name = 'OFPAT_SET_TP_TOS'
    fields_desc = [ShortEnumField('type', 8, ofp_action_types), ShortField('len', 4)]


class OFPATSetTpSrcID(OFPATID):
    name = 'OFPAT_SET_TP_SRC'
    fields_desc = [ShortEnumField('type', 9, ofp_action_types), ShortField('len', 4)]


class OFPATSetTpDstID(OFPATID):
    name = 'OFPAT_SET_TP_DST'
    fields_desc = [ShortEnumField('type', 10, ofp_action_types), ShortField('len', 4)]
...
class OFPATSetMPLSLabelID(OFPATID):
    name = 'OFPAT_SET_MPLS_LABEL'
    fields_desc = [ShortEnumField('type', 13, ofp_action_types), ShortField('len', 4)]


class OFPATSetMPLSTCID(OFPATID):
    name = 'OFPAT_SET_MPLS_TC'
    fields_desc = [ShortEnumField('type', 14, ofp_action_types), ShortField('len', 4)]
...
class OFPATCopyTTLOutID(OFPATID):
    name = 'OFPAT_COPY_TTL_OUT'
    fields_desc = [ShortEnumField('type', 11, ofp_action_types), ShortField('len', 4)]


class OFPATCopyTTLInID(OFPATID):
    name = 'OFPAT_COPY_TTL_IN'
    fields_desc = [ShortEnumField('type', 12, ofp_action_types), ShortField('len', 4)]


class OFPATSetMPLSTTLID(OFPATID):
    name = 'OFPAT_SET_MPLS_TTL'
    fields_desc = [ShortEnumField('type', 15, ofp_action_types), ShortField('len', 4)]


class OFPATDecMPLSTTLID(OFPATID):
    name = 'OFPAT_DEC_MPLS_TTL'
    fields_desc = [ShortEnumField('type', 16, ofp_action_types), ShortField('len', 4)]


class OFPATPushVLANID(OFPATID):
    name = 'OFPAT_PUSH_VLAN'
    fields_desc = [ShortEnumField('type', 17, ofp_action_types), ShortField('len', 4)]


class OFPATPopVLANID(OFPATID):
    name = 'OFPAT_POP_VLAN'
    fields_desc = [ShortEnumField('type', 18, ofp_action_types), ShortField('len', 4)]


class OFPATPushMPLSID(OFPATID):
    name = 'OFPAT_PUSH_MPLS'
    fields_desc = [ShortEnumField('type', 19, ofp_action_types), ShortField('len', 4)]


class OFPATPopMPLSID(OFPATID):
    name = 'OFPAT_POP_MPLS'
    fields_desc = [ShortEnumField('type', 20, ofp_action_types), ShortField('len', 4)]


class OFPATSetQueueID(OFPATID):
    name = 'OFPAT_SET_QUEUE'
    fields_desc = [ShortEnumField('type', 21, ofp_action_types), ShortField('len', 4)]


class OFPATGroupID(OFPATID):
    name = 'OFPAT_GROUP'
    fields_desc = [ShortEnumField('type', 22, ofp_action_types), ShortField('len', 4)]


class OFPATSetNwTTLID(OFPATID):
    name = 'OFPAT_SET_NW_TTL'
    fields_desc = [ShortEnumField('type', 23, ofp_action_types), ShortField('len', 4)]


class OFPATDecNwTTLID(OFPATID):
    name = 'OFPAT_DEC_NW_TTL'
    fields_desc = [ShortEnumField('type', 24, ofp_action_types), ShortField('len', 4)]


class OFPATSetFieldID(OFPATID):
    name = 'OFPAT_SET_FIELD'
    fields_desc = [ShortEnumField('type', 25, ofp_action_types), ShortField('len', 4)]


class OFPATPushPBBID(OFPATID):
    name = 'OFPAT_PUSH_PBB'
    fields_desc = [ShortEnumField('type', 26, ofp_action_types), ShortField('len', 4)]


class OFPATPopPBBID(OFPATID):
    name = 'OFPAT_POP_PBB'
    fields_desc = [ShortEnumField('type', 27, ofp_action_types), ShortField('len', 4)]


class OFPATExperimenterID(OFPATID):
    name = 'OFPAT_EXPERIMENTER'
    fields_desc = [ShortEnumField('type', 65535, ofp_action_types), ShortField('len', None)]
...
ofp_instruction_types = {1: 'OFPIT_GOTO_TABLE', 2: 'OFPIT_WRITE_METADATA', 3: 'OFPIT_WRITE_ACTIONS', 4: 'OFPIT_APPLY_ACTIONS', 5: 'OFPIT_CLEAR_ACTIONS', 6: 'OFPIT_METER', 65535: 'OFPIT_EXPERIMENTER'}
...
class OFPITGotoTable(OFPIT):
    ...
    fields_desc = [ShortEnumField('type', 1, ofp_instruction_types), ShortField('len', 8), ByteEnumField('table_id', 0, ofp_table), X3BytesField('pad', 0)]


class OFPITWriteMetadata(OFPIT):
    name = 'OFPIT_WRITE_METADATA'
    fields_desc = [ShortEnumField('type', 2, ofp_instruction_types), ShortField('len', 24), XIntField('pad', 0), LongField('metadata', 0), LongField('metadata_mask', 0)]


class OFPITWriteActions(OFPIT):
    name = 'OFPIT_WRITE_ACTIONS'
    fields_desc = [ShortEnumField('type', 3, ofp_instruction_types), ShortField('len', None), XIntField('pad', 0), PacketListField('actions', [], OFPAT, length_from=lambda pkt: pkt.len - 8)]


class OFPITApplyActions(OFPIT):
    name = 'OFPIT_APPLY_ACTIONS'
    fields_desc = [ShortEnumField('type', 4, ofp_instruction_types), ShortField('len', None), XIntField('pad', 0), PacketListField('actions', [], OFPAT, length_from=lambda pkt: pkt.len - 8)]


class OFPITClearActions(OFPIT):
    name = 'OFPIT_CLEAR_ACTIONS'
    fields_desc = [ShortEnumField('type', 5, ofp_instruction_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPITMeter(OFPIT):
    name = 'OFPIT_METER'
    fields_desc = [ShortEnumField('type', 6, ofp_instruction_types), ShortField('len', 8), IntEnumField('meter_id', 1, ofp_meter)]


class OFPITExperimenter(OFPIT):
    name = 'OFPIT_EXPERIMENTER'
    fields_desc = [ShortEnumField('type', 65535, ofp_instruction_types), ShortField('len', None), IntField('experimenter', 0)]


ofp_instruction_cls = {1: OFPITGotoTable, 2: OFPITWriteMetadata, 3: OFPITWriteActions, 4: OFPITApplyActions, 5: OFPITClearActions, 6: OFPITMeter, 65535: OFPITExperimenter}
...
class OFPITGotoTableID(OFPITID):
    name = 'OFPIT_GOTO_TABLE'
    fields_desc = [ShortEnumField('type', 1, ofp_instruction_types), ShortField('len', 4)]


class OFPITWriteMetadataID(OFPITID):
    name = 'OFPIT_WRITE_METADATA'
    fields_desc = [ShortEnumField('type', 2, ofp_instruction_types), ShortField('len', 4)]


class OFPITWriteActionsID(OFPITID):
    name = 'OFPIT_WRITE_ACTIONS'
    fields_desc = [ShortEnumField('type', 3, ofp_instruction_types), ShortField('len', 4)]


class OFPITApplyActionsID(OFPITID):
    name = 'OFPIT_APPLY_ACTIONS'
    fields_desc = [ShortEnumField('type', 4, ofp_instruction_types), ShortField('len', 4)]


class OFPITClearActionsID(OFPITID):
    name = 'OFPIT_CLEAR_ACTIONS'
    fields_desc = [ShortEnumField('type', 5, ofp_instruction_types), ShortField('len', 4)]


class OFPITMeterID(OFPITID):
    name = 'OFPIT_METER'
    fields_desc = [ShortEnumField('type', 6, ofp_instruction_types), ShortField('len', 4)]


class OFPITExperimenterID(OFPITID):
    name = 'OFPIT_EXPERIMENTER'
    fields_desc = [ShortEnumField('type', 65535, ofp_instruction_types), ShortField('len', None)]


ofp_instruction_id_cls = {1: OFPITGotoTableID, 2: OFPITWriteMetadataID, 3: OFPITWriteActionsID, 4: OFPITApplyActionsID, 5: OFPITClearActionsID, 6: OFPITMeterID, 65535: OFPITExperimenterID}
...
class OFPBucket(_ofp_header_item):
    ...
    fields_desc = [
        ...
        XIntField('pad', 0),
        PacketListField('actions', [], OFPAT, length_from=lambda pkt: pkt.len - 16),
    ]
...
ofp_queue_property_types = {0: 'OFPQT_NONE', 1: 'OFPQT_MIN_RATE'}
...
class OFPQTNone(OFPQT):
    name = 'OFPQT_NONE'
    fields_desc = [ShortEnumField('type', 0, ofp_queue_property_types), ShortField('len', 8), XIntField('pad', 0)]


class OFPQTMinRate(OFPQT):
    name = 'OFPQT_MIN_RATE'
    fields_desc = [ShortEnumField('type', 1, ofp_queue_property_types), ShortField('len', 16), XIntField('pad1', 0), ShortField('rate', 0), XBitField('pad2', 0, 48)]


ofp_queue_property_cls = {0: OFPQTNone, 1: OFPQTMinRate}


class OFPPacketQueue(Packet):
    name = 'OFP_PACKET_QUEUE'
    fields_desc = [IntEnumField('queue_id', 0, ofp_queue), ShortField('len', None), XShortField('pad', 0), PacketListField('properties', [], OFPQT, length_from=lambda pkt: pkt.len - 8)]  # noqa: E501
...
ofp_meter_band_types = {0: 'OFPMBT_DROP', 1: 'OFPMBT_DSCP_REMARK', 65535: 'OFPMBT_EXPERIMENTER'}
...
class OFPMBTDrop(OFPMBT):
    name = 'OFPMBT_DROP'
    fields_desc = [ShortEnumField('type', 0, ofp_queue_property_types), ShortField('len', 16), IntField('rate', 0), IntField('burst_size', 0), XIntField('pad', 0)]


class OFPMBTDSCPRemark(OFPMBT):
    name = 'OFPMBT_DSCP_REMARK'
    fields_desc = [ShortEnumField('type', 1, ofp_queue_property_types), ShortField('len', 16), IntField('rate', 0), IntField('burst_size', 0), ByteField('prec_level', 0), X3BytesField('pad', 0)]


class OFPMBTExperimenter(OFPMBT):
    name = 'OFPMBT_EXPERIMENTER'
    fields_desc = [ShortEnumField('type', 65535, ofp_queue_property_types), ShortField('len', 16), IntField('rate', 0), IntField('burst_size', 0), IntField('experimenter', 0)]


ofp_meter_band_cls = {0: OFPMBTDrop, 1: OFPMBTDSCPRemark, 2: OFPMBTExperimenter}
...
ofp_version = {0x01: 'OpenFlow 1.0', 0x02: 'OpenFlow 1.1', 0x03: 'OpenFlow 1.2', 0x04: 'OpenFlow 1.3', 0x05: 'OpenFlow 1.4'}
...
class OFPTHello(_ofp_header):
    name = 'OFPT_HELLO'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 0, ofp_type), ShortField('len', None), IntField('xid', 0), PacketListField('elements', [], OFPHET, length_from=lambda pkt: pkt.len - 8)]
...
class OFPETHelloFailed(_ofp_header):
    ...
    fields_desc = [
        ByteEnumField('version', 0x04, ofp_version),
        ByteEnumField('type', 1, ofp_type),
        ShortField('len', None),
        IntField('xid', 0),
        ShortEnumField('errtype', 0, ofp_error_type),
        ShortEnumField('errcode', 0, {0: 'OFPHFC_INCOMPATIBLE', 1: 'OFPHFC_EPERM'}),
...
class OFPETFlowModFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPFMFC_UNKNOWN', 1: 'OFPFMFC_TABLE_FULL', 2: 'OFPFMFC_BAD_TABLE_ID', 3: 'OFPFMFC_OVERLAP', 4: 'OFPFMFC_EPERM', 5: 'OFPFMFC_BAD_TIMEOUT', 6: 'OFPFMFC_BAD_COMMAND', 7: 'OFPFMFC_BAD_FLAGS'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETPortModFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPPMFC_BAD_PORT', 1: 'OFPPMFC_BAD_HW_ADDR', 2: 'OFPPMFC_BAD_CONFIG', 3: 'OFPPMFC_BAD_ADVERTISE', 4: 'OFPPMFC_EPERM'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETTableModFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPTMFC_BAD_TABLE', 1: 'OFPTMFC_BAD_CONFIG', 2: 'OFPTMFC_EPERM'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETQueueOpFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPQOFC_BAD_PORT', 1: 'OFPQOFC_BAD_QUEUE', 2: 'OFPQOFC_EPERM'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETSwitchConfigFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPSCFC_BAD_FLAGS', 1: 'OFPSCFC_BAD_LEN', 2: 'OFPSCFC_EPERM'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETRoleRequestFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPRRFC_STALE', 1: 'OFPRRFC_UNSUP', 2: 'OFPRRFC_BAD_ROLE'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPETTableFeaturesFailed(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortEnumField('errcode', 0, {0: 'OFPTFFC_BAD_TABLE', 1: 'OFPTFFC_BAD_METADATA', 2: 'OFPTFFC_BAD_TYPE', 3: 'OFPTFFC_BAD_LEN', 4: 'OFPTFFC_BAD_ARGUMENT', 5: 'OFPTFFC_EPERM'}),
        OFPacketField('data', '', Raw),
    ]
...
class OFPTEchoRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 2, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTEchoReply(_ofp_header):
    name = 'OFPT_ECHO_REPLY'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 3, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTExperimenter(_ofp_header):
    name = 'OFPT_EXPERIMENTER'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 4, ofp_type), ShortField('len', None), IntField('xid', 0), IntField('experimenter', 0), IntField('exp_type', 0)]


class OFPTFeaturesRequest(_ofp_header):
    name = 'OFPT_FEATURES_REQUEST'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 5, ofp_type), ShortField('len', None), IntField('xid', 0)]
...
class OFPTGetConfigRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 7, ofp_type), ShortField('len', None), IntField('xid', 0)]
...
class OFPTGetConfigReply(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        IntField('xid', 0),
        ShortEnumField('flags', 0, {0: 'FRAG_NORMAL', 1: 'FRAG_DROP', 2: 'FRAG_REASM', 3: 'FRAG_MASK'}),
...
class OFPTPacketIn(_ofp_header):
    ...
    fields_desc = [
        ...
        ByteEnumField('reason', 0, {0: 'OFPR_NO_MATCH', 1: 'OFPR_ACTION', 2: 'OFPR_INVALID_TTL'}),
...
class OFPTFlowRemoved(_ofp_header):
    ...
    fields_desc = [
        ...
        ByteEnumField('reason', 0, {0: 'OFPRR_IDLE_TIMEOUT', 1: 'OFPRR_HARD_TIMEOUT', 2: 'OFPRR_DELETE', 3: 'OFPRR_GROUP_DELETE'}),
        ByteEnumField('table_id', 0, ofp_table),
...
class OFPTPortStatus(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        IntField('xid', 0),
        ByteEnumField('reason', 0, {0: 'OFPPR_ADD', 1: 'OFPPR_DELETE', 2: 'OFPPR_MODIFY'}),
        XBitField('pad', 0, 56),
...
class OFPTPacketOut(_ofp_header):
    ...
    fields_desc = [
        ...
        XBitField('pad', 0, 48),
        PacketListField('actions', [], OFPAT, OFPAT, length_from=lambda pkt: pkt.actions_len),
        PacketField('data', '', Ether),
    ]
...
class OFPTFlowMod(_ofp_header):
    ...
    fields_desc = [
        ...
        ByteEnumField('table_id', 0, ofp_table),
        ByteEnumField('cmd', 0, {0: 'OFPFC_ADD', 1: 'OFPFC_MODIFY', 2: 'OFPFC_MODIFY_STRICT', 3: 'OFPFC_DELETE', 4: 'OFPFC_DELETE_STRICT'}),
        ShortField('idle_timeout', 0),
        ShortField('hard_timeout', 0),
        ShortField('priority', 0),
        IntEnumField('buffer_id', 'NO_BUFFER', ofp_buffer),
        ...
        FlagsField('flags', 0, 16, ['SEND_FLOW_REM', 'CHECK_OVERLAP', 'RESET_COUNTS', 'NO_PKT_COUNTS', 'NO_BYT_COUNTS']),
        XShortField('pad', 0),
        MatchField('match'),
        PacketListField('instructions', [], OFPIT, length_from=lambda pkt: pkt.len - 48 - (pkt.match.len + (8 - pkt.match.len % 8) % 8)),
    ]  # noqa: E501
...
class OFPTGroupMod(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        IntField('xid', 0),
        ShortEnumField('cmd', 0, {0: 'OFPGC_ADD', 1: 'OFPGC_MODIFY', 2: 'OFPGC_DELETE'}),
        ByteEnumField('group_type', 0, {0: 'OFPGT_ALL', 1: 'OFPGT_SELECT', 2: 'OFPGT_INDIRECT', 3: 'OFPGT_FF'}),
        XByteField('pad', 0),
        IntEnumField('group_id', 0, ofp_group),
        PacketListField('buckets', [], OFPBucket, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPFlowStats(_ofp_header_item):
    ...
    fields_desc = [
        ...
        LongField('byte_count', 0),
        MatchField('match'),
        PacketListField('instructions', [], OFPIT, length_from=lambda pkt: pkt.len - 56 - pkt.match.len),
    ]  # noqa: E501
...
class OFPMPReplyFlow(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('flow_stats', [], OFPFlowStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPTableStats(Packet):
    ...
    fields_desc = [ByteEnumField('table_id', 0, ofp_table), X3BytesField('pad1', 0), IntField('active_count', 0), LongField('lookup_count', 0), LongField('matched_count', 0)]
...
class OFPMPReplyTable(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('table_stats', None, OFPTableStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPMPReplyPortStats(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('port_stats', None, OFPPortStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPQueueStats(Packet):
    ...
    fields_desc = [IntEnumField('port_no', 0, ofp_port_no), IntEnumField('queue_id', 0, ofp_queue), LongField('tx_bytes', 0), LongField('tx_packets', 0), LongField('tx_errors', 0), IntField('duration_sec', 0), IntField('duration_nsec', 0)]
...
class OFPMPReplyQueue(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('queue_stats', None, OFPQueueStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPBucketStats(Packet):
    ...
    fields_desc = [LongField('packet_count', 0), LongField('byte_count', 0)]
...
class OFPGroupStats(_ofp_header_item):
    ...
    fields_desc = [
        ...
        IntField('duration_sec', 0),
        IntField('duration_nsec', 0),
        PacketListField('bucket_stats', None, OFPBucketStats, length_from=lambda pkt: pkt.len - 40),
    ]
...
class OFPMPReplyGroup(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('group_stats', [], OFPGroupStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPGroupDesc(_ofp_header_item):
    ...
    fields_desc = [
        ShortField('len', None),
        ByteEnumField('type', 0, {0: 'OFPGT_ALL', 1: 'OFPGT_SELECT', 2: 'OFPGT_INDIRECT', 3: 'OFPGT_FF'}),
        XByteField('pad', 0),
        IntEnumField('group_id', 0, ofp_group),
        PacketListField('buckets', None, OFPBucket, length_from=lambda pkt: pkt.len - 8),
    ]
...
class OFPMPReplyGroupDesc(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('group_descs', [], OFPGroupDesc, length_from=lambda pkt: pkt.len - 16),
    ]
...
ofp_action_types_flags = [v for v in six.itervalues(ofp_action_types) if v != 'OFPAT_EXPERIMENTER']
...
class OFPMPReplyGroupFeatures(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        FlagsField('types', 0, 32, ['ALL', 'SELECT', 'INDIRECT', 'FF']),
        FlagsField('capabilities', 0, 32, ['SELECT_WEIGHT', 'SELECT_LIVENESS', 'CHAINING', 'CHAINING_CHECKS']),
...
class OFPMeterBandStats(Packet):
    ...
    fields_desc = [LongField('packet_band_count', 0), LongField('byte_band_count', 0)]
...
class OFPMeterStats(Packet):
    ...
    fields_desc = [
        ...
        IntField('duration_sec', 0),
        IntField('duration_nsec', 0),
        PacketListField('band_stats', None, OFPMeterBandStats, length_from=lambda pkt: pkt.len - 40),
    ]
...
class OFPMPReplyMeter(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('meter_stats', [], OFPMeterStats, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPMeterConfig(_ofp_header_item):
    ...
    fields_desc = [ShortField('len', None), FlagsField('flags', 0, 16, ['KBPS', 'PKTPS', 'BURST', 'STATS']), IntEnumField('meter_id', 1, ofp_meter), PacketListField('bands', [], OFPMBT, length_from=lambda pkt: pkt.len - 8)]
...
class OFPMPReplyMeterConfig(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('meter_configs', [], OFPMeterConfig, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPMPReplyMeterFeatures(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('band_types', 0, 32, ['DROP', 'DSCP_REMARK', 'EXPERIMENTER']),
        FlagsField('capabilities', 0, 32, ['KPBS', 'PKTPS', 'BURST', 'STATS']),
...
class OFPTFPTInstructions(OFPTFPT):
    ...
    fields_desc = [ShortField('type', 0), ShortField('len', None), PacketListField('instruction_ids', [], OFPITID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTInstructionsMiss(OFPTFPT):
    name = 'OFPTFPT_INSTRUCTIONS_MISS'
    fields_desc = [ShortField('type', 1), ShortField('len', None), PacketListField('instruction_ids', [], OFPITID, length_from=lambda pkt: pkt.len - 4)]
...
class OFPTFPTNextTables(OFPTFPT):
    ...
    fields_desc = [ShortField('type', 2), ShortField('len', None), PacketListField('next_table_ids', None, OFPTableID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTNextTablesMiss(OFPTFPT):
    name = 'OFPTFPT_NEXT_TABLES_MISS'
    fields_desc = [ShortField('type', 3), ShortField('len', None), PacketListField('next_table_ids', None, OFPTableID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTWriteActions(OFPTFPT):
    name = 'OFPTFPT_WRITE_ACTIONS'
    fields_desc = [ShortField('type', 4), ShortField('len', None), PacketListField('action_ids', [], OFPATID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTWriteActionsMiss(OFPTFPT):
    name = 'OFPTFPT_WRITE_ACTIONS_MISS'
    fields_desc = [ShortField('type', 5), ShortField('len', None), PacketListField('action_ids', [], OFPATID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTApplyActions(OFPTFPT):
    name = 'OFPTFPT_APPLY_ACTIONS'
    fields_desc = [ShortField('type', 6), ShortField('len', None), PacketListField('action_ids', [], OFPATID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTApplyActionsMiss(OFPTFPT):
    name = 'OFPTFPT_APPLY_ACTIONS_MISS'
    fields_desc = [ShortField('type', 7), ShortField('len', None), PacketListField('action_ids', [], OFPATID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTMatch(OFPTFPT):
    name = 'OFPTFPT_MATCH'
    fields_desc = [ShortField('type', 8), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTWildcards(OFPTFPT):
    name = 'OFPTFPT_WILDCARDS'
    fields_desc = [ShortField('type', 10), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTWriteSetField(OFPTFPT):
    name = 'OFPTFPT_WRITE_SETFIELD'
    fields_desc = [ShortField('type', 12), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTWriteSetFieldMiss(OFPTFPT):
    name = 'OFPTFPT_WRITE_SETFIELD_MISS'
    fields_desc = [ShortField('type', 13), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTApplySetField(OFPTFPT):
    name = 'OFPTFPT_APPLY_SETFIELD'
    fields_desc = [ShortField('type', 14), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTApplySetFieldMiss(OFPTFPT):
    name = 'OFPTFPT_APPLY_SETFIELD_MISS'
    fields_desc = [ShortField('type', 15), ShortField('len', None), PacketListField('oxm_ids', [], OXMID, length_from=lambda pkt: pkt.len - 4)]


class OFPTFPTExperimenter(OFPTFPT):
    name = 'OFPTFPT_EXPERIMENTER'
    fields_desc = [ShortField('type', 65534), ShortField('len', None), IntField('experimenter', 0), IntField('exp_type', 0), PacketLenField('experimenter_data', None, Raw, length_from=lambda pkt: pkt.len - 12)]


class OFPTFPTExperimenterMiss(OFPTFPT):
    name = 'OFPTFPT_EXPERIMENTER_MISS'
    fields_desc = [ShortField('type', 65535), ShortField('len', None), IntField('experimenter', 0), IntField('exp_type', 0), PacketLenField('experimenter_data', None, Raw, length_from=lambda pkt: pkt.len - 12)]


ofp_table_features_prop_cls = {
    0: OFPTFPTInstructions,
...
class OFPTableFeatures(_ofp_header_item):
    ...
    fields_desc = [
        ...
        IntEnumField('config', 0, {0: 'OFPTC_NO_MASK', 3: 'OFPTC_DEPRECATED_MASK'}),
        IntField('max_entries', 0),
        PacketListField('properties', [], OFPTFPT, length_from=lambda pkt: pkt.len - 64),
    ]
...
class OFPMPRequestTableFeatures(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_request_flags),
        XIntField('pad1', 0),
        PacketListField('table_features', [], OFPTableFeatures, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPMPReplyPortDesc(_ofp_header):
    ...
    fields_desc = [
        ...
        FlagsField('flags', 0, 16, ofpmp_reply_flags),
        XIntField('pad1', 0),
        PacketListField('ports', None, OFPPort, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPTBarrierRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 20, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTBarrierReply(_ofp_header):
    name = 'OFPT_BARRIER_REPLY'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 21, ofp_type), ShortField('len', None), IntField('xid', 0)]


class OFPTQueueGetConfigRequest(_ofp_header):
    name = 'OFPT_QUEUE_GET_CONFIG_REQUEST'
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 22, ofp_type), ShortField('len', None), IntField('xid', 0), IntEnumField('port_no', 'ANY', ofp_port_no), XIntField('pad', 0)]
...
class OFPTQueueGetConfigReply(_ofp_header):
    ...
    fields_desc = [
        ...
        XIntField('pad', 0),
        PacketListField('queues', [], OFPPacketQueue, length_from=lambda pkt: pkt.len - 16),
    ]
...
class OFPTRoleRequest(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        IntField('xid', 0),
        IntEnumField('role', 0, {0: 'OFPCR_ROLE_NOCHANGE', 1: 'OFPCR_ROLE_EQUAL', 2: 'OFPCR_ROLE_MASTER', 3: 'OFPCR_ROLE_SLAVE'}),
        XIntField('pad', 0),
...
class OFPTGetAsyncRequest(_ofp_header):
    ...
    fields_desc = [ByteEnumField('version', 0x04, ofp_version), ByteEnumField('type', 26, ofp_type), ShortField('len', 8), IntField('xid', 0)]


ofp_packet_in_reason = ['NO_MATCH', 'ACTION', 'INVALID_TTL']

ofp_port_reason = ['ADD', 'DELETE', 'MODIFY']

ofp_flow_removed_reason = ['IDLE_TIMEOUT', 'HARD_TIMEOUT', 'DELETE', 'GROUP_DELETE']
...
class OFPTMeterMod(_ofp_header):
    ...
    fields_desc = [
        ...
        ShortField('len', None),
        IntField('xid', 0),
        ShortEnumField('cmd', 0, {0: 'OFPMC_ADD', 1: 'OFPMC_MODIFY', 2: 'OFPMC_DELETE'}),
        FlagsField('flags', 0, 16, ['KBPS', 'PKTPS', 'BURST', 'STATS']),
        IntEnumField('meter_id', 1, ofp_meter),
        PacketListField('bands', [], OFPMBT, length_from=lambda pkt: pkt.len - 16),
    ]
