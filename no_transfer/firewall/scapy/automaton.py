...
class Automaton_metaclass(type):
    def __new__(cls, name, bases, dct):
        ...
        decorated = [v for v in six.itervalues(members) if hasattr(v, 'atmt_type')]
        ...
        for v in itertools.chain(six.itervalues(cls.conditions), six.itervalues(cls.recv_conditions), six.itervalues(cls.ioevents)):
    ...
    def build_graph(self):
        ...
        for c, k, v in (
            [('purple', k, v) for k, v in self.conditions.items()]  # noqa: E501
            ...
            try:
                ...
                if self.state.state in self.breakpoints and self.state.state != self.breakpointed:  # noqa: E501
                    ...
                    yield self.Breakpoint(
                        'breakpoint triggered on state %s' % self.state.state,  # noqa: E501
                ...
                if self.state.error:
                    raise self.ErrorState(
                        'Reached %s: [%r]' % (self.state.state, state_output),  # noqa: E501
                        result=state_output,
                        state=self.state.state,
                    )  # noqa: E501
                if self.state.final:
