...
ReadOnlyAttribute = functools.partial(Interceptor, hook=(lambda name, *args, **kwargs: _readonly(name)))
    ...
    def __repr__(self):
        ...
        for num, layer in six.iteritems(self.num2layer):
            if layer in self.layer2num and self.layer2num[layer] == num:
                ...
                dir = ' ->'
            lst.append((num, '%#6x %s %-20s (%s)' % (num, dir, layer.__name__, layer._name)))
        for layer, num in six.iteritems(self.layer2num):
            if num not in self.num2layer or self.num2layer[num] != layer:
                lst.append((num, '%#6x <-  %-20s (%s)' % (num, layer.__name__, layer._name)))
        lst.sort()
...
class CacheInstance(Dict[str, Any], object):
    ...
    def flush(self):
        # type: () -> None
        CacheInstance.__init__(self, name=self.name, timeout=self.timeout)
...
def _set_conf_sockets():
    ...
    if LINUX:
        ...
        conf.L3socket6 = cast('Type[SuperSocket]', functools.partial(L3PacketSocket, filter='ip6'))
...
class Conf(ConfClass):
    ...
    bufsize = 2**16
    #: history file
    histfile = os.getenv('SCAPY_HISTFILE', os.path.join(os.path.expanduser('~'), '.scapy_history'))
    ...
    use_pcap = Interceptor('use_pcap', os.getenv('SCAPY_USE_LIBPCAP', '').lower().startswith('y'), _socket_changer)
    ...
    def __getattribute__(self, attr):
        ...
        if attr == 'iface6':
            warnings.warn('conf.iface6 is deprecated in favor of conf.iface', DeprecationWarning)
            attr = 'iface'
...
def crypto_validator(func):
    ...
    def func_in(*args, **kwargs):
        ...
        if not conf.crypto_valid:
            raise ImportError('Cannot execute crypto-related method! ' 'Please install python-cryptography v1.7 or later.')  # noqa: E501
