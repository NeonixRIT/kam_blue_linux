...
/i7kinChIXSAmRgA==\n""",
    ...
    def get_local_dict(cls):
        return {x: y.name for (x, y) in six.iteritems(cls.__dict__) if isinstance(y, File)}

    get_local_dict = classmethod(get_local_dict)

    def get_URL_dict(cls):
        return {x: y.URL for (x, y) in six.iteritems(cls.__dict__) if isinstance(y, File)}
...
def execute_campaign(TESTFILE, OUTPUTFILE, PREEXEC, NUM, KW_OK, KW_KO, DUMP, DOCS, FORMAT, VERB, ONLYFAILED, CRC, INTERPRETER, autorun_func, theme, pos_begin=0, scapy_ses=None):  # noqa: E501
    # Parse test file
    try:
...
def main():
    ...
    if OUTPUTFILE == sys.stdout:
        ...
        with open(OUTPUTFILE, 'wb') as f:
            f.write(glob_output.encode('utf8', 'ignore') if 'b' in f.mode or six.PY2 else glob_output)
